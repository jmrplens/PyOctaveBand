...
def test_truncated_sweep_keeps_the_exact_synthesis_magnitude() -> None:
    ...
    res = room.shaped_sweep_signal(FS, 100.0, 10000.0, 1.5, target="white")
...
def test_crest_factor_stays_near_the_swept_sine_ideal() -> None:
    for target in ("pink", "white"):
        res = room.shaped_sweep_signal(FS, 50.0, 5000.0, 2.0, target=target)
...
def test_crest_factor_is_finite_when_the_core_window_collapses() -> None:
    ...
    res = room.shaped_sweep_signal(FS, 50.0, 5000.0, 1e-6, start_delay=1.0)
...
def test_shaped_sweep_deconvolves_a_known_system() -> None:
    ...
    # truncation bias.
    ir = room.impulse_response(recorded, excitation, FS, return_full=True)
...
def test_result_behaves_like_array() -> None:
    res = room.shaped_sweep_signal(FS, 100.0, 5000.0, 0.5)
