...
def test_long_direct_field_60_db_at_1_2_m() -> None:
    ...
    lp = float(np.asarray(speech_direct_level(1.2))[()])
    assert round(lp) == 60
...
def test_long_self_noise_63_db_for_one_talker() -> None:
    ...
    noise is 63 dB"."""
    lp = float(np.asarray(crowd_noise_level(1, LONG_HARD_ABSORPTION))[()])
    assert round(lp) == 63


def test_long_signal_to_noise_is_minus_3_db() -> None:
    """Long p. 665: "our signal-to-noise ratio is -3 dB"."""
    signal = float(np.asarray(speech_direct_level(1.2))[()])
    noise = float(np.asarray(crowd_noise_level(1, LONG_HARD_ABSORPTION))[()])
...
def test_long_twenty_talkers_reach_76_db() -> None:
    ...
    lp = float(np.asarray(crowd_noise_level(20, LONG_HARD_ABSORPTION))[()])
    assert round(lp) == 76
...
def test_long_treated_room_drops_the_20_table_level_to_66_db() -> None:
    ...
    lp = float(np.asarray(crowd_noise_level(20, total))[()])
    assert round(lp) == 66
...
def test_long_communication_bound_is_6_3_square_metres_at_1_m() -> None:
    ...
    a_tab = float(np.asarray(absorption_per_table(1.0, COMMUNICATION_SNR))[()])
...
def test_long_privacy_bound_is_20_square_metres_at_2_5_m() -> None:
    ...
    a_tab = float(np.asarray(absorption_per_table(2.5, PRIVACY_SNR))[()])
...
def test_long_privacy_constant_is_3_16() -> None:
    ...
    for rt in (1.0, 2.0, 3.5):
        a_tab = float(np.asarray(absorption_per_table(rt, PRIVACY_SNR))[()])
...
def test_communication_constant_is_6_31_not_the_printed_6_33() -> None:
    ...
    printed equation.
    """
    expected = 16.0 * math.pi * 10.0 ** (-0.6) / TALKER_DIRECTIVITY
    a_tab = float(np.asarray(absorption_per_table(1.0, COMMUNICATION_SNR))[()])
...
def test_speech_to_noise_is_independent_of_power_and_occupancy() -> None:
    ...
    """
    a_tab, r = 8.0, 1.2
    direct = float(np.asarray(speech_direct_level(r))[()])
    for n in (1.0, 5.0, 20.0):
        noise = float(np.asarray(crowd_noise_level(n, n * a_tab))[()])
        assert direct - noise == pytest.approx(
            float(np.asarray(speech_to_noise_ratio(r, a_tab))[()])
        )


def test_absorption_per_table_inverts_the_ratio() -> None:
    for snr in (-3.0, -6.0, -9.0, -12.0):
        a_tab = float(np.asarray(absorption_per_table(1.7, snr))[()])
        assert float(np.asarray(speech_to_noise_ratio(1.7, a_tab))[()]) == (
            pytest.approx(snr)
        )
...
def test_limit_of_a_perfectly_absorptive_room() -> None:
    ...
    """
    levels = np.asarray(
        crowd_noise_level(20, np.array([100.0, 200.0, 400.0, 1e12]))
    )
...
def test_empty_room_is_the_single_talker_case() -> None:
    ...
    one = float(np.asarray(crowd_noise_level(1, 50.0))[()])
    ten = float(np.asarray(crowd_noise_level(10, 50.0))[()])
    assert ten - one == pytest.approx(10.0)


def test_crowd_noise_result_shape_and_fields() -> None:
    result = crowd_noise([20.0, 190.0], talkers=[1.0, 20.0])
    assert isinstance(result, CrowdNoiseResult)
    ...
    assert result.sound_power_level == NORMAL_VOICE_POWER_LEVEL
    assert result.directivity == TALKER_DIRECTIVITY
    # Above this noise level the -6 dB communication limit is breached.
    assert result.communication_level == pytest.approx(
        result.signal_level - COMMUNICATION_SNR
    )
...
def test_plot_draws_one_curve_per_absorption_plus_two_references() -> None:
    ...
    result = crowd_noise([20.0, 90.0, 190.0])
    ax = result.plot()
...
@pytest.mark.parametrize(
    ("call", "match"),
    [
        (lambda: crowd_noise_level(0, 20.0), "at least 1"),
        (lambda: crowd_noise_level(1, 0.0), "positive and finite"),
        (lambda: speech_direct_level(0.0), "positive and finite"),
        (lambda: speech_direct_level(1.0, directivity=0.0), "directivity"),
        (lambda: speech_to_noise_ratio(1.0, -1.0), "positive and finite"),
        (lambda: absorption_per_table(1.0, math.nan), "finite"),
        (lambda: crowd_noise([]), "non-empty"),
        (lambda: crowd_noise(20.0, talkers=[0.5]), "at least 1"),
        (lambda: crowd_noise(-1.0), "positive and finite"),
    ],
)
