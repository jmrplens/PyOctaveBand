...
def _vigran_flat_stiffness() -> float:
    ...
    return plate_bending_stiffness(
...
def _vigran_corrugated() -> tuple[float, tuple[float, float, float]]:
    ...
    mass = VIGRAN_MASS * corrugated_plate_mass_factor(
        VIGRAN_AMPLITUDE, VIGRAN_WAVELENGTH
    )
    stiffness = corrugated_plate_stiffness(
...
def test_flat_plate_eigenfrequencies_match_vigran_example() -> None:
    ...
    assert orthotropic_plate_resonance(1, 1, **kwargs) == pytest.approx(4.9, abs=0.05)
    assert orthotropic_plate_resonance(2, 2, **kwargs) == pytest.approx(19.7, abs=0.05)
...
def test_corrugated_plate_eigenfrequencies_match_vigran_example() -> None:
    ...
    assert orthotropic_plate_resonance(1, 1, **kwargs) == pytest.approx(25.5, abs=0.05)
    assert orthotropic_plate_resonance(2, 2, **kwargs) == pytest.approx(102.0, abs=0.1)
...
def test_corrugation_mass_factor_is_the_developed_length() -> None:
    ...
    assert corrugated_plate_mass_factor(
        VIGRAN_AMPLITUDE, VIGRAN_WAVELENGTH
    ) == pytest.approx(numeric, rel=1e-10)
    assert corrugated_plate_mass_factor(1e-12, 0.1) == pytest.approx(1.0, abs=1e-12)
...
def test_orthotropic_resonance_collapses_to_the_isotropic_formula() -> None:
    ...
    for i, n in ((1, 1), (2, 3), (4, 2)):
        ...
        assert orthotropic_plate_resonance(
...
def test_hopkins_table_a2_h_fc_products() -> None:
    ...
    for name, (c_l, product) in zip(HOPKINS_TABLE_A2_NAMES, rows, strict=True):
        b = plate_bending_stiffness(rho * c_l**2 * (1.0 - nu**2), h, nu)
        fc = coincidence_frequency(rho * h, b)
...
def test_orthotropic_critical_frequencies_bound_the_isotropic_one() -> None:
    ...
    b, m2 = 19.23, 7.8
    fc = coincidence_frequency(m2, b)
    assert orthotropic_critical_frequencies(m2, b, b) == pytest.approx((fc, fc))
    ...
    low, high = orthotropic_critical_frequencies(m2, 100.0 * b, b)
    assert low == pytest.approx(fc / 10.0)
    assert high == pytest.approx(fc)
    assert orthotropic_critical_frequencies(m2, b, 100.0 * b) == (low, high)
...
def test_corrugating_vigran_plate_moves_the_coincidence_range() -> None:
    ...
    flat_fc = coincidence_frequency(VIGRAN_MASS, _vigran_flat_stiffness())
...
def _fig627(
    ...
    *,
    ...
    return orthotropic_transmission_loss(
        freq, FIG627_MASS,
...
def test_orthotropic_integral_reduces_to_the_exact_mass_law_integral() -> None:
    ...
    res = orthotropic_transmission_loss(
...
def test_orthotropic_integral_flattens_below_the_flat_plate() -> None:
    ...
    flat = single_panel_transmission_loss(
        bands, VIGRAN_MASS,
        critical_frequency=coincidence_frequency(
...
def test_orthotropic_result_carries_the_coincidence_range() -> None:
    res = _fig627(BANDS)
    assert isinstance(res, SoundReductionResult)
...
def test_orthotropic_helpers_reject_bad_input() -> None:
    with pytest.raises(ValueError, match="must be positive"):
        corrugated_plate_mass_factor(-0.01, 0.1)
    ...
    with pytest.raises(ValueError, match="must be positive"):
        orthotropic_critical_frequencies(7.8, 0.0, 1.0)
