...
def test_critical_bandwidth_grows_with_frequency() -> None:
    assert psychoacoustics.critical_bandwidth_engineering(
        2000.0
    ) > psychoacoustics.critical_bandwidth_engineering(100.0)
...
def test_critical_band_corners_geometry() -> None:
    ...
    for ft in (55.0, 137.3, 592.2, 1582.7):
        f1, f2 = psychoacoustics.critical_band_corners(ft)
        assert math.sqrt(f1 * f2) == pytest.approx(ft, rel=1e-12)
        assert f2 - f1 == pytest.approx(
            psychoacoustics.critical_bandwidth_engineering(ft), rel=1e-12
        )
        assert 0.0 < f1 < ft < f2
...
def test_masking_index_closed_form() -> None:
    ...
    assert psychoacoustics.masking_index(f) == pytest.approx(expected)


def test_masking_index_matches_annex_e() -> None:
    assert psychoacoustics.masking_index(137.3) == pytest.approx(
        ref.ISO20065_AV_137, abs=0.005
    )
    assert psychoacoustics.masking_index(592.2) == pytest.approx(
        ref.ISO20065_AV_592, abs=0.005
    )


def test_masking_index_decreases_with_frequency() -> None:
    assert (
        psychoacoustics.masking_index(2000.0)
        < psychoacoustics.masking_index(100.0)
        < 0.0
    )
...
def test_critical_band_level_bandwidth_ratio() -> None:
    ...
    dfc = psychoacoustics.critical_bandwidth_engineering(ft)
    assert psychoacoustics.critical_band_level(ls, ft, df) == pytest.approx(
...
def test_energy_sum_hanning_correction() -> None:
    ...
    assert psychoacoustics.energy_sum_level([80.0, 80.0]) == pytest.approx(
        83.0103 - 10.0 * math.log10(1.5), abs=1e-4
    )


def test_energy_sum_rectangular_is_plain_sum() -> None:
    value = psychoacoustics.energy_sum_level(
        [80.0, 80.0], effective_bandwidth_factor=1.0
    )
    assert value == pytest.approx(10.0 * math.log10(2.0e8), abs=1e-6)


def test_hanning_factor_constant() -> None:
    assert psychoacoustics.HANNING_BANDWIDTH_FACTOR == 1.5
...
def test_mean_narrowband_level_rectangular_higher() -> None:
    ...
    ls_h = psychoacoustics.mean_narrowband_level(
        ref.ISO20065_E1_LEVELS, ref.ISO20065_E1_FREQUENCIES, 137.3
    )
    ls_r = psychoacoustics.mean_narrowband_level(
...
def test_tone_level_single_line_when_isolated() -> None:
    ...
    ls = 45.0
    lt = psychoacoustics.tone_level(levels, freqs, 105.4, ls)
...
def test_single_line_tone_audibility_flip_regression() -> None:
    ...
    result = psychoacoustics.analyze_spectrum(levels, freqs, df)
...
def test_mean_narrowband_level_few_lines_returns_unpruned_mean() -> None:
    ...
    ls = psychoacoustics.mean_narrowband_level(levels, freqs, 1000.0)
...
def test_analyze_spectrum_detects_annex_e_tones() -> None:
    result = psychoacoustics.analyze_spectrum(
...
def test_analyze_spectrum_rejects_inaudible_peak() -> None:
    ...
    with pytest.raises(ValueError, match="No audible tone"):
        psychoacoustics.analyze_spectrum(levels, freqs, 2.7)


def test_combined_tone_level_matches_annex_e() -> None:
    value = psychoacoustics.combined_tone_level(
        ref.ISO20065_E1_LEVELS,
        ref.ISO20065_E1_FREQUENCIES,
...
def test_combined_tone_level_single_tone_equals_tone_level() -> None:
    ...
    fg = psychoacoustics.combined_tone_level(
...
def _e1_line_energy(indices: list[int]) -> float:
    ...
    return 10.0 * math.log10(total) - 10.0 * math.log10(
        psychoacoustics.HANNING_BANDWIDTH_FACTOR
    )
...
def test_combined_tone_level_counts_a_shared_line_once() -> None:
    ...
    naive_same = psychoacoustics.energy_sum_level(
        [
            psychoacoustics.tone_level(lev, freq, 134.6, ls),
            psychoacoustics.tone_level(lev, freq, 137.3, ls),
        ],
        effective_bandwidth_factor=1.0,
    )
    ...
    naive_nested = psychoacoustics.energy_sum_level(
        [
            psychoacoustics.tone_level(lev, freq, 156.1, ls),
            psychoacoustics.tone_level(lev, freq, 158.8, ls),
        ],
        effective_bandwidth_factor=1.0,
    )
...
def test_combined_tone_level_no_double_counting() -> None:
    ...
    singles = [
        psychoacoustics.tone_level(
            ref.ISO20065_E1_LEVELS, ref.ISO20065_E1_FREQUENCIES, f, 49.22
        )
...
def test_separation_frequency_grows_either_side_of_reference() -> None:
    ...
    base = psychoacoustics.two_tone_separation_frequency(212.0)
    assert psychoacoustics.two_tone_separation_frequency(120.0) > base
    assert psychoacoustics.two_tone_separation_frequency(600.0) > base
...
def test_resolve_separately_uses_more_prominent_tone() -> None:
    ...
    d_lo = psychoacoustics.two_tone_separation_frequency(f1)  # ≈ 23.02 Hz
    d_hi = psychoacoustics.two_tone_separation_frequency(f2)  # ≈ 23.91 Hz
    assert d_lo < abs(f2 - f1) < d_hi
    assert (
        psychoacoustics.resolve_tones_separately(f1, f2, 5.0, 3.0) is True
    )  # fT = 300, thr low
    assert (
        psychoacoustics.resolve_tones_separately(f1, f2, 3.0, 5.0) is False
    )  # fT = 323.1, thr hi
...
# assess_tones result
# ---------------------------------------------------------------------------
def _annex_e_result() -> psychoacoustics.ToneAudibilityResult:
...
def test_assess_tones_audible_mask() -> None:
    ...
    quiet = psychoacoustics.assess_tones([200.0], [40.0], [50.0], 2.7)
...
def test_table_e2_fg_uncertainty() -> None:
    ...
    u = psychoacoustics.audibility_uncertainty(
...
def test_table_e3_decisive_audibilities_and_mean() -> None:
    ...
    assert psychoacoustics.mean_audibility(decisives) == pytest.approx(
...
def test_din_anhang_i_decisive_tone_from_spectrum() -> None:
    ...
    lt = psychoacoustics.tone_level(
        ref.DIN45681_I9_LEVELS, ref.DIN45681_I9_FREQUENCIES, ft, ls
    )
    ...
    assert psychoacoustics.masking_index(ft) == pytest.approx(av_p, abs=0.01)
    assert psychoacoustics.tone_audibility(lt, ls, ft, df) == pytest.approx(
        dl_p, abs=0.03
    )
...
def test_din_anhang_i_decisive_tone_uncertainty() -> None:
    ...
    u = psychoacoustics.audibility_uncertainty(
        lev[16:20], lev[kept], ft, ref.DIN45681_LINE_SPACING
    )
...
def test_analyze_spectrum_step3_merges_chained_clusters() -> None:
    ...
    result = psychoacoustics.analyze_spectrum(lev, freq, df)
    assert result.group_sizes is not None
    fg = result.group_sizes > 1
    assert int(np.sum(fg)) == 1
    assert int(result.group_sizes[fg][0]) == 3
