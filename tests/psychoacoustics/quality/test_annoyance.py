...
def test_pa_matches_hand_worked_tuple() -> None:
    ...
    res = psychoacoustics.psychoacoustic_annoyance(n5, s, f, r)
...
def test_pa_reduces_to_loudness_without_terms() -> None:
    ...
    res = psychoacoustics.psychoacoustic_annoyance(12.0, 1.5, 0.0, 0.0)
    assert res.w_s == 0.0
    assert res.w_fr == 0.0
...
def test_pa_sharpness_term_formula() -> None:
    res = psychoacoustics.psychoacoustic_annoyance(20.0, 3.0, 0.0, 0.0)
...
def test_pa_fluctuation_roughness_term_formula() -> None:
    res = psychoacoustics.psychoacoustic_annoyance(8.0, 1.0, 1.2, 0.7)
...
def test_pa_result_fields_and_plot() -> None:
    res = psychoacoustics.psychoacoustic_annoyance(15.0, 2.2, 0.4, 0.6)
    assert isinstance(res, psychoacoustics.PsychoacousticAnnoyanceResult)
    assert res.n5 == 15.0
...
def test_pa_from_signal_runs_and_is_positive() -> None:
    ...
    res = psychoacoustics.psychoacoustic_annoyance_from_signal(x, fs)
    assert isinstance(res, psychoacoustics.PsychoacousticAnnoyanceResult)
