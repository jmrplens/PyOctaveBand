...
1. **Exact limits against already-anchored code.** As the frame is made
   ...
   on :func:`~phonometry.materials.porous_absorber.limp_frame`. Both are tested
...
2. **Two independent derivations of the same physics.** The chapter 6 closed
   ...
   :func:`~phonometry.materials.biot.biot_waves` and agree to machine
   precision.
...
def test_global_assembly_reproduces_the_chapter_6_closed_form() -> None:
    ...
    :func:`~phonometry.materials.biot.biot_waves`, so agreement at machine
...
def test_rigid_frame_limit_converges_on_the_jca_equivalent_fluid(
    ...
    :class:`~phonometry.materials.porous_absorber.PorousLayer` builds from a
    :func:`~phonometry.materials.porous_absorber.johnson_champoux_allard`
