...
def body_waves() -> ElasticFDTDResult:
    ...
    return elastic_fdtd_simulation(
        CP_AL, CS_AL, 0.002, 2.2e-4, rho=RHO_AL, shape=(501, 501),
        sources=[
            ExplosionSource(ix=250, iy=250,
...
def test_flexural_dispersion_of_thin_plate_strip() -> None:
    ...
    sim = ElasticFDTD2D(CP_ST, CS_ST, dx, rho=RHO_ST, shape=(ny, nx),
                        free_sides=("top", "bottom"), damping=sig)
    w = 2e-4
    src_ix = 400
    sim.add_source(ForceSource(
        ix=src_ix, iy=4, direction="y", amplitude=1e6,
        waveform=GaussianPulse(0, 0, width=w).value))
    ...
    for k, ix in enumerate(probe_ix):
        d = (ix - src_ix) * dx
        gate = (np.clip((times - (t0 + d / 2600.0)) / 3e-4, 0.0, 1.0)
                * np.clip(((t0 + d / 700.0 + 2.0 * w) - times) / 3e-4,
                          0.0, 1.0))
        gated[k] = rec[k] * gate
    ...
    wraps = round(((-omega[k_ref] * distance / c_corrected[k_ref])
                   - phase[k_ref]) / (2.0 * np.pi))
...
def test_immersed_plate_reproduces_normal_incidence_mass_law() -> None:
    ...
    def run(with_plate: bool) -> np.ndarray:
        ...
        sim = ElasticFDTD2D(c_p, c_s, dx, rho=rho)
        y = (np.arange(ny) + 0.5) * dx
        p0 = (np.exp(-(((y - 0.6) / 0.06) ** 2))
              * np.sin(2.0 * np.pi * f0 * (y - 0.6) / CP_W))
        sim.txx[:] = -p0[:, np.newaxis]
        sim.tyy[:] = -p0[:, np.newaxis]
        y_face = np.arange(1, ny) * dx + 0.5 * CP_W * sim.dt
        v0 = (np.exp(-(((y_face - 0.6) / 0.06) ** 2))
    ...
    tl_meas = 20.0 * np.log10(np.abs(reference).max()
                              / np.abs(through).max())
    tl_exact = float(mass_law_transmission_loss(
        f0, RHO_ST * 0.003, incidence="normal",
        speed_of_sound=CP_W, air_density=RHO_W))
...
def test_mu_zero_everywhere_reproduces_acoustic_fdtd() -> None:
    ...
    acoustic = FDTD2D(c, 0.01, rho=rho, sponge_width=10,
                      sponge_sides=("left", "top"), obstacle_mask=mask)
    elastic = ElasticFDTD2D(c, 0.0, 0.01, rho=rho, sponge_width=10,
                            sponge_sides=("left", "top"),
                            obstacle_mask=mask)
    assert elastic.dt == acoustic.dt
    width = 8 * acoustic.dt
    acoustic.add_source(GaussianPulse(ix=20, iy=20, width=width))
    elastic.add_source(ExplosionSource(
        ix=20, iy=20, waveform=GaussianPulse(0, 0, width=width).value))
    for _ in range(400):
        acoustic.step()
        elastic.step()
    ...
    np.testing.assert_array_equal(elastic.txy,
                                  np.zeros_like(elastic.txy))
...
def test_energy_is_conserved_in_a_closed_domain() -> None:
    ...
    sim.add_source(ExplosionSource(
        ix=60, iy=60, waveform=GaussianPulse(0, 0, width=1e-6).value))
    sim.add_source(ForceSource(
        ix=40, iy=40, direction="x", amplitude=100.0,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"c_p": 6000.0, "c_s": -1.0, "shape": (10, 10)},
         "c_s must be non-negative"),
        ({"c_p": 6000.0, "c_s": 3000.0, "shape": (10, 10),
          "rho": np.ones((9, 10)) * 2700.0}, "rho map must match"),
        ({"c_p": 6000.0, "c_s": 3000.0, "shape": (10, 10), "rho": 0.0},
         "rho must be strictly positive"),
        ({"c_p": 6000.0, "c_s": 3000.0, "shape": (10, 10), "cfl": 1.0},
         "cfl must lie in"),
        ({"c_p": 6000.0, "c_s": 3000.0, "shape": (1, 10)},
         "at least 2 x 2"),
        ({"c_p": 6000.0, "c_s": 3000.0, "shape": (10, 10),
          "free_sides": "north"}, "unknown free sides"),
        ({"c_p": 6000.0, "c_s": 3000.0, "shape": (10, 10),
          "free_sides": "top", "sponge_width": 3}, "both free and absorbing"),
        ({"c_p": 6000.0, "c_s": 3000.0, "shape": (10, 10),
...
def test_lambda_zero_medium_is_accepted() -> None:
    ...
    c_s = 3000.0
    sim = ElasticFDTD2D(float(np.sqrt(2.0)) * c_s, c_s, 0.01, rho=2700.0,
                        shape=(10, 10))
...
def test_source_validation() -> None:
    ...
    mask[8:12, 8:12] = True
    sim = ElasticFDTD2D(6000.0, 3000.0, 0.01, rho=2700.0, shape=(20, 20),
                        obstacle_mask=mask)
...
def test_simulation_rejects_invalid_arguments(
    ...
    full: dict[str, object] = {
        ...
        "sources": [ExplosionSource(
            ix=5, iy=5, waveform=GaussianPulse(0, 0, width=1e-4).value)],
        **kwargs,
    }
    duration = full.pop("duration")
    with pytest.raises(ValueError, match=match):
        elastic_fdtd_simulation(6000.0, 3000.0, 0.05, duration,
                                rho=2700.0, shape=(30, 30), **full)
...
def _small_run(**kwargs: object) -> ElasticFDTDResult:
    ...
    return elastic_fdtd_simulation(
        6000.0, 3000.0, 0.02, 2e-3, rho=2700.0, shape=(40, 60),
...
def test_orthogonal_free_sides_pin_both_corner_stresses() -> None:
    ...
    sim = ElasticFDTD2D(6000.0, 3000.0, 0.01, rho=2700.0, shape=(40, 40),
...
def test_plot_probes(small_result: ElasticFDTDResult) -> None:
    ...
    np.testing.assert_allclose(lines[0].get_xdata(),
