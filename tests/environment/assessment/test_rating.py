...
def test_ldn_constant_level_analytic() -> None:
    ...
    assert environment.ldn(60.0, 60.0) == pytest.approx(
        60.0 + expected_offset, abs=1e-9
    )
...
def test_composite_rating_level_formula5() -> None:
    ...
    got = environment.composite_rating_level([(l_rd, d, k_d), (l_rn, n, k_n)])
    assert got == pytest.approx(expected, abs=1e-12)


def test_composite_generalizes_lden() -> None:
    ld, le, ln_ = 63.0, 58.0, 51.0
    composite = environment.composite_rating_level(
        [(ld, 12, 0.0), (le, 4, 5.0), (ln_, 8, 10.0)]
    )
    assert composite == pytest.approx(environment.lden(ld, le, ln_), abs=1e-12)
