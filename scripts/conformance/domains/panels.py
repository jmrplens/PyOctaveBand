...
def _chk_mass_law_octave_slope() -> Outcome:
    lo = float(ph.building.mass_law_transmission_loss(500.0, 20.0, incidence="normal"))
    hi = float(ph.building.mass_law_transmission_loss(1000.0, 20.0, incidence="normal"))
...
def _chk_mass_law_mass_slope() -> Outcome:
    lo = float(ph.building.mass_law_transmission_loss(500.0, 20.0, incidence="normal"))
    hi = float(ph.building.mass_law_transmission_loss(500.0, 40.0, incidence="normal"))
    return numeric(6.0206, hi - lo, 0.01, unit="dB")


@register(
    _PANEL, "Bies 5e Eq. 7.42 (field incidence)", "One-third-octave correction 5.5 dB"
)
def _chk_field_incidence_correction() -> Outcome:
    n = float(ph.building.mass_law_transmission_loss(500.0, 20.0, incidence="normal"))
    fld = float(ph.building.mass_law_transmission_loss(500.0, 20.0, incidence="field"))
    return numeric(5.5, n - fld, 0.001, unit="dB")


@register(
    _PANEL, "Hopkins Eq. 2.201 / Bies Eq. 7.3", "Coincidence frequency, 6 mm glass"
)
...
def _chk_radiation_above_coincidence() -> Outcome:
    ...
    return numeric(1.0 / math.sqrt(1.0 - 0.5), float(res.radiation_efficiency[0]), 1e-9)


@register(
    _PANEL,
    "Bies Eq. 7.62 / Hopkins Eq. 4.73",
...
@register(
    _PANEL, "Bies Eq. 7.64 (double wall)", "Below f0 = mass law of the combined mass"
)
...
@register(_PANEL, "Hopkins Eq. 4.92 (composite)", "1 % open area caps R at 10 lg(S/Sa)")
def _chk_composite_open_area_limit() -> Outcome:
    r = float(ph.building.composite_transmission_loss([0.99, 0.01], [60.0, 0.0]))
    return numeric(10.0 * math.log10(1.0 / 0.01), r, 0.05, unit="dB")


@register(
    _PANEL,
    "Vigran Building Acoustics Eq. (3.109), printed p. 96",
...
@register(
    _PANEL,
    "Vigran Eqs. (3.113)/(3.115), printed p. 96",
...
def _chk_heckl_coincidence_constant() -> Outcome:
    ...
    tl = float(
        ph.building.orthotropic_transmission_loss(
            [f],
            mass,
...
@register(
    _PANEL,
    "Bies 5e Eq. (7.60) / Vigran Eq. (6.112)",
...
@register(
    _PANEL,
    "Hopkins Table A2, printed p. 608",
...
def _chk_hopkins_table_a2_products() -> Outcome:
    ...
    for c_l, product in ref.HOPKINS_TABLE_A2_H_FC:
        b = ph.vibration.plate_bending_stiffness(rho * c_l**2 * (1.0 - nu**2), h, nu)
        worst = max(
            worst,
...
@register(
    _PANEL,
    "Hopkins Eq. 4.99/4.101 (Gomperts slit)",
...
def _chk_slit_resonance() -> Outcome:
    ...
    res = ph.building.slit_transmission_coefficient(f, 0.005, 0.1, field="normal")
