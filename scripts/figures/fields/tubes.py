...
def _anim_tube_hardware(
    ...
    *,
    ...
    _anim_speaker(
        ax, 0.0, 0.5 * bore, bore, label_y=-0.55 * bore if label_speaker else None
    )
...
def animate_fdtd_impedance_tube(output_dir: str) -> None:
    ...
    fig = _anim_figure()
    fig.suptitle(
        T("The virtual impedance tube: standing waves read the absorption (2D FDTD)")
    )
    axes = fig.subplots(2, 1, sharex=True)
    titles = [
        T(r"Rigid end: deep minima, $|r| \sim 1$"),
    ...
    a_txt = axes[1].text(
        ...
        color="white",
        zorder=7,
        bbox={"boxstyle": _ANIM_PILL_BOX, "facecolor": "#737373", "edgecolor": "none"},
    )
    t_txt = fig.text(
        0.988,
        0.93,
    ...
    def update(k: int) -> tuple[Any, ...]:
        ...
        for im, line, (p_all, env) in zip(ims, lines, ((p_e, env_e), (p_s, env_s))):  # noqa: B905
            ...
            line.set_data(
                x_env[env_from:], env_base + env_row[env_from:] / env_max * env_h
            )
        a_txt.set_text(
            T(f"$\\alpha$ = {alpha:.2f} at {_VTUBE_F:.0f} Hz") if k >= reveal else ""
        )
    ...
    _render_clip(
        ...
        update,
        output_dir,
        "anim_fdtd_impedance_tube",
        frames=n_active + _VTUBE_HOLD,
        fps=_VTUBE_FPS,
        gif_fps=8,
    )
...
def _transmission_tube_fields(
    ...
    for with_sample in (False, True):
        ...
        sim.add_plane_wave(
            "right", center=0.35, width=0.10, wavelength=343.0 / _VTUBE_F
        )
        ps: list[Any] = []
        ts: list[float] = []
    ...
    rc = 1.2 * 343.0
    combo = (
        np.cos(kd) + 1j * z2 * np.sin(kd) / rc + rc * 1j * np.sin(kd) / z2 + np.cos(kd)
    )
...
def animate_fdtd_transmission_tube(output_dir: str) -> None:
    ...
    length, bore = 1.6, 0.1
    fig = _anim_figure()
    fig.suptitle(T("The virtual transmission tube: what gets through (2D FDTD)"))
    axes = fig.subplots(2, 1, sharex=True)
    titles = [
        T("Empty tube: the packet crosses unchanged"),
    ...
    def update(k: int) -> tuple[Any, ...]:
        ...
        tl_txt.set_text(
            T(f"TL = {tl:.1f} dB at {_VTUBE_F:.0f} Hz") if k >= reveal else ""
        )
...
def animate_standing_wave_tube(output_dir: str) -> None:
    ...
    fig = _anim_figure()
    fig.suptitle(
        T("Standing wave in the impedance tube (ISO 10534-2)"),
    )
    ...
    for row, (title, refl) in enumerate(cases):
        ...
        for yw in (-1.0, 1.0):
            ax.plot(
                [x0 - 0.1, xs + (0.8 if refl < 1.0 else 0.0)],
                [yw, yw],
                color=COLOR_FG,
                lw=1.8,
            )
        ...
        env = amp * np.sqrt(1.0 + refl**2 + 2.0 * refl * np.cos(2.0 * k * (xs - x)))
        env_lines = [
            ax.plot(x, s * env, color=COLOR_FG, lw=1.0, ls="--", alpha=0.0)[0]
            for s in (1.0, -1.0)
        ]
        (mic_dots,) = ax.plot(
            [], [], marker="o", ms=5, ls="none", color=COLOR_SECONDARY
        )
        # left of the shared legend row, clear of the bottom panel's strip
        note = ax.text(
            2.5, -1.42, "", ha="center", va="top", color=COLOR_FG, fontsize=9
        )
        readout = ax.text(
            13.9,
            1.7,
    ...
    def update(kf: int) -> tuple[Any, ...]:
        ...
        for pn in panels:
            ...
            if a_env > 0.0:
                ...
                pn["readout"].set_text(
                    T(f"$|R|$ = {refl:.2f}   $\\alpha$ = {1.0 - refl**2:.2f}")
                )
