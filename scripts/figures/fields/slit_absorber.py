...
def _lossy_fluid_params(rho_c: complex, kappa_c: complex) -> tuple[float, float, float]:
...
def _slit_absorber_fields(
    ...
    neck_f = _lossy_fluid_params(
        *(p.item() for p in rectangular_duct_properties(freq, side=res.neck_side))
    )
    cav_f = _lossy_fluid_params(
        *(p.item() for p in rectangular_duct_properties(freq, side=res.cavity_side))
    )
    ...
    for h in (h0, _SLIT_ABS_DETUNE * h0):
        slit_f = _lossy_fluid_params(
            *(p.item() for p in slit_effective_properties(freq, slit_height=h))
        )
...
def animate_fdtd_slit_absorber(output_dir: str) -> None:
    ...
    fig = _anim_figure()
    fig.suptitle(T("Slow-sound slit absorber at critical coupling (2D FDTD)"))
    gs = fig.add_gridspec(2, 2, width_ratios=[1.55, 0.52])
    titles = [
        T("Critically coupled slit: the wave dies inside"),
    ...
    for row, (title, h) in enumerate(zip(titles, heights, strict=True)):
        ...
        x_mouth = _SLIT_ABS_TUBE + 0.5 * _SLIT_ABS_LATTICE
        x_neck = (x_mouth - 0.5 * res.neck_side, x_mouth + 0.5 * res.neck_side)
        x_cav = (x_mouth - 0.5 * res.cavity_side, x_mouth + 0.5 * res.cavity_side)
        body = [
    ...
    def fit_resonator_labels() -> None:
        for helm in helm_txts:
            _fit_text_x(
                fig, helm.axes, helm, x_zoom0, x_end, margin=0.05 * (x_end - x_zoom0)
            )
...
def _anim_slit_tube_walls(
    ...
    _anim_speaker(ax, 0.0, 0.5 * bore, bore, label_y=-0.30 * bore if speaker else None)
