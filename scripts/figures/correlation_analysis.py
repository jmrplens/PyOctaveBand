...
def generate_gcc_phat_delay(output_dir: str) -> None:
    ...
    phat = signals.time_delay(
        x, y, fs, method="gcc", weighting="phat", nperseg=2048, max_delay=0.01
    )
    ...
    ax.axvline(
        ...
        linewidth=1.4,
        alpha=0.7,
        label="True delay (20 samples)",
    )
    ax.set_xlabel("Lag [ms]")
    ax.set_ylabel("Normalized correlation")
    ax.set_title(
        "Time-Delay Estimation: GCC-PHAT vs Direct Correlation (Knapp & Carter)", pad=12
    )
...
def generate_cepstrum_echo(output_dir: str) -> None:
    ...
    half = res.nfft // 2 + 1
    ax.plot(
        1e3 * res.quefrencies[:half],
        res.cepstrum[:half],
        color=COLOR_PRIMARY,
        linewidth=1.1,
        label="Power cepstrum",
    )
    ax.axvspan(
        1e3 * res.search_range[0],
        1e3 * res.search_range[1],
    ...
    ax.axvline(
        ...
        linewidth=1.3,
        alpha=0.7,
        label="True echo delay (8 ms)",
    )
    ax.plot(
        [1e3 * res.delay],
        [res.reflection_coefficient],
        "v",
        color=COLOR_SECONDARY,
        markersize=10,
    ...
    ax.set_title("Echo Detection on the Power Cepstrum (Quefrency Analysis)", pad=12)
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
...
def generate_envelope_spectrum(output_dir: str) -> None:
    ...
    x = a0 * (1.0 + m * np.cos(2.0 * np.pi * fm * t)) * np.cos(2.0 * np.pi * 1000.0 * t)
    ...
    ax.set_title("Envelope Spectrum of an AM Tone (Bendat & Piersol 13.3)", pad=12)
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
    ax.legend(loc="upper right", fontsize=9)
    ax.text(
...
def generate_synchronous_average(output_dir: str) -> None:
    ...
    t_ms = 1e3 * res.times
    ax0.plot(
        t_ms, signal[:m], color=COLOR_GRID, linewidth=1.0, label="One noisy period"
    )
    ax0.plot(
        t_ms,
        res.period_waveform,
    ...
    ax1.set_title("Rejecting a Tone by Choosing $N$ (McFadden 1987)", pad=10)
...
def generate_cepstrum_variants(output_dir: str) -> None:
    ...
    s = np.zeros(4096)
    s[37 : 37 + 256] = scipy_signal.lfilter(b, a_coef, np.r_[1.0, np.zeros(255)])
    ...
    ax.set_ylim(-0.3, 0.6)
    ax.set_title("The Three Cepstrum Variants of One Echo-Carrying Record", pad=12)
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
    ax.legend(loc="upper right", fontsize=9)
    plt.tight_layout()
...
def generate_lifter_split(output_dir: str) -> None:
    ...
    axes[1].semilogx(
        high.frequencies[band],
        high.liftered_db[band],
        color=COLOR_SECONDARY,
        linewidth=1.1,
    ...
    for bound in (20.0 * np.log10(1.5), 20.0 * np.log10(0.5)):
        axes[1].axhline(
            ...
            linewidth=1.2,
            label=(
                r"closed-form ripple bounds $20\log_{10}(1\pm a)$"
                if bound > 0
                else None
            ),
        )
...
def generate_correlation_normalizations(output_dir: str) -> None:
    ...
    coeff = signals.correlation(x, y, fs, normalization="coefficient", max_lag=0.05)
    ...
    for ax in (ax_c, ax_n):
        ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
        ax.set_axisbelow(True)
    ax_c.set_title("Correlation Normalizations of a Two-Sensor Delay Model", pad=12)
    plt.tight_layout()
...
def generate_ir_alignment(output_dir: str) -> None:
    ...
    _fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(
        t_ms, res.reference, color=COLOR_PRIMARY, linewidth=1.6, label="Reference IR"
    )
    ax.plot(
        t_ms,
        ir_b,
        ...
        linestyle="--",
        alpha=0.8,
        label="Measured IR (delayed 7.37 samples)",
    )
    ax.plot(
        t_ms,
        res.aligned[: t.size],
        ...
        linewidth=1.0,
        linestyle=":",
        label="Aligned IR (delay removed)",
    )
    ax.text(
        0.985,
        0.05,
    ...
    ax.set_title("Sub-Sample Impulse-Response Alignment", pad=12)
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
    ax.legend(loc="upper right", fontsize=9)
    plt.tight_layout()
...
def generate_hilbert_envelope(output_dir: str) -> None:
    ...
    ax_e.plot(t, res.signal, color=COLOR_PRIMARY, linewidth=0.6, label="Signal")
    ax_e.plot(
        res.times,
        res.envelope,
        color=COLOR_SECONDARY,
    ...
    for ax in (ax_e, ax_f):
        ...
        ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
        ax.set_axisbelow(True)
    ax_e.set_title("Hilbert Envelope and Instantaneous Frequency", pad=12)
    plt.tight_layout()
...
def generate_tsa_noise_reduction(output_dir: str) -> None:
    ...
    ax.loglog(
        ...
        linewidth=1.6,
        markersize=6,
        label="Measured RMS error of the average",
    )
    ax.loglog(
        counts,
        1.0 / np.sqrt(np.asarray(counts, dtype=np.float64)),
        color=COLOR_SECONDARY,
        linestyle="--",
        linewidth=1.4,
    ...
    ax.set_title(r"TSA Noise Reduction: the $\sqrt{N}$ Law (McFadden 1987)", pad=12)
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5, which="both")
    ax.set_axisbelow(True)
    ax.legend(loc="upper right", fontsize=9)
    plt.tight_layout()
