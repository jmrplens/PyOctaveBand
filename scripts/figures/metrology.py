...
def generate_calibration_stability(output_dir: str) -> None:
    ...
    unstable = (
        0.5 * (1 + 0.03 * np.sin(2 * np.pi * 2.0 * tt)) * np.sin(2 * np.pi * 1000 * tt)
    )
    ...
    ax.set_title("Calibration Tone Stability Check (IEC 60942:2017, 5.3.3)", pad=12)
...
def generate_calibration_narrowband_bias(output_dir: str) -> None:
    ...
    for snr in snr_db:
        ...
        with warnings.catch_warnings():
            ...
            err_rms.append(20 * np.log10(metrology.sensitivity(x, fs=fs) / s_true))
            err_nb.append(
                20 * np.log10(metrology.sensitivity(x, fs=fs, narrowband=True) / s_true)
            )
    ...
    ax.axvline(
        crossing, color=COLOR_SECONDARY, linestyle="--", linewidth=1.2, alpha=0.8
    )
    ax.annotate(
        f"below {crossing:.1f} dB SNR the estimator\nalone spends the "
        "whole class 1 limit",
        xy=(crossing, -limit),
        xytext=(crossing + 1.5, -0.95),
        fontsize=9,
        color=COLOR_SECONDARY,
        arrowprops={"arrowstyle": "->", "color": COLOR_SECONDARY, "lw": 1.1},
    )
    ax.set_xlim(0, 40)
    ax.set_ylim(-2.0, 0.5)
    ...
    ax.set_title(
        "Broadband noise in the calibrator take biases every later level", pad=12
    )
...
def generate_dbfs_versus_spl(output_dir: str) -> None:
    ...
    ax_l.semilogx(
        ...
        markersize=3.5,
        linewidth=1.8,
        label="dB SPL (factor from the calibrator)",
    )
    ax_l.semilogx(
        bands,
        dbfs + offset,
    ...
    ax_r.plot(
        axis_ms, band, color=COLOR_MUTED, linewidth=0.8, label="1 kHz band signal"
    )
    ax_r.axhline(
        drive,
        color=COLOR_FG,
    ...
    ax_r.set_title("mode='peak' reads the filter's onset transient", pad=10)
...
def generate_trend_test(output_dir: str) -> None:
    ...
    ax.set_title("Nonparametric Trend Test by Reverse Arrangements (B&P 4.5.2)", pad=12)
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
...
def generate_stationarity_test(output_dir: str) -> None:
    ...
    ax.set_title("Stationarity Test by Reverse Arrangements (B&P 10.3.1.1)", pad=12)
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
    ax.legend(loc="upper left", fontsize=9)
    ax.text(
...
def generate_rice_level_crossings(output_dir: str) -> None:
    ...
    res = metrology.level_crossing_rate(x, fs, levels=np.linspace(-3.5, 3.5, 29))

    _fig, ax = plt.subplots(figsize=(10, 6))
    order = np.argsort(res.levels)
    ax.plot(
        res.levels[order],
        res.rice_rates[order],
        color=COLOR_SECONDARY,
        linewidth=1.6,
    ...
    ax.set_title("Level-Crossing Rates of Bandlimited Gaussian Noise (Rice)", pad=12)
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
...
def generate_rice_peak_distribution(output_dir: str) -> None:
    ...
    ax.plot(
        ...
        color=COLOR_PRIMARY,
        linewidth=1.2,
        label="Empirical peak exceedance (0-2 kHz noise)",
    )
    ax.set_yscale("log")
    ax.set_xlim(-2.5, 4.5)
    ...
    ax.set_title("Peak-Height Distribution and the Irregularity Factor (Rice)", pad=12)
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
...
def generate_uncertainty(output_dir: str) -> None:
    ...
    def model(a: float, b: float, c: float, d: float) -> float:
        return a + b + c + d
    ...
    gauss = np.exp(
        -0.5 * ((grid - result.value) / result.combined_uncertainty) ** 2
    ...
    ax_m.set_title(
        f"$Y$ = {result.value:.2f} dB,  $U$ = {big:.2f} dB ($k$ = {k:.2f})", pad=10
    )
...
def generate_stationarity_glide_blind_spot(output_dir: str) -> None:
    ...
    sos = scipy_signal.butter(
        6, [lo / (fs / 2), hi / (fs / 2)], btype="band", output="sos"
    )
    ...
    axes[0].axhspan(
        lo,
        hi,
        color=theme_fill(COLOR_SECONDARY, axes[0]),
        zorder=0,
        label=f"the {lo:.0f}-{hi:.0f} Hz band of panel (c)",
    )
    ...
    axes[0].set_title(
        f"(a) The record: a {f0:.0f} Hz to {f1:.0f} Hz glide at constant amplitude",
        pad=8,
    )
    ...
    for ax, res, color, title in (
        ...
        ax.plot(idx, res.segment_values, "o-", color=color, linewidth=1.6, markersize=5)
        ax.set_xlabel("Segment index")
        ax.set_ylabel("Segment mean square")
        ...
        ax.text(
            0.98,
            0.08,
            f"segment values span {low:.4f} to {high:.4f}",
            transform=ax.transAxes,
            ha="right",
            fontsize=9,
            color=COLOR_MUTED,
...
def generate_rice_nongaussian_screen(output_dir: str) -> None:
    ...
    impulsive[rng.choice(n, 400, replace=False)] += 6.0 * rng.choice([-1.0, 1.0], 400)
    ...
    for record, color, marker, size, label in (
        ...
        (impulsive, COLOR_TERTIARY, "^", 4, r"Gaussian + sparse $6\,\sigma$ spikes"),
    ):
        res = metrology.level_crossing_rate(record, fs, levels=levels * np.std(record))
        ax_l.plot(
            res.levels / np.std(record),
            res.rice_rates,
            color=color,
            linestyle="--",
            linewidth=1.2,
    ...
    ax_l.set_title("Measured rates against each record's own Rice curve", pad=10)
    ...
    cutoffs = np.array(
        [2500.0, 4000.0, 6000.0, 8000.0, 10000.0, 13000.0, 16000.0, 19000.0, 22000.0]
    )
    factors = []
    for cut in cutoffs:
    ...
    ax_r.set_title("A floor 50 dB down, and $m_4$ weighting by $f^4$", pad=10)
    ax_r.grid(color=COLOR_GRID, linestyle="-", alpha=0.5)
    ax_r.set_axisbelow(True)
...
def generate_uncertainty_gum_vs_mc(output_dir: str) -> None:
    ...
    q_a = [
        ph.metrology.rectangular(0.0, 1.0, name="Dominant rectangular"),
        ph.metrology.Quantity(0.0, 0.1, name="Small Gaussian"),
    ]
    panels.append(
        (
            "A dominant rectangular input",
            lambda a, b: a + b,
            q_a,
            "Correction [dB]",
            None,
    ...
    panels.append(
        (
            "An output against a physical bound",
            lambda a: np.minimum(a, 1.0),
            q_c,
...
def generate_uncertainty_correlation(output_dir: str) -> None:
    ...
    u_i = 0.3
    pair = [
        ph.metrology.Quantity(0.0, u_i, name="Term 1"),
    ...
    _fig, (ax_l, ax_r) = plt.subplots(1, 2, figsize=(12.5, 5.4))
    ax_l.plot(
        rho, same, color=COLOR_PRIMARY, lw=2.0, label="sensitivities of the same sign"
    )
    ax_l.plot(
        rho,
        opposite,
    ...
    ax_r.annotate(
        f"+{100 * (linear / quad - 1):.0f} % understated",
        xy=(1, linear),
        xytext=(0.28, linear * 0.72),
        fontsize=10,
        color=COLOR_SECONDARY,
        arrowprops={"arrowstyle": "->", "color": COLOR_SECONDARY, "lw": 1.2},
    )
...
def generate_runs_test(output_dir: str) -> None:
    ...
    for ax, seq in zip(axes, sequences, strict=True):
        ...
        above = seq > median
        ax.plot(idx, seq, color="#7f7f7f", linewidth=0.7, alpha=0.7, zorder=1)
        ax.scatter(
            idx[above],
            seq[above],
            color=COLOR_PRIMARY,
        ...
        ax.axhline(
            ...
            linewidth=1.1,
            alpha=0.7,
            label="Sequence median",
        )
        verdict = "trend-free" if res.trend_free else "rejected"
        ax.set_title(
            f"$r$ = {res.statistic} runs, accept "
