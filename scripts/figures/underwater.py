...
def generate_ship_source_level(output_dir: str) -> None:
    ...
    ax.semilogx(
        ...
        linewidth=2.0,
        markersize=4,
        label=r"Source level $L_\mathrm{s}$",
    )
    ax.semilogx(
        freqs,
        res.radiated_noise_level,
        "s--",
        color=COLOR_SECONDARY,
        linewidth=1.6,
        markersize=3,
        alpha=0.8,
        label="Radiated noise level",
    )
    ax.set_xlabel("Frequency [Hz]")
    ax.set_ylabel("Level [dB re 1 µPa·m]")
    ax.set_title("Ship Equivalent Monopole Source Level (ISO 17208-2)", pad=12)
    ...
    twin.semilogx(
        freqs,
        res.surface_correction,
        ":",
        color=COLOR_TERTIARY,
        linewidth=2.0,
        label=r"Surface correction $\Delta L$",
    )
...
def generate_pile_driving(output_dir: str) -> None:
    ...
    _fig, (ax_w, ax_c) = plt.subplots(
        2, 1, figsize=(10, 7.2), gridspec_kw={"height_ratios": [1.4, 1.0]}
    )
    ...
    ax_w.plot(
        ...
        color=COLOR_SECONDARY,
        markersize=8,
        label=f"Peak = {res.peak_spl:.0f} dB re 1 µPa",
    )
    ax_w.set_xlabel("Time [ms]")
    ax_w.set_ylabel("Pressure [Pa]")
    ax_w.set_title("Percussive Pile-Driving Strike (ISO 18406)", pad=12)
...
def generate_underwater_propagation_loss(output_dir: str) -> None:
    ...
    ax.set_title("Underwater Propagation Loss (Francois–Garrison)", pad=12)
    ax.invert_yaxis()
...
def generate_weston_regimes(output_dir: str) -> None:
    ...
    for curve, onset, label, color, style in (
        ...
        ax.plot(
            res.range_m, shown, linestyle=style, linewidth=1.3, color=color, label=label
        )
    ax.plot(
        res.range_m,
    ...
    for boundary, name in (
        ...
        ax.axvline(
            boundary,
            color=theme_line(COLOR_SECONDARY, ax, quiet=0.6),
            linestyle="--",
            linewidth=0.9,
        )
        ax.annotate(
    ...
    ax.set_title("Weston Shallow-Water Propagation Regimes (Ainslie §9.1.1.2)", pad=12)
...
def generate_marine_mammal_weighting(output_dir: str) -> None:
    ...
    ax.set_title("Marine-Mammal Auditory Weighting (NMFS 2024, v3.0)", pad=12)
...
def generate_underwater_sound_speed(output_dir: str) -> None:
    ...
    ax.set_title("Sea-Water Sound-Speed Profile (UNESCO)", pad=12)
    ax.invert_yaxis()
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
    ax.legend(loc="lower left", fontsize=9)
...
def generate_numerical_propagation(output_dir: str) -> None:
    ...
    axes[2].plot(
        pe.ranges / 1000.0,
        pe.propagation_loss[zi],
        color=COLOR_SECONDARY,
        linewidth=0.8,
        alpha=0.7,
        label="Parabolic equation",
    )
    axes[2].invert_yaxis()
...
def generate_seawater_absorption(output_dir: str) -> None:
    ...
    styles = (
        ("francois-garrison", COLOR_PRIMARY, "-", 2.2),
    ...
    ax_a.set_title("Volume Absorption (10 °C, 35 ppt, 100 m)", pad=12)
    ...
    ax_r.text(14.0, 10.5, "±10 % of Francois-Garrison", fontsize=9, color=COLOR_FG)
    for name, color, ls, lw in styles[1:]:
        ax_r.semilogx(
            freqs,
            100.0 * (alpha[name] / ref - 1.0),
            ls,
            color=color,
            linewidth=lw,
            label=name,
    ...
    ax_r.set_title("Where Each Simplification Is Honest", pad=12)
...
def generate_sound_speed_models(output_dir: str) -> None:
    ...
    ax_d.set_title(f"Spread on This Profile: up to {spread:.1f} m/s", pad=12)
...
def generate_detection_range(output_dir: str) -> None:
    ...
    first = underwater.detection_range_from_curve(fom, ranges, pl, crossing="first")
    last = float(ranges[crossings[-1]])
    ax_m.annotate(
        f"first crossing: {first / 1000.0:.2f} km\n"
...
def generate_normal_modes(output_dir: str) -> None:
    ...
    ax.plot(
        freqs,
        2.0 * freqs * depth_m / 1500.0,
        color=COLOR_SECONDARY,
        linewidth=1.6,
        linestyle="--",
        label=r"$M = kD/\pi = 2fD/c$",
    )
    ax.set_xlabel("Frequency [Hz]")
...
def generate_sonar_budget(output_dir: str) -> None:
    ...
    fom_full = float(
        underwater.passive_sonar_equation(
            source_level=140.0,
            propagation_loss=np.array([50.0]),
            noise_level=60.0,
    ...
    for label, freq, law, r0, color, ls in cases:
        ...
        ax.plot(ranges / 1000.0, loss.pl, ls, color=color, linewidth=1.9, label=label)
    for fom, style, lab, dy in (
        (fom_full, "-", f"FOM = {fom_full:.1f} dB (DI 15)", -15),
        (fom_trim, ":", f"FOM = {fom_trim:.1f} dB (DI 7.5)", 8),
    ):
        ax.axhline(
            fom, color=COLOR_FG, linewidth=1.4, linestyle=style, alpha=0.8, label=lab
        )
        for k, (_label, freq, law, r0, color, _ls) in enumerate(cases):
            r50 = (
                float(
...
def generate_ray_turning_point(output_dir: str) -> None:
    ...
    colors = (COLOR_PRIMARY, COLOR_TERTIARY, COLOR_SECONDARY, "#8c8c8c", "#7b5ea7")
    for angle, color in zip(angles, colors, strict=True):
        rays = underwater.ray_trace(
            depths,
            speeds,
...
def generate_gaussian_beam_caustic(output_dir: str) -> None:
    ...
    filled = ax.contourf(
        ...
        field,
        levels=edges,
        colors=[tuple(rgba) for rgba in bands],
        extend="both",
    )
    ...
    for i in range(0, rays.ranges.shape[0], 2):
        ax.plot(
            rays.ranges[i], rays.depths[i], color=COLOR_MUTED, linewidth=0.7, zorder=3
        )
...
def generate_eigenray_arrivals(output_dir: str) -> None:
    ...
    touches = np.asarray(arr.surface_reflections + arr.bottom_reflections, dtype=int)
    ...
    sc = ax.scatter(
        ...
        cmap="viridis",
        vmin=1,
        vmax=int(touches.max()),
        s=52,
        zorder=4,
        edgecolors=COLOR_FG,
        linewidths=0.6,
    )
    cbar = fig.colorbar(sc, ax=ax, pad=0.02, ticks=np.arange(1, int(touches.max()) + 1))
    cbar.set_label("Boundary reflections")
    ax.vlines(t_ms[direct], base, loss[direct], color=COLOR_PRIMARY, lw=2.0, zorder=4)
    ax.plot(t_ms[direct], loss[direct], "o", color=COLOR_PRIMARY, ms=8, zorder=5)
...
def generate_pe_paraxial_error(output_dir: str) -> None:
    ...
    def _range_average(pl: "np.ndarray", width: int = 61) -> "np.ndarray":
        ...
        return -10.0 * np.log10(np.convolve(10.0 ** (-pl / 10.0), kernel, mode="same"))
    ...
    ax_pl.plot(
        ranges / 1000.0,
        nm,
        color=theme_line(COLOR_PRIMARY, ax_pl, quiet=0.35),
        linewidth=0.7,
    )
    ax_pl.plot(
        ranges / 1000.0,
        pe,
        color=theme_line(COLOR_SECONDARY, ax_pl, quiet=0.35),
        linewidth=0.7,
    )
    ax_pl.plot(
        ranges[inner] / 1000.0,
        ...
        color=COLOR_PRIMARY,
        linewidth=2.2,
        label="Normal modes, range-averaged (reference)",
    )
    ax_pl.plot(
        ranges[inner] / 1000.0,
        sm_pe[inner],
        color=COLOR_SECONDARY,
        linewidth=2.2,
        label="Parabolic equation, range-averaged",
    )
    ax_pl.set_ylim(30.0, 92.0)
    ax_pl.invert_yaxis()
    ax_pl.set_xlabel("Range [km]")
    ax_pl.set_ylabel("Propagation loss [dB]")
    ax_pl.set_title("50 Hz in 100 m of Water, Receiver at 60 m", pad=12)
    ...
    ax_ang.set_title(
        f"{int(np.sum(~inside))} of {grazing.size} Modes Are Outside It", pad=12
    )
...
def generate_marine_mammal_audiograms(output_dir: str) -> None:
    ...
    ax_g.set_title("Southall et al. (2019) Group Audiograms", pad=12)
    ...
    ax_o.semilogx(
        ...
        color=COLOR_PRIMARY,
        linewidth=2.0,
        label="orca_audiogram (three branches)",
    )
    ...
    ax_o.plot(
        ...
        markersize=7,
        zorder=5,
        label=f"third branch at 50 kHz: {at_50k:.1f} dB",
    )
    ...
    ax_o.set_title("Killer Whale (Ainslie 2010, Eq. 11.159)", pad=12)
...
def generate_marine_mammal_assessment(output_dir: str) -> None:
    ...
    axes[0].set_title("Step 1: Single-Strike SEL by Band", pad=12)
...
def generate_marine_mammal_exposure_functions(output_dir: str) -> None:
    ...
    palette = (COLOR_PRIMARY, COLOR_SECONDARY, COLOR_TERTIARY, "#8c8c8c", "#7b5ea7")
    ...
    for group, color in zip(groups, palette, strict=True):
        ...
        ax.semilogx(res.frequencies, exposure, color=color, linewidth=1.8, label=group)
        imin = int(np.argmin(exposure))
        ax.plot(
            [res.frequencies[imin]],
            [exposure[imin]],
            "o",
            color=color,
            markersize=5,
    ...
    p = lf.parameters
    ax.annotate(
        "each minimum is that group's weighted TTS onset "
    ...
    ax.set_title("What a Band Level Is Compared Against", pad=12)
    ...
    ax.set_title("What $b$ = 5 Changed for LF Cetaceans", pad=12)
    ...
    ax.set_title("Impulsive Onset Criteria (NMFS 2024)", pad=12)
...
def generate_piling_campaign_accumulation(output_dir: str) -> None:
    ...
    fs = 48_000
    spectrum = underwater.strike_sel_spectrum(_piling_strike(fs), fs, fraction=3)
    ...
    for group, color in zip(groups, palette, strict=True):
        ...
        levels = [
            (value, style)
            for value, style in ((crit.tts_sel, ":"), (crit.injury_sel, "--"))
            if value is not None
        ]
        for level, ls in levels:
            ax.axhline(level, color=color, linewidth=0.9, linestyle=ls, alpha=0.7)
    ...
    ax.set_title(
        "Accumulation Against the Criteria (dotted TTS, dashed AUD INJ)", pad=12
    )
    ax.grid(which="both", color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
