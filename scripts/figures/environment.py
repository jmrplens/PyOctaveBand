...
def generate_lden_profile(output_dir: str) -> None:
    ...
    ax.step(
        np.r_[hours, 24],
        np.r_[laeq_h, laeq_h[-1]],
        where="post",
        color=COLOR_FG,
        linewidth=1.6,
        label=r"Hourly $L_{\mathrm{Aeq}}$",
    )
    ax.hlines(
        ld,
        7,
    ...
    ax.set_title(r"Day-Evening-Night Level $L_{\mathrm{den}}$ (ISO 1996-1)", pad=12)
    ax.set_xlim(0, 24)
    ax.set_ylim(42, 80)
...
def generate_wind_turbine_tonality(output_dir: str) -> None:
    ...
    res = environment.wind_turbine_tonality(levels, freqs, tone_frequency=200.0)

    _fig, ax = plt.subplots(figsize=(10, 6))
    band_lo, band_hi = _critical_band_edges(res.tone_frequency)
    ax.axvspan(
        band_lo, band_hi, color=COLOR_TERTIARY, alpha=0.15, label="Critical band"
    )
    ax.plot(
        freqs, levels, color=COLOR_PRIMARY, linewidth=1.0, label="Narrowband spectrum"
    )
    ax.axhline(
        res.masking_level,
        color="#ff7f0e",
    ...
    ax.plot(
        ...
        color=COLOR_SECONDARY,
        markersize=9,
        label=f"Tone = {res.tone_level:.1f} dB",
    )
    ax.set_xlabel("Frequency [Hz]")
    ax.set_ylabel("Level [dB]")
    ax.set_title("Wind-Turbine Tonal Audibility (IEC 61400-11)", pad=12)
    ...
    ax.text(
        0.02,
        0.95,
        "Tonal audibility $\\Delta L_{\\mathrm{a}}$ = "
...
def generate_air_absorption_alpha(output_dir: str) -> None:
    ...
    ax.set_title(r"ISO 9613-1 Atmospheric Absorption $\alpha(f)$", pad=12)
...
def generate_cnossos_road_emission(output_dir: str) -> None:
    ...
    ax.bar(
        x,
        result.total_line_power,
        color=COLOR_MUTED,
        edgecolor=COLOR_FG,
        linewidth=0.6,
        label="Total source line",
        zorder=2,
    )
    labels = {
        "1": "Light vehicles (1)",
...
def generate_cnossos_road_speed_law(output_dir: str) -> None:
    ...
    for category, color, name in [
        ...
        rolling = np.array(
            [a_weighted(environment.road_rolling_noise(category, v)) for v in speeds]
        )
        propulsion = np.array(
            [a_weighted(environment.road_propulsion_noise(category, v)) for v in speeds]
        )
        total = np.array(
            [
    ...
    ax.set_ylabel(r"A-weighted sound power $L_{W\!,\mathrm{A}}$ [dB(A) re 1 pW]")
...
def generate_ground_effect_spherical(output_dir: str) -> None:
    ...
    ax.axhline(0.0, color=COLOR_FG, linewidth=0.8, alpha=0.6)
    ax.set_xscale("log")
    ax.set_title("Spherical-Wave Ground Effect (Weyl-Van der Pol)", pad=12)
    ax.set_xlabel("Frequency [Hz]")
...
def generate_atmospheric_refraction(output_dir: str) -> None:
    ...
    axes[0].plot(
        [0.0], [zs], "o", color=COLOR_SECONDARY, ms=7, zorder=4, label="Source"
    )
    ...
    fig.suptitle(
        "Atmospheric Refraction: Ray Bending and the Acoustic Shadow", fontsize=13
    )
    plt.tight_layout()
...
def generate_impulse_prominence(output_dir: str) -> None:
    ...
    ax_p.set_title(
        r"$P = 3\,\log_{10}(\mathrm{OR}) + 2\,\log_{10}(\mathrm{LD})$", pad=10
    )
    ...
    ax_k.plot(
        grid,
        environment.impulse_adjustment(grid),
        color=COLOR_PRIMARY,
    ...
    ax_k.set_ylabel(r"Adjustment $K_{\mathrm{I}}$ [dB]")
    ax_k.set_title(r"Adjustment to $L_{\mathrm{Aeq}}$", pad=10)
...
def generate_tonal_audibility(output_dir: str) -> None:
    ...
    assessed = [
        environment.assess_tonal_audibility(lpt, lpn, fc) for lpt, lpn, fc in examples
    ]
    ...
    ax.scatter(
        [a.audibility for a in assessed],
        [a.adjustment for a in assessed],
        color=COLOR_SECONDARY,
        marker="o",
        s=70,
        zorder=6,
    ...
    ax.scatter(
        ...
        s=150,
        zorder=7,
        label="mid-range tone",
    )

    ax.set_xlabel(r"Tonal audibility $\Delta L_{\mathrm{ta}}$ [dB]")
    ax.set_ylabel(r"Tonal adjustment $K_{\mathrm{t}}$ [dB]")
    ax.set_ylim(-0.3, 6.6)
...
def generate_atmospheric_sound_speed_profiles(output_dir: str) -> None:
    ...
    ax.plot(
        ...
        color=COLOR_PRIMARY,
        linewidth=2.0,
        label="Downward refraction ($b$ = +1 m/s)",
        zorder=3,
    )
    ax.plot(
        up.sound_speeds,
        ...
        color=COLOR_SECONDARY,
        linewidth=2.0,
        label="Upward refraction ($b$ = −1 m/s)",
        zorder=3,
    )
    ...
    ax.set_title("Effective Sound-Speed Profiles (Salomons Eq. 4.5)", pad=12)
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
...
def generate_atmospheric_ray_fan(output_dir: str) -> None:
    ...
    ax.plot([0.0], [zs], "o", color=COLOR_SECONDARY, ms=7, zorder=4, label="Source")
    ax.axhline(0.0, color=COLOR_FG, lw=1.0, alpha=0.7)
    ax.set_xlabel("Range [m]")
    ax.set_ylabel("Height [m]")
    ax.set_xlim(0.0, 600.0)
    ax.set_ylim(0.0, 40.0)
    ax.set_title("Sound Rays under Downward Refraction ($b$ = +1 m/s)", pad=12)
    ax.text(
        0.985,
        0.94,
...
def generate_atmospheric_pe_range(output_dir: str) -> None:
    ...
    x_sh = environment.shadow_zone_distance(float(grad), zs, zr, ground_speed=c0)
    ax.axvline(
        x_sh,
        color=COLOR_SECONDARY,
        ls=":",
    ...
    ax.set_title("GFPE Relative Level at the Receiver Height (400 Hz, 2 m)", pad=12)
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
    ax.legend(loc="lower left", fontsize=9)
    plt.tight_layout()
...
def generate_barrier_insertion_loss_methods(output_dir: str) -> None:
    ...
    ax.plot(
        freqs,
        il_ka.insertion_loss,
        color=COLOR_TERTIARY,
        lw=1.8,
        ls="--",
        label="Kurze-Anderson (thin screen)",
        zorder=3,
    )
    ax.plot(
        freqs,
        il_ex.insertion_loss,
        color=COLOR_PRIMARY,
        lw=1.8,
        label="Exact rigid half-plane",
        zorder=3,
    )
    ax.plot(
        freqs,
        il_gr.insertion_loss,
        color=COLOR_SECONDARY,
        lw=1.4,
        alpha=0.9,
        label="Exact + coherent ground (four paths)",
        zorder=2,
    )
    ax.axhline(
        5.0,
    ...
    ax.set_title("Wave-Theoretic Barrier Insertion Loss", pad=12)
    ax.grid(which="both", color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
    ax.legend(loc="upper left", fontsize=9)
    plt.tight_layout()
...
def generate_cnossos_rail_roughness_shift(output_dir: str) -> None:
    ...
    for speed, color, marker in (
        (60.0, COLOR_PRIMARY, "o"),
        (160.0, COLOR_TERTIARY, "s"),
...
def generate_outdoor_level_cascade(output_dir: str) -> None:
    ...
    ax.plot(
        ...
        linewidth=2.0,
        markersize=6,
        label=r"$L_{fT}(\mathrm{DW})$ at the receiver",
        zorder=5,
    )
    ax.axhline(0.0, color=COLOR_MUTED, linewidth=0.9, zorder=1)
    ax.set_title("Where 95 dB of Source Power Goes (ISO 9613-2, 200 m)", pad=12)
    ax.set_xticks(x)
    ax.set_xticklabels([f"{b:g}" for b in bands])
    ax.set_xlabel(LABEL_FREQ_HZ)
    ax.set_ylabel("Level [dB]")
...
def generate_iso9613_screening_anatomy(output_dir: str) -> None:
    ...
    for h in heights:
        ...
        single.append(
            float(
                environment.barrier_attenuation(
        ...
        double.append(
            float(
                environment.barrier_attenuation(
                    environment.Barrier(
                        source_to_edge=leg, edge_to_receiver=leg, edge_separation=2.0
                    ),
                    200.0,
                    [500.0],
                )[0]
    ...
    left.axhline(
        ...
        linestyle=":",
        linewidth=1.6,
        label="25 dB cap (double)",
        zorder=3,
    )
    left.set_title("$D_z$ at 500 Hz against barrier height ($d$ = 200 m)", pad=10)
    ...
    right.plot(
        ...
        linewidth=2.0,
        markersize=6,
        label=r"$A_{\mathrm{bar}} = \max(D_z - A_{\mathrm{gr}}, 0)$ (Eq. (12))",
        zorder=4,
    )
    right.fill_between(
        x,
        ...
        color=COLOR_TERTIARY,
        alpha=0.20,
        label=r"$A_{\mathrm{gr}}$, spent on the screened path",
        zorder=2,
    )
    right.axhline(20.0, color=COLOR_MUTED, linestyle=":", linewidth=1.2, zorder=1)
    right.set_title("The ground effect is spent, not kept", pad=10)
    right.set_xticks(x)
...
def generate_ground_reflection_coefficient(output_dir: str) -> None:
    ...
    with _warnings.catch_warnings():
        ...
        for h in heights:
            res = environment.ground_effect(
                [freq], float(h), float(h), dist, flow_resistivity=2e5, model="miki"
            )
    ...
    left.invert_xaxis()
    left.set_title("The coefficients part as the path grazes", pad=10)
    ...
    right.semilogx(
        ...
        linewidth=2.0,
        linestyle="--",
        label=r"with $R_{\mathrm{p}}$ alone (plane wave)",
        zorder=4,
    )
    right.invert_xaxis()
    ...
    right.set_title("What the ground wave keeps alive (500 Hz, 50 m)", pad=10)
...
def generate_barrier_thickness_gain(output_dir: str) -> None:
    ...
    with _warnings.catch_warnings():
        ...
        for freq, color in (
            ...
            left.plot(
                widths, gain, color=color, linewidth=2.0, label=f"{freq:g} Hz", zorder=4
            )
    left.set_title("Wave-theoretic model: the path length alone", pad=10)
    ...
    for freq, color in (
        ...
        thin = float(
            environment.barrier_attenuation(
                environment.Barrier(
                    source_to_edge=dss, edge_to_receiver=math.hypot(50.0, 2.5)
                ),
                100.0,
                [freq],
            )[0]
    ...
    right.set_title("ISO 9613-2: the path length plus the $C_3$ factor", pad=10)
...
def generate_shadow_zone_map(output_dir: str) -> None:
    ...
    pairs = (
        (0.5, 1.5, COLOR_PRIMARY),
        (2.0, 2.0, COLOR_TERTIARY),
    ...
    for hs, hr, color in pairs:
        ax.loglog(
            gradients,
            [
                environment.shadow_zone_distance(-float(g), hs, hr, ground_speed=340.0)
                for g in gradients
            ],
            color=color,
            linewidth=2.0,
    ...
    ax.plot(
        [grad_log],
        [environment.shadow_zone_distance(-grad_log, 2.0, 2.0, ground_speed=340.0)],
        "s",
        color=COLOR_SECONDARY,
        markersize=8,
        zorder=5,
    ...
    ax.set_title("Where the Acoustic Shadow Starts ($c_0$ = 340 m/s)", pad=12)
    ...
    secondary = ax.secondary_xaxis(
        "top",
        functions=(
            lambda g: 340.0 / np.maximum(g, 1e-9),
            lambda r: 340.0 / np.maximum(r, 1e-9),
        ),
    )
    secondary.set_xlabel(r"Radius of curvature $R_{\mathrm{c}} = c_0/|g|$ [m]")
    plt.tight_layout()
...
def generate_refraction_homogeneous_check(output_dir: str) -> None:
    ...
    _fig, (top, bottom) = plt.subplots(
        2, 1, figsize=(11, 7.2), sharex=True, gridspec_kw={"height_ratios": [2.2, 1.0]}
    )
    top.plot(
        ranges,
        solver,
    ...
    top.plot(
        ...
        linewidth=1.6,
        linestyle="--",
        label="Coherent two-ray closed form",
        zorder=4,
    )
    top.axhline(
        6.0,
        ...
        linestyle=":",
        linewidth=1.3,
        label="+6 dB (coherent sum)",
        zorder=2,
    )
    top.set_ylim(-35.0, 10.0)
    top.set_ylabel("Level re free field [dB]")
    top.set_title(
        "The Homogeneous Limit: 500 Hz, Rigid Ground, "
    ...
    bottom.plot(ranges, solver - two_ray, color=COLOR_TERTIARY, linewidth=1.6, zorder=4)
    bottom.axhline(0.6, color=COLOR_MUTED, linestyle=":", linewidth=1.3, zorder=2)
    bottom.axhline(-0.6, color=COLOR_MUTED, linestyle=":", linewidth=1.3, zorder=2)
...
def generate_cnossos_road_corrections(output_dir: str) -> None:
    ...
    light = [
        environment.RoadTraffic(environment.RoadVehicleCategory.LIGHT, 1000.0, 50.0)
    ]
    heavy = [
        environment.RoadTraffic(environment.RoadVehicleCategory.HEAVY, 1000.0, 50.0)
    ]

    _fig, (t_ax, s_ax, j_ax) = plt.subplots(1, 3, figsize=(14.5, 4.8))
    temps = np.linspace(-10.0, 40.0, 60)
    for flow, name, color in (
        (light, "Light (1)", COLOR_PRIMARY),
    ...
    for edge in (50.0, 90.0):
        s_ax.axvline(edge, color=COLOR_MUTED, linestyle=":", linewidth=1.3, zorder=2)
    ...
    for junction, name, color in (
        ...
        j_ax.plot(
            ...
            [
                a_line(
                    traffic=light, junction_distance=float(x), junction_type=junction
                )
                - ref
                for x in xs
            ],
    ...
    j_ax.axvline(100.0, color=COLOR_MUTED, linestyle=":", linewidth=1.3, zorder=2)
...
def generate_cnossos_road_gradient(output_dir: str) -> None:
    ...
    for edge in (-12.0, 12.0):
        ax.axvline(edge, color=COLOR_MUTED, linestyle=":", linewidth=1.3, zorder=2)
    ax.axhline(0.0, color=COLOR_MUTED, linewidth=0.9, zorder=1)
    ax.set_title("CNOSSOS-EU Road-Gradient Correction (2.2.13-2.2.16)", pad=12)
...
def generate_cnossos_road_surfaces(output_dir: str) -> None:
    ...
    for surface, color, style in rows:
        ...
        ax.semilogx(
            ...
            markerfacecolor="white",
            markeredgewidth=1.2,
            label=f"{surface.value} ({span})",
            zorder=4,
        )
    ax.axhline(0.0, color=COLOR_MUTED, linewidth=0.9, zorder=1)
    ax.set_title("CNOSSOS-EU Road Surfaces (Table F-4, light vehicles)", pad=12)
    ax.set_xlabel(LABEL_FREQ_HZ)
...
def generate_cnossos_rail_components(output_dir: str) -> None:
    ...
    palette = {
        "rolling": COLOR_PRIMARY,
        "traction": COLOR_TERTIARY,
        "aerodynamic": COLOR_SECONDARY,
        "bridge": "#9467bd",
    }
    for name, spectrum in result.components.items():
        for row, height, style in ((0, "0,5 m", "-"), (1, "4,0 m", "--")):
            ...
            left.semilogx(
                ...
                values,
                style,
                color=palette.get(name, COLOR_MUTED),
                linewidth=1.9,
                label=f"{name} @ {height}",
                zorder=4,
            )
    left.set_title("Components at 160 km/h, by source height", pad=10)
    ...
    for row, label, color in (
        (0, "Source A (0,5 m)", COLOR_PRIMARY),
    ...
    right.axvline(
        ...
        linestyle=":",
        linewidth=1.4,
        label="Aerodynamic threshold (2.3.13)",
        zorder=2,
    )
...
def generate_cnossos_rail_directivity(output_dir: str) -> None:
    ...
    for band, name, color in bands:
        for edition, style in (
            ...
            values = [
                float(environment.vertical_directivity(float(a), edition=edition)[band])
                for a in psi
            ]
            left.plot(
                np.radians(psi),
    ...
    left.set_title("Vertical correction of source A (2.3.16)", pad=18)
    ...
    right.plot(
        phi,
        10.0 * np.log10(0.01 + 0.99 * np.sin(phi) ** 2),
        color=COLOR_TERTIARY,
        linewidth=2.0,
    )
    ...
    fig.suptitle("CNOSSOS-EU Railway Source Directivity", fontsize=13)
    plt.tight_layout()
...
def generate_wind_turbine_apparent_power(output_dir: str) -> None:
    ...
    lwa = np.array(
        [
            float(
                environment.apparent_sound_power_level(
    ...
    ax.plot(
        ...
        color="#e8a838",
        markersize=9,
        label="3-6 dB margin: reported with an asterisk",
        zorder=5,
    )
    ax.plot(
        [bins[voided]],
        ...
        markersize=12,
        markeredgewidth=2.4,
        label=r"Margin $\leq$ 3 dB: bin voided",
        zorder=5,
    )
    ax.set_title("Apparent Sound Power against Wind Speed (IEC 61400-11)", pad=12)
    ax.set_xlabel("Hub-height wind speed [m/s]  (0,5 m/s bins)")
    ax.set_ylabel(r"$L_{W\!\mathrm{A}}$ [dB(A) re 1 pW]")
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5, zorder=0)
    ax.set_axisbelow(True)
    ax.legend(loc="lower right", fontsize=9)
...
def generate_wind_turbine_audibility_criterion(output_dir: str) -> None:
    ...
    left.loglog(
        ...
        color=COLOR_PRIMARY,
        linewidth=2.0,
        label="IEC 61400-11 critical band (Zwicker)",
        zorder=4,
    )
    left.loglog(
        fc,
    ...
    left.set_title("Two critical bandwidths on one page", pad=10)
    left.set_xlabel(r"Tone frequency $f_{\mathrm{c}}$ [Hz]")
    ...
    right.semilogx(
        ...
        color=COLOR_PRIMARY,
        linewidth=2.0,
        label=r"Tonality needed to be audible "
        r"($\Delta L_{\mathrm{a}} > 0$)",
        zorder=4,
    )
    right.semilogx(
        fc,
    ...
    right.axhline(0.0, color=COLOR_MUTED, linewidth=0.9, zorder=1)
    right.set_title("The audibility criterion, read as required tonality", pad=10)
...
def generate_rd1367_kf_ki(output_dir: str) -> None:
    ...
    kf = [
        environment.low_frequency_correction(lceq=63.0 + float(d), laeq=63.0)
        for d in diffs
    ]
    ki = [
        environment.impulsive_correction(laieq=63.0 + float(d), laeq=63.0)
        for d in diffs
    ]
    ...
    ax.plot(
        ...
        markersize=9,
        zorder=5,
        label=r"worked example: $L_{\mathrm{f}}$ = 13 dB → "
        r"$K_{\mathrm{f}}$ = 3 dB",
    )
    ax.plot(
        [5.0],
        ...
        markersize=9,
        zorder=5,
        label=r"worked example: $L_{\mathrm{i}}$ = 5 dB → "
        r"$K_{\mathrm{i}}$ = 0 dB",
    )
    ax.set_title("RD 1367/2007: the Low-Frequency and Impulsive Corrections", pad=12)
...
def generate_rd1367_vs_iso_tonal(output_dir: str) -> None:
    ...
    deep = np.array([58.0, 60.0, 59.0, 61.0, 72.0, 62.0, 60.0, 58.0, 56.0, 54.0, 52.0])
    shallow = deep.copy()
    shallow[4] = 69.0

    _fig, axes = plt.subplots(1, 2, figsize=(13, 5.8), sharey=True)
    for ax, spectrum, title in (
        (axes[0], deep, r"$L_{\mathrm{t}}$ = 10.5 dB: both methods agree"),
        (axes[1], shallow, r"$L_{\mathrm{t}}$ = 7.5 dB: the verdicts split"),
    ):
        rd = environment.tonal_correction(spectrum, freqs)
        flags = np.asarray(environment.tonal_seeking_survey(spectrum, freqs))
        colors = [
            COLOR_SECONDARY if i == 4 else COLOR_PRIMARY for i in range(spectrum.size)
        ]
        ax.bar(
        ...
        ax.plot(
            ...
            markersize=22,
            markeredgewidth=2.4,
            label="Arithmetic mean of the two neighbours",
            zorder=5,
        )
        ...
        ax.text(
            0.03,
            0.97,
            f"RD 1367 tonal_correction: $K_{{\\mathrm{{t}}}}$ = "
            f"{rd.correction:.0f} dB\n"
            f"ISO 1996-2 survey flag at 250 Hz: {bool(flags[4])}\n"
            f"$L_{{\\mathrm{{t}}}}$ at 250 Hz = "
            f"{_fmt_minus(rd.differences[4], '.1f')} dB",
            transform=ax.transAxes,
            va="top",
            fontsize=10,
            color=COLOR_FG,
