...
def generate_epnl(output_dir: str) -> None:
    ...
    ax.plot(
        res.times, res.pnl, color="#8c8c8c", linestyle="--", linewidth=1.4, label="PNL"
    )
...
def generate_airport_contour(output_dir: str) -> None:
    ...
    sel = [
        [98.0, 92.0, 86.0, 80.0, 74.0, 68.0, 62.0, 56.0],
        [104.0, 98.0, 92.0, 86.0, 80.0, 74.0, 68.0, 62.0],
    ]
    lmax = [
        [94.0, 88.0, 82.0, 76.0, 70.0, 64.0, 58.0, 52.0],
...
def generate_airport_sor(output_dir: str) -> None:
    ...
    jet = np.array([aircraft.start_of_roll_directivity(p, dsor, "jet") for p in psi])
    prop = np.array(
    ...
    for a in (90.0, 120.0, 150.0, 180.0, 210.0, 240.0, 270.0):  # radial spokes
        ...
        ax.plot(
            sx, sy, color=COLOR_FG, linestyle="--", linewidth=0.9, alpha=0.28, zorder=0
        )
    for g in (-4.0, -8.0, -12.0):  # inner radial grid arcs (dB)
        gx, gy = _xy(az, g - r0)
        ax.plot(
            gx, gy, color=COLOR_FG, linestyle="--", linewidth=0.9, alpha=0.28, zorder=0
        )
    ...
    for data, color, label in (
        (jet, COLOR_PRIMARY, "Turbofan jet (Eq. 4-24a)"),
...
def generate_rotorcraft_flyover_event(output_dir: str) -> None:
    ...
    track = np.column_stack(
        [np.zeros_like(t), speed * (t - 65.0), np.full_like(t, 150.0)]
    )
    ...
    ax.set_title("Rotorcraft Flyover Time History (ECAC Doc 32)", pad=12)
...
def generate_rotorcraft_insertion_loss(output_dir: str) -> None:
    ...
    ax.axvspan(
        float(delta.min()),
        0.0,
        color=theme_fill(COLOR_MUTED, ax),
        zorder=0,
        label="Below line of sight",
    )
    colours = series_colors(len(bands))
    for colour, label, values in zip(colours, band_labels, ld.T, strict=True):
        ax.plot(delta, values, color=colour, linewidth=2.0, label=label, zorder=3)
    ...
    ax.set_title(
        "Rotorcraft Diffraction Attenuation vs Path Difference (ECAC Doc 32 / NORAH2)",
        pad=12,
    )
    ax.grid(color=COLOR_GRID, linestyle="--", alpha=0.5)
    ax.set_axisbelow(True)
    ...
    ax.text(
        0.98,
        0.05,
        f"edge height $h_0$ = {h0:.1f} m, single edge",
        transform=ax.transAxes,
        ha="right",
        va="bottom",
        fontsize=9,
...
def generate_anp_profile(output_dir: str) -> None:
    ...
    ax.text(
        0.98,
        0.06,
        f"stage length {profile.stage_length}, {profile.path.shape[0]} fixed points",
        transform=ax.transAxes,
        va="bottom",
        ha="right",
        fontsize=9,
...
def _doc29_departure() -> "tuple[Any, Any, Any]":
    ...
    path = np.column_stack(
        [xs, np.zeros_like(xs), z, power, np.full_like(xs, _VREF_MS)]
    )
...
def generate_airport_segment_breakdown(output_dir: str) -> None:
    ...
    ax.set_title("Doc 29 Segment Contributions at One Receiver", pad=12)
    ax.annotate(
        f"closest segment: {seg[top]:.1f} dB",
        xy=(top, seg[top]),
        xytext=(top + 5.0, seg[top] + 7.0),
        color=COLOR_FG,
        fontsize=9,
...
def generate_airport_segment_corrections(output_dir: str) -> None:
    ...
    for mounting, color, label in (
        ("wing", COLOR_PRIMARY, "Wing-mounted"),
    ...
    ax_i.set(
        xlabel=r"Depression angle $\varphi$ [°]",
        ylabel=r"$\Delta I(\varphi)$ [dB]",
        xlim=(0.0, 180.0),
    )
    ax_i.set_title(
    ...
    for ell, color in (
        (100.0, COLOR_TERTIARY),
        (300.0, COLOR_SECONDARY),
        (914.0, COLOR_PRIMARY),
    ):
    ...
    ax_l.set(
        xlabel=r"Elevation angle $\beta$ [°]",
        ylabel=r"$\Lambda(\beta, \ell)$ subtracted [dB]",
        xlim=(0.0, 90.0),
    )
    ax_l.set_title(
    ...
    for length, color, label in (
        (464.0, COLOR_PRIMARY, r"$\lambda$ = 464 m"),
        (2000.0, COLOR_SECONDARY, r"$\lambda$ = 2 000 m"),
    ):
        frac = np.linspace(-1.0, 2.0, 601)
        df = np.array(
            [aircraft.noise_fraction(f * length, length, d_lambda) for f in frac]
        )
    ...
    ax_v.set(
        xlabel=r"Segment speed $V_{\mathrm{seg}}$ [m/s]", ylabel=r"$\Delta V$ [dB]"
    )
    ax_v.set_title(
        "(d) Duration correction (Eq. 4-14)",
        fontsize=11,
    )
    ax_v.text(
        0.98,
...
def generate_anp_contour(output_dir: str) -> None:
    ...
    ax.plot(
        ...
        solid_capstyle="butt",
        zorder=3,
        label="take-off ground roll",
    )
    ax.plot(
        profile.path[inside, 0] / 1000.0,
        profile.path[inside, 1] / 1000.0,
        color=COLOR_FG,
        lw=1.4,
        ls="--",
        zorder=3,
        label="default ground track",
    )
    ax.plot(
        [3.0],
        [0.5],
    ...
    ax.set_title(f"ANP Departure SEL Contour - {record.description}", pad=12)
...
def _synthetic_hemisphere(masked: bool = False) -> "Any":
    ...
    bvi = (
        7.0
        * np.exp(-((po - 140.0) ** 2) / (2 * 16.0**2))[None, :, None]
        * np.exp(-(az**2) / (2 * 34.0**2))[:, None, None]
        * band[None, None, :]
    )
    levels = (
        spectrum[None, None, :] + rear[None, :, None] + lateral[:, None, None] + bvi
    )
    if masked:
        unseen = (
            (np.abs(az)[:, None] > 60.0) | (po[None, :] < 40.0) | (po[None, :] > 140.0)
        )
...
def generate_rotorcraft_hemisphere(output_dir: str) -> None:
    ...
    filled = np.array(
        [
            [aircraft.hemisphere_source_level(h, float(a), float(p))[idx] for p in po]
            for a in az
        ]
    )
    ...
    ax2.set_title(f"{freqs[idx]:.0f} Hz band, the most directive one", pad=10)
    ax2.text(
        0.02,
        0.04,
...
def generate_rotorcraft_hover_ring(output_dir: str) -> None:
    ...
    directivity = 3.5 * np.cos(np.radians(bearings)) + 3.0 * np.exp(
        -((bearings - 120.0) ** 2) / (2.0 * 40.0**2)
    )
    ...
    for label, hemisphere, color, style in conditions:
        section = np.array(
            [aircraft.hemisphere_source_level(hemisphere, 0.0, t)[k] for t in theta]
        )
    ...
    ax2.set_title(r"Derived hover and idle sources (Table 3, $\varphi$ = 0°)", pad=10)
...
def generate_rotorcraft_contour(output_dir: str) -> None:
    ...
    sigma = np.where(
        np.abs(y)[:, None] < 300.0, 20.0e6, np.full((y.size, x.size), 200.0e3)
    )
    ...
    for ax, ground, title in (
        (
            axes[0],
            aircraft.RotorcraftGround(flow_resistivity="D"),
            "Uniform pasture (class D)",
        ),
        (
            axes[1],
            aircraft.RotorcraftGround(flow_resistivity=sigma),
...
def generate_rotorcraft_mean_ground_plane(output_dir: str) -> None:
    ...
    for (px, py), color, label in (
        (src, COLOR_SECONDARY, rf"$h_\mathrm{{s}}$ = {hs_eq:.0f} m"),
        (rcv, COLOR_PRIMARY, rf"$h_\mathrm{{r}}$ = {hr_eq:.1f} m"),
    ):
    ...
    true = aircraft.ground_effect_adjustment(
        freqs, hs_true, hr_true, rcv[0], flow_resistivity="D"
    )
    equiv = aircraft.ground_effect_adjustment(
        freqs, hs_eq, hr_eq, rcv[0], flow_resistivity="D"
    )
    ax2.axhline(0.0, color=COLOR_FG, lw=1.0, alpha=0.5)
    ax2.plot(
        freqs,
        ...
        marker="s",
        ms=3,
        label=f"true heights ({hs_true:.0f} m, {hr_true:.1f} m)",
    )
    ax2.plot(
        freqs,
        equiv,
...
def generate_rotorcraft_flight_conditions(output_dir: str) -> None:
    ...
    for axis, pts, tri, weights, qi, qo, title, xlabel, ylabel in (
        ...
        axis.triplot(pts[:, 0], pts[:, 1], tri, color=COLOR_MUTED, lw=0.9, zorder=1)
        # The simplex that encloses the query, and the three weights it blends.
        corner = np.array([pts[i] for i, _ in weights])
        axis.fill(
            corner[:, 0], corner[:, 1], color=COLOR_TERTIARY, alpha=0.18, zorder=0
        )
        axis.plot(
            pts[:, 0],
            pts[:, 1],
        ...
        axis.plot(
            [qo[0], pts[w_out[0][0], 0]],
            [qo[1], pts[w_out[0][0], 1]],
            color=COLOR_SECONDARY,
            lw=1.2,
            ls=":",
            zorder=2,
