...
_ES_EXACT = {
    ...
    r"$s^{\prime}_\mathrm{a}$, enclosed gas (Formula 7, $\varepsilon = 0.9$)":
        r"$s^{\prime}_\mathrm{a}$, aire encerrado (Fórmula 7, $\varepsilon = 0{,}9$)",
    r"installed $s^{\prime} = s^{\prime}_\mathrm{t} + s^{\prime}_\mathrm{a}$ (clause 8.2)":
        r"instalada $s^{\prime} = s^{\prime}_\mathrm{t} + s^{\prime}_\mathrm{a}$ (apartado 8.2)",
    ...
    r"$r < 10$  →  $s^{\prime} = s^{\prime}_\mathrm{t}$ only if $s^{\prime}_\mathrm{a}$ is negligible":
        r"apartado 8.2:  $r \geq 100$ kPa·s/m²  →  $s^{\prime} = s^{\prime}_\mathrm{t}$" "\n"
        r"$10 \leq r < 100$  →  $s^{\prime} = s^{\prime}_\mathrm{t} + s^{\prime}_\mathrm{a}$" "\n"
        r"$r < 10$  →  $s^{\prime} = s^{\prime}_\mathrm{t}$ solo si $s^{\prime}_\mathrm{a}$ es despreciable",
    "the worked determination:\n$d$ = 20 mm, 4.94 + 5.56 = 10.49":
        "la determinación del ejemplo:\n$d$ = 20 mm, 4,94 + 5,56 = 10,49",
    r"$f_\mathrm{u} = 0.58\,c_0/d$ (Clause 5.4)":
        r"$f_\mathrm{u} = 0{,}58\,c_0/d$ (apartado 5.4)",
    "$|H_\\mathrm{i}|$, the windowed free-field reference":
        "$|H_\\mathrm{i}|$, la referencia de campo libre enventanada",
    "$|H_\\mathrm{r}|$, the windowed surface reflection":
        "$|H_\\mathrm{r}|$, la reflexión de la superficie enventanada",
    "$h_i$: free field, the rig clear of every surface":
        "$h_i$: campo libre, el equipo lejos de toda superficie",
    ...
    "$h_\\mathrm{r}$ = road - free field, the surface alone":
        "$h_\\mathrm{r}$ = pavimento - campo libre, solo la superficie",
    "$2 d_\\mathrm{m}/c$ = 1.47 ms": "$2 d_\\mathrm{m}/c$ = 1,47 ms",
    ...
    "Periodic, 6 × $N$ = 7  ($d$ = 0.22)":
        "Periódico, 6 × $N$ = 7  ($d$ = 0,22)",
    "Modulated, period + inverse  ($d$ = 0.32)":
        "Modulado, periodo + inverso  ($d$ = 0,32)",
    "$f_0$: the one band the\nperiodic array wins":
        "$f_0$: la única banda en la que\ngana la disposición periódica",
    "small excitation: $f_\\mathrm{r}$ = 25.0 Hz":
        "excitación pequeña: $f_\\mathrm{r}$ = 25,0 Hz",
    ...
    r"$\alpha_{\mathrm{spec}} - \alpha_\mathrm{s}$  (numerator of Eq. (5))":
        r"$\alpha_{\mathrm{spec}} - \alpha_\mathrm{s}$  (numerador de la Ec. (5))",
    r"$\alpha_{\mathrm{spec}}$, rotating turntable (T3, T4)":
        r"$\alpha_{\mathrm{spec}}$, plataforma giratoria (T3, T4)",
    r"$\alpha_\mathrm{s}$, static turntable (T1, T2)":
        r"$\alpha_\mathrm{s}$, plataforma estática (T1, T2)",
    "Absorption coefficient": "Coeficiente de absorción",
    r"$s = (\alpha_{\mathrm{spec}} - \alpha_\mathrm{s})/(1 - \alpha_\mathrm{s})$   Eq. (5)":
        r"$s = (\alpha_{\mathrm{spec}} - \alpha_\mathrm{s})/(1 - \alpha_\mathrm{s})$   Ec. (5)",
    ...
    "(d) $h_4$: deconvolved by $h_3$, Formula (1)":
        "(d) $h_4$: deconvolución por $h_3$ (Fórm. 1)",
    "(e) windowed, Clause 7.4.3": "(e) enventanada, apartado 7.4.3",
    "room": "sala",
    "$h_4$ arrives 2 ms earlier than $h_1$:\n"
    "dividing by $h_3$ removes its delay too":
        "$h_4$ llega 2 ms antes que $h_1$:\n"
    ...
    "$N$ = 7 QRD, $f_0$ = 500 Hz, 5 periods":
        "QRD $N$ = 7, $f_0$ = 500 Hz, 5 periodos",
    ...
    "On the rig: reading $f_\\mathrm{r}$": "En el banco: leer $f_\\mathrm{r}$",
    "Installed: only well above $f_0$":
        "Instalado: solo muy por encima de $f_0$",
    ...
    "$f_0$ of a 120 kg/m² screed [Hz]":
        "$f_0$ de un recrecido de 120 kg/m² [Hz]",
    ...
    r"Reading $f_\mathrm{r}$ on the EN 29052-1 rig":
        r"Lectura de $f_\mathrm{r}$ en el banco EN 29052-1",
    ...
    r"below $f_\mathrm{r}$: the plate follows the force":
        r"por debajo de $f_\mathrm{r}$: la placa sigue a la fuerza",
    r"at $f_\mathrm{r}$: a quarter cycle behind, amplitude peaks":
        r"en $f_\mathrm{r}$: un cuarto de ciclo por detrás, la amplitud es máxima",
    r"above $f_\mathrm{r}$: the plate moves against the force":
        r"por encima de $f_\mathrm{r}$: la placa se mueve contra la fuerza",
    "phase": "fase",
    ...
    "$R_\\mathrm{w}$ alone is not a specification":
        "$R_\\mathrm{w}$ por si solo no es una especificacion",
    # Bar-group tick label, symbols only: reads the same in Spanish.
    "$R_\\mathrm{w} + C_\\mathrm{tr}$": "$R_\\mathrm{w} + C_\\mathrm{tr}$",
    "shifted reference (both, $R_\\mathrm{w}$ = 49 dB)":
        "curva de referencia desplazada (ambas, $R_\\mathrm{w}$ = 49 dB)",
    ...
    "Signal-to-background margin $L_\\mathrm{sb} - L_\\mathrm{b}$ [dB]":
        "Margen senal-fondo $L_\\mathrm{sb} - L_\\mathrm{b}$ [dB]",
    "Correction applied, $L_\\mathrm{sb} - L$ [dB]":
    ...
    "laboratory $R$ ($R_\\mathrm{w}$ = 49 dB)": "$R$ de laboratorio ($R_\\mathrm{w}$ = 49 dB)",
    r"field $R^{\prime}$, good junctions ($R^{\prime}_\mathrm{w}$ = 47 dB)":
        r"$R^{\prime}$ in situ, uniones buenas ($R^{\prime}_\mathrm{w}$ = 47 dB)",
    r"field $R^{\prime}$, flanking dominant ($R^{\prime}_\mathrm{w}$ = 42 dB)":
        r"$R^{\prime}$ in situ, flancos dominantes ($R^{\prime}_\mathrm{w}$ = 42 dB)",
    ...
    "Window $R_\\mathrm{A}$ [dBA]  (2 m² of an 8 m² facade)":
        "$R_\\mathrm{A}$ de la ventana [dBA]  (2 m² de una fachada de 8 m²)",
    "Overall facade $R_\\mathrm{A}$ [dBA]": "$R_\\mathrm{A}$ global de la fachada [dBA]",
    ...
    "$F_{pI} = L_p - L_{I\\mathrm{n}}$ (Formula (10))":
        "$F_{pI} = L_p - L_{I\\mathrm{n}}$ (Formula (10))",
    ...
    "critical frequency $f_\\mathrm{c}$": "frecuencia crítica $f_\\mathrm{c}$",
    ...
    r"flat 1 mm sheet (isotropic, single $f_\mathrm{c}$)":
        r"chapa plana de 1 mm (isótropa, un solo $f_\mathrm{c}$)",
    ...
    r"Normalised effective density $\rho_\mathrm{e}/\rho_0$":
        r"Densidad efectiva normalizada $\rho_\mathrm{e}/\rho_0$",
    ...
    r"Normalised surface impedance $Z_\mathrm{s}/\rho_0 c_0$":
        r"Impedancia superficial normalizada $Z_\mathrm{s}/\rho_0 c_0$",
    ...
    "Area ratio $m = S_{\\mathrm{exp}}/S_{\\mathrm{duct}}$":
        "Relación de áreas $m = S_{\\mathrm{exp}}/S_{\\mathrm{duct}}$",
    ...
    r"Diffraction attenuation $\Delta L_\mathrm{d}$ [dB]":
        r"Atenuación por difracción $\Delta L_\mathrm{d}$ [dB]",
    ...
    r"Ground-effect adjustment $\Delta L_\mathrm{g}$ [dB]":
        r"Ajuste por efecto de suelo $\Delta L_\mathrm{g}$ [dB]",
    ...
    "ISO 15186-1 Intensity Sound Reduction Index ($R_\\mathrm{I}$ and $R_\\mathrm{I,M}$)":
        "Índice de reducción acústica por intensidad ISO 15186-1 "
        "($R_\\mathrm{I}$ y $R_\\mathrm{I,M}$)",
    "Sound reduction index [dB]": "Índice de reducción acústica [dB]",
    "$K_\\mathrm{c}$ adaptation": "Adaptación $K_\\mathrm{c}$",
    "$R_\\mathrm{I}$ (intensity)": "$R_\\mathrm{I}$ (intensidad)",
    # intensity_element_insulation info box: rating and formula, symbols only.
    "$D_\\mathrm{{I,n,e,w}}$($C$;$C_\\mathrm{{tr}}$) = 29(−1;−2) dB\n"
    "$D_\\mathrm{I,n,e} = L_{p1} - 6 - [L_{I\\mathrm{n}} + 10\\,\\log_{10}(S_\\mathrm{m}/A_0)]"
    " + 10\\,\\log_{10} N$":
        "$D_\\mathrm{{I,n,e,w}}$($C$;$C_\\mathrm{{tr}}$) = 29(−1;−2) dB\n"
        "$D_\\mathrm{I,n,e} = L_{p1} - 6 - [L_{I\\mathrm{n}} + 10\\,\\log_{10}(S_\\mathrm{m}/A_0)]"
        " + 10\\,\\log_{10} N$",
    ...
    "$D_\\mathrm{nT}$ (standardized)": "$D_\\mathrm{nT}$ (estandarizada)",
    ...
    r"$\pm U$ ($k$ = 2), reproducibility":
        r"$\pm U$ ($k$ = 2), reproducibilidad",
    r"$\alpha_\mathrm{s}$ (ISO 354)": r"$\alpha_\mathrm{s}$ (ISO 354)",
    r"$\sigma_\mathrm{R} = m\,\alpha_\mathrm{s} + n$  (Table 1)" "\n"
    r"$U = k\,u$,  $k$ = 2  (95 %)":
        r"$\sigma_\mathrm{R} = m\,\alpha_\mathrm{s} + n$  (Tabla 1)" "\n"
        r"$U = k\,u$,  $k$ = 2  (95 %)",
    ...
    "Suspended-ceiling plenum path\n(one-dimensional model, $L_\\mathrm{R}$ = 4.75 m, "
    ...
    "$D_\\mathrm{n,c}$ (measured)": "$D_\\mathrm{n,c}$ (medido)",
    ...
    "$R_\\mathrm{cl}$, plenum $h$ = 0.43 m": "$R_\\mathrm{cl}$, plenum $h$ = 0,43 m",
    "$R_\\mathrm{cl}$, plenum $h$ = 0.86 m": "$R_\\mathrm{cl}$, plenum $h$ = 0,86 m",
    ...
    "$\\Delta L_\\mathrm{w}$ = 32.2 dB  (ISO 12354-2 Formula C.4)":
    ...
    r"Tonal audibility $\Delta L_\mathrm{ta}$ [dB]":
        r"Audibilidad tonal $\Delta L_\mathrm{ta}$ [dB]",
    r"Tonal adjustment $K_\mathrm{t}$ [dB]": r"Ajuste tonal $K_\mathrm{t}$ [dB]",
    r"$K_\mathrm{t}(\Delta L_\mathrm{ta})$ (Formulae C.4-C.6)":
        r"$K_\mathrm{t}(\Delta L_\mathrm{ta})$ (Fórmulas C.4-C.6)",
    ...
    r"$s^{\prime}_\mathrm{a} = p_0/(d\,\varepsilon) \approx 111/d$ MN/m³  (NOTE)":
        ...
        r"$s^{\prime} = s^{\prime}_\mathrm{t} + s^{\prime}_\mathrm{a}$  (apartado 8.2)" "\n"
        r"$s^{\prime}_\mathrm{t} = 4\pi^2 m^{\prime}_\mathrm{t} f_\mathrm{r}^2$  (Fórmula 4)" "\n"
        r"$s^{\prime}_\mathrm{a} = p_0/(d\,\varepsilon) \approx 111/d$ MN/m³  (NOTA)",
    ...
    r"Structure-borne power level $L_{W\mathrm{s}}$ [dB re 1 pW]":
        r"Nivel de potencia estructural $L_{W\mathrm{s}}$ [dB re 1 pW]",
    ...
    r"characteristic $L_{W\mathrm{s,c}}$ (EN 15657)":
        r"característica $L_{W\mathrm{s,c}}$ (EN 15657)",
    r"installed $L_{W\mathrm{s,inst}} = L_{W\mathrm{s,c}} - D_\mathrm{C}$":
        r"instalada $L_{W\mathrm{s,inst}} = L_{W\mathrm{s,c}} - D_\mathrm{C}$",
    ...
    "$D_\\mathrm{C} = 10\\,\\log_{10}(|Y_\\mathrm{s}+Y_i|^2 / (|Y_\\mathrm{s}|\\,\\mathrm{Re}\\,Y_i))$\n"
    "$L_{n,s,ij} = L_{W\\mathrm{s,inst}} - D_\\mathrm{sa} - R_{ij}"
    " - 10\\,\\log_{10}(S_i/S_0) - 10\\,\\log_{10}(A_0/4)$\n"
    "$L_\\mathrm{n,s} = 10\\,\\log_{10}(\\Sigma\\,10^{L_{n,s,ij}/10})$,  "
    "$S_0 = A_0$ = 10 m²":
        "$D_\\mathrm{C} = 10\\,\\log_{10}(|Y_\\mathrm{s}+Y_i|^2 / (|Y_\\mathrm{s}|\\,\\mathrm{Re}\\,Y_i))$\n"
        "$L_{n,s,ij} = L_{W\\mathrm{s,inst}} - D_\\mathrm{sa} - R_{ij}"
        " - 10\\,\\log_{10}(S_i/S_0) - 10\\,\\log_{10}(A_0/4)$\n"
    ...
    "$d\\!f_\\mathrm{c} = 25 + 75\\,(1 + 1.4\\,(f_\\mathrm{T}/1000)^2)^{0.69}$\n"
    "$L_\\mathrm{G} = L_\\mathrm{S} + 10\\,\\log_{10}(d\\!f_\\mathrm{c}/d\\!f)$,"
    "  $a_\\mathrm{v} = -2 - \\log_{10}(1 + (f/502)^{2.5})$\n"
    "$\\Delta L = L_\\mathrm{T} - L_\\mathrm{G} - a_\\mathrm{v}$  (combustion engine, Annex E)":
        "$d\\!f_\\mathrm{c} = 25 + 75\\,(1 + 1{,}4\\,(f_\\mathrm{T}/1000)^2)^{0{,}69}$\n"
        "$L_\\mathrm{G} = L_\\mathrm{S} + 10\\,\\log_{10}(d\\!f_\\mathrm{c}/d\\!f)$,"
        "  $a_\\mathrm{v} = -2 - \\log_{10}(1 + (f/502)^{2{,}5})$\n"
        "$\\Delta L = L_\\mathrm{T} - L_\\mathrm{G} - a_\\mathrm{v}$  (motor de combustión, Anexo E)",
    ...
    "$R_\\mathrm{p}$ — wall": "$R_\\mathrm{p}$ — muro",
    "$R_\\mathrm{p}$ — window": "$R_\\mathrm{p}$ — ventana",
    "$R_\\mathrm{p}$ — skylight": "$R_\\mathrm{p}$ — claraboya",
    "$R_\\mathrm{p}$ — air inlet": "$R_\\mathrm{p}$ — entrada de aire",
    ...
    r"Porous layer 50 mm ($\sigma$ = 20 kPa·s/m²)":
        r"Capa porosa de 50 mm ($\sigma$ = 20 kPa·s/m²)",
    ...
    "Membrane 2 kg/m² + air + porous 38 mm":
        "Membrana 2 kg/m² + aire + poroso 38 mm",
    ...
    "Exposure duration $D_\\mathrm{y}$ [years]":
        "Duración de la exposición $D_\\mathrm{y}$ [años]",
    ...
    "$K_2$ saturates, and inherits every error in $A$":
        "$K_2$ se satura, y hereda cada error de $A$",
    "Source-to-background margin $\\Delta L_p$ [dB]":
        "Margen entre fuente y fondo $\\Delta L_p$ [dB]",
    "Background correction $K_1$ [dB]":
        "Corrección por ruido de fondo $K_1$ [dB]",
    ...
    "Contribution to $L_W - \\bar{L}_p$ [dB]":
        "Aportación a $L_W - \\bar{L}_p$ [dB]",
    ...
    "$f_{\\mathrm{max}} = 0.1\\,c/\\Delta r$":
        "Extremo alto: el sesgo de diferencias finitas, y "
        "$f_{\\mathrm{max}} = 0{,}1\\,c/\\Delta r$",
    ...
    "$\\delta_{pI0}$ margin,\nre 25 mm [dB]":
        "Margen de $\\delta_{pI0}$,\nre 25 mm [dB]",
    "−0.3 dB: the bound $f_{\\mathrm{max}}$ is quoted against":
        "−0,3 dB: el sesgo contra el que se cita la cota $f_{\\mathrm{max}}$",
    ...
    "ISO 4871 Annex B: $L_{W\\!Ad} = L_{W\\!A} + K_{W\\!A}$, "
    "verified when $L_1 \\leq L_{W\\!Ad}$":
        "ISO 4871 Anexo B: $L_{W\\!Ad} = L_{W\\!A} + K_{W\\!A}$, "
        "verificado si $L_1 \\leq L_{W\\!Ad}$",
    ...
    "$L_\\mathrm{eq}$ over the whole event": "$L_\\mathrm{eq}$ de todo el evento",
    ...
    "Hourly $L_\\mathrm{Aeq}$": "$L_\\mathrm{Aeq}$ horario",
    ...
    r"$L_{p,\mathrm{A,S},4\,\mathrm{m}} > 50$ dB: poor":
        r"$L_{p,\mathrm{A,S},4\,\mathrm{m}} > 50$ dB: deficiente",
    r"$L_{p,\mathrm{A,S},4\,\mathrm{m}} \leq 48$ dB: good target":
        r"$L_{p,\mathrm{A,S},4\,\mathrm{m}} \leq 48$ dB: objetivo bueno",
    ...
    "Adjustment to $L_\\mathrm{Aeq}$": "Ajuste a $L_\\mathrm{Aeq}$",
    "Impulses": "Impulsos",
    ...
    "Loud": "Fuerte",
    "Shout": "Grito",
    r"ISO 1999 — NIPTS at $L_\mathrm{EX,8h}$ = 95 dB":
        r"ISO 1999 — NIPTS a $L_\mathrm{EX,8h}$ = 95 dB",
    ...
    "$s_\\mathrm{u}$ (worse than the median)": "$s_\\mathrm{u}$ (peor que la mediana)",
    "$s_\\mathrm{l}$ (better than the median)": "$s_\\mathrm{l}$ (mejor que la mediana)",
    "$s_\\mathrm{u} - s_\\mathrm{l}$: the asymmetry": "$s_\\mathrm{u} - s_\\mathrm{l}$: la asimetría",
    ...
    "$R_\\mathrm{w}$ ($C$ ; $C_\\mathrm{tr}$) = 30 (−2 ; −3) dB":
        "$R_\\mathrm{w}$ ($C$ ; $C_\\mathrm{tr}$) = 30 (−2 ; −3) dB",
    ...
    "Normalized impact sound pressure level $L_\\mathrm{n}$ [dB]":
        "Nivel de presión acústica de impactos normalizado $L_\\mathrm{n}$ [dB]",
    "Measured $L_\\mathrm{n}$ (third octave)": "$L_\\mathrm{n}$ medido (tercios de octava)",
    ...
    "Threshold $f_\\mathrm{D}$ (Formula 19)": "Umbral $f_\\mathrm{D}$ (Fórmula 19)",
    "Frequency of the more audible tone $f_\\mathrm{T}$ [Hz]":
        "Frecuencia del tono más audible $f_\\mathrm{T}$ [Hz]",
    ...
    "18.9 Hz apart — below $f_\\mathrm{D}$, so combined":
        ...
        "separados 18,9 Hz, por debajo de $f_\\mathrm{D}$: se combinan",
    ...
    "Choosing Between $H_1$ and $H_2$ (Bendat & Piersol)":
        "Elegir entre $H_1$ y $H_2$ (Bendat y Piersol)",
    "Noise on the output — $H_1$ is unbiased":
        "Ruido en la salida — $H_1$ es el insesgado",
    "Noise on the input — $H_2$ is unbiased":
        "Ruido en la entrada — $H_2$ es el insesgado",
    ...
    "and $d_3$ by 10.3 dB, and lifts the 10th order and above":
        "para un fundamental de 100 Hz la red recorta $d_2$ en 13,8 dB\n"
    ...
    "each order rolls off at the 3 kHz post-filter, not at $n f_2$":
        "punteado: los niveles de Chebyshev $1 + 3a_3/4$, $a_2/2$ y "
    ...
    "$f_2$ = 6 kHz: the Farina band stops here":
        "$f_2$ = 6 kHz: la banda de Farina se detiene aquí",
    ...
    "true phase of $H_2$: $-\\pi/2$ at every frequency":
        "fase verdadera de $H_2$: $-\\pi/2$ en todas las frecuencias",
    ...
    "Source level $L_\\mathrm{s}$": "Nivel de fuente $L_\\mathrm{s}$",
    ...
    r"Every Ray Turns Where $c(z_\mathrm{t}) = c(z_\mathrm{s})/\cos\theta_0$":
        r"Cada rayo gira donde $c(z_\mathrm{t}) = c(z_\mathrm{s})/\cos\theta_0$",
    ...
    r"Line power $L^{\prime}_{W,\mathrm{eq,line}}$ [dB re 1 pW/m]":
        r"Potencia de la línea $L^{\prime}_{W,\mathrm{eq,line}}$ [dB re 1 pW/m]",
    ...
    "$L_\\mathrm{EX,8h}$ contribution [dB]": "Contribución a $L_\\mathrm{EX,8h}$ [dB]",
    "Measurement task": "Tarea de medición",
    "planning/breaks": "planificación/pausas",
    "welding": "soldadura",
    "cutting/grinding": "corte/amolado",
    "Daily $L_\\mathrm{EX,8h}$": "$L_\\mathrm{EX,8h}$ diario",
    "$L_\\mathrm{EX,8h} + U$ (one-sided 95 %)": "$L_\\mathrm{EX,8h} + U$ (unilateral 95 %)",
    ...
    "Magnitude": "Módulo",
    "$|R_\\mathrm{p}|$ — plane wave": "$|R_\\mathrm{p}|$ — onda plana",
    "$|Q|$ — spherical wave": "$|Q|$ — onda esférica",
    "with $Q$ (spherical wave)": "con $Q$ (onda esférica)",
    "with $R_\\mathrm{p}$ alone (plane wave)": "solo con $R_\\mathrm{p}$ (onda plana)",
    ...
    "Radius of curvature $R_\\mathrm{c} = c_0/|g|$ [m]":
        "Radio de curvatura $R_\\mathrm{c} = c_0/|g|$ [m]",
    "representative −0.1 s⁻¹: 233 m": "−0,1 s⁻¹ representativo: 233 m",
    "the page's $b$ = −1 m/s case: 109 m":
        "el caso $b$ = −1 m/s de la página: 109 m",
    "The Homogeneous Limit: 500 Hz, Rigid Ground, $h_\\mathrm{s} = h_\\mathrm{r}$ = 2 m":
        "El límite homogéneo: 500 Hz, suelo rígido, $h_\\mathrm{s} = h_\\mathrm{r}$ = 2 m",
    ...
    r"$Q_{\mathrm{stud}}$ = 0.2, $T_\mathrm{s}$ = 4 months":
        r"$Q_{\mathrm{stud}}$ = 0,2, $T_\mathrm{s}$ = 4 meses",
    r"$Q_{\mathrm{stud}}$ = 0.5, $T_\mathrm{s}$ = 4 months":
        r"$Q_{\mathrm{stud}}$ = 0,5, $T_\mathrm{s}$ = 4 meses",
    ...
    "Tone frequency $f_\\mathrm{c}$ [Hz]": "Frecuencia del tono $f_\\mathrm{c}$ [Hz]",
    ...
    r"Tonality needed to be audible ($\Delta L_\mathrm{a} > 0$)":
        r"Tonalidad necesaria para ser audible ($\Delta L_\mathrm{a} > 0$)",
    r"Tonality needed to be reportable ($\Delta L_\mathrm{a} \geq -3$ dB)":
        r"Tonalidad necesaria para ser declarable ($\Delta L_\mathrm{a} \geq -3$ dB)",
    r"Required tonality $\Delta L_\mathrm{tn}$ [dB]":
        r"Tonalidad exigida $\Delta L_\mathrm{tn}$ [dB]",
    ...
    "$K_\\mathrm{f}$, from $L_{Ceq,Ti} - L_{Aeq,Ti}$":
        "$K_\\mathrm{f}$, a partir de $L_{Ceq,Ti} - L_{Aeq,Ti}$",
    "$K_\\mathrm{i}$, from $L_{AIeq,Ti} - L_{Aeq,Ti}$":
        "$K_\\mathrm{i}$, a partir de $L_{AIeq,Ti} - L_{Aeq,Ti}$",
    "worked example: $L_f$ = 13 dB → $K_\\mathrm{f}$ = 3 dB":
        "ejemplo resuelto: $L_f$ = 13 dB → $K_\\mathrm{f}$ = 3 dB",
    "worked example: $L_i$ = 5 dB → $K_\\mathrm{i}$ = 0 dB":
        "ejemplo resuelto: $L_i$ = 5 dB → $K_\\mathrm{i}$ = 0 dB",
    "$L_\\mathrm{t}$ = 10.5 dB: both methods agree":
        "$L_\\mathrm{t}$ = 10,5 dB: los dos métodos coinciden",
    "$L_\\mathrm{t}$ = 7.5 dB: the verdicts split":
        "$L_\\mathrm{t}$ = 7,5 dB: los veredictos se separan",
    ...
    r"Through-origin quadratic fit  $\Delta p = a\,u + b\,u^2$":
        r"Ajuste cuadrático por el origen  $\Delta p = a\,u + b\,u^2$",
    ...
    "Linear airflow velocity $u$ [mm/s]":
        "Velocidad lineal del aire $u$ [mm/s]",
    r"Pressure drop $\Delta p$ [Pa]": r"Caída de presión $\Delta p$ [Pa]",
    r"Absorption coefficient $\alpha = 1 - |r|^2$":
        r"Coeficiente de absorción $\alpha = 1 - |r|^2$",
    r"Standing-wave level difference $L_{\mathrm{max}} - L_{\mathrm{min}}$ [dB]":
        r"Diferencia de nivel de onda estacionaria "
        r"$L_{\mathrm{max}} - L_{\mathrm{min}}$ [dB]",
    r"Sound absorption coefficient $\alpha$":
        r"Coeficiente de absorción acústica $\alpha$",
    ...
    "Reflection factor magnitude $|r|$":
        "Módulo del factor de reflexión $|r|$",
    ...
    r"100 mm bore, $s$ = 100 mm   ·   top end: spacing $0.45\,c/s$":
        r"tubo de 100 mm, $s$ = 100 mm   ·   techo: separación $0{,}45\,c/s$",
    r"100 mm bore, $s$ = 100 mm (ASTM E2611)   ·   top end: spacing $0.40\,c/s$":
        r"tubo de 100 mm, $s$ = 100 mm (ASTM E2611)   ·   techo: "
        r"separación $0{,}40\,c/s$",
    r"100 mm bore, $s$ = 50 mm   ·   top end: cut-on $0.58\,c/d$":
        r"tubo de 100 mm, $s$ = 50 mm   ·   techo: corte $0{,}58\,c/d$",
    r"29 mm bore, $s$ = 20 mm   ·   top end: cut-on $0.58\,c/d$":
        r"tubo de 29 mm, $s$ = 20 mm   ·   techo: corte $0{,}58\,c/d$",
    ...
    "Equivalent absorption area [m²]":
        "Área de absorción equivalente [m²]",
    r"Band height as $\alpha_\mathrm{s} = (A_2 - A_1)/S$":
        r"Altura de la franja como $\alpha_\mathrm{s} = (A_2 - A_1)/S$",
    ...
    "2 kPa·s/m²  (too transparent)":
        "2 kPa·s/m²  (demasiado transparente)",
    "8 kPa·s/m²  (window)": "8 kPa·s/m²  (ventana)",
    "20 kPa·s/m²  (window)": "20 kPa·s/m²  (ventana)",
    "33 kPa·s/m²  (window)": "33 kPa·s/m²  (ventana)",
    "100 kPa·s/m²  (too reflecting)":
        "100 kPa·s/m²  (demasiado reflectante)",
    ...
    "$0.01 < X < 1$": "$0{,}01 < X < 1$",
    r"Characteristic impedance, $\sigma$ = 20 kPa·s/m²":
        r"Impedancia característica, $\sigma$ = 20 kPa·s/m²",
    ...
    r"$\mathrm{Re}(Z_\mathrm{s})/(\rho_0 c_0)$,  50 mm hard-backed layer":
        r"$\mathrm{Re}(Z_\mathrm{s})/(\rho_0 c_0)$,  capa de 50 mm con respaldo rígido",
    ...
    r"$\alpha_\mathrm{dif}$  bulk (Paris integral)":
        r"$\alpha_\mathrm{dif}$  volumétrica (integral de Paris)",
    r"$\alpha_\mathrm{st}$  locally reacting (closed form)":
        r"$\alpha_\mathrm{st}$  reacción local (forma cerrada)",
    ...
    "solved $h$ = 0.978 mm:\n"
    r"$\mathrm{Re}(z) = 1$ and $\mathrm{Im}(z) = 0$ together":
        "$h$ resuelta = 0,978 mm:\n"
    ...
    "A-weighted level $L_\\mathrm{AF}$ [dB]": "Nivel ponderado A $L_\\mathrm{AF}$ [dB]",
    "$L_\\mathrm{AF}$ (A-weighted, Fast)": "$L_\\mathrm{AF}$ (ponderado A, Fast)",
    ...
    "normalized $L_\\mathrm{n}$": "$L_\\mathrm{n}$ normalizado",
    "Impact sound pressure level $L_\\mathrm{n}$ [dB]":
        "Nivel de presión acústica de impactos $L_\\mathrm{n}$ [dB]",
    "ISO 15186-1 Small-Element Insulation by Intensity":
        "Aislamiento de elementos pequeños por intensidad ISO 15186-1",
    "$D_\\mathrm{I,n,e}$ (element)": "$D_\\mathrm{I,n,e}$ (elemento)",
    ...
    "$D_\\mathrm{n,f}$ (flanking)": "$D_\\mathrm{n,f}$ (flancos)",
    ...
    "$L_\\mathrm{n,w,eq} = 164 - 35\\,\\log_{10}(m^{\\prime}/m^{\\prime}_0)$ = 76.2 dB\n"
    ...
    "$L_W = L_{p,\\mathrm{in}} + C_\\mathrm{d} - R^{\\prime} + 10\\,\\log_{10}(S/S_0)$\n"
    "wall 176 m² + industrial door 24 m², $C_\\mathrm{d}$ = −5 dB":
        "$L_W = L_{p,\\mathrm{in}} + C_\\mathrm{d} - R^{\\prime} + 10\\,\\log_{10}(S/S_0)$\n"
    ...
    "Insertion loss, stiff source ($Z_\\mathrm{s} = 20\\,\\rho c/S$)":
        "Pérdida por inserción, fuente rígida ($Z_\\mathrm{s} = 20\\,\\rho c/S$)",
    "Insertion loss, matched source ($Z_\\mathrm{s} = \\rho c/S$)":
        "Pérdida por inserción, fuente adaptada ($Z_\\mathrm{s} = \\rho c/S$)",
    ...
    "$55\\,\\mathrm{lg}(V/V_0)$: 16.6 dB per\ndoubling, every band":
        "$55\\,\\mathrm{lg}(V/V_0)$: 16,6 dB por\nduplicación, en cada banda",
    "peak band $f_\\mathrm{P} = 48.8\\,U_\\mathrm{G}$ moves up one octave per\ndoubling of the "
    "face velocity; ASHRAE Table 9 caps\nthe neck of an RC 30 supply "
    "outlet at 2.2 m/s":
        "la banda de pico $f_\\mathrm{P} = 48{,}8\\,U_\\mathrm{G}$ sube una octava por\n"
    ...
    "blades, 2 kHz if the fan is selected faster":
        "el incremento $C_{\\mathrm{BFI}}$ es de 2 dB y cae entero en la\n"
    ...
    "Target $\\mathrm{{IL}}$ = $L_{p1}$ − NC 45":
        "$\\mathrm{{IL}}$ objetivo = $L_{p1}$ − NC 45",
    "Interior correction $10\\,\\lg(S_\\mathrm{E}/R_\\mathrm{i})$":
        "Corrección interior $10\\,\\lg(S_\\mathrm{E}/R_\\mathrm{i})$",
    ...
    "30°$":
        "$60° \\leq |\\mathrm{acimut}| \\leq 120°$,  $|\\mathrm{elevación}| "
        "< 30°$",
    "Azimuth [°]": "Acimut [°]",
    "Elevation [°]": "Elevación [°]",
    # true_peak_intersample: the intersample peak and the closed-form bound
    "A 12 kHz tone at $\\pi/4$: every sample misses the peak":
        "Un tono de 12 kHz en $\\pi/4$: ninguna muestra cae en el pico",
    ...
    "Coupling term $D_\\mathrm{C}$ [dB]": "Término de acoplamiento $D_\\mathrm{C}$ [dB]",
    "EN 12354-5 Coupling Term and Its Two Limits":
        "Término de acoplamiento de la EN 12354-5 y sus dos límites",
    "$D_\\mathrm{C} = 10\\,\\log_{10}(|Y_\\mathrm{s} + Y_i + Y_k|^2 / (|Y_\\mathrm{s}|\\,\\mathrm{Re}\\{Y_i\\}))$\n"
    ...
    r"$|F_n|_{\mathrm{upper}} = 2\,m\,v_\text{h}/T_i$  (rebound)":
        r"$|F_n|_{\mathrm{upper}} = 2\,m\,v_\text{h}/T_i$  (con rebote)",
    r"$|F_n|_{\mathrm{lower}} = m\,v_\text{h}/T_i$  (no rebound)":
        r"$|F_n|_{\mathrm{lower}} = m\,v_\text{h}/T_i$  (sin rebote)",
    ...
    r"Apparent normalized impact level $L^{\prime}_\mathrm{n}$ [dB]":
        r"Nivel de impactos normalizado aparente $L^{\prime}_\mathrm{n}$ [dB]",
    ...
    r"$L_{W\mathrm{s}}$ measured on the test plate ($Y = 5.34\times10^{-6}$)":
        r"$L_{W\mathrm{s}}$ medido sobre la placa de ensayo ($Y = 5{,}34\times10^{-6}$)",
    r"$L_{Wsn}$ on the standard plate ($Y = 5\times10^{-6}$): what is declared":
        r"$L_{Wsn}$ sobre la placa normalizada ($Y = 5\times10^{-6}$): "
        "lo que se declara",
    r"$L_{W\mathrm{s,inst}}$ on the receiving wall ($Y = 24.1\times10^{-6}$)":
        r"$L_{W\mathrm{s,inst}}$ sobre el muro receptor "
        r"($Y = 24{,}1\times10^{-6}$)",
    r"$L_{W\mathrm{s,c}}$ with the source mobility ($Y = 10^{-3}$): "
    "the input to EN 12354-5":
        r"$L_{W\mathrm{s,c}}$ con la movilidad de la fuente ($Y = 10^{-3}$): "
    ...
    "$L_\\mathrm{AF}(t)$, Fast A-weighted level":
        "$L_\\mathrm{AF}(t)$, nivel ponderado A con Fast",
    ...
    "$L_\\mathrm{eq}$ minus the arithmetic dB mean [dB]":
        "$L_\\mathrm{eq}$ menos la media aritmética en dB [dB]",
    ...
    "Two noises with the same $L_\\mathrm{Aeq}$": "Dos ruidos con el mismo $L_\\mathrm{Aeq}$",
    ...
    "at $2 f_\\mathrm{m}$": "en $2 f_\\mathrm{m}$",
    "at $4 f_\\mathrm{m}$": "en $4 f_\\mathrm{m}$",
    ...
    "$L_\\mathrm{AF}$ [dB re 20 µPa]": "$L_\\mathrm{AF}$ [dB re 20 µPa]",
    ...
    "resolution bias, $-10\\lg(1+\\varepsilon_\\mathrm{b})$ [dB]":
        "sesgo de resolución, $-10\\lg(1+\\varepsilon_\\mathrm{b})$ [dB]",
    ...
    "ISO/TS 7849-1 assumes $\\varepsilon = 1$ here":
        "ISO/TS 7849-1 supone $\\varepsilon = 1$ aquí",
...
_ES_PATTERNS = [
    ...
    ((r"^five paths, not thirteen: only the floor is excited\n"
      r"\$L\^\{\\prime\}_\\mathrm\{n,w\}\$ \(\$C_\\mathrm\{I\}\$\) = (.+) \((.+)\) dB$"),
    ...
    ((r"^\$L_\{W\\mathrm\{s,c\}\} - D_\\mathrm\{C\}\$, \$D_\\mathrm\{C\}\$ = (\d+)\.(\d+) dB: "
    ...
    (r"^blind part \$R_\\mathrm\{A\}\$ = (\d+) dBA$", r"parte ciega $R_\\mathrm{A}$ = \1 dBA"),
    ...
    (r"^\$n_d\$ = (\d+) averages, \$\\varepsilon_\\mathrm\{r\}\$ = (\d+)\.(\d+) %$",
     r"$n_d$ = \1 promedios, $\\varepsilon_\\mathrm{r}$ = \2,\3 %"),
    ...
    (r"^\$h_\\mathrm\{s\}\$ = (\d+) m, \$h_\\mathrm\{r\}\$ = (\d+)\.(\d+) m, \$d_\\mathrm\{p\}\$ = (\d+) m$",
     r"$h_\\mathrm{s}$ = \1 m, $h_\\mathrm{r}$ = \2,\3 m, $d_\\mathrm{p}$ = \4 m"),
    ...
    (r"^Governing  \$K_\\mathrm\{I\}\$ = (\d+)\.(\d+) dB$", r"Determinante  $K_\\mathrm{I}$ = \1,\2 dB"),
    ...
    (r"^\$K_\\mathrm\{t\}\$ = (\d+) dB, but the interval reaches into \$K_\\mathrm\{t\}\$ = (\d+) dB$",
     r"$K_\\mathrm{t}$ = \1 dB, pero el intervalo entra en $K_\\mathrm{t}$ = \2 dB"),
    ...
    (r"^\$\\alpha_\\mathrm\{w\}\$ = (\d+)\.(\d+)(.*)$", r"$\\alpha_\\mathrm{w}$ = \1,\2\3"),
    ...
    (r"^Linear term \$a\$ = (.+) Pa·s/m  \(= \$R_\\mathrm\{s\}\$ at \$u \\to 0\$\)$",
    ...
    ((r"^Weighted seat acceleration \(ISO 2631-1\)  "
     ...
     r"Aceleración ponderada del asiento (ISO 2631-1)  $a_\\mathrm{w}$ = \1,\2 m/s²"),
    ...
    (r"^\$\\mathrm\{VDV\}/\(a_\\mathrm\{w\} T\^\{1/4\}\)\$ = (\d+)\.(\d+)   \(> 1\.75\)$",
    ...
    ((r"^Treated: \$D_\{2,\\mathrm\{S\}\}\$ = (\d+) dB, "
     ...
     r"Tratada: $D_{2,\\mathrm{S}}$ = \1 dB, $L_{p,A,S,4\,\\mathrm{m}}$ = \2 dB"),
    ((r"^Untreated: \$D_\{2,\\mathrm\{S\}\}\$ = (\d+) dB, "
      r"\$L_\{p,\\mathrm\{A,S\},4\\,\\mathrm\{m\}\}\$ = (\d+) dB$"),
     r"Sin tratar: $D_{2,\\mathrm{S}}$ = \1 dB, $L_{p,A,S,4\,\\mathrm{m}}$ = \2 dB"),
    (r"^Treated: \$r_\\mathrm\{D\}\$ = (\d+)\.(\d+) m, \$r_\\mathrm\{P\}\$ = (\d+) m$",
     r"Tratada: $r_\\mathrm{D}$ = \1,\2 m, $r_\\mathrm{P}$ = \3 m"),
    (r"^Untreated: \$r_\\mathrm\{D\}\$ = (\d+)\.(\d+) m, \$r_\\mathrm\{P\}\$ = (\d+) m$",
    ...
    (r"^coincidence \$f_\\mathrm\{c\}\$ = (\d+) Hz$", r"coincidencia $f_\\mathrm{c}$ = \1 Hz"),
    (r"^\$L_\{W\\mathrm\{A\}\}\$ = (\d+)\.(\d+) dB\(A\)$", r"$L_{W\\mathrm{A}}$ = \1,\2 dB(A)"),
    ...
    ((r"^\$D_\{ls,2m,nT,w\}\$\(\$C\$;\$C_\\mathrm\{tr\}\$\) = (.+) dB\n"
      r"45° loudspeaker method \(−1\.5 dB on \$R\^\{\\prime\}\$\)$"),
     ("$D_{ls,2m,nT,w}$($C$;$C_\\\\mathrm{tr}$) = \\1 dB\n"
    ...
    ((r"^\$L\^\{\\prime\}_\\mathrm\{nT,w\}\$\(\$C_\\mathrm\{I\}\$\) = (.+) dB\n"
     ...
     ("$L^{\\\\prime}_\\\\mathrm{nT,w}$($C_\\\\mathrm{I}$) = \\1 dB\n"
      "atención al signo menos: un recinto vivo reduce $L^{\\\\prime}_\\\\mathrm{nT}$")),
    ((r"^\$D_\\mathrm\{nT,w\}\$\(\$C\$;\$C_\\mathrm\{tr\}\$\) = (.+) dB\n"
      r"\$R\^\{\\prime\}_\\mathrm\{w\}\$ = (\d+) dB   \(\$S\$ = 12\.5 m², \$V\$ = 30\.4 m³\)$"),
     ("$D_\\\\mathrm{nT,w}$($C$;$C_\\\\mathrm{tr}$) = \\1 dB\n"
      "$R^{\\\\prime}_\\\\mathrm{w}$ = \\2 dB   ($S$ = 12,5 m², $V$ = 30,4 m³)")),
    ...
    (r"^Tonal audibility \$\\Delta L_\\mathrm\{a\}\$ = ([+−]?\d+)\.(\d+) dB\naudible$",
    ...
    (r"^\$h_\\mathrm\{s\}/h_\\mathrm\{r\}\$ = (\d+)\.(\d+) / (\d+)\.(\d+) m$",
     r"$h_\\mathrm{s}/h_\\mathrm{r}$ = \1,\2 / \3,\4 m"),
    (r"^\$h_\\mathrm\{s\}/h_\\mathrm\{r\}\$ = (\d+) / (\d+) m$", r"$h_\\mathrm{s}/h_\\mathrm{r}$ = \1 / \2 m"),
    ...
    ((r"^quiet background, three events:  \$L_\{10\} - L_\{90\}\$ = "
      r"(\d+)\.(\d+) dB   \|   \$L_\\mathrm\{Aeq\} - L_\{50\}\$ = (−?\d+)\.(\d+) dB$"),
     ("fondo silencioso, tres eventos:  $L_{10} - L_{90}$ = \\1,\\2 dB   |   "
      "$L_\\\\mathrm{Aeq} - L_{50}$ = \\3,\\4 dB")),
    (r"^\$L_\{\\mathrm\{A\}E\}\$ = (\d+)\.(\d+) dB: the whole event energy in 1 s$",
     r"$L_{\\mathrm{A}E}$ = \1,\2 dB: toda la energía del evento en 1 s"),
    ...
    ((r"^each minimum is that group's weighted TTS onset \$T_\\mathrm\{w\} = K \+ C\$\n"
      r"LF: below \$f_1\$ the filter falls at \$20a\$ = (\d+) dB/decade, "
      r"above \$f_2\$ at \$20b\$ = (\d+) dB/decade$"),
     ("cada mínimo es el inicio de TTS ponderado de su grupo $T_\\\\mathrm{w} = K + C$\n"
