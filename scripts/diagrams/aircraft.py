...
def _d_doc29_segment_geometry(s: SVG, th: Theme) -> None:
    ...
    s.text(60.0, 70.0, "(a) Observer alongside the segment", 17, th.fg,
    ...
    s.text(478.0, 70.0, "(b) Observer behind the segment", 17, th.fg,
           bold=True, anchor="start")
    x_s1b = 626.0
    ...
    s.text(60.0, 412.0, "(c) In the plane normal to the flight path", 17, th.fg,
           bold=True, anchor="start")
    gy = 578.0
    ...
    s.text(obs_x + 22.0, gy + 20.0, "receiver, 1.2 m", 14, th.muted,
           anchor="middle")
    s.text(62.0, 448.0, "wing plane", 14, th.accent, anchor="start")

    # --- (d) start-of-roll azimuth in plan (Fig. 4-8) -----------------------
    s.text(478.0, 412.0, "(d) Behind the take-off roll, in plan", 17, th.fg,
           bold=True, anchor="start")
    cx, cy = 700.0, 522.0
    s.rect(cx, cy - 9.0, 138.0, 18.0, th.panel, th.fg, sw=1.6)
    s.text(cx + 70.0, cy - 18.0, "runway", 14, th.muted)
...
def _d_aircraft_noise_station(s: SVG, th: Theme) -> None:
    ...
    # --- site elevation -----------------------------------------------------
    s.text(56.0, 70.0, "Site", 17, th.fg, bold=True, anchor="start")
    ...
    s.text(380.0, 84.0, "reference flight path", 15, th.fg)
    s.ground(gy, 60.0, 866.0)
    s.line(378.0, 112.0, mx + 4.0, gy - 30.0, th.primary, 2.2)
    s.text(360.0, 232.0, "slant path", 14, th.primary, anchor="start")
    ...
    s.text(mx + 140.0, gy - 38.0, "80° half-angle about the vertical", 14,
    ...
    s.text(612.0, gy - 112.0, "inside the cone: site rejected", 13,
           th.secondary)

    # The station itself, small at this scale, and the tracking sensor.
    s.mic(mx, gy - 26.0, gy, 0.7)
    s.text(mx, gy + 22.0, "microphone", 14, th.muted)
    ...
    s.text(84.0, gy + 44.0, "tracking, independent of the cockpit", 14,
    ...
    s.text(56.0, 386.0, "The microphone, at 60 px per metre", 17, th.fg,
           bold=True, anchor="start")
    dgy = 540.0
    dx = 210.0
    ...
    s.text(dx - 84.0, cap - 42.0, "arriving ray", 14, th.primary, anchor="end")
    s.text(dx + 40.0, cap - 26.0, "windscreen: insertion loss", 14, th.muted,
           anchor="start")
    s.text(dx + 40.0, cap - 8.0, "within ±1.5 dB, and corrected for", 14,
           th.muted, anchor="start")
    s.dim(dx - 54.0, dgy, dx - 54.0, cap, "1.2 m", offset=0, size=15)
    s.line(dx - 26.0, cap, dx - 54.0, cap, th.muted, 0.9, dash="3,3")
    s.text(dx, dgy + 24.0, "sensing element 1.2 m above local ground", 14,
           th.muted)

    # --- plan: why the capsule points across the track ----------------------
    s.text(470.0, 386.0, "Plan: the capsule axis", 17, th.fg, bold=True,
           anchor="start")
    s.line(482.0, 452.0, 862.0, 452.0, th.muted, 1.2, dash="7,5")
    s.arrow(836.0, 452.0, 866.0, 452.0, th.muted, 1.5)
    s.text(858.0, 440.0, "track", 13, th.muted, anchor="end")
    ...
    s.text(672.0, 582.0, "24 one-third-octave bands, 50 Hz to 10 kHz", 14, th.fg)
    s.text(672.0, 600.0, "one sample every 500 ms ± 5 ms", 13, th.muted)
...
def _d_rotorcraft_hemisphere(s: SVG, th: Theme) -> None:
    ...
    gy = 532.0
    s.text(52.0, 386.0, "(c) The 60 m sphere on a flight path", 17, th.fg,
           bold=True, anchor="start")
