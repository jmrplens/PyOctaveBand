...
def _d_outdoor(s: SVG, th: Theme) -> None:
    ...
    s.text(66.0, gy + 26.0, "Ground ($G_s$, $G_m$, $G_r$)", 15, th.muted,
           anchor="start")
    ...
    s.text(ex + 16.0, (ey + gy) / 2 + 6.0, "Barrier", 17, th.secondary,
    ...
    s.text(285.0, sy + 40.0, "direct path (blocked)", 14, c_direct,
    ...
    s.text(450.0, gy + 58.0, "z = dss + dsr − d   (path difference)", 16,
           th.fg, bold=True)
    s.text(450.0, gy + 84.0,
           "Dz = 10 log10[ 3 + (C₂/λ) C₃ z Kmet ]   (Eq. 14)", 15, th.muted)
...
def _d_impulse_prominence(s: SVG, th: Theme) -> None:
    ...
    s.text(cx, 82, "A-weighted level history  $L_{pAF}$  (time weighting F)", 16,
           th.fg, "middle", bold=True)
    s.text(cx, 103, "an onset = a stretch where the gradient exceeds 10 dB/s "
           "(clauses 4.5-4.7)", 11, th.muted, "middle")
...
def _d_wind_turbine(s: SVG, th: Theme) -> None:
    ...
    s.text(pcx + 34, pcy + 52.0, "60°", 11, th.muted)
    s.text(pcx, pcy + pr + 30.0, "reference 1 (downwind)", 12,
           th.secondary)
...
def _d_ground_reflection(s: SVG, th: Theme) -> None:
    ...
    s.text(560.0, 88.0, "path difference  $δ = r_2 − r_1$", 17, th.fg,
           bold=True)
    s.text(560.0, 114.0, "phase difference  $Δφ = 2π δ / λ$  (+ $arg Q$)", 15,
           th.fg)
...
def _d_atmospheric_refraction(s: SVG, th: Theme) -> None:
    ...
    s.text(188.0, 414.0, "acoustic shadow", 13, th.secondary, italic=True,
           anchor="start")
    ...
    s.dim(sx + 34, gy, sx + 34, sy, "2 m", offset=0, size=13,
          label_side="right")
    ...
    s.dim(mrx + 38, gy, mrx + 38, gy - 46.0, "1.5 m", offset=0, size=12,
          label_side="right")
...
def _d_ground_barrier(s: SVG, th: Theme) -> None:
    ...
    s.text(ex + 18, ey + 74, "Barrier", 16, th.secondary, bold=True,
           anchor="start")
...
def _d_ground_regions(s: SVG, th: Theme) -> None:
    ...
    s.rect(grass_end, gy - band, rec_start - grass_end, band, th.muted,
           th.accent, sw=2.0)
    s.text((src_end + grass_end) / 2, gy - band - 12, "grass, $G = 1$", 15,
           th.fg)
    s.text((grass_end + rec_start) / 2, gy - band - 12, "asphalt, $G = 0$", 15,
           th.fg)
    ...
    s.text(x0 + 12, gy - hs - 40, "Source", 17, th.fg, anchor="start",
           bold=True)
    s.text(x0 + 12, gy - hs - 18, "$h_s$ = 1,5 m", 15, th.muted, anchor="start")
    s.mic(x1, gy - hs, gy, 1.0)
    s.text(x1 - 12, gy - hs - 40, "Receiver", 17, th.fg, anchor="end",
           bold=True)
    ...
    s.dim(ix0, iy + 48, ix1, iy + 48,
          "$d_p$ = 60 m: the regions overlap, so there is no middle region", 0, 15)

    # The two reading boxes, on one row under the drawing.
    by = 500.0
    s.rect(70.0, by, 380, 100, th.panel, th.muted, rx=8, sw=1.2)
    s.text(84.0, by + 28, "$G$ is the porous fraction of its region", 16, th.fg,
           anchor="start", bold=True)
    s.text(84.0, by + 54, "$G_s = 25/45$ = 0,55", 15, th.fg, anchor="start")
    s.text(84.0, by + 78, "$G_m = 70/110$ = 0,64   $G_r$ = 1,00", 15, th.fg,
           anchor="start")

    s.rect(470.0, by, 380, 100, th.panel, th.muted, rx=8, sw=1.2)
    s.text(484.0, by + 28, "$q = 1 − 30(h_s + h_r)/d_p$ = 0,55", 16, th.fg,
           anchor="start")
    s.text(484.0, by + 54, "below $30(h_s + h_r)$ = 90 m: $q = 0$, $A_m = 0$",
           15, th.secondary, anchor="start")
    s.text(484.0, by + 78, "and ground_middle is ignored entirely", 15, th.fg,
           anchor="start")
...
def _d_barrier_four_paths(s: SVG, th: Theme) -> None:
    ...
    s.text(ex + 14, gy - 16, "4 m screen", 16, th.fg, anchor="start",
           bold=True)
    ...
    s.text(lx + 14, ly + 26, "Four coherent routes over one edge", 16, th.fg,
           anchor="start", bold=True)
    ...
    s.text(tx + 14, ty + 26, "Thick barrier: two edges", 16, th.fg,
           anchor="start", bold=True)
...
def _d_cnossos_road(s: SVG, th: Theme) -> None:
    ...
    s.text(rx1 + 10, py - lane / 2 + 6, "source line,", 15, th.secondary,
           anchor="start")
    s.text(rx1 + 10, py + lane / 2 + 6, "one per lane centre", 15, th.secondary,
           anchor="start")
    ...
    s.text(rx0, cy + 62, "each segment carries L'W,eq,line,i + 10 lg(dL)", 15,
    ...
    s.text(gx0 - 12, gy0 - 56, "$max(1 − |x|/100, 0)$", 13, th.fg,
           anchor="start")
    ...
    s.text(rxr - 46, sy - 162, "receiver point", 15, th.fg, anchor="start",
           bold=True)
...
def _d_cnossos_rail(s: SVG, th: Theme) -> None:
    ...
    s.text(cx - 26, ay - 6, "A — rolling, impact, squeal,", 14, th.fg,
           anchor="end")
    s.text(cx - 26, ay + 16, "bridge, low traction", 14, th.fg, anchor="end")
    s.text(cx - 26, by + 16, "B — exhausts, roof apparatus,", 14, th.fg,
           anchor="end")
    ...
    s.text(ox - 96, py + 26, "−20 dB along the track", 15, th.accent,
           anchor="end")
    s.arrow(ox, py, ox + 140, py - 78, th.primary, 1.8)
    s.text(ox + 148, py - 82, "receiver bearing", 15, th.primary,
           anchor="start")
    s.text(ox + 56, py - 18, "$φ$", 17, th.primary, bold=True)
    s.rect(600.0, py - 140, 280, 112, th.panel, th.muted, rx=8, sw=1.2)
    s.text(614.0, py - 110, "Impact noise applies from 50 m", 14, th.fg,
           anchor="start")
    s.text(614.0, py - 90, "before a joint to 50 m after it", 14, th.fg,
           anchor="start")
    s.text(614.0, py - 60, "Curve squeal needs ≥ 50 m", 14, th.fg,
           anchor="start")
...
def _d_wind_turbine_board(s: SVG, th: Theme) -> None:
    ...
    s.text((bx0 + bx1) / 2, gy + 86,
           "plywood ≥ 12,0 mm  ·  metal ≥ 2,5 mm", 15, th.fg)
    ...
    s.text(mcx, gy + 58, "capsule diaphragm in the board plane, ≤ 13 mm", 14,
           th.fg)
    ...
    s.text(mcx, gy - 166, "document and correct its insertion loss", 14,
           th.muted)
...
def _d_rd1367_chain(s: SVG, th: Theme) -> None:
    ...
    s.text(x0, y0 - 12, "1 — the day, split into evaluation periods (Annex I A.1)",
           16, th.fg, anchor="start", bold=True)
    ...
    s.text(x0, py + 58, "noise phases $T_i$ of uniformly perceived level", 15,
           th.muted, anchor="start")

    # Stage 2: the phase corrections.
    y1 = 250.0
    s.rect(x0, y1, 360, 108, th.panel, th.muted, rx=8, sw=1.2)
    s.text(x0 + 14, y1 + 28, "2 — each phase, corrected", 16, th.fg,
           anchor="start", bold=True)
    ...
    s.text(x0 + 14, y1 + 56, "LKeq,Ti = LAeq,Ti + Kt + Kf + Ki", 15, th.fg,
           anchor="start", mono=True)
    s.text(x0 + 14, y1 + 82, "$K_t + K_f + K_i ≤ 9$ dB (Annex IV A.3.3)", 15,
    ...
    s.text(x0 + 450, y1 + 28, "3 — the period level", 16, th.fg, anchor="start",
           bold=True)
    s.text(x0 + 450, y1 + 54, "LKeq,x = 10 lg[ (1/T) Σ Ti", 14, th.fg,
           anchor="start", mono=True)
    s.text(x0 + 450, y1 + 74, "                10^(LKeq,Ti/10) ]", 14, th.fg,
           anchor="start", mono=True)
    s.text(x0 + 450, y1 + 98, "round_reported_level → 57 dB", 15, th.accent,
           anchor="start", mono=True)

    # Stage 4: the annual mean.
    y2 = 400.0
    s.rect(x0, y2, 360, 96, th.panel, th.muted, rx=8, sw=1.2)
    s.text(x0 + 14, y2 + 28, "4 — the annual value", 16, th.fg, anchor="start",
           bold=True)
    s.text(x0 + 14, y2 + 56, "$L_{K,x}$ over the operating days", 15, th.fg,
           anchor="start")
    s.text(x0 + 14, y2 + 82, "303 open / 62 closed → 56 dB", 15, th.accent,
           anchor="start", mono=True)
    ...
    s.text(x0, y2 + 146, "Article 25.2 drops the third criterion for an "
           "activity already in operation", 14, th.muted, anchor="start")
