...
def _d_hydrophone_deployment(s: SVG, th: Theme) -> None:
    ...
    s.text(750, 220, "lDW = 2 dCPA tan 30°", 14, th.fg, mono=True)

    # Run schedule: what the data window sits inside (Clause 5.5-5.6).
    s.text(752, 384, "Run schedule (not to scale)", 17, th.fg, bold=True)
    s.line(630, 430, 878, 430, th.fg, 1.8)
    for tx, tlab in ((640.0, "COMEX"), (866.0, "FINEX")):
        s.line(tx, 420, tx, 440, th.fg, 1.6)
        s.text(tx, 412, tlab, 14, th.fg)
    s.line(722, 430, 784, 430, th.accent, 4.0)
    s.text(753, 458, "data window", 14, th.accent)
    s.dim(640, 444, 722, 444, "≥ 2 × DWL", offset=0, size=13)
    s.dim(784, 444, 866, 444, "≥ 2 × DWL", offset=0, size=13)
...
def _d_pile_driving_deployment(s: SVG, th: Theme) -> None:
    ...
    # --- Section view -------------------------------------------------------
    s.text(left + 4, 64, "Section: the minimum campaign", 19, th.fg, bold=True,
           anchor="start")
    ...
    s.text(300, bed + 30, "seabed", 14, th.muted)

    # Water depth, inside the 4 m to 100 m scope of Clause 1.
    s.dim(left + 22, surf, left + 22, bed, "30 m", offset=0, size=16)
    ...
    s.text(px_ + 34, 100, "impact hammer", 15, th.fg, anchor="start")
    s.text(176, 214, "monopile", 15, th.fg, anchor="start")
    s.line(172, 209, px_ + 13, 209, th.muted, 0.9)
    s.line(px_ + 11, bed + 44, px_ + 60, bed + 44, th.muted, 0.9, dash="3,3")
    s.dim(px_ + 48, bed, px_ + 48, bed + 44, "penetration", offset=0, size=14,
          label_side="right")
    ...
    s.text(px_ + 52, 330, "bubble curtain, if used", 14, th.muted, anchor="start")
    ...
    s.dim(px_, 134, rx_, 134, "as close as possible to 750 m", offset=0, size=17)
    ...
    s.text(right - 2, y_half - 30, "lower half of the water column:", 14,
           th.fg, anchor="end")
    s.text(right - 2, y_half - 12, "2 m above the bed to ½ depth", 14, th.fg,
           anchor="end")
    ...
    for frac, lab in ((0.5, "½ depth"), (0.75, "¾ depth")):
        ...
        s.text(rx_ + 14, hy + 5, lab, 14, th.fg, anchor="start", mono=True)
    s.text(rx_ + 2, bed + 34, "bottom-mounted recorder", 14, th.muted)
    ...
    s.text(176, 372, "engines, generator and echo-sounder off;", 14,
           th.muted, anchor="start")
    s.text(176, 390, "flow noise, cable strum and surface heave", 14,
           th.muted, anchor="start")
    s.text(176, 408, "all read as signal", 14, th.muted, anchor="start")

    # --- Plan view ----------------------------------------------------------
    s.text(742, 64, "Plan: where else to measure", 19, th.fg, bold=True)
    cx_, cy_ = 742.0, 300.0
    ...
    s.text(cx_, cy_ - 18, "pile", 14, th.fg)
    ...
    s.text(cx_, cy_ + 58, "3 × water depth = 90 m:", 13, th.muted)
    s.text(cx_, cy_ + 74, "nothing inside", 13, th.muted)
    s.text(cx_ + 64, cy_ - 92, "750 m", 15, th.secondary, anchor="start",
           mono=True)
    s.text(cx_, cy_ + 176, "further positions on a transect,", 15, th.fg)
    s.text(cx_, cy_ + 196, "clear of banks and trenches", 15, th.fg)
    s.text(cx_, cy_ + 216, "(radii not to scale)", 13, th.muted)
...
def _d_sonar_equation(s: SVG, th: Theme) -> None:
    ...
    # --- Passive ------------------------------------------------------------
    s.text(60, 70, "Passive: the target radiates, one way", 19, th.fg,
           bold=True, anchor="start")
    s.rect(60, 84, 780, 214, th.bg, th.muted, rx=4, sw=1.0)
    _noise_field(74, 700, 106, 250)
    s.text(300, 118, "ambient noise field NL", 15, th.muted)
    ...
    s.text(132, 152, "SL", 18, th.secondary, bold=True)
    s.text(132, 252, "source level", 14, th.fg)
    ...
    s.path("M 636 196 L 566 168 L 566 224 Z", fill="none", stroke=th.accent,
           sw=1.6, dash="6,4")
    s.text(600, 268, "DI", 18, th.accent, bold=True)
    s.text(600, 288, "array gain", 14, th.fg)
    ...
    s.text(818, 380, "sea surface", 13, th.muted, anchor="end")
    s.ground(566, 74, 826, hatch=30)

    # Transmitter / receiver at one point.
    s.rect(96, 452, 34, 48, th.panel, th.fg, rx=3, sw=1.8)
    s.text(113, 434, "SL, DI, DT", 14, th.fg)
    s.text(113, 522, "transmit and", 14, th.fg)
    s.text(113, 540, "receive here", 14, th.fg)
    ...
    s.text(566, 440, "TS", 18, th.secondary, bold=True)
    s.text(566, 420, "target strength", 14, th.fg)
    for r_ in (20.0, 30.0):
...
def _d_underwater_waveguide(s: SVG, th: Theme) -> None:
    ...
    s.text(208, axis_y - 8, "channel axis", 13, th.muted, anchor="end")
...
def _d_marine_mammal_exposure(s: SVG, th: Theme) -> None:
    ...
    # --- Section ------------------------------------------------------------
    s.text(60, 74, "Section: pile, hydrophone, animal", 19, th.fg, bold=True,
           anchor="start")
    dsur = f"M 60 {surf}"
    x = 60.0
    while x < 520:
    ...
    s.text(px_ + 22, 250, "pile", 14, th.fg, anchor="start")
    ...
    s.text(hx_, 326, "calibrated hydrophone", 14, th.fg)
    s.text(hx_, 344, "band SEL measured here", 14, th.secondary)
    s.dim(px_, 128, hx_, 128, "750 m", offset=0, size=15)
    ...
    s.text(290, 486, "weighted cumulative SEL  vs  AUD INJ / TTS", 15, th.fg)
    s.text(290, 510, "unweighted peak SPL  vs  the flat criterion", 15, th.fg)

    # --- Plan: the two isopleths -------------------------------------------
    s.text(700, 74, "Plan: the two isopleths", 19, th.fg, bold=True)
    cx_, cy_ = 700.0, 260.0
    ...
    s.circle(cx_, cy_, 3, th.fg)
    s.text(cx_, cy_ - 18, "pile", 14, th.fg)
    s.text(cx_, cy_ + 88, "peak SPL isopleth", 13, th.secondary)
    s.text(cx_, cy_ + 138, "weighted cumulative SEL isopleth", 13, th.accent)
    s.text(cx_, cy_ + 168, "the larger isopleth governs", 16, th.fg, bold=True)
    s.text(cx_, cy_ + 196, "VHF, impulsive, NMFS 2024:", 14, th.fg)
    s.text(cx_, cy_ + 216, "144 / 159 dB re 1 µPa²·s weighted", 14, th.fg)
