...
def _d_hearing_threshold(s: SVG, th: Theme) -> None:
    ...
    s.text(cx, 106, "audiometric frequencies 125 Hz – 8000 Hz", 13, th.muted, "middle")
    ...
    s.text(cx, 476, "referenced to the audiometric zero", 12, th.primary, "middle")
...
def _d_nihl(s: SVG, th: Theme) -> None:
    ...
    _step(lxc, 150, "Age threshold  $H$  (HTLA)", "ISO 7029 fractile, dB", th.primary)
...
def _d_zwicker(s: SVG, th: Theme) -> None:
    ...
    _step(
        132,
        "Equal-loudness correction and lower critical bands  (Clause 5.4, Table A.3)",
...
def _d_loudness_capture(s: SVG, th: Theme) -> None:
    ...
    for k in range(11):
        wx = ax0 + 20 + k * 36
        s.path(
            f"M {wx} 110 L {wx + 30} 110 L {wx + 15} 134 Z",
            fill=th.panel,
            stroke=th.muted,
            sw=1.2,
        )
    ...
    s.text(
        450,
        588,
        "free-field equalization only for one frontal source "
...
def _d_mg_capture_routes(s: SVG, th: Theme) -> None:
    ...
    for k in range(3):
        s.path(f"M {124 + k * 20} {y + 52} q 9 30 0 60", stroke=th.secondary, sw=1.4)
    ...
    s.path(
        f"M {hx + 30} {hy - 9} L {hx + 96} {hy - 17} L {hx + 96} "
        f"{hy + 17} L {hx + 30} {hy + 9} Z",
        fill=th.bg,
        stroke=th.fg,
        sw=1.6,
    )
    ...
    hx, hy = 108.0, y + 82.0
    s.path(
        f"M {hx - 32} {hy + 32} L {hx - 22} {hy + 4} L {hx + 22} {hy + 4} "
        f"L {hx + 32} {hy + 32} Z",
        fill=th.panel,
        stroke=th.fg,
        sw=1.8,
    ...
    s.text(
        450,
        646,
        "presentation: monaural is one ear alone, diotic the "
    ...
    s.text(
        450,
        666,
        "a diotic sound is about 1.5 times as loud as the same "
        "sound at one ear (clause 8.1)",
        12,
        th.muted,
    )
...
def _d_tone_audibility_acquisition(s: SVG, th: Theme) -> None:
    ...
    states = (
        (0.0, 0.25, "idle", th.primary),
        (0.25, 0.75, "full load", th.secondary),
        (0.75, 1.0, "idle", th.primary),
    )
    for a, b, name, color in states:
        s.rect(x0 + a * span, 80, (b - a) * span, 42, th.panel, color, rx=6, sw=2)
    ...
    s.path(
        f"M {x0} 326 L {x1} 326 L {x1 - 170} 358 L {x0 + 170} 358 Z",
        fill=th.panel,
        stroke=th.muted,
        sw=1.4,
    )
    ...
    s.text(
        450, 379, "Energy mean of the $J$ decisive audibilities", 14, th.fg, bold=True
    )
    s.text(
        450,
        399,
...
def _d_dosimeter(s: SVG, th: Theme) -> None:
    ...
    s.path(
        f"M {mx:.0f} {sh_y:.0f} C {mx + 26:.0f} {sh_y + 56:.0f} "
    ...
    s.dim(
        mx + 44, sh_y, mx + 44, cap_y, "≈ 0.04 m", offset=0, size=15, label_side="right"
    )
    ...
    s.line(px + 24, head_y + 8, mx - 4, cap_y + 4, th.secondary, 1.4, dash="5,4")
    s.text(px, head_y - 82, "≥ 0.1 m from the ear canal,", 15, th.secondary)
    s.text(px, head_y - 62, "most-exposed side", 15, th.secondary)

    # --- Right: the three sampling strategies as day timelines -------------
    s.text(620, 84, "Measurement strategies (Clauses 9–11)", 19, th.fg, bold=True)
    x0, x1 = 390.0, 850.0
    bw = x1 - x0
    ax_y = 132.0
    ...
    def strip(y: float, title: str, caption: str) -> None:
        ...
        s.text(x0, y + 68, caption, 14, th.muted, "start", italic="$" not in caption)
    ...
    for frac in (0.05, 0.24, 0.46, 0.65, 0.86):
        s.rect(x0 + bw * frac, y2 + 6, bw * 0.06, 32, th.panel, th.primary, rx=4, sw=2)
...
def _d_sound_quality(s: SVG, th: Theme) -> None:
    ...
    metrics = (
        ...
        (
            ...
            "Roughness $R$",
            "ECMA-418-2 clause 7",
            "fast envelope modulation,",
            "band-pass peaking near 70 Hz",
            ("1 kHz, 100 % AM at 70 Hz, 60 dB",),
            "→ $R$ = 0.9999 asper",
        ),
        (
            702.0,
            "Fluctuation strength $F$",
            "ECMA-418-2 clause 9 (HSA)",
    ...
    s.text(
        450,
        463,
        "$N_5$, $S$, $R$ and $F$ feed the Fastl and Zwicker "
        "psychoacoustic annoyance $PA = N_5·(1 + √(w_S^2 + w_{FR}^2))$",
        10,
        th.fg,
    )
...
def _d_tone_audibility(s: SVG, th: Theme) -> None:
    ...
    s.rect(130, 488, 640, 68, "none", th.secondary, rx=10, sw=1.6, dash="6,5")
    s.text(
        cx,
        515,
    ...
    s.text(
        cx,
        539,
        "energy mean of the five spectra $ΔL$ = 6.98 dB → tonal "
        "adjustment $K_t$ = 4 dB (Table J.1)",
        11,
        th.fg,
    )
...
def _d_psychoacoustic_annoyance(s: SVG, th: Theme) -> None:
    ...
    inputs = (
        ...
        (
            262.0,
            "$N_5$ = 30 sone",
            "percentile loudness (ISO 532-1)",
            "exceeded 5 % of the time",
        ),
        (482.0, "$F$ = 0.5 vacil", "fluctuation strength", "slow modulation, ≈ 4 Hz"),
    ...
    s.text(260, 230, "Sharpness weighting $w_S$ = 0.1001", 13, th.fg, bold=True)
    s.text(260, 252, "$w_S = (S − 1.75) · 0.25 · log_{10}(N_5 + 10)$", 11, th.primary)
    s.text(260, 274, "zero for $S ≤ 1.75$ acum", 10, th.muted)
    s.rect(470, 204, 340, 86, th.panel, th.fg, rx=10, sw=2)
    s.text(
        640, 230, "Roughness and fluctuation $w_{FR}$ = 0.2125", 13, th.fg, bold=True
    )
    s.text(640, 252, "$w_{FR} = 2.18 / N_5^{0.4} · (0.4·F + 0.6·R)$", 11, th.primary)
    s.text(640, 274, "roughness weighs more: 0.6 against 0.4", 10, th.muted)
    ...
    s.text(
        450, 374, "$PA = N_5 · (1 + √(w_S^2 + w_{FR}^2))$ = 37.05", 15, th.fg, bold=True
    )
    s.text(450, 399, "Fastl and Zwicker Eq. 16.2 (origin Widmann 1992)", 10, th.muted)
    ...
    s.text(
        450,
        464,
        "a neutral sound ($S ≤ 1.75$ acum, $F = R = 0$) sits on "
        "the baseline $PA = N_5$",
        12,
        th.fg,
    )
    s.text(
        450,
        488,
        "sharpness, roughness and fluctuation only ever lift "
        "the annoyance above the loudness",
        11,
        th.muted,
    )
...
def _d_soundfield_audiometry(s: SVG, th: Theme) -> None:
    ...
    def ghost_subject(x: float) -> float:
        ...
        q = 0.15 * scale
        s.rect(
            x - q, ref_y - q, 2 * q, 2 * q, "none", th.accent, rx=3, sw=1.4, dash="5,4"
        )
    ...
    notes(
        xa,
        th.accent,
        "on the reference axis, 0° azimuth",
    ...
    xb = px0[1]
    room(xb, "B · Diffuse field", "third-octave noise band · binaural", th.accent)
    s.ground(gy, xb, xb + pw)
    sx = xb + pw / 2 + 20.0
    ref_y = ghost_subject(sx)
    for dx, dy in ((-112.0, -96.0), (108.0, -100.0), (104.0, 62.0), (-108.0, 58.0)):
    ...
    xc = px0[2]
    room(xc, "C · Earphone", "supra-aural or insert · monaural", th.secondary)
    s.ground(gy, xc, xc + pw)
    sx = xc + 74.0
    s.person(sx, gy, 190.0, seated=True)
    ref_y = gy - 190.0 + 19.0
    s.path(
        f"M {sx - 20:.0f} {ref_y - 8:.0f} Q {sx:.0f} {ref_y - 42:.0f} "
        f"{sx + 20:.0f} {ref_y - 8:.0f}",
        stroke=th.secondary,
        sw=2.6,
    )
    ...
    s.text(xc + pw / 2, 148, "not this standard: the level lives", 11, th.secondary)
    ...
    s.text(
        450,
        514,
        "Reference point: the midpoint of the line joining the "
        "listener's ear-canal openings",
        14,
        th.fg,
        bold=True,
    )
    s.text(
        450,
        540,
        "the listener in the listening position; in A and B the "
        "level is measured there with the subject and chair absent",
        12,
        th.muted,
    )
...
def _d_slm_workstation(s: SVG, th: Theme) -> None:
    ...
    def machine(x: float) -> None:
        ...
        for k in range(3):
            s.path(
                f"M {x + 96 + k * 15:.0f} {gy - 100:.0f} q 8 11 0 22 q -8 11 0 22",
                stroke=th.secondary,
                sw=1.6,
            )

    # ---------------- Left: worker absent ---------------------------------
    panel(xa, "Worker absent", "the preferred Clause 12.4 placement", th.primary)
    s.ground(gy, xa, xa + pw)
    machine(xa + 20)
    mx = xa + 276.0
    ...
    panel(xb, "Worker present", "hand-held meter, most-exposed ear", th.secondary)
    s.ground(gy, xb, xb + pw)
    machine(xb + 18)
    px = xb + 242.0
    ph = 1.75 * m
    ...
    s.text(
        450,
        514,
        "A fixed microphone position under-reads a hand-held "
        "tool close to the ear (13.1)",
        14,
        th.fg,
        bold=True,
    )
    s.text(
        450,
        540,
        "that is exactly when the worn personal exposure meter "
        "of Clause 12.3 is the right instrument",
        12,
        th.muted,
    )
...
def _d_sti_setup(s: SVG, th: Theme) -> None:
    ...
    room(xa, "A · Unamplified talker", "acoustical input, clause 7.2", th.primary)
    tx = xa + 74.0
    ...
    s.path(
        f"M {tx + 16:.0f} {mouth - 14:.0f} L {tx + 40:.0f} {mouth - 24:.0f} "
    ...
    s.text(tx + m + 8, mouth - 118, "60 dB(A) here, or the", 11, th.secondary, "start")
    s.text(
        tx + m + 8, mouth - 102, "Annex J operational level", 11, th.secondary, "start"
    )
    ...
    s.line(rx1 + 6, gy - 1.2 * m, rx1 + 26, gy - 1.2 * m, th.muted, 0.9, dash="3,3")
    notes(
        xa,
        th.primary,
    ...
    for k in range(3):
        ...
        s.path(
            f"M {lx - 26:.0f} {ceil + 56:.0f} L {lx:.0f} {ceil + 24:.0f} "
            f"L {lx + 26:.0f} {ceil + 56:.0f}",
            stroke=th.accent,
            sw=1.2,
            dash="5,4",
        )
    ...
    s.text(xb + 6, rack_y + 84, "at the Annex J level", 10, th.secondary, "start")
    ...
    s.text(
        450,
        514,
        "The rating of the space is the mean of the positions "
        "minus one standard deviation (7.6.4)",
        14,
        th.fg,
        bold=True,
    )
    s.text(
        450,
        540,
        "a plain mean over the positions overstates coverage; "
        "better still, plot the whole distribution",
        12,
        th.muted,
    )
...
def _d_stoi_bench(s: SVG, th: Theme) -> None:
    ...
    def box(
        x: float, y: float, w: float, h: float, title: str, sub: str, color: str
    ) -> None:
    ...
    s.path(
        f"M 122 {ly - 14:.0f} C 168 {ly - 14:.0f} 168 {fy + 26:.0f} 212 {fy + 26:.0f}",
        stroke=th.primary,
        sw=2.2,
    )
    ...
    s.path(
        f"M 690 {ly:.0f} C 714 {ly:.0f} 714 {ly - 40:.0f} 714 {ly - 62:.0f}",
        stroke=th.secondary,
        sw=2.2,
    )
    ...
    s.text(415, 416, "run it once with the device bypassed", 13, th.primary, bold=True)
    s.text(
        415,
        438,
        "the loudspeaker, the box noise and the microphone "
        "are scored as degradation too",
        11,
        th.muted,
    )
    ...
    s.text(
        450,
        518,
        "the index is invariant to level, the device is not; a "
        "single capture carries the acoustic path's run-to-run spread",
        11,
        th.muted,
    )
