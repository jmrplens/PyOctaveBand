...
def _d_multiple_shock(s: SVG, th: Theme) -> None:
    ...
    s.text(cx, 72, "Vertical seat acceleration  $a_{z}(t)$", 16, th.fg,
           "middle", bold=True)
    s.text(cx, 92,
           "conditioned per 5.1.3:  HP 0.01 Hz (2nd order) / LP 80 Hz "
           "(4th order)", 11, th.muted, "middle")
    s.arrow(cx, 106, cx, 136, th.fg, 1.8)
    s.text(cx - 26, 128, "not the ISO 2631-1 0.4 Hz / 100 Hz filters", 10,
           th.secondary, "end")
    ...
    s.text(cx, 513, "Injury probability  $P(R) = 1 − exp(−(R/α)^β)$  "
           "(Formula C.5)", 15, th.fg, "middle", bold=True)
    s.text(cx, 533, "Weibull risk of lumbar injury, by sex (Table C.1/C.2)", 11,
           th.muted, "middle")
...
def _d_hand_arm_vibration(s: SVG, th: Theme) -> None:
    ...
    s.text(258, 74, "On the tool (ISO 5349-2, 6.1.3)", 15, th.fg, "middle",
           bold=True)
    ...
    s.text(45, 366, "1  middle of the gripping zone, under the hand",
           13, th.fg, "start", bold=True)
    s.text(62, 385, "the most representative location; needs an adaptor",
           13, th.muted, "start")
    s.text(45, 410, "2  either side of the hand — usual practice", 13, th.fg,
           "start", bold=True)
    s.text(62, 429, "on a side handle, average the two positions", 13,
           th.muted, "start")
    s.text(45, 454, "3  underside of the handle, next to the hand", 13, th.fg,
           "start", bold=True)
    s.text(45, 480, "grip and push force change the reading: report the",
           12, th.muted, "start")
    s.text(45, 497, "posture and the applied forces (7.1, clause 9 g))", 12,
           th.muted, "start")
    ...
    s.text(cx, 464, "EAV 2.5 m/s²   ·   ELV 5 m/s²", 16, th.fg, "middle",
           bold=True)
...
def _d_iso2631_5_setup(s: SVG, th: Theme) -> None:
    ...
    s.text(681, 78, "The record, split where contact is lost", 15, th.fg,
           "middle", bold=True)
    ...
    for lo, hi in gaps:
        s.text(ax0 + 0.5 * (lo + hi) * (ax1 - ax0), base - 100,
               "no contact", 12, th.secondary, "middle")
    s.text(ax0, base + 118, "$a_{z}(t)$, conditioned per 5.1.3", 12, th.muted,
           "start")
    ...
    s.text(681, base + 186,
           "each segment is conditioned separately (5.1.3, second step)", 12,
           th.fg, "middle")
...
def _d_fault_kinematics(s: SVG, th: Theme) -> None:
    ...
    s.text(cx, cy + r_out_o + 34, "inner race turns at $f_s$ = 33.33 Hz,", 13,
           th.fg)
    s.text(cx, cy + r_out_o + 56, "outer race stationary", 13, th.fg)
    s.text(cx, cy + r_out_o + 82, "cage FTF = 13.8 Hz = 0.41 $f_s$", 13,
           th.accent)
    ...
    s.text(690, axy + 206,
           ...
           13, th.muted)
    s.text(690, axy + 226,
           "and $φ > 0$ for angular-contact and tapered-roller types", 13,
           th.muted)
    ...
    s.text(450, 730, "$GMF = N f_s = 28 × 25$ = 700 Hz, and a chipped tooth "
           "modulates it once per revolution: sidebands at $± f_s$", 14,
           th.fg)
...
def _d_machine_diagnostics(s: SVG, th: Theme) -> None:
    ...
    s.text(328, 322, "radial, in the load zone, on the housing itself: no "
                     "joint between bearing and sensor", 13, th.muted)
    ...
    s.text(70, 386, "Mounting sets the usable upper frequency", 15, th.fg,
    ...
    s.text(492, 450, "$T$ = 2 s at 2000 r/min = 67 revolutions", 14, th.fg,
           anchor="start")
    s.text(492, 480, "$Δf = 1/T$ = 0.5 Hz, against $f_s$ = 33.3 Hz", 14,
           th.fg, anchor="start")
    s.text(492, 522, "enough to resolve the $± f_s$ sidebands, which are", 12,
           th.muted, anchor="start")
    s.text(492, 542, "what separates an inner-race defect from an", 12,
           th.muted, anchor="start")
...
def _d_mobility_rig(s: SVG, th: Theme) -> None:
    ...
    s.text(470, beam_bot + 32, "Structure under test (free-free beam)", 16,
           th.fg, bold=True)
    ...
    s.text(60, 402, "$F$ and $a$ at the drive point", 12, th.muted,
           anchor="start")
    ...
    s.text(210, 218, "driving point:  $Y_{ii} = v_i / F_i$", 14, th.fg,
           anchor="start")
    ...
    s.text(450, 520,
           ...
           "impact hammer (Part 5)", 15, th.fg)
    s.text(450, 546,
           "same measurement, three FRFs: $x/F$ receptance · $v/F$ mobility "
           "· $a/F$ accelerance", 13, th.muted)
    ...
    s.text(450, 600, "Where the accelerometer and the force transducer go "
           "(clause 6.4.4)", 16, th.fg, bold=True)
    verdicts = (
    ...
    s.text(450, 830, "and the suspension at least 10× more mobile than the "
           "structure at each attachment point (5.3)", 13, th.muted)
...
def _d_transfer_stiffness_rig(s: SVG, th: Theme) -> None:
    ...
    for cx, head in ((250.0, "Direct method (Part 2)"),
        ...
        s.text(cx + 28, 260, "isolator under test", 14, th.accent,
               anchor="start")
    s.text(48, 296, "$a′_1$: unwanted transverse input,", 12, th.secondary,
           anchor="start")
    s.text(48, 314, "≥ 15 dB below $a_1$ (Inequality 3)", 12, th.secondary,
           anchor="start")
    ...
    s.text(cx, 470, "output blocked:  $u_2 ≈ 0$ → measure $F_{2,b}$", 14,
           th.fg)
    ...
    s.text(508, 942, "roller bearings, or two symmetrical", 12, th.muted,
           anchor="start")
    s.text(508, 962, "elements, suppress the unwanted input", 12, th.muted,
           anchor="start")
    ...
    s.text(508, 1000, "output shear force summed from two", 12, th.fg,
           anchor="start")
    s.text(508, 1020, "transducers,  $F_2 = F_2′ + F_2″$", 12, th.fg,
           anchor="start")
...
def _d_junction_rig(s: SVG, th: Theme) -> None:
    ...
    s.text(80, 536, "$l_{ij} ≥ 2.3$ m along the junction; element sizes "
           "3.0 m $≤ l_i <$ 6.0 m", 15, th.fg, anchor="start")
    s.text(80, 564, "≥ 4 excitation positions on $i$; accelerometers "
           "≥ 0.25 m from edges, ≥ 0.5 m apart", 15, th.fg, anchor="start")
...
def _d_power_injection_rig(s: SVG, th: Theme) -> None:
    ...
    def _pair(x0: float, driven: int, head: str) -> None:
        ...
        s.text(x0 + 160, 150, "subsystem 2", 15, th.secondary,
               anchor="start")
        ...
        if driven == 1:
            ...
            s.text(x0 + 176, gy + 20, "impedance head", 13, th.secondary,
                   anchor="start")
        ...
        s.text(x0 + 40, gy + 96,
               ...
               f"$Π_{'2' if driven == 1 else '1'} = 0$", 14, th.fg,
               anchor="start")
    ...
    s.text(450, 566, "$Π_{in} = ½ Re{F v*}$ at the drive point, from an "
           "impedance head — not the amplifier setting", 15, th.fg)
    s.text(450, 592, "$⟨v^2⟩$ space-averaged over several positions per "
           "subsystem, away from the edges and from the drive point", 15,
           th.fg)
    s.text(450, 618, "one run gives $η_{12}$ only if $η_1$ and $η_2$ come "
                     "from a decay measurement; two runs solve all four", 15,
           th.primary, bold=True)
    s.text(450, 644, "bands wide enough to hold several modes of each "
           "subsystem: the modal densities decide how wide", 13, th.muted)
