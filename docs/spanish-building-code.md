...
is in [Insulation Ratings (ISO 717)](insulation-ratings.md).
...
$$
I_x = -10 \lg \sum_i 10^{(L_{x,i} - X_i)/10} \quad \text{[dBA]}
$$
...
bands, 100 Hz to 5 kHz, four more than the sixteen of ISO 717-1 (100 Hz to
3150 Hz), and those four high bands are the first reason the two routes do not
always agree. The band centres are in `DB_HR_FREQUENCIES`.

| Normalised spectrum | Annex A table | Quantity it defines |
| :--- | :--- | :--- |
| A-weighted pink noise, $L_{Ar,i}$ | Table A.5 | $R_A$, $D_{nT,A}$ |
| Road traffic, $L_{Atr,i}$ | Table A.3 | $R_{A,tr}$, $D_{2m,nT,Atr}$ |
| Railway noise, $L_{Aef,i}$ | Table A.4 (numerically identical to A.3) | facades with dominant rail noise |
| Aircraft noise, $L_{Aav,i}$ | Table A.2 | facades with dominant aircraft noise |
...
`ra()`, `ra_tr()`, `dnt_a()` and `d2m_nt_atr()` are named shortcuts over
...
the document itself accepts the approximations
$D_{nT,A} \approx D_{nT,w} + C_{100-5000}$ and
$D_{2m,nT,Atr} \approx D_{2m,nT,w} + C_{tr,100-5000}$ as long as the two routes
differ by less than 1 dB. On the two published oracles this page reproduces,
the agreement is exact to the integer:

| Case | Direct route (Annex A) | ISO 717-1 route |
| :--- | :--- | :--- |
| Separating wall of Ejemplo 7.2 | $R'_A = 51.4$ dBA, rounded to 51 | $R'_w = 52$ with $C_{100-5000} = -1$, i.e. 51 |
| Facade of Ejercicio 7.1 | $D_{2m,nT,Atr} = 32.8$ dBA, rounded to 33 | $D_{2m,nT,w} = 38$ with $C_{tr,100-5000} = -5$, i.e. 33 |

The real trap is the name of the spectrum adaptation terms. DB-HR calls them
simply $C$ and $C_{tr}$, but they are the **enlarged-range** terms
$C_{100-5000}$ and $C_{tr,100-5000}$, not the core-range terms of ISO 717-1. On
the facade of Ejercicio 7.1 the core-range $C$ is $-2$ while $C_{100-5000}$ is
$-1$: using the wrong term would give 36 dBA instead of 37 for the
$D_{2m,nT,A}$ index of that same facade.
...
[Predicting Panel Sound Insulation](panel-sound-insulation.md). With the 8 m²
