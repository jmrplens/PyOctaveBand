...
An `AnpAircraft` carries the metadata the Doc 29 chain needs downstream: the
engine type and count, the ICAO wake weight class, and the **power parameter**
the NPD table is indexed by. That last one matters because it is not a force in
newtons but whatever quantity the manufacturer tabulated against, corrected net
thrust in pounds for most jets, and the value you pass later has to be in those
units.
