...
you pass `fs` as below, also validates the recording's short-term stability the
way IEC 60942 qualifies the calibrator itself, so a badly coupled microphone is
caught here instead of corrupting every level downstream. Omit `fs` and you get
the factor on its own, with no check.
