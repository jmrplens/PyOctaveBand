...
the 15 848.9 Hz row falls 16.2 dB below its design goal (class 1 allows
...
−5.0 dB). `verify_weighting_class` measures that whole path, so it reports
class 2 at 32 kHz and no class at all at 16 kHz rather than a grade the
filter cannot deliver. Sample at 44.1 kHz or above for a class 1 A-weighting
