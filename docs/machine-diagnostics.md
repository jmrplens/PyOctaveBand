...
already provides - the [envelope spectrum and cepstrum](cepstrum-echoes.md) and
...
elements of diameter `d` on a pitch diameter `D`, a contact angle `φ`, and
writing `g = (d/D) cos φ` and `fs = N/60`:

| Name | Meaning | Expression | Norton eq. |
|---|---|---|---|
| `shaft` | shaft rotational frequency | `fs = N/60` | (8.4) |
| `FTF` | cage (fundamental train) frequency | `(fs/2)(1 - g)` | (8.5) |
| `FTF_rel` | cage relative to the rotating race | `fs - FTF` | (8.11) |
| `BSF` | rolling-element rotational frequency | `(fs/2)(D/d)(1 - g²)` | (8.7) |
| `BDF` | rolling-element spin frequency | `2 BSF` | (8.10) |
| `BPFO` | element pass frequency, outer race | `Z (fs/2)(1 - g)` | (8.8) |
| `BPFI` | element pass frequency, inner race | `Z (fs/2)(1 + g)` | (8.9) |
...
errors instantly: `BPFO + BPFI = Z fs` always, and `BPFO = Z × FTF` whenever
...
`GMF = N fs` (Eq. 8.3), with integer harmonics. What distinguishes the fault
...
`n = 1, 2, …`. Dynamic eccentricity dresses the dominant slot harmonic with
sidebands at ± the shaft rate and ± the slip frequency.
...
`fe = fs p / (2 (1 - s))`; Norton's Eq. (8.19) writes `fe = fs p / 2`, which is
the same expression at zero slip but does not reduce Eq. (8.20) to the physical
rotor-bar passing rate `R fs` when the machine is loaded. Under load the result
also carries the slip frequency and the pole-pass line `FP = p × f_slip`, the
sideband spacing that a broken rotor bar puts around `1x`.

## 4. Fans, blowers and pumps

The blade-passing frequency of an impeller with `N` blades is `fb = n N fs`
(Eq. 8.15); a pump's hydraulic pulsations follow the same form with `N` pumping
events per revolution (Eq. 8.18), and a rotary positive-displacement blower
repeats four times per revolution, so it is handled by passing `4 × r/min`.

In a **ducted axial fan** the blades also interact with the stator vanes and set
up rotating pressure patterns with `mL = n N ± k V` lobes for `V` vanes
(Eq. 8.16), turning at `n N fs / mL` (Eq. 8.17). A pattern that spins faster
...
2. **Detect and transform the envelope.** The repetition rate of the impacts
   ...
   [cepstrum and envelope-spectrum guide](cepstrum-echoes.md).
...
[Time synchronous averaging](synchronous-averaging.md) removes everything not
locked to one shaft, which separates a gearbox's two shafts before the
sidebands are read; and the [cepstrum](cepstrum-echoes.md) collapses a whole
...
above 4 damaged). The pole-pass frequency `FP = p × f_slip` is standard
...
- [Cepstrum, echoes and the envelope spectrum](cepstrum-echoes.md): the
  envelope spectrum these lines are drawn on, and the cepstral collapse of a
  harmonic family.
- [Time synchronous averaging](synchronous-averaging.md): separating one
...
- [Bending-wave transmission at plate junctions](junction-transmission.md): what
