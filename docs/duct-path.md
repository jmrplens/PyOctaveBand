...
The terminal device is the element a real sheet never models. Neither
ASHRAE nor Long publishes a closed form for the sound power of a grille,
register or diffuser: it comes from manufacturer data measured to ASHRAE
Standard 70. What the library offers instead are the two screening rules of
ASHRAE Chapter 49. `air_terminal_velocity_limit` (Table 9) gives the
maximum neck velocity for a design RC, for use when no sound data exists
for the selected device, and `air_terminal_damper_correction` (Table 10)
gives the penalty for throttling a balancing damper, which is where a great
many finished installations fail.
...
- Long, M. (2014). *Architectural acoustics* (2nd ed.). Academic Press.
  ...
  The fan sound-power model (Ch. 13, Eq. 13.1 and Tables 13.5-13.8), the
  duct attenuation, flexible duct, split loss, end reflection, silencer
