...
res = detailed_airborne_prediction(
    ...
    flanking_paths=[d1, ...])          # the twelve paths of the four elements
...
floor would help; from 100 Hz upwards it has vanished from the budget and the
flanking paths across the walls set $R'$, so no amount of work on the floor
would move the result. The single number $R'_w = 57$ dB says none of that.*
