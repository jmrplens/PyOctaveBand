...
The two acquisition tools operate inside the same two-channel frame: the
electrical drive is the reference, the microphone is the response, and the
Welch cross-spectra between them give the frequency response plus a
coherence that polices it. The regularized inversion then works on the
measured response as a post-processing step.
