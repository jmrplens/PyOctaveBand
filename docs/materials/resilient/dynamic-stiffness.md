...
response peak; Formula 4 turns it into $s'_t$. That extraction is the
laboratory's: the Clause 7 procedures for reading $f_r$ from the raw
...
`apparent_dynamic_stiffness` expects the already-extrapolated $f_r$.
