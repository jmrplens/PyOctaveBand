...
true level within $\pm 2s$ of the measured one at 95 % confidence; and a band
that failed criterion 2 may still be recorded, provided the 95 % confidence
interval of Formula (B.3) accompanies it:

```python
print(power.expanded_uncertainty)            # [6. 4. 4. 3. 3. 3.] dB
print(np.round(power.confidence_interval[0], 1))     # [-1.7  1.2] dB
```

<details>
<summary>Show the code for the sound power figure</summary>
...
fig, ax = plt.subplots()
ax.bar(np.arange(power.frequencies.size), np.nan_to_num(power.sound_power_level),
       color="#1f77b4")
ax.set_xticks(np.arange(power.frequencies.size))
ax.set_xticklabels([f"{f:g}" for f in power.frequencies])
ax.set_xlabel("Frequency [Hz]")
ax.set_ylabel("Sound power level $L_W$ [dB re 1 pW]")
ax.set_title("ISO 9614-1 sound power spectrum "
             f"($L_{{WA}}$ = {power.sound_power_level_a:.1f} dB(A))")
plt.show()
```
...
procedure cannot help. That case, and the case of no qualifying subset, are
both refused with a message naming Table B.3 action (d), which is what the
standard falls back to.
