...
position for the reference source, so $K_1$ takes its 1,3 dB cap and the
88,8 dB the band reports is an **upper bound**, which is what the `False`
says.
...
3744 Annex E octave values digit for digit); `frequencies` must be those
eight nominal centres.
