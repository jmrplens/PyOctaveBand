...
### The same step by VDI 2081

VDI 2081 Blatt 1 closes its own chain with the same relation, Equation (36),
written in the equivalent absorption area $A$ rather than the room constant
$R$. The two are not the same number, $R = A/(1-\bar\alpha)$, so each has its
own argument and neither stands in for the other. The guideline also reads
the directivity off a chart against frequency, so $Q$ may be given one value
...
# Room 102 of the guideline's worked example: A = 20 m2, listener 1.5 m away,
# and the directivity its Figure 30 gives for the ceiling diffuser.
...
# Where the reverberant field takes over, Equation (36a): rH = 0,2 sqrt(A).
print(round(float(room.critical_distance(absorption_area=20.0, directivity=2.0)), 2))  # 0.89
# And the absorption a measured reverberation time implies, Equation (37).
print(round(float(room.sabine_absorption_area(54.4, 0.44, speed_of_sound=339.0)), 1))  # 20.2
```

`sabine_absorption_area` is the familiar Sabine relation read the other way
about, from the time to the area, which is the direction a room survey
actually delivers. Its leading constant follows the speed of sound: $0{,}161$
at 343 m/s, and the $0{,}163$ the guideline prints, which is the same
expression at 339 m/s.

Figure 30 itself is not implemented. It is a chart of $Q$ against the product
of frequency and the square root of the outlet area, for two radiation angles
and four positions in the room, and the copy in hand does not resolve it to
better than about a tenth of the quantity it carries. Read it, or take $Q$
from the diffuser's own data, and pass the eight values.
...
- [Industrial noise control](noise-control.md): the rest of the
  ...
  insertion loss.
- [Room-noise criteria (NC / RC Mark II)](../../buildings/rooms/room-noise.md): the ANSI/ASA
  S12.2-2019 families the received spectrum is judged against.
- [Steady-state room field](../../buildings/rooms/room-image-sources.md): the room constant
...
- Verein Deutscher Ingenieure. (2001). *Geräuscherzeugung und Lärmminderung
  in Raumlufttechnischen Anlagen* (VDI 2081 Blatt 1:2001-07). Section 6.7.3:
  the level an air outlet produces in a room (Equation (36)), the equivalent
  absorption area a reverberation time implies (Equation (37)) and the
  reverberation radius $r_H = 0{,}2\sqrt{A}$. Element 20 of the worked example in its
  Part 2:2005-05 is the case checked here.
