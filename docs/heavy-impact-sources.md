...
energy average, so the usual $10\lg(T/T_0)$ standardization is wrong: a Fast
...
$$
L'_{i,F\max,V,T} = L_{i,F\max} + 10\lg\frac{V}{V_0}
- 10\lg\frac{g(C)}{g(C_0)},\qquad
...
$$
X_{iA,F\max} = 10\lg\!\left(\sum_j 10^{(X_{i,F\max,j} + A_j)/10}\right),
$$
