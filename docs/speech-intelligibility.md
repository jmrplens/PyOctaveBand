...
convenient population input, see [hearing threshold](hearing-threshold.md).
...
step over the band limits, which is the figure above. Tables 1, 2 and 4 are
tabulated for normal vocal effort only, so `"raised"`, `"loud"` and `"shout"`
are available on the one-third-octave procedure alone.
