...
Steps 2 and 3 are one correction inverted and then applied, which is what
keeps the adjustment from drifting away from the masking and threshold model
the forward measurement uses: it is that model, run backwards and then
forwards.
