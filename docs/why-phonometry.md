...
| ISO 226:2023 Annex B | Equal-loudness contours, loudness levels and hearing threshold against the Annex B tables | `tests/psychoacoustics/test_loudness_contours.py` |
...
what the document says, why it cannot be right, the independent evidence, the
reading the library implements, and the regression test that pins that reading.
...
- **MoSQITo** focuses on psychoacoustic sound-quality metrics (loudness,
  sharpness, roughness). That used to be a clean split, and it no longer is:
  phonometry now implements the same family, with ISO 532-1, ISO 532-2 and
  ISO 532-3 loudness, ECMA-418-2 tonality, roughness and fluctuation strength,
  DIN 45692 sharpness and the Fastl & Zwicker annoyance model. What differs is
  the setting. Here those metrics sit in the same conformance harness as the
  metrology core and compose directly with the weighting filters, ballistics,
  band filtering and calibration that feed them.
- **pyroomacoustics** simulates room impulse responses for audio and machine
  ...
  arrival). phonometry's room and simulation modules answer the measurement
  question instead: its image-source and FDTD solvers are pinned to published
  closed forms, and what comes out of them goes through the same ISO 3382
  parameter analysis as a measured impulse response.
