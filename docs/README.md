...
- [Underwater propagation solvers](underwater/underwater-solvers.md): the numerical solvers of the range-independent stratified ocean (Jensen et al.): the finite-difference normal-mode sum, Runge-Kutta ray tracing with Snell's invariant, and the split-step Fourier parabolic equation, each validated against an exact closed form, plus the guidance for choosing between them and the closed forms
