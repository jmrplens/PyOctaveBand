...
functions `leq`, `ln_levels`, `sel` and `lc_peak` go further and take the
`Signal` itself — the object already knows its rate and its calibration, and
...
`read_blocks()` yields exactly what `read()` would return, block by block —
same scaling, same channel convention, base-install seek-and-decode for
linear WAV including RF64 — so the stateful filters of
...
runs; the [weighting guide](../signals/levels/weighting.md) covers
