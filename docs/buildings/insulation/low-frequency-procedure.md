...
[Field Insulation Measurement (ISO 16283)](insulation-field.md), the envelope
version is [Façade Sound Insulation](facade-insulation.md), and the
...
defect of the print, recorded in the [errata registry](../../ERRATA.md); the
...
measured. Second, Part 3 Clause 6 says that "for the element and global road
...
not depend on, and the library enforces only the one that can be read off the
data it is given.

| Requirement | Where | Enforced? |
| :--- | :--- | :--- |
| A minimum of four corners per source position | Part 1 and Part 2 Clause 8.3, Part 3 Clause 7.3.2 | warned (`LowFrequencyWarning`) |
...
| A background measurement in **every** corner used | Part 1 Clause 9.1, Part 3 Clause 7.4.1 | no |

The levels handed in are assumed already corrected for background noise, which
ISO 16283-2 states inside Formula (15)'s own where-list. The per-band point
...
# The whole spectrum, by hand, against the default procedure alone:
...
result. The eight rows this module carries in the
[conformance report](../../CONFORMANCE.md) judge printed numbers and closed
forms rather than a published result. What stands in for an oracle, one row at
a time:

- **The printed numbers.** The 25 m³ trigger and its strictness, the rounding
  on the boundary, and the 50 / 63 / 80 Hz band set are exact quantities read
  off the page, and are pinned as such.
- **The closed forms of Formula (13).** Degeneracy at
  $L_\mathrm{Corner} = L$, strict monotonicity in $L_\mathrm{Corner}$, and the
  $10 \log_{10}(2/3) + L$ floor that is approached and never crossed.
- **The reason Clause 10.4 gives, measured rather than asserted.** A known
  single-sloped decay is put through the reverberation machinery already in
  this library, and the 63 Hz octave band recovers it with a smaller error
  than the three one-third-octave bands do, which is NOTE 1 turned into a
  measurement.
- **One implementation, proved and not claimed.** A conformance row asserts
...
- [Room Acoustics](../rooms/room-acoustics.md): the reverberation-time
  machinery the 63 Hz octave value comes out of.
- [Errata found in published sources](../../ERRATA.md): the two defects of
