...
standardized level difference $D_\mathrm{nT} = D + k$, the normalized
$D_\mathrm{n} = D + k + 10\log_{10}(A_0 T_0/(0.16\,V))$, the apparent
$R' = D + k + 10\log_{10}(S T_0/(0.16\,V))$ (using $V/7.5$ for $S$ where
that is larger), and, for impacts and façades, $L'_\mathrm{nT} = L_\mathrm{i} - k$ and
$D_{2\mathrm{m,nT}} = D_{2\mathrm{m}} + k$. The clause references follow ISO 10052:2021; the formulas
