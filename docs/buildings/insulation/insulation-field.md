...
All three warn when you forget. A `volume` that rounds below 25 m³ beside a
`frequencies` vector naming the three bands tells the function that Clause 8.1
is in force, and with no procedure to run it says so with a
