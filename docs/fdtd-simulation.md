...
run transform directly into the *scattered* far field, no reference run
needed.
...
probe.reset()                              # then integrate whole periods
...
- Brekhovskikh, L. M., & Godin, O. A. (1990). *Acoustics of layered media
  ...
  The fluid-solid oracles of §6: the oblique reflection coefficient with
  mode conversion (Eqs. 4.2.22-4.2.26) and its critical-angle limits
  (Eqs. 4.2.27-4.2.31), the exact Scholte characteristic equation
  (Eqs. 4.4.18-4.4.20) with the light-fluid asymptotics (Eqs. 4.4.21-4.4.24),
  and the three-media layer transmission (Eqs. 2.4.10-2.4.19).
- van Vossen, R., Robertsson, J. O. A., & Chapman, C. H. (2002).
  ...
  Scholte test of §6 and the >= 15 points-per-wavelength rule for the
  O(2,2) scheme with an interface wave.
...
dispersion, the fluid-solid reflection at normal and oblique incidence
with mode conversion, the exact Scholte-wave speed, the immersed-plate
transmission with its thickness resonance and the immersed-panel mass
law); two of those anchors run in the
