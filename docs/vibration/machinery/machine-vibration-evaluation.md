...
reading is graded on its own, which is the series Criterion II is judged on:
that criterion does not read the zone at all, it reads the change from the
established baseline, and a machine can be well inside zone B and still owe an
investigation.
...
There are three ratings, one per quantity: `DR` for shaft relative
peak-to-peak displacement, `VR` for housing r.m.s. velocity and `AR` for
housing true peak acceleration. Table 1 fixes the unit each is stated in,
micrometres, millimetres per second and metres per second squared; the three
names are spelled out under Table 5 and defined in Annex A. Tables 2, 3 and 4
give the boundaries.
