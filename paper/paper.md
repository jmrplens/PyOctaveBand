...
computed value, delta, verdict) is published with the documentation. A
...
specialised packages such as `MOSQITO` [@mosqito] implement individual
psychoacoustic models. What none of them offers is systematic, published
verification of each implementation against the normative text it claims to
implement. When implementations disagree, and they do, the user has no way to
tell which one the standard would side with.
