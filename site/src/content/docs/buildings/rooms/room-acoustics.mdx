...
$T_s$ integrate *from* $t = 0$, so an onset placed one millisecond late throws
away part of the direct sound and can move C50 and C80 by several decibels and
$T_s$ by tens of milliseconds, while leaving T20 and T30 almost untouched — the
...
+3 dB as dry; $T_s$ in the same halls runs between 60 ms and 260 ms and moves
...
$f_s \approx 2000\sqrt{T/V}$ ($V$ in m³, $T$ in s) these
...
The transition is soft, so a band straddling $f_s$ should be checked position by
...
- Theory: [Room and building acoustics](/phonometry/reference/theory/rooms-buildings/#room-and-building-acoustics-iso-18233-iso-3382-iso-16283-iso-10140-en-12354-iso-12999-iso-717-iso-354): the decay model behind T20, T30 and EDT and the energy ratios the clarity and definition parameters are built from.
