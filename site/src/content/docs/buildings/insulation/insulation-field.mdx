...
$D_\mathrm{nT}$ and $R'$, the impact levels $L'_\mathrm{nT}$ and $L'_\mathrm{n}$, the field test
...
is a regulatory question, not a measurement one.** $D_\mathrm{nT}$ describes the room
...
by its area, and is what a comparison against a laboratory $R_\mathrm{w}$ or an EN 12354
prediction needs. Measure once, then report the quantity the requirement is
written in and say which it is: an $R'_\mathrm{A}$ cannot be checked against a
$D_\mathrm{nT,A}$ limit without converting through the room's $T$ and the partition
area first — see
[Spanish Building Code (CTE DB-HR)](/phonometry/buildings/insulation/spanish-building-code/),
which states its between-rooms requirement in $D_\mathrm{nT,A}$ and its
partition-wall requirement in $R_\mathrm{A}$.
...
national requirements typically sit around $D_\mathrm{nT,w} = 50$ dB and rarely below
...
signal-plus-background level $L_\mathrm{sb}$:

- margin $\ge 10$ dB: use $L_\mathrm{sb}$ unchanged;
- $6 < \text{margin} < 10$ dB: subtract the background in energy,
  $L = 10\log_{10}(10^{L_\mathrm{sb}/10} - 10^{L_\mathrm{b}/10})$ (Formula 14), with both
...
background noise, and form $D_\mathrm{nT}$ or $R'$ once. With a **single loudspeaker
...
cubic metre, so a 25.4 m³ bedroom does not trigger it and a 24.4 m³ one does —
...
$L_\mathrm{LF}$ replaces $L$ in those three bands, and $D_\mathrm{nT}$ or $R'$ is then formed
...
**Clause 10.4: the reverberation time as well.** Under the same trigger the
50 Hz, 63 Hz and 80 Hz one-third-octave reverberation times stop being measured
and one **63 Hz octave band** value takes their place for all three. It is a
prescription about what to measure, not a claim that the values are equal: in a
small room a one-third-octave decay rarely has enough modes to be
single-sloped, and in timber or steel frame construction it can be shorter than
the analyser's own one-third-octave filter. Below the trigger there is nothing
to fall back on either, because Clause 10.3 confines the default
reverberation-time procedure to 100 Hz and above once the room is that small.
The clause is a *receiving-room* clause in all three parts, even in Part 1
where the corner procedure itself also admits the source room.
...
# Clause 8.5: the loudest corner, band by band (Formula (12) at q = 1).
l_corner = building.corner_level(corners)
print(l_corner)                           # [54.6 56.9 52.7]  (three different corners)

l_lf = building.low_frequency_level(l_default, l_corner)   # Formula (13)
...
print(building.low_frequency_procedure_applies(25.4))   # False, rounds to 25
```

In a whole measurement the two halves travel together in a
`LowFrequencyProcedure`, which `airborne_insulation()` applies to the band
vector for you: it rewrites the three bands with $L_\mathrm{LF}$ and puts the
63 Hz octave value over the same three reverberation times, leaving 100 Hz and
above untouched.

```python
# 50, 63, 80 Hz plus the first three core bands, so the substitution is visible.
freqs = np.array([50.0, 63.0, 80.0, 100.0, 125.0, 160.0])
l1_lf = np.array([88.4, 90.1, 87.6, 85.2, 84.7, 84.1])
l2_lf = np.array([49.5, 51.2, 48.6, 45.6, 43.2, 41.8])
t2_lf = np.array([0.62, 0.58, 0.54, 0.49, 0.47, 0.45])

receiving = building.LowFrequencyProcedure(
    volume=22.0,
    corner_levels=corners,
    reverberation_63_octave=0.72,         # Clause 10.4
)
small = building.airborne_insulation(l1_lf, l2_lf, t2_lf, frequencies=freqs,
                                     receiver_low_frequency=receiving)
print(np.round(small.l2, 1))              # [51.9 54.  50.4 45.6 43.2 41.8]
print(small.t2)                           # [0.72 0.72 0.72 0.49 0.47 0.45]
```

`impact_insulation()` and `facade_insulation()` take the same object under a
`low_frequency=` keyword and reach the same code: Formula (16) of Part 2 and
Formula (5) of Part 3 are Formula (13) with different subscripts. Two
differences between the parts are real and enforced. Part 1 tests the two rooms
independently, so `airborne_insulation()` also takes a `source_low_frequency=`,
and a small source room beside a large receiving room moves $L_1$ and leaves
$L_2$ and $T$ as measured. Part 3 confines the procedure to the element and
global **loudspeaker** methods (Clause 6 NOTE 1 records that there is no
experience of running it with traffic as the source), so passing it with
`method="road_traffic"` raises rather than answering.
...
$L'_\mathrm{nT,w} = 79$ dB is a bare structural floor with no covering and no floating
...
`quantity="l_n"`, the normalized $L'_\mathrm{n}$ fiche (Figure C.2). Each fiche names
...
(the energy-average $L_1$ and $L_2$, or $L_\mathrm{i}$, and the reverberation time
...
standardized $D_\mathrm{nT}$ and apparent $R'$ of Clause 3.12-3.15 and the Clause 7.8
...
does not match the field thresholds.) ISO 16283-1/-2's own position
requirements — the counts, distances and placements of [Taking the
measurement](#taking-the-measurement-iso-16283-1-clauses-7-and-9) — are
documented here and checked nowhere: energy-averaging happens once positions
are supplied, but nothing verifies how many were taken or where. The
low-frequency procedure of [Small
rooms](#small-rooms-the-low-frequency-procedure-clause-8) *is* implemented —
the 25 m³ trigger, Formula (12), Formula (13) and the 63 Hz octave
reverberation time of Clause 10.4 — but it too is fed corner levels the
operator measured, and the only sampling requirement it enforces is a warning
below the four corners per source position Clause 8.3 asks for. The other members of the family are covered by their own guides: the
