...
$L_\mathrm{LF}$ replaces $L$ in those three bands, and $D_\mathrm{nT}$ or $R'$ is then
formed exactly as before. Under the same trigger, Clause 10.4 also stops the
50 Hz, 63 Hz and 80 Hz one-third-octave reverberation times being measured and
puts one **63 Hz octave band** value in their place.
...
vector for you, and `impact_insulation()` and `facade_insulation()` take the
same object under a `low_frequency=` keyword.
...
rooms](/phonometry/buildings/insulation/low-frequency-procedure/) *is* implemented —
