...
façade** $L_{1,2m}$, giving the level difference $D_{2m}$ and, exactly as in
...
$R'_{tr,s}$ assumes the sound arrives "from all angles" (3.13, Note 2). So the
1.5 dB between the two methods is a statement about the *source*, not about the
wall, and it is why a loudspeaker result is labelled $D_{ls,2m,nT}$ and a
traffic result $D_{tr,2m,nT}$ rather than both being called the same thing.
...
$D_{2m,nT}$ is measured in front of a reflecting surface and is therefore
higher than the free field the same source would produce with the building
removed, so a $D_{2m,nT}$ and an internal $D_{nT}$ are not the same kind of
...
$D_{2m,nT}$ (or $D_{2m,n}$) and "quantify the airborne sound insulation of a
...
$R'_{45°}$ or $R'_{tr,s}$ and are the route when the aim is to characterise one
...
it to elements or façades with $R'_w < 40$ dB (Clause 10.1 NOTE).
...
ISO 16283-3:2016 (the field façade quantities $D_{2m}$, $D_{2m,nT}$,
$D_{2m,n}$, $R'_{45°}$ and $R'_{tr,s}$ of the loudspeaker and road-traffic
...
transmission-factor summation of Formula 10, the $R_{45}$ and $R_{tr,s}$
indices, the $D_{2m,nT}$ of Formula 13 and the Annex C façade-shape term) via
...
- [Insulation Ratings (ISO 717)](/phonometry/buildings/insulation/insulation-ratings/): the
  reference-curve engine behind the $D_{2m,nT,w}$, $R'_{45,w}$ and
  $R'_{tr,s,w}$ single numbers.
