...
comparable with an $L'_{nT,w}$ or an $L'_{n,w}$ from the tapping machine, and
...
the $L'_{iA,F\max}$ of this section (through KS F ISO 10140-5 and
...
$L_{i,F\max}$ through to $L'_{iA,F\max,V,T}$, so the standardization chain is
