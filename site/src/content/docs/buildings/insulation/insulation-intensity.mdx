...
**Qualifying the surface (Clause 6.4.2).** Measure $L_{In}$ and, if possible
simultaneously, the surface-averaged pressure level $L_p$, and form the
surface pressure-intensity indicator $F_{pI} = L_p - L_{In}$ (Formula (10)).
...
A large or inhomogeneous specimen is scanned as several subareas $S_{mi}$, and
`combine_subareas` performs the area-weighted energy average of Formula (11)
with $S_m = \sum_i |S_{mi}|$ (Formula (12)). Clause 6.4.6 adds the rule with no
...
sign goes before that $S_{mi}$ in Formula (11). Express it by passing that
subarea's area as a negative number; its energy is then subtracted from the
numerator while $S_m$ keeps the unsigned sum.
...
$S_m$ keeps the unsigned area because it is the area actually scanned, which
is what the $10\log_{10}(S_m/S)$ term of $R_I$ needs.
...
ISO 15186-1:2000's intensity sound reduction index $R_I$ (Clause 3.8), its
$K_c$-modified $R_{I,M}$ (Annex B) and the element-normalized $D_{I,n,e}$
...
acquisition and phase-mismatch calibration behind $L_{p1}$ and $L_{In}$) is
...
- [Laboratory Insulation Measurement](/phonometry/buildings/insulation/insulation-lab/):
  the ISO 10140 pressure method that the $K_c$ adaptation reproduces.
...
- [Insulation Ratings (ISO 717)](/phonometry/buildings/insulation/insulation-ratings/):
  the reference-curve engine behind $R_{I,w}$ and $D_{I,n,e,w}$.
