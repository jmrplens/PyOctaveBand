...
### Table D.1: $Y_s$ from the machine's own parts
...
| `"bar_end"` | $\rho$ [kg/m3], $c_L$ [m/s], $S$ [m²] | $[\rho c_L S]^{-1}$ |
| `"beam"` | $\rho$, $c_L$, $t$ [m], $w$ [m] | $[7{,}6\,\rho t w\sqrt{c_L t f}]^{-1}$ |
| `"plate"` | $\rho$, $c_L$, $t$ [m] | $[2{,}3\,c_L\rho t^2]^{-1}$ |
| `"pipe"` | $\rho$, $c_L$, $t$ [m], $r$ [m] | $[63\,\rho t r\sqrt{c_L r f}]^{-1}$ |
...
`"plate"` is Formula (F.4) again, written in $\rho$, $c_L$ and $t$ instead of
...
$\lvert Y_s\rvert$ at its damping-limited *minimum* — which is the frequency
at which the mount injects the **most** power, because a small source mobility
is a small $D_C$. That is the mounting resonance, and $\eta$ is the only thing
...
page takes: $L_{Ws,c} = L_F - 5 - 10\log_{10}f$ (Formula D.9a) and
$D_{C,i} = -10\log_{10}(\omega M Y_i) + 10\log_{10}[1 + (\omega M Y_i)^2]$
(Formula D.9b, which is Formula 19b with $Y_s = 1/\mathrm{j}\omega M$).
