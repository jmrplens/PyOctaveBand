...
the weakest element on the list at $R_w$ = 33 dB, yet the 33,5 dB tabulated for
its $K_{Ff}$ takes its Ff path out of the sum entirely. On this model a bad
...
$R_{s,w} = 57$ dB and $R'_w = 52$ dB. The dashed line is the assembled $R'_w$,
...
$$
\Delta R_{ij,w} = \Delta R_{i,w} + \frac{\Delta R_{j,w}}{2}
\quad\text{or}\quad
\Delta R_{j,w} + \frac{\Delta R_{i,w}}{2}\ \text{dB},
$$
...
$\Delta L_w$ and is subtracted separately.
...
($R_w$, $\Delta R_w$, $\Delta L_w$, mass per unit area) are taken as given,
