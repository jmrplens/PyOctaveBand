...
$f_\text{co}$ = 589 Hz and $f_\text{limit}$ = 555 Hz, so the spectrum bends
...
distance between them is $f_\text{co}$. The stiff PVC puts it at 2318 Hz, above
...
worth 17.0 dB at 250 Hz and reaches 69.6 dB at 5 kHz. Above $f_\text{co}$ both
...
$s'_\text{tot} = (\sum 1/s'_i)^{-1}$, is followed verbatim by: this holds only if
