...
plate, so only the corner path applies there. The tabulated coefficients also
assume a *symmetric* junction (opposite plates identical), and there is no
closed form here for one that is not. On the experimental side, the inversion
of section 6 is written for **two** subsystems only, and
`PowerInjectionResult.modal_density_ratio` is a consistency check the reader
performs rather than one the library automates.
