...
and `vibration_vector_change`.
</ScopeClaim>
<ScopeClaim status="not-covered" label="Not covered">
The numeric zone boundaries of the machine-specific parts, which this page
takes as inputs; the ALARM and TRIP settings of 6.4, which the standard
describes without numbers and leaves to the parts; and the shaft criterion of
Figure 10, whose values Annex C.1 declines to give for machines no part covers,
on the grounds that such machines are not normally fitted with shaft
transducers.
</ScopeClaim>
</Scope>
