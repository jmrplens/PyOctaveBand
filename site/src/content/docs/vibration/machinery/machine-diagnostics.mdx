...
| $f_s$ | shaft rotational frequency, $f_s = N/60$ | Hz | all |
...
| $s$, $f_e$ | slip (a fraction, not a percentage), supply frequency | —, Hz | 3 |
...
| $m_L$ | number of lobes of a rotating interaction pattern | — | 4 |
...
inner-race one, so $\text{BPFO} + \text{BPFI} = Z f_s$; and with a stationary
...
- an **inner-race** defect turns with the shaft, so it passes in and out of the
  ...
  shaft rate, and BPFI carries $\pm f_s$ sidebands around the line and around
...
$\pm f_s$ sideband costs record length, which is why the acquisition of section
...
Every frequency on this page is strictly proportional to $f_s = N/60$: the
...
$m_L = nN \pm kV$, which is why the lobe count has to be part of the line's
name. The $m_L = 2$ pattern turns at $nNf_s/m_L = 6 \times 58.3/2 = 175$ Hz,
three times the shaft speed, so it is the strong radiator and the line to check
against the duct cut-on frequency. The $m_L = 10$ pattern turns at 35 Hz, below
...
faults: a chipped tooth gives low, flat sidebands at $\pm f_s$ and harmonics
...
why the rotor-slot harmonic and its $\pm f_s$ sidebands need a logarithmic
...
gets is $1/T$ and the sideband spacing to be resolved is $f_s$ itself. Two
seconds at 2000 r/min is 67 revolutions and 0.5 Hz of resolution against a
33 Hz shaft rate, which is why the shaft line and the $\pm f_s$ sidebands of
