...
input cross-spectral matrix `Gxx` and the input-output vector `Gxy` it is
...
where `Gnn` is the residual output spectrum uncorrelated with every input. If
the output carries additive uncorrelated noise of per-band signal-to-noise
ratio `SNR`, this is exactly $\mathrm{SNR}/(1+\mathrm{SNR})$, the closed-form
...
The 4th-edition definition keeps the *total* output `Gyy` in the denominator
...
output spectrum `Gyy·q!` remains. This is the whole point of the method: in
the figure above, input 2 is `0.7·x1 + independent noise`, so it *correlates*
...
factor `Liy`) estimates, the linear model that predicts the output from
