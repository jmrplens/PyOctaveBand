...
(`sound_power_intensity`): the partial powers, the `FpI` surface
pressure-intensity and `F+/-` negative-partial-power field indicators, the
dynamic capability `Ld`, the repeatability criterion, the per-band achieved
