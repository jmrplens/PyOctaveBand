...
lie. `weston_propagation_loss` assembles the composite loss $PL = -10 \lg F$
...
:::note
Ainslie's printed Equation (9.57) for $r_{MS}$ reads $k^2H_e^3/(9\eta)$, a
factor $\pi$ larger than the value its own derivation rule gives. The library
implements the consistent form; see
...
- API reference: [`underwater.propagation`](/phonometry/reference/api/underwater/propagation/) and [`underwater.sound_speed`](/phonometry/reference/api/underwater/sound-speed/).
