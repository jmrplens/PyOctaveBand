...
applies time weighting F ($\tau$ = 125 ms), samples the level history `LpAF`
...
impulse) above the adjustment-curve plot `K`<sub>I</sub>`(P)` with the candidate
impulses marked, the boxed governing prominence `P` together with the derived
`L`<sub>Aeq</sub> adjustment `K`<sub>I</sub> (Formula 2), an optional PASS/FAIL
