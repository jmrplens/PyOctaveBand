...
bias correction and its usable-bandwidth bound (clause 7.3, Table 3), and the
residual pressure-intensity index `δpI0` as the instrument's phase-error
floor. ISO 9614-1:1993 for the Annex A field indicators F2, F3 and F4
(`field_indicators`) and the Annex B dynamic-capability criterion
...
ISO 9614-3) live in the [sound power guide](/phonometry/guides/sound-power/).
IEC 61043's class 1 / class 2 residual-index acceptance limits are not
verified by the library either: `δpI0` is a value the caller measures on
their probe-analyzer chain and supplies, not one phonometry classifies.
