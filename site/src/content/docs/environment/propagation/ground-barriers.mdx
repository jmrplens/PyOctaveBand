...
ground wave is irrelevant. By 0.02 m, $R_p$ has reached $-1$ (magnitude 0.993 at
...
| `receiver_height` | float | m | ≥ 0 | $h_r$ (**before** the distance) |
...
| `receiver_height` | float | m | ≥ 0 | $h_r$ (**after** the distance) |
...
are its $d_{ss}$ and $d_{sr}$. The tabulated $A_{gr}$ and $D_z$ of the
...
**Covered.** The Weyl-Van der Pol spherical-wave ground reflection
coefficient (`ground_effect`, Attenborough Eq. 2.40a/c, Salomons Eq. 3.2/D.58,
the Faddeeva-function boundary-loss factor), with its hard-ground +6 dB,
grazing-incidence and reciprocity limits pinned as tests. Wave-theoretic
barrier diffraction (`barrier_insertion_loss`): the Kurze-Anderson closed
form (`kurze_anderson_attenuation`, Bies Eq. 5.138), the exact rigid
half-plane (MacDonald / Hadden & Pierce, Attenborough Eqs. 9.19-9.20), thick
barriers through the double-edge Fresnel number (Bies Eq. 5.157), and the
coherent four-path barrier-on-ground model weighted by the `ground_effect`
...
the separate [atmospheric refraction](/phonometry/environment/propagation/atmospheric-refraction/)
guide instead. The coherent barrier-on-ground model weights all four
diffracted paths with a single reflection coefficient $Q$ computed over the
overall source-receiver geometry rather than a separate coefficient per
image path, so it is coherent and reciprocal but not a full boundary-element
solution.
...
- Theory: [Outdoor propagation and occupational exposure](/phonometry/reference/theory/environment-transport/#outdoor-propagation-and-occupational-exposure-iso-9613-12-iso-9612): the ground and barrier terms of ISO 9613-2 in the context of the whole attenuation sum.
