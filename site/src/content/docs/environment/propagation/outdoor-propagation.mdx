...
equivalent source to the receiver exceeds twice the largest dimension $H_{max}$
of the group ($d > 2H_{max}$). If $d \le 2H_{max}$, or if the propagation
...
an A-weighted $L_{WA}$ is available, the standard's own fallback (Clause 1,
...
$G_s$, $G_m$ and $G_r$ are not a vague softness: each is the *porous fraction* of
a region whose extent the standard fixes (7.3.1, figure 1). Measured along the
ground-plane distance $d_p$, the **source region** runs $30 h_s$ from the source
towards the receiver (capped at $d_p$), the **receiver region** runs $30 h_r$
...
On the 200 m example of this section ($h_s = h_r = 1.5$ m, so $d_p \approx d$)
...
the plant across a 20 m concrete apron takes $G_s = 25/45 \approx 0.55$, and a
middle region of 70 m of grass and 40 m of asphalt takes
$G_m = 70/110 \approx 0.64$.

The middle region can also disappear. Table 3, note 2 defines
$q = 1 - 30(h_s + h_r)/d_p$ when $d_p > 30(h_s + h_r)$ and $q = 0$ below it, and
$A_m$ is proportional to $q$: with these two heights the threshold sits at
$d_p = 30(1.5 + 1.5) = 90$ m, so on a 60 m path `ground_middle` is **ignored
entirely** — passing 0.0 or 1.0 returns the identical $A_{gr}$, silently. The
same $q$ gives the sanity check for the whole term. Over hard ground everywhere
$A_s = A_r = -1.5$ dB and $A_m = -3q$ in every band, so the 200 m path must
return $A_{gr} = -4.65$ dB ($q = 0.55$) and the 60 m path the familiar $-3$ dB of
...
at 63 Hz — which is why the barrier there has to spend $A_{gr}$ before it buys
anything.*
...
$h_m$ is **not** the average of the two end heights. Figure 3 defines it as
$h_m = F/d$, where $F$ is the area enclosed between the straight source-receiver
line and the ground profile: over flat ground it reduces to
$(h_s + h_r)/2$, over a cutting it collapses towards zero, and over a valley
...
$A_{gr}$ is capped below at 0, so a path riding high above the ground earns no
ground attenuation at all (at $d = 200$ m, $h_m = 8$ m still gives 3.32 dB but
$h_m = 8$ m at 50 m gives 0.00 dB), while a grazing path earns the full 4.8 dB.
...
screen on this path the single-edge curve is flat at 20 dB, and $K_{met}$ has
already taken 1 to 2 dB off it at 200 m. Right: the two curves are the same
barrier; the gap between them is $A_{gr}$, and where the ground was already
worth more than the diffraction (the 250 Hz octave) the net $A_{bar}$ is zero.*
...
- [Atmospheric refraction](/phonometry/environment/propagation/atmospheric-refraction/): what the favourable-condition assumption and $C_{met}$ stand in for.
...
- Theory: [Outdoor propagation and occupational exposure](/phonometry/reference/theory/environment-transport/#outdoor-propagation-and-occupational-exposure-iso-9613-12-iso-9612): the ISO 9613-2 attenuation terms derived one by one, and the atmospheric absorption of Part 1.
