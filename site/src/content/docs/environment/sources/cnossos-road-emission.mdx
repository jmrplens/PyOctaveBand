...
| `RoadTraffic.studded_fraction`, `studded_months` ($Q_{\mathrm{stud,ratio}}$, $T_s$) | Fleet share and season | The average ratio of light vehicles per hour fitted with studded tyres **during** the months they are fitted, and the number of those months; the method forms $p_s = Q_{\mathrm{stud,ratio}}\,T_s/12$ itself (2.2.7) | Passing the year-averaged share, which applies $T_s/12$ twice |
...
temperature and a night speed is what makes the resulting line power an $L_{den}$
...
$\Delta L_{WR,\mathrm{acc}} = C_{R,m,k}\cdot\max(1 - |x|/100,\ 0)$ for rolling
noise and $\Delta L_{WP,\mathrm{acc}} = C_{P,m,k}\cdot\max(1 - |x|/100,\ 0)$ for
...
attributed equally to all octave bands. The rolling coefficients $C_R$ are
*negative* (traffic near a junction is slower), the propulsion coefficients $C_P$
