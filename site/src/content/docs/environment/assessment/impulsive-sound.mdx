...
is built from level *differences*) the reported and adjusted $L_{Aeq}$ are not.
...
| `calibration_offset` | `0.0` dB | Added to the level history to bridge a digital scale to a calibrated one: the difference between the calibrator's known level and the level this same processing reports for the calibration recording. It moves $L_{Aeq}$ and the adjusted $L_{Aeq}$ and leaves $K_I$ alone |
| `laeq` | `None` | Supply the $L_{Aeq}$ measured over the assessment period instead of letting it be computed from the excerpt, which is what a real report does |
...
level difference; the prominence $P$ and the adjustment $K_I$; the source
category the result puts the source in; and the $L_{Aeq}$ the adjustment rides
on, with the instrumentation and calibration behind it. The fiche below fills the
computed half automatically — the per-impulse table, the governing $P$, $K_I$ and
...
impulse) above the adjustment-curve plot $K_I(P)$ with the candidate
impulses marked, the boxed governing prominence $P$ together with the derived
$L_{Aeq}$ adjustment $K_I$ (Formula 2), an optional PASS/FAIL
...
prominence $P$ (clause 7, Formula 1), the graduated adjustment $K_I$ (clause
...
chain built on the same prominence formula: $L_{pAF}$ sampling, onset
...
adjustment based on subjective judgement, recommended when $K_I > 3$ for noise
characteristic of working operations, offered as a fallback where the
graduated method cannot be applied. Only the graduated $K_I$ of Formula 2 is
