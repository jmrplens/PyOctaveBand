...
**Covered.** NT ACOU 112:2002: the onset definition (clauses 4.5 to 4.7),
the predicted prominence $P$ (clause 7, Formula 1), the graduated adjustment
$K_I$ (clause 8, Formula 2) and the rating level over a reference time
...
the objective measurement chain built on the same prominence formula: $L_{pAF}$
...
when $K_I > 3$ for noise characteristic of working operations, offered as a
fallback where the graduated method cannot be applied. Only the graduated
$K_I$ of Formula 2 is implemented; the flat 5 dB alternative is not. ISO
