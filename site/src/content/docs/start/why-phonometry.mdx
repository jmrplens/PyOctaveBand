...
it from rest over a burst of duration $T_b$ and the envelope reaches
$(1-e^{-T_b/\tau})$ of the steady value the same tone would eventually produce,
so the maximum time-weighted level relative to steady state is

$$
\delta_{\text{ref}} = 10\lg\!\left(1 - e^{-T_b/\tau}\right)
$$

which is IEC 61672-1:2013 Equation (7). With $\tau_F = 0.125$ s that gives
−0.98 dB at 200 ms, −4.82 dB at 50 ms, −11.14 dB at 10 ms and −20.99 dB at
1 ms: the whole "IEC target" column of Table 4, to the standard's own rounding.
CI asserts the identity for every Fast and Slow row
...
function of $T_b/\tau$, a detector that really solves the equation tracks the
whole column at once; a detector that averages over fixed 125 ms blocks has no
$T_b$ in it at all, and can only be right where the burst happens to fill a
block.
...
<Video
  ...
  width={2400}
  height={1350}
  title="The same tone burst slid across a 125 ms block boundary"
...
*The exponential trace is flat because $10\lg(1-e^{-T_b/\tau})$ contains
$T_b$ and nothing about the clock; the block trace is a picture of the
...
- If you need **standard-compliant Fast/Slow/Impulse envelopes** (sound level
  ...
  [`time_weighting`](/phonometry/signals/levels/time-weighting/).
- If you need **block-averaged $L_{eq}$ per interval**, that is a different, equally
  valid metric; you can compute it with [`leq`](/phonometry/signals/levels/levels/) over consecutive
  slices.
...
a table the library never reads, the entry says so instead. There are
{errataEntries} of them now, across standards, guidance documents, textbooks
and journal papers, each marked as reported to the issuing body or not. A defect listed there is never a defect of the
