...
$c_P^{\max}$, exactly as in the acoustic solver (Virieux Eqs. 6-7).
...
must satisfy $c_P^2 \ge 2 c_S^2$, which the constructor enforces, and that
...
speeds: the dotted arcs are drawn at the exact $c_P t$ and $c_S t$ radii, so
...
their peaks is the measurement: the simulated $c_R$ against 2921 m/s, the root
...
| Constant | $c_P$ [m/s] | $c_S$ [m/s] | $\rho$ [kg/m³] |
|---|---|---|---|
| `AIR` | 343 | 0 | 1.2 |
| `WATER` | 1480 | 0 | 1000 |
...
$c_P^2 \ge 2\,c_S^2$: that is the statement that the first Lamé parameter is
...
so each has its own critical angle: $\arcsin(c_1/c_{P2})$ for the
compressional wave and $\arcsin(c_1/c_{S2})$ for the shear wave. Both follow
...
the same $c_P$ and $\rho$ — the model the underwater module carries — and
...
|---|---|---|---|---|
| 0° | P only | 0.938 | 0.938 | no shear is excited, so the steel is exactly a liquid of its $\rho$ and $c_P$ |
...
against $f_n = n\,c_P/(2h)$. The same time-gating logic is what an in-situ
...
the two regimes meet is one division for any plate: $c_P/(2h)$, which is
