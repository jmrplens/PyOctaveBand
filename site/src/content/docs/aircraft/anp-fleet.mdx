...
- Opening the shipped EASA ANP database, or another CSV export, with
  `load_anp_database`.
- What one aircraft record holds: metadata, the power parameter its NPD table is
  indexed by, the NPD curves per operation and metric, and the default profiles.
- Reading and interpolating the NPD surface, and drawing the curve family.
- The fixed-point trajectories, the stage-length bins, and why most aircraft
  carry procedural steps this bridge does not fly.
- Running the Doc 29 single-event level and ground-grid contour straight from an
  aircraft identifier.
