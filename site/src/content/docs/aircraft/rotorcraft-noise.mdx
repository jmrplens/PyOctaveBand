...
near 0.6 dB while the hard one is still ringing at 2.4 dB at 5 kHz. All of it is
...
bearing — 0° at the nose, positive to starboard — reduced to the ring's
polar distance, commonly 70 m rather than the flyover database's 60 m.
Guidance §A.3.5 defines four conditions — in-ground hover (HIGE),
out-of-ground hover (HOGE), reduced-rpm idle and full-rpm idle — and three
approaches in descending priority: measure all four rings; measure the HIGE
ring and the other three in the 0° direction only; or measure the HIGE ring
alone and apply fixed offsets.
...
by exactly that value, which is how Table 3 (stated on $L_A$ levels) extends
to band spectra.
...
*The ring on the left is the only measurement; everything on the right is
derived from it. The HIGE section starts at the ring's nose value and ends at
its tail value — that is the constant-φ extension at work — and the other
three sources are the same directivity shifted whole: +12 dB up to HOGE,
−2.5 dB to full-rpm idle, −12 dB to reduced-rpm idle.*
...
:::note[The published offsets and the shipped ones]
The guidance warns that its constants come from measurements with inverted
microphones on ground plates and may not hold for other setups — and the
NORAH2 public database actually ships **+8/−10/−2 dB** in every type's
interpolation file. Pass those as `offset_db` to reproduce the prototype;
the published Table 3 constants are the defaults. The Approach 3 prose also
derives full-rpm idle from HOGE where its own table derives it from HIGE,
12 dB apart; the table, backed by the database, is what is implemented. Both
divergences are recorded in the
...
*A single number tells the whole story only when both the wavelength and
$C_h$ are fixed: the 4 kHz curve is already at the 25 dB cap by $\delta$ =
0.68 m, the 1 kHz curve is still climbing at $\delta$ = 1.5 m (22.5 dB, short
of the cap), and the 63 Hz curve reaches only 7.2 dB there, held back by its
own $C_h$ = 0.63 (Eq. 43, $h_0$ = 2.5 m) on top of the longer wavelength. At
grazing incidence the three bands whose $C_h$ = 1 cross 4.8 dB, but 63 Hz
crosses at 3.0 dB instead, because that value scales with $C_h$ too, not
with wavelength alone.*
...
heights from 0.01 m to 8.2 m and three ground classes; the residual peaks on
...
hover, idle and taxi source derivation of Table 3
(`hover_ring_hemisphere`, `hover_derived_hemisphere`), the
...
end to end against the NORAH2 prototype's ARP verification cases, including
its out-of-ground-hover operation.

**Not covered.** The §A.3.5 fallback for a type with no hover data at all —
two side-line level-flight microphones corrected to the 150 m hover circle by
the ICAO Annex 16 integrated method — is a measurement-correction chain this
module, which consumes already-reduced hemispheres, does not model. **No
hemisphere database
