...
*A single number tells the whole story only when the wavelength is fixed: the
4 kHz curve is already at the 25 dB cap by $\delta$ = 0.68 m, the 1 kHz curve
does not reach it until $\delta$ = 2.7 m, and the 63 Hz curve is still under
10 dB at $\delta$ = 4 m, held back by its own $C_h$ = 0.63 (Eq. 43, $h_0$ = 2.5 m)
on top of the longer wavelength. All four still cross 4.8 dB at grazing
incidence, because that value never depends on frequency.*
