...
(§A.4): `spherical_spreading_adjustment` ($\Delta L_\mathrm{s} = -20\log_{10}(r/60)$,
Eq. 24), `atmospheric_adjustment` ($\Delta L_\mathrm{a} = -\alpha(f)\,(r - 60)$ with
...
by exactly that value, which is how Table 3 (stated on $L_\mathrm{A}$ levels) extends
to band spectra.
...
$\Phi = \arctan(K V_\mathrm{g}^2 / g)$ and path angle
...
recorded time $t_\mathrm{r} = t_\mathrm{e} + r/c$ (Eq. 22, $c = 346.1\ \text{m/s}$) and
integrated:
...
recorded time $t_\mathrm{r} = t_\mathrm{e} + r/c$, and $r$ is largest far from the closest point
...
$L_\mathrm{AS,max}$ = 78.5 dB(A) arrives at 64.56 s, a full second **before** the sound
...
$\mathrm{SEL} - L_\mathrm{ASmax}$ flyover integral. End to end it reproduces the
...
The ECAC Doc 32 hemisphere method and the EASA NORAH2 equation- level guidance
(§A.3-A.5): the noise hemisphere and its bilinear lookup
...
hover, idle and taxi source derivation of Table 3 (`hover_ring_hemisphere`,
`hover_derived_hemisphere`), the flight-path kinematics
(`flight_path_kinematics`), the single-event SEL, LASmax and EPNL
(`rotorcraft_event_level`), ground-grid contours (`rotorcraft_noise_contour`)
and the terrain mean-ground-plane and diffraction-screening chain
(`mean_ground_plane`, `mean_flow_resistivity`, `diffraction_attenuation`,
`terrain_screening_adjustment`). Validated against the NORAH2 guidance Table
4, closed-form geometric and grazing limits, and end to end against the NORAH2
prototype's ARP verification cases, including its out-of-ground-hover
operation.
</ScopeClaim>

<ScopeClaim status="not-covered" label="Not covered">
The §A.3.5 fallback for a type with no hover data at all — two side-line
level-flight microphones corrected to the 150 m hover circle by the ICAO Annex
16 integrated method — is a measurement-correction chain this module, which
consumes already-reduced hemispheres, does not model. **No hemisphere database
ships with phonometry, and there is no NORAH file reader**: the method is
implemented, the data is not, so the arrays come from a measurement campaign
or from the NORAH2 reference database and are assembled by the reader.
Refraction by atmospheric gradients is outside Doc 32 itself, as is the
flight-condition interpolation, which follows the NORAH2 guidance instead.
</ScopeClaim>
</Scope>
