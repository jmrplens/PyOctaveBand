...
`ElasticFDTDResult` congelado con el mismo `.plot()`. Lo que la simulación
...
en un `ElasticBoundaries` (los lados y el grosor de la esponja). La cota de Courant
depende solo de la $c_P$ más rápida del mapa (Virieux, Ecs. 6-7),
mientras que la regla de resolución usa la velocidad de onda más lenta de
todo el dominio: la $c_S$ no nula más lenta de las celdas sólidas o, si
una región fluida lleva una $c_P$ aún más lenta, esa velocidad del
fluido: $\Delta x \le c_\text{min} / (10 f)$ (en un mapa completamente sólido la
longitud de onda S es siempre la más corta; en dominios mixtos la longitud
de onda P del fluido puede ser más corta, como en agua a 1480 m/s sobre un
sedimento con cizalla de 2000 m/s).
