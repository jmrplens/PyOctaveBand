...
Todo $a_{hv}$ que alimenta el cálculo de exposición de más abajo es un promedio,
...
por encima de 20 Hz, así que $a_w$ = 1,026 m/s² lo fijan tres bandas de
...
ponderada: el **valor eficaz móvil** $a_w(t_0)$ (ecs. (2)/(3)) y su máximo, el
...
de cuarta potencia $\text{VDV} = \left(\int a_w^4\,dt\right)^{1/4}$
...
$\text{MTVV}/a_w$ pregunta si algún segundo suelto del registro es mucho peor
que el registro entero; $\text{VDV}/(a_w T^{1/4})$ pregunta si la cuarta potencia
...
ax_t.plot(time, a_ws, lw=1.0, color="#1f77b4", label="$a_w(t)$, ponderada Wk")
...
$\text{MSDV}_z = a_w T_0^{1/2}$ (método (b)), cuyo periodo de medida no debería
...
corta cuando $a_w$ pasa de 0,5 m/s². Las mujeres son más propensas que los
...
| Valor total de vibración global $a_v$ | Reacción probable |
| :--- | :--- |
...
$A_i(8)$ va como $\sqrt{T_i}$ mientras que va con $a_{hv}$ directamente: partir
...
MTVV, VDV, MSDV y factor de cresta, los cocientes de comparación del apartado
6.3.3, la escala de confort del Anexo C y la lectura de mareo del Anexo D
(`running_rms`, `motion_sickness_dose_value`), y la magnitud
energético-equivalente del
