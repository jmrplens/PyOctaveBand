...
| $f_s$ | frecuencia de giro del eje, $f_s = N/60$ | Hz | todas |
...
| $s$, $f_e$ | deslizamiento (una fracción, no un porcentaje), frecuencia de alimentación | —, Hz | 3 |
...
| $m_L$ | número de lóbulos de un patrón rotativo de interacción | — | 4 |
...
$\text{BPFO} + \text{BPFI} = Z f_s$; y con la pista exterior quieta la jaula
...
- un defecto en la **pista interior** gira con el eje, así que entra y sale de
  ...
  $\pm f_s$ alrededor de la línea y de cada armónico, a menudo más visibles que
  la propia portadora;
...
componente. Resolver una banda lateral a $\pm f_s$ cuesta longitud de registro,
...
$f_s = N/60$: las siete líneas del rodamiento, el engrane y sus bandas
...
frecuencia física de paso de barras $R f_s = 60 \times 60$ Hz.
...
$f_e = f_s\,p/2$, que es la misma expresión con deslizamiento nulo pero no
...
$n N f_s / m_L$ (Ec. 8.17). Un patrón que gira más rápido que las propias palas
...
en $m_L = nN \pm kV$, que es la razón de que el recuento de lóbulos tenga que
formar parte del nombre de la línea. El patrón de $m_L = 2$ gira a
$nNf_s/m_L = 6 \times 58{,}3/2 = 175$ Hz, tres veces la velocidad del eje, así
que es el que radia con fuerza y la línea que hay que contrastar con la
frecuencia de corte del conducto. El patrón de $m_L = 10$ gira a 35 Hz, por
...
$\pm f_s$ y armónicos que caen; un desgaste distribuido da grupos altos de
...
y sus bandas laterales a $\pm f_s$ necesiten un eje logarítmico para verse
...
de bandas laterales que hay que resolver es el propio $f_s$. Dos segundos a
...
$\pm f_s$ de la sección 1 sean separables siquiera. Registra más tiempo si la
