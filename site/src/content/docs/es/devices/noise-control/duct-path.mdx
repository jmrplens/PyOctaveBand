...
noise_control.fan_sound_power(relative_efficiency_percent=80.0, fan_static_pressure_pa=**servicio).plot(language="es")
plt.show()

fig, ax = plt.subplots()
for rendimiento in (90.0, 80.0, 55.0):
    res = noise_control.fan_sound_power(relative_efficiency_percent=rendimiento, fan_static_pressure_pa=**servicio)
