...
- **Codo** (apartado 6.2, Tabla 8): según el tamaño del propio codo, con
  `elbow_insertion_loss`.
- **Cambio de sección** (apartado 6.3): la reducción que da la VDI 3733,
  limitada a 5 dB, porque el valor impreso solo se alcanza con el conducto
  terminado anecoicamente por los dos extremos.
- **Ramificación** (apartado 6.5): la parte del caudal que se lleva la rama,
  con `split_loss`.
...
- **Un tramo recto** (Ecuación (16)) y **un codo o una ramificación**
  (Ecuación (17)), con `flow_noise_straight_duct` y `flow_noise_bend`. Ambas
  están escritas sobre un número de Strouhal construido con el diámetro del
  propio elemento y la velocidad que lo atraviesa, y las dos figuras declaran
  que solo valen por encima de $St = 1$, así que la biblioteca no devuelve nada
  por debajo en lugar de extrapolar una curva que la directriz no dibuja.
