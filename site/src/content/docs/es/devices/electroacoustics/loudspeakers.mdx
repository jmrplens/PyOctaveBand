...
detrás, y la del altavoz es la que hace tropezar: una normalización de potencia
y distancia con dos trampas independientes en una sola frase del apartado 20.3.
...
en $2\pi$ y la respuesta cae hasta 6 dB — el escalón de la pantalla —, con la
difracción en los cantos añadiendo unos decibelios de rizado alrededor; nada de
...
altavoz sobre una pantalla grande, y como caso de referencia contra el que se
...
`RadiatingPistonResult`, que es la forma a la que echar mano cuando se
dimensiona un cono o se mete la pantalla en una transparencia.
...
del 80 % es la razón de que las especificaciones nominales de 4 y 8 Ω sobrevivan
sobre un módulo que varía en un orden de magnitud a lo largo de la banda.
...
efectivo del apartado 21.2, leídos contra la línea de $-10$ dB referida a la
...
*Se lee contra la respuesta de encima: un pico de distorsión que coincide con un
pico de respuesta es una resonancia excitada con fuerza, y una subida hacia baja
frecuencia es el cono quedándose sin excursión. La distorsión se cita a una
excitación declarada, así que una curva de THD sin su tensión de entrada y sin
el nivel de presión acústica que produjo a 1 m es ininterpretable (apartado
24.1.2.7).*
...
$D_i = L_{ax} - L_p + 10\lg(T/T_0) - 10\lg(V/V_0) + 25$ dB, con $T_0 = 1$ s y
...
abiertos** $\Delta L_\text{nom} = 10\log_{10} N_m$ (ec. (18.23)). El criterio queda
...
1. **Ajusta el sistema una vez y no lo toques.** $L_{H\text{-}M}$ y
   $L_{H\text{-}L}$ son un *par* leído con un mismo ajuste de ganancia sin
...
2. **Excita con ruido rosa** y lee el nivel en la posición del micrófono con el
   ...
   hacia dónde apunta forman parte del lazo. Eso es $L_{H\text{-}M}$. Mover la
   cápsula 0,3 m hacia el altavoz o alejándola de él lo cambia en varios
   decibelios, y es el arreglo más barato que hay cuando el criterio no se
   cumple.
3. **Lee $L_{H\text{-}L}$** con el mismo ajuste, promediado sobre tres a cinco
   butacas representativas.
4. **Mide $L_{T\text{-}M}$** con un hablante o una boca artificial a la
   distancia de hablante de diseño; eso fija
   $Z_S = L_{H\text{-}L} - L_{T\text{-}M}$.
5. **Lee $D_M(\theta)$** en los propios datos polares del micrófono, en la
...
$+D_M(\theta)$ en el segundo miembro donde las ecs. (18.20) a (18.22) exigen
$-D_M(\theta)$, lo que convertiría la ventaja de un micrófono direccional en una
penalización. `feedback_stability` implementa el signo de la ec. (18.20), de
modo que con $N_m = 1$ reproduce exactamente los casos particulares del propio
...
| :--- | :--- | :--- | :--- | :--- |
| `open_loop_gain` | float | dB | finito | $Z_S = L_{H\text{-}L} - L_{T\text{-}M}$; unos -6 dB en un auditorio |
| `level_loudspeaker_at_microphone` | float | dB | finito | $L_{H\text{-}M}$ |
| `level_loudspeaker_at_listener` | float | dB | finito | $L_{H\text{-}L}$ |
| `microphone_directivity` | float | dB | defecto 0 | $D_M(\theta)$, 0 omni, unos -2 cardioide |
| `open_microphones` | int | — | ≥ 1, defecto 1 | $N_m$ |
...
apartado 7.1) y las condiciones en las que quedan definidas: los tres
montajes del apartado 10, el plano, el punto y el eje de referencia del
apartado 15, el preacondicionamiento de 1 h del apartado 12, la excitación
con ruido rosa limitado en banda de 20.1.2 con su sustituto por tercios de
octava, los requisitos de instrumentación de los apartados 8 y 9, el campo
libre simulado de 5.5/7.3, y la geometría polar, la regla de excitación y las
dos vías del índice de directividad del apartado 23. El modelo de radiación
del pistón en pantalla
...
**No cubierto.** Ninguna de las condiciones de arriba la comprueban las
funciones: reducen y presentan las curvas que se les entregan, así que el
montaje, la excitación, el enventanado y la condición de campo quedan a cargo
de quien opera, que además tiene que declararlos junto al resultado. Los
valores eléctricos y mecánicos que la norma define
