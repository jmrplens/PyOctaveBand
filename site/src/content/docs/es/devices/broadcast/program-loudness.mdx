...
el nivel de entrega del que depende todo lo demás. Las dos salidas legítimas son
un limitador de pico verdadero aplicado a los momentos más sonoros (que baja algo
el LRA, y hay que declararlo) o renegociar el objetivo de esa entrega.
...
nada aquí lo ejecuta. Tampoco hay un *ayudante* de normalización: la sección 7
enseña la ganancia única, y aplicarla es una multiplicación. Y nada de esta
página es un **modelo** de sonoridad — la ponderación K es un filtro lineal fijo,
así que el enmascaramiento, la dependencia con el nivel, las bandas críticas y la
suma binaural quedan fuera por construcción; eso vive en
