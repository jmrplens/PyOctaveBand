...
# uno a uno: (sucesos, posiciones, bandas), tercios de octava de 100 Hz a 10 kHz.
slam = np.linspace(88.0, 76.0, freqs.size)      # nivel de suceso aislado, dB re E0
...
slam_lj = emission.sound_energy_reverberation(
    ...
    background_levels=np.full(freqs.size, 45.0),   # promediado en los mismos 4 s
...
print(np.round(lj_octave, 1))         # 99.8 ... 87.7 dB, tres tercios sumados por octava
```
