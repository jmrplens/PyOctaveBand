...
description: "La determinación de precisión del nivel de potencia acústica en una cámara reverberante cualificada según ISO 3741: el método directo mediante el área de absorción de Sabine con las correcciones de Waterhouse y meteorológicas C1/C2, el método de comparación con una fuente sonora de referencia, los avisos de cualificación, la ficha de ensayo de estilo acreditado, y el nivel de energía acústica LJ de una ráfaga por los mismos dos métodos."
references:
  - type: book
...
cualificación, la ficha de ensayo de estilo acreditado y, en la sección 4, el
nivel de energía acústica $L_J$ que los mismos dos métodos dan a un suceso
aislado. Qué vía encaja con
...
una carrera de prensa o un portazo, se describe en cambio con el nivel de
energía acústica $L_J$ del apartado 9.2, que cubre la sección 4.
...
## 4. Nivel de energía acústica de un suceso aislado (apartado 9.2)

Los dos métodos anteriores promedian en energía el nivel de la cámara y dan un
ritmo de flujo de energía, así que una máquina que dispara una vez y calla no
tiene $L_W$ que determinar. El apartado 9.2 le da el **nivel de energía
acústica** $L_J = 10\log_{10}(J/J_0)$, $J_0 = 1$ pJ (apartado 3.18), y lo
determina por las mismas dos vías con el nivel de presión acústica de suceso
aislado integrado en el tiempo $L_E$ (apartado 3.4) en lugar del $L_p$
promediado en el tiempo: la Ec. 30 es la Ec. 20 con $\overline{L_E(\text{ST})}$
donde estaba $\overline{L_p(\text{ST})}$, y la Ec. 31 es la Ec. 21 de igual
modo,
...
La medición es el apartado 8.5: al menos **cinco** sucesos en cada una de las
seis posiciones, uno a uno o como una sola lectura que los abarque todos, a
través de un intervalo que contenga toda parte significativa del suceso
*incluida su caída*, y sin micrófono móvil para un impulso no repetitivo. El
apartado 9.2.1 reduce ambas lecturas al nivel de un suceso en cada posición
(Ec. 22, la media energética de $N_\mathrm{e}$ lecturas individuales, o Ec. 23,
una lectura menos $10\log_{10} N_\mathrm{e}$; `mean_single_event_level` en la
[página de métodos de presión](/phonometry/es/devices/emission/sound-power-pressure/#cinco-sucesos-uno-a-uno-o-todos-a-la-vez)
es el mismo par). El apartado 9.2.2 corrige cada posición por su propio fondo
con el criterio dependiente de la frecuencia de 9.1.2 (Ec. 25/26), y el
apartado 9.2.3 promedia en energía las posiciones corregidas (Ec. 27). El fondo
es el nivel promediado en el tiempo medido sobre el mismo tiempo de integración
$T$ que los sucesos, y se compara como su exposición sobre esa ventana,
$L_{pi(\mathrm{B})} + 10\log_{10}(T/T_0)$, para que las energías que resta la
Ec. 25 compartan una referencia; `integration_time` es por tanto obligatorio
con `background_levels`. Las razones se detallan en la página de métodos de
presión y en el [registro de
erratas](https://github.com/jmrplens/phonometry/blob/main/docs/ERRATA.es.md).

`sound_energy_reverberation` y `sound_energy_comparison` admiten `levels` en
las formas que admiten las funciones de potencia acústica, más una: un array
3D `(Ne, NM, NB)` de los sucesos medidos uno a uno, o una lectura 1D/2D con
`events=Ne` para una sola medición de varios. La cámara de la sección 1 y la
fuente de referencia de la sección 2 sirven para ambas:

```python
import numpy as np
from phonometry import emission

# Cinco portazos registrados en las seis posiciones de la cámara de la sección 1,
# uno a uno: (sucesos, posiciones, bandas), los 21 tercios de octava de 100 Hz a 10 kHz.
...
slam = np.linspace(88.0, 76.0, thirds.size)      # nivel de suceso aislado, dB re E0
...
slam_lj = emission.sound_energy_reverberation(
    ...
    background_levels=np.full(thirds.size, 45.0),   # promediado en los mismos 4 s
...
# Método de comparación: la fuente de referencia de la sección 2, en régimen estacionario.
...
slam_lj.plot()   # LJ por tercio de octava, LJA en el título (requiere matplotlib)
```

La identidad con la cadena estacionaria es exacta también aquí: el nivel medio
de cámara de la sección 1 leído como ventana de 10 s,
$L_E = L_p + 10\log_{10}(T/T_0)$ en cada posición, da $L_J = L_W + 10$ dB en
todas las bandas a través de la misma área de absorción, el mismo término de
Waterhouse y los mismos $C_1$/$C_2$, que es la razón de que la Ec. 30 exprese
$L_J$ en las condiciones meteorológicas de referencia exactamente como la
Ec. 20 expresa $L_W$:

```python
lp_thirds = np.linspace(80.0, 70.0, thirds.size)     # el SPL medio de cámara de la sección 1
...
### Bandas de octava y total ponderado A (Anexo F)

Ambos métodos se definen sobre bandas de tercio de octava, y el Anexo F forma
después los niveles por bandas de octava y ponderados A, para la potencia
acústica (Ec. F.1, F.2) y para la energía acústica (Ec. F.4, F.5) por igual. El
total ponderado A es el `sound_energy_level_a` de todos los resultados
anteriores; las bandas de octava son la suma energética de los tres tercios
$k = 3i - 2$ a $3i$ que la Tabla F.1 asigna a la octava $i$, numerados desde
$k = 1$ en 50 Hz, que `octave_band_levels` aplica a cualquier espectro por
bandas cuyos tercios lleguen en tripletes completos:
...
print(np.round(lj_octave, 1))         # de 99.8 dB a 87.7 dB, tres tercios sumados por octava
```

### Parámetros de `sound_energy_reverberation()` y `sound_energy_comparison()`

| Parámetro | Tipo | Unidades | Rango / valor por defecto | Notas |
| :--- | :--- | :--- | :--- | :--- |
| `levels` | array 1D, 2D o 3D | dB | por banda, `(NM, NB)` o `(Ne, NM, NB)` | Niveles de suceso aislado: por suceso en el primer eje (Ec. 22), una lectura de `events` sucesos (Ec. 23), o la media de un suceso |
| `t60`, `volume`, `surface_area`, `frequencies` | como en la sección 1 | | | Solo método directo. Ahí `frequencies` es obligatorio; el método de comparación lo pide solo cuando se da `background_levels`, porque la corrección es por banda |
| `levels_ref`, `lw_ref`, `background_levels_ref` | como en la sección 2 | | | Solo método de comparación; la fuente de referencia es estacionaria y conserva la corrección de 9.1.2 |
| `events` | int o None | | `>= 1`, por defecto `None` | $N_\mathrm{e}$ abarcados por una lectura (Ec. 23); al menos 5 sin aviso |
| `background_levels` | array 1D o 2D | dB | coincide con `levels` | Fondo promediado en el tiempo sobre la misma ventana; $K_{1i}$ por posición (Ec. 25/26) |
| `integration_time` | float | s | > 0, obligatorio con `background_levels` | La ventana $T$; el fondo se compara como $L_{pi(\mathrm{B})} + 10\log_{10}(T/T_0)$ |
| `temperature`, `static_pressure` | float | °C, kPa | por defecto `23.0`, `101.325` | Fijan $c$, $C_1$, $C_2$ como en la sección 1 |

Ambas devuelven un `ReverberationSoundEnergyResult` (`sound_energy_level`,
`mean_event_level`, `absorption_area`, `waterhouse_correction`,
`background_correction`, `c1`, `c2`, `speed_of_sound`, `sound_energy_level_a`,
`method`, `events`, `integration_time`), con los mismos campos `NaN` para el
método de comparación que su gemelo de potencia acústica, y los mismos avisos
de cualificación.
...
como ficha de potencia acústica de estilo acreditado mediante `.report()`. El
nivel de energía acústica $L_J$ de un suceso aislado por los mismos dos
métodos (`sound_energy_reverberation`, Ec. 30; `sound_energy_comparison`,
Ec. 31): los $N_\mathrm{e}$ sucesos reducidos a uno (Ec. 22/23), el $K_{1i}$
por posición de la Ec. 25 con el fondo comparado sobre la misma ventana, la
media de la Ec. 27, y los niveles por bandas de octava
(`octave_band_levels`, Ec. F.1/F.4) y ponderados A (Ec. F.2/F.5) del Anexo F.
</ScopeClaim>

<ScopeClaim status="not-covered" label="No cubierto">
Los resultados de suceso aislado no llevan ficha `.report()`, y el promedio
sobre varias posiciones de fuente de la Ec. 24 (y de la Ec. 13 para $L_W$) se
deja a quien llama, que aporta niveles ya promediados sobre las posiciones del
apartado 8.4.2.4. La propia cualificación de la cámara
