...
print(np.round(burst.sound_energy_level, 1))               # LJ por banda, 95.1 ... 90.8 dB
...
print(round(float(five_at_once[0]), 2))          # 90.01 dB: 97 - 10 lg 5 (Ec. 20)
one_by_one = emission.mean_single_event_level(np.array([89.0, 91.0, 90.0, 90.5, 89.5]))
print(round(float(one_by_one), 2))               # 90.05 dB: la media energética (Ec. 19)
```
...
print(np.round(ten_seconds.sound_energy_level - res.sound_power_level, 6))   # 10.0 en todas las bandas
