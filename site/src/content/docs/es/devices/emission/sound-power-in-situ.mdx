...
125 Hz a 8 kHz** (apartado 3.11), el único rango que tabula el Anexo D.
...
La banda de 125 Hz es la que hay que leer dos veces. En dos de las cuatro
posiciones el margen sobre el ruido de la nave está por debajo de 6 dB para
la máquina, y por debajo de 6 dB en todas las posiciones para la fuente de
referencia, así que $K_1$ toma allí su tope de 1,3 dB y los 88,8 dB que
reporta la banda son una **cota superior**, que es lo que dice el `False`.
Todas las demás bandas superan los 6 dB en todas partes, la de 250 Hz con
correcciones de medio decibelio que la Ec. 7 evalúa en lugar de ignorar, y
de 1 kHz en adelante el margen supera los 15 dB en la mayoría de las
posiciones y la corrección es cero.

<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/in_situ_sound_power_es.svg" alt="Dos paneles sobre las siete bandas de octava de 125 Hz a 8 kHz. El panel superior representa los niveles de presión acústica en las cuatro posiciones de micrófono: los niveles corregidos de la fuente en ensayo como círculos azules rellenos con su media energética como línea azul, los niveles corregidos de la fuente de referencia como cuadrados verdes con su media como línea verde, los niveles medidos antes de la corrección por ruido de fondo como círculos huecos unidos a los corregidos allí donde la corrección los movió, y el ruido de fondo en cada posición como aspas grises. Una nota enmarcada en 125 Hz dice que el margen está por debajo de 6 dB en dos posiciones, que K1 se limita a 1,3 dB y que la banda es una cota superior. El panel inferior dibuja el nivel de potencia acústica resultante de la fuente en ensayo como barras azules, la de 125 Hz rayada como cota superior, con el nivel de potencia acústica calibrado de la fuente de referencia como línea roja discontinua con rombos, y declara en su título un nivel ponderado A de 96,1 dB(A), grado 2, y una incertidumbre expandida de 3,2 dB con k = 2 y una desviación de 0,5 dB por condiciones de funcionamiento" width="96%" />
...
| `frequencies` | array 1D | Hz | octavas de 63 Hz a 8 kHz | Frecuencias centrales nominales de octava de la Tabla D.1; obligatorio |
| `background_levels` | array 1D o 2D | dB | `(bandas,)` o `(n, bandas)` | $L_{pi(\mathrm{B})}$; un espectro sirve para todas las posiciones |
...
| `excess_levels` | array 1D | dB | `(n,)` | $\Delta L_{f\mathrm{A}}$ en cada posición (Anexo A); con `directivity_range` decide el grado |
| `directivity_range` | float | dB | $\ge 0$ | Semiamplitud del rango de directividad ponderado A de la fuente (7.2) |
...
| `events` | int | | $\ge 1$ | $N$ para la forma 2D (Ec. 17); debe ser `None` con la forma 3D |
...
`frequencies` sean esos ocho centros nominales, la fila de 63 Hz solo para
uso donde el entorno y la instrumentación son satisfactorios allí (Tabla
D.1, nota a).
