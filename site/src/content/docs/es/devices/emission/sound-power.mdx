...
description: "El nivel de potencia acústica LW como descriptor de emisión independiente del dispositivo: la elección entre las seis vías de determinación normalizadas (ISO 3744/3746, ISO 3745, ISO 3741, ISO 9614-2, ISO 9614-3, ISO/TS 7849), qué prometen los grados de precisión, y la declaración de emisión sonora de ISO 4871 con sus formas de número doble y único y su verificación."
references:
  - type: book
...
ponderado A $L_{WA}$, pero bajo entornos, grados de precisión y restricciones
prácticas distintos.

| Método | Norma | Magnitud medida | Entorno | Grado de precisión | Cuándo usarlo |
| :--- | :--- | :--- | :--- | :--- | :--- |
| Superficie envolvente | **ISO 3744** (ingeniería) / **ISO 3746** (inspección) | Presión acústica sobre una semiesfera o caja | Campo esencialmente libre sobre uno o varios planos reflectantes | Grado 2 ($\sigma_{R0} \approx 1{,}5\ \text{dB}$) / grado 3 ($\approx 3{,}0\ \text{dB}$) | In situ o en una sala grande; sin instalación de ensayo especial disponible |
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/sound_power_methods_es.svg" alt="Las seis vías de potencia acústica en dos filas de tres, cada celda con los mismos cinco atributos. Fila superior: la superficie envolvente de presión de ISO 3744/3746 sobre un plano reflectante, el conjunto fijo de micrófonos de ISO 3745 en una cámara anecoica revestida de cuñas y el campo difuso de una cámara reverberante de ISO 3741. Fila inferior: el barrido serpenteante de intensidad de ISO 9614-2, el barrido de precisión más estricto de ISO 9614-3 y la retícula de acelerómetros de ISO/TS 7849 sobre una carcasa radiante, sin micrófono alguno" width="92%" loading="eager" />
...
El grado es una afirmación sobre la **reproducibilidad**: $\sigma_{R0}$ es la
...
típicos son $\sigma_{R0} \approx 0{,}5\ \text{dB}$ para el grado 1 (ISO 3741), 1,5 dB para el
...
cobertura), donde $\sigma_\text{tot} = \sqrt{\sigma_{R0}^2 + \sigma_\text{omc}^2}$
...
En la práctica: un $L_{WA}$ de grado 2 de 92,4 dB lleva $U \approx 3\ \text{dB}$, así que dos
...
anchura convertida en $K_{WA}$ y sumada al valor medido, que es la razón de que
...
acústica ponderado A $L_{WA}$, acompañado opcionalmente del nivel de presión
acústica de emisión ponderado A $L_{pA}$ en un puesto de trabajo.

Una declaración adopta una de dos formas alternativas (apartado 4):

- la forma de **doble número** (apartado 3.16): el valor medido $L_{WA}$ y su
  incertidumbre $K_{WA}$ indicados juntos pero por separado; y
- la forma de **número único** (apartado 3.15): el valor declarado derivado
  $L_{WAd} = L_{WA} + K_{WA}$, un límite superior que es improbable que las mediciones
  repetidas superen al nivel de confianza indicado.

$K_{WA}$ combina la incertidumbre de medición (reproducibilidad) y, para un lote,
...
de ISO 4871 (dos modos de funcionamiento, $L_{WA} = 88$ y 95 dB con $K_{WA} = 2$
dB, que dan los valores declarados $L_{WAd} = 90$ y 97 dB):
...
verifica frente a $L_{WAd}$, la suma sin redondear $L_{WA} + K_{WA}$ redondeada
una sola vez; una declaración de **número doble** se verifica frente a
$\mathrm{redondeo}(L_{WA}) + \mathrm{redondeo}(K_{WA})$, que puede diferir en un
...
**nivel de presión acústica de emisión** ponderado A $L_{pA}$ en el puesto de
...
métodos y la ruta de decisión, las condiciones de funcionamiento y de montaje
que la preceden, qué afirma cada grado de precisión sobre la
...
formas de número doble y de número único, $L_{WAd} = L_{WA} + K_{WA}$ y la
verificación del apartado 6.2 con sus dos reglas de redondeo (`verified`,
`verified_dual`), presentadas como ficha de declaración mediante `.report()`.

**No cubierto.** Las seis determinaciones viven en sus guías de método: la
...
[Intensidad acústica](/phonometry/es/devices/emission/intensity/).
La ISO 3743-1, la ISO 3743-2 y la ISO 3747, los miembros restantes de la
familia ISO 3740, tampoco están implementadas, ni lo está el nivel de *energía*
acústica $L_J$ para sucesos aislados. El nivel de presión acústica de emisión
$L_{pA}$ que acompaña a $L_{WA}$ en una declaración se consume, no se determina:
la ISO 11201, la ISO 11202 y la ISO 11204 quedan fuera de la biblioteca. Los
...
allá del caso de máquina única
...
El grado es una afirmación sobre la reproducibilidad: $\sigma_{R0}$ es la desviación
...
$\sigma_{R0} \approx 0{,}5\ \text{dB}$ para el grado 1 (ISO 3741), 1,5 dB para el grado 2
(ISO 3744, ISO 9614-2) y 3 dB o más para el grado 3. Un $L_{WA}$ de grado 2
