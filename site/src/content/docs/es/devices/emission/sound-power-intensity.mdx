...
ingeniería a inspección da por sí sola 3 dB de $L_d$, porque $K$ pasa de 10 dB a
...
determinación: un $L_{WA}$ informado sin decir qué bandas se descartaron no es
...
| `frequencies` | array 1D | Hz | octava 63 Hz-4 kHz, tercio 50 Hz-6,3 kHz | $L_{WA}$ y límites de la Tabla 2; fuera de ese rango la Tabla 2 no tiene $s$ y la llamada lanza un error |
...
$L_{WA}$), `surface_area`, `sound_power_level_a` y `grade`.
...
$F_{pI_n}$, la no uniformidad del campo $F_S$ y la variabilidad temporal $F_T$
...
2. **suficiencia de la capacidad dinámica**,
   $L_d \ge F_{pI_n}^{\text{con signo}}$ con $L_d = \delta_{pI0} - K$ y
   $K = 10$ dB (Ec. C.2);
3. $F_{pI_n}^{\text{con signo}} - F_{pI_n}^{\text{sin signo}} \le 3$ dB
   (Ec. C.3), la contrapartida de precisión del límite de $F_{+/-}$ de la
   ISO 9614-2;
4. **no uniformidad del campo**, $F_S \le 2$ (Ec. C.4);
5. **convergencia con la densidad de barrido**,
   $0{,}83 \le F_S(1)/F_S(2) \le 1{,}2$ entre la densidad inicial y una doblada
   (Ec. C.5).
...
4 kHz el indicador con signo (12,2 dB y 9,2 dB) supera la $L_d = 5$ dB de la
...
$F_T$, $F_{p|I_n|}$, $F_{pI_n}$ y $F_S$ (apartado 10 f) 1)) y la celda de grado
que deciden los criterios.

Si le pasas la cualificación calculada arriba, la hoja hace lo que el apartado
...
9.2). Por eso el $L_{WA}$ del recuadro puede diferir de
`result.sound_power_level_a`, que se calcula antes de evaluar ningún criterio y
por tanto suma todas las bandas aplicables. Sin `criteria`, la ficha recuadra el
...
sonda (apartado 10 d) 5)) con la capacidad dinámica $L_d$ que se deduce de él;
...
# result, ind y crit son la determinación de precisión, sus indicadores del
# Anexo B y sus criterios del Anexo C de los dos bloques anteriores.
result.report(
    ...
    metadata=ReportMetadata(
        ...
        requirement=98.0,
        notes="Superficie en caja a 0,25 m; cuatro superficies parciales barridas dos veces.",
    ),
    indicators=ind, criteria=crit, residual_index=15.0,
    language="es",
)   # 250 Hz omitida (apartado 9.2), 4 kHz omitida (criterio 2)
```
...
$L_d$, el criterio de repetibilidad, el grado alcanzado por banda y el marcado
