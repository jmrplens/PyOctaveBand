...
`sound_exposure` y `lex_8h`) van más allá y aceptan el propio `Signal`: el objeto ya conoce su frecuencia y su
calibración, y pedirte que repitas cualquiera de las dos es pedir un error
de transcripción.
...
vez. `read_blocks()` entrega exactamente lo que `read()` devolvería, bloque
a bloque — mismo escalado, mismo convenio de canales, y en la instalación
base posicionamiento y descodificación propios para el WAV lineal, RF64
incluido — de modo que los filtros con estado de la guía de
