...
así que toda función `(x, fs, ...)` de la biblioteca lo acepta hoy. Las
funciones de nivel (`leq`, `laeq`, `ln_levels`, `sel`, `lc_peak`,
`sound_exposure` y `lex_8h`) y los filtros (`octave_filter`,
`weighting_filter`, `time_weighting`, `linkwitz_riley`, `parametric_eq`, y los
objetos de proceso por bloques `OctaveFilterBank`, `WeightingFilter`,
`TimeWeighting` y `ParametricEQ`) van más allá y aceptan el propio
`Signal`: el objeto ya conoce su frecuencia y su calibración, y pedirte que
repitas cualquiera de las dos es pedir un error de transcripción. La frecuencia
que lleva manda, y una explícita que discrepe lanza en vez de arbitrarse.

Un `Signal` calibrado se *procesa* en pascales, que es una sola regla con tres
lecturas según lo que devuelva la llamada: `weighting_filter`, `linkwitz_riley`
y `parametric_eq` devuelven una forma de onda, y esa forma de onda va en
pascales; `time_weighting` eleva su entrada al cuadrado, así que su envolvente
va en Pa²; y `octave_filter` y las funciones de nivel devuelven un nivel, así
que lo que compra ahí la calibración es dB SPL re 20 µPa en vez de un número en
unidades digitales. Las rutas `dbfs` son la excepción deliberada: su referencia
...
nivel: el `Signal` lleva los dos. Un argumento explícito sigue ganando al
que viene en el objeto — quien llama sabe más que un objeto — y un array
desnudo sin `fs` se rechaza nombrando el argumento en vez de adivinarlo.
...
y el flujo aporta cada muestra exactamente una vez. (La comparación de una
pasada dice `high_accuracy=False` porque ese es el filtro que ejecuta una
ponderación con estado; la
[guía de ponderación](/phonometry/es/signals/levels/weighting/) cubre la
distinción.) La misma construcción alimenta un `OctaveFilterBank` con
