...
$$
f_m = 1000 \cdot G^{(x-30)/b} \quad (b \text{ impar}), \qquad
f_m = 1000 \cdot G^{(2x-59)/(2b)} \quad (b \text{ par})
$$
...
**Centro exacto frente a etiqueta impresa.** El $f_m$ de arriba es la frecuencia
...
Es idéntico al $L_{EP,d}$ de la Directiva 86/188/CEE y al $L_{EX,8h}$ de ISO 1999 (IEC 61252, 3.3 NOTAS 5–6). El ancla de IEC 61252 (3.3 NOTA 4): una exposición de **3,2 Pa²h corresponde a un $L_{EX,8h}$ de exactamente 90 dB**. ISO 9612 prescribe cómo muestrear una jornada de trabajo para obtenerlo y cómo acotar el resultado; consulta [Exposición al ruido en el trabajo](/phonometry/es/reference/theory/environment-transport/#exposición-al-ruido-en-el-trabajo-iso-9612).
