...
| :--- | :--- | :--- |
| $A$ | Área de absorción equivalente, m² ($A_0 = 10$ m² de referencia, $A_T$ de la probeta, $A_{obj}$, $A_{air}$) | en toda la página |
...
| $C$, $C_{tr}$ | Términos de adaptación espectral, dB | índices de ISO 717 |
| $T$ | Tiempo de reverberación, s ($T_s$ es el tiempo central) | en toda la página |
...
$L_{LF} = 10 \log_{10}\left[(10^{0{,}1 L_{esquina}} + 2 \cdot 10^{0{,}1 L})/3\right]$
(Fórmula 13), y ese $L_{LF}$ sustituye al nivel por defecto **solo en las bandas
...
**Cómo se leen $C$ y $C_{tr}$.** El índice ponderado condensa una curva entera
...
negativos, y el valor son los decibelios que hay que *sumar* a $R_w$ para obtener
la diferencia de niveles que esa fuente va a experimentar realmente, que es la
razón por la que las reglamentaciones enuncian los requisitos como $R_w + C$ o
$R_w + C_{tr}$ y no como $R_w$ a secas. Eso convierte a $C_{tr}$ en un
...
$R_w$ pueden diferir en muchos decibelios bajo tráfico. Los términos se calculan
...
disposición muy especificada, y sin ella $\alpha_s$ no significa nada. La cámara
...
la razón por la que un $\alpha_s$ de catálogo sin su código de montaje no sirve, y
...
registrado. Y un $\alpha_s$ por encima de 1 no es un error, sino la firma de la
...
$K_{ij}$ y la longitud de acoplamiento $\ell_f$ de la fórmula de abajo, y el
elemento separador es donde se mide $S_s$. Cada unión aporta tres términos de
...
$f_l = c_0/(2\pi d)$ (Ec. 7.64); un relleno poroso baja $f_0$. El 60 de $f_0$ es
...
condiciones ordinarias y cambia con una cámara llena de gas o al vacío; y $f_l$
...
equipos de trabajo distintos) la $\sigma_{situ}$ in situ; la situación **C** (misma
