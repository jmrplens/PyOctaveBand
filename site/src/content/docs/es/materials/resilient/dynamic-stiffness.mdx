...
respuesta y la resonancia vertical fundamental $f_r$ del sistema placa-probeta
se lee del pico de la respuesta; la Fórmula 4 la convierte en $s'_t$.

### 1.1 Obtener $f_r$

La biblioteca toma $f_r$ como entrada, y el apartado 7 es cómo la produce un
...
El problema está en que $f_r$ suele depender de esa fuerza, porque una capa
...
2. Repetir en **al menos tres** amplitudes de fuerza dentro del intervalo que
   ...
   $s' \le 50$ MN/m³.
3. Extrapolar linealmente los puntos $(F, f_r)$ hasta fuerza nula. **Esa
   ordenada en el origen** es la $f_r$ que espera la biblioteca.
...
más afilada, y ver un barrido pasar por $f_r$ enseña por qué.
...
mientras que la fase pasa por $-90°$ de forma abrupta y lo cruza en $f_r$ sea
...
La $f_r$ medida la deciden el asiento y las tolerancias del banco al menos
...
- **Asiento.** Cubrir la probeta con una lámina de plástico impermeable de unos
  ...
  probeta, rigidiza la lectura y sesga $f_r$ hacia arriba.
...
$s'_t$ que midió el banco ya lo contiene y no se añade nada. Con $r$
**intermedia** escapa de la probeta pequeña pero no de un suelo de varios metros
de lado, así que la norma calcula $s'_a$ y lo vuelve a sumar. Con $r$ **baja**
escapa de los dos: no hay resorte de gas que sumar, pero el método tampoco puede
ya separar las dos contribuciones. En ese tercer caso, declarar $s' = s'_t$ con
la reserva, y enunciar el error estimado de despreciar $s'_a$: la norma pide
...
porosidad $\varepsilon$ de $s'_a$ como el 0,9 de la norma salvo que se haya
medido.
...
por métodos sinusoidal, de ruido blanco o de impulso, no está implementado:
pasa una $f_r$ ya extrapolada, obtenida como describe la sección 1.1 (la
extrapolación en sí es un ajuste por recta, que la sección 1.1 muestra).
