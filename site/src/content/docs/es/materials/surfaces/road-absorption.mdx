...
que vale $2/3$ para la geometría obligatoria $d_s = 1{,}25$ m, $d_m = 0{,}25$ m.
...
los espectros incidente y reflejado enventanados $H_i$, $H_r$:
...
$2 d_m / c \approx 1{,}5$ ms, muchísimo menos de lo necesario para separarlas
...
3. Repetir la medición idéntica con el conjunto apartado de cualquier superficie
   reflectante. Esa respuesta de campo libre **es** $h_i$. Corregir cualquier
...
4. Restar: $h_r = h_\text{pavimento} - h_i$. Todo lo que comparten las dos
...
5. Enventanar $h_i$ y $h_r$ con la **misma** ventana aplicada en sus respectivos
   tiempos de llegada ($T_{w,\text{directo}} = T_{w,\text{reflejado}}$),
...
El cociente en bruto $|H_r/H_i|^2/K_r^2$ supone que la cadena electroacústica y
...
ventana; el ejemplo resuelto del Anexo A ($d_s = 1{,}25$ m, $d_m = 0{,}25$ m,
...
- **El área máxima muestreada debe estar despejada.** Cualquier objeto que no
  ...
  quede fuera debe estar más lejos del micrófono que $d_s$, es decir, a más de
...
(`materials.absorption_reference_corrected`) y el caso de incidencia oblicua
del Anexo F (`materials.geometric_spreading_factor_angle`,
`materials.msa_major_axis` y el argumento `incidence_angle` de
`materials.insitu_absorption_spectrum`); y el límite de frecuencia de
...
[guía del Tubo de impedancia](/phonometry/es/materials/absorbers/impedance-tube/).
Tampoco se hacen cumplir en ningún sitio del código ni la adquisición en sí ni
los filtros de campo: el promediado, la referencia de campo libre, las
