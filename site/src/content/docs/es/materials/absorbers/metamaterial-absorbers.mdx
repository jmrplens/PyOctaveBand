...
Todos los símbolos de esa cadena: $M_s$ es el medio paso de red de la propia
ranura, $M_{HR}^{(n)}$ la admitancia en derivación del $n$-ésimo resonador con
$Z_{HR}$ su impedancia de entrada (cuello más cavidad, con las correcciones de
radiación de extremo de Jiménez et al. 2016), y $M_{\Delta l}$ la corrección de
radiación de la boca de la ranura. $\kappa_s$, $\rho_s$ y $S_s$ son el módulo
...
reflexión queda sobre el eje de frecuencia real. También están expuestos los
dos bloques constructivos viscotérmicos con los que se ensambla la cadena
(`slit_effective_properties`, `rectangular_duct_properties` y
`helmholtz_resonator_impedance`, todos con un estado `AirProperties`), y
...
la sección 5 expone la vía por la que se verificaría un panel construido, en el
...
(ISO 354), pero nada de aquí ejecuta nada de eso: las cifras de tolerancia y de
