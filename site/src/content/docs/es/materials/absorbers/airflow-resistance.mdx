...
resistividad al flujo de aire $\sigma$ es $R_s$ por metro de espesor. Dos
...
area)` es el $R_s = R\,A$ del apartado 3.2, que además admite la otra vía,
...
forma cerrada $R_s = 8\eta h/(\phi r^2)$ de una placa perforada, de modo que el
...
con $P_S$ la presión estática (atmosférica), $f$ la frecuencia del pistón, $V$
el volumen de la cavidad de aire (incluida toda tubería de conexión, con el
pistón a media carrera), $L_{p,s}$ y $L_{p,t}$ los niveles de presión acústica
de la cavidad medidos con la celda de medición que lleva la probeta y con la
terminación hermética, $h_s$ y $h_t$ las amplitudes de carrera del pistón
...
nivel de la cavidad cae, y esa caída $L_{p,t} - L_{p,s}$ *es* la medida. La
relación de carreras $h_t/h_s$ solo normaliza las dos tandas a la misma
...
las tandas lo fija el apartado 8.5/8.6: medir $P_S$, fijar la carrera a $h_s$,
leer la frecuencia del pistón, el nivel de fondo $L_{p,b}$ (con la fuente en
marcha y sin celda ni terminación montadas) y después $L_{p,s}$; solo entonces
cambiar a la terminación hermética, fijar $h_t$, confirmar que la frecuencia no
se ha movido y leer $L_{p,t}$.
...
$(h_t/h_s)\,10^{(L_{p,s}-L_{p,t})/20} < 0{,}3$, mantiene la Fórmula (2)
linealizada lo bastante cerca de la relación exacta del Anexo B; la Fórmula (4),
$L_{p,s} - L_{p,b} > 10$ dB, mantiene la influencia del fondo sobre $L_{p,s}$
...
en la ventana recomendada: el apartado 6.2 da $q_v = 2\pi f h A_P$ y la
Fórmula (1) $v_s = q_v/A$, y recomienda velocidades eficaces entre 0,5 mm/s y
...
hay `.plot()` ni ficha de ISO 9053-2, y los dos pasos hasta $R_s$ y $\sigma$ son
