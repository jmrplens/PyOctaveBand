...
[Ponderaciones especiales](/phonometry/es/signal/levels/special-weightings/), junto
...
[Ponderaciones especiales](/phonometry/es/signal/levels/special-weightings/): `'G'`
...
| `fs` | int | Hz | > 0 | |
| `curve` | str | — | `'A'` (por defecto), `'B'`, `'C'`, `'D'`, `'G'`, `'AU'`, `'Z'` | `'G'` según ISO 7196 (infrasonido), `'B'`/`'D'` históricas y `'AU'` según IEC 61012 se tratan en [Ponderaciones especiales](/phonometry/es/signal/levels/special-weightings/); `'Z'` es un bypass |
...
Consulta [Procesado por bloques](/phonometry/es/signal/filters/block-processing/) para
