...
partiendo del reposo sobre una ráfaga de duración $T_b$ y la envolvente alcanza
$(1-e^{-T_b/\tau})$ del valor estacionario que acabaría produciendo ese mismo
...
que es la Ecuación (7) de IEC 61672-1:2013. Con $\tau_F = 0,125$ s da
...
propia norma. CI exige la identidad en todas las filas de Fast y Slow
(`test_delta_ref_equation7_consistency`, con una tolerancia de 0,15 dB), de modo
que la transcripción de la tabla está comprobada y no simplemente creída.

La consecuencia sostiene el resto de esta página. Como la respuesta es una
función suave de $T_b/\tau$, un detector que resuelva de verdad la ecuación
sigue la columna entera de una vez; un detector que promedia sobre bloques fijos
de 125 ms no tiene ningún $T_b$ dentro, y solo puede acertar allí donde la
ráfaga llene un bloque por casualidad.
...
exactitud, sino una banda de tolerancia con nombre alrededor de una respuesta
nominal: la clase 1 es la banda estrecha, pensada para trabajo de precisión, y
la clase 2 la ancha, para reconocimientos generales, y las dos se ensanchan en
los extremos del rango de frecuencias, que es donde más cuesta cumplirlas. La
clase 0 era una banda todavía más estrecha, de la edición retirada de 1995 de
IEC 61260, y aquí se mantiene como objetivo adicional voluntario para el banco
Butterworth por defecto. Así que un PASA en esta página significa que la
respuesta calculada no se sale nunca de la banda que dibuja la norma; no
...
análisis espectral calibrado y el solucionador FDTD de ondas. El informe numérico
...
- **pyroomacoustics** simula respuestas al impulso de salas para cadenas de
  ...
  ISO 3382. Los solucionadores de fuentes imagen y FDTD de phonometry están fijados a
