...
| Comportamiento | Verdadera ponderación temporal IEC | Más cercano a un integrador por bloques ($L_{eq,\tau}$) |
...
de 125 ms no contiene ningún $T_b$, y solo puede acertar allí donde la ráfaga
...
*La traza exponencial es plana porque $10\lg(1-e^{-T_b/\tau})$ contiene $T_b$
...
- Si necesitas un **$L_{eq}$ promediado por bloques para cada intervalo**, esa
