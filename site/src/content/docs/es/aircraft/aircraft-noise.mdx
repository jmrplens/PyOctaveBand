...
sí sola se lleva el 85 % del total y todo lo demás junto se acredita con el
...
Sobre el sobrevuelo, $\mathrm{PNLT} = \mathrm{PNL} + C$ y su máximo es
...
el ajuste por reparto de bandas) sobre la gráfica apaisada a todo el ancho de
...
corrección tonal, se devuelve el ajuste por reparto de bandas del día de ensayo,
...
(§8.3.6). $\Delta_{\text{peak}}$ se ocupa de un máximo secundario que quede a
menos de 2 dB del $\mathrm{PNLTM}$ y que adelante al máximo ajustado (§8.3.3);
...
$\delta_t = \alpha\,s$ a la atenuación de banda $\delta_B$, consistente con el
