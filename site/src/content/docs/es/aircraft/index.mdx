...
hemisferios de ruido son *datos de entrada*: la biblioteca interpola las
tablas publicadas para un tipo y no las sintetiza a partir de datos de motor,
y la base de datos ANP se lee y nunca se escribe (la versión 2.3 se distribuye
tal cual). De las entradas de ANP, solo las que tienen perfiles de punto fijo
traen una trayectoria lista para usar, porque convertir una salida por pasos
de procedimiento en una trayectoria de vuelo exige el modelo de prestaciones
de mecánica del vuelo del Doc 9911 de la OACI, que no está implementado.
</ScopeClaim>
...
**no** está implementada: el ruido de aeronaves aquí es la familia de la OACI
y la ECAC, que es un conjunto de modelos distinto del de las fuentes de
carretera y ferrocarril de [Fuentes
ambientales](/phonometry/es/environment/sources/), y los dos no deben
mezclarse dentro de un mismo mapa estratégico sin advertirlo.
</ScopeClaim>
</Scope>
