...
`spherical_spreading_adjustment` ($\Delta L_s = -20\log_{10}(r/60)$, Ec. 24),
`atmospheric_adjustment` ($\Delta L_a = -\alpha(f)\,(r - 60)$ con el
coeficiente de
...
exactamente ese valor, que es como la Tabla 3 (definida sobre niveles $L_A$)
...
# Un anillo de estacionario con la forma de una campaña real: 31 espectros de
# tercio de octava, uno por cada 30° de rumbo, reducidos a los 70 m polares.
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/rotorcraft_hover_ring_es.svg" alt="Dos paneles de la derivación de la fuente de estacionario. A la izquierda, el anillo de estacionario con efecto suelo en la banda de 315 Hz: nivel de banda a 70 m frente al rumbo del anillo de menos 180 a 180 grados, con el máximo delante del morro y en el cuarto trasero de estribor. A la derecha, las secciones proa-popa de las cuatro fuentes derivadas frente al ángulo polar: el hemisferio HIGE construido desde el anillo, y las curvas de estacionario sin efecto suelo, ralentí a régimen pleno y ralentí a régimen reducido a más 12, menos 2,5 y menos 12 decibelios de él, con una nota de que directividad constante en phi significa que cada ángulo polar lee el anillo en más o menos ese rumbo" width="92%" />
...
el de la cola (esa es la extensión de φ constante trabajando), y las otras
...
viento), rumbo, curvatura, ángulo de alabeo $\Phi = \arctan(K V_g^2 / g)$ y
...
tercios de octava se expresa en tiempo registrado $t_r = t_e + r/c$ (Ec. 22,
...
tiempo registrado $t_r = t_e + r/c$, y $r$ es mayor lejos del punto de máxima
...
$L_{AS,max}$ = 78,5 dB(A) llega a los 64,56 s, un segundo entero **antes** que
...
lorentziana $\mathrm{SEL} - L_{ASmax}$ del sobrevuelo. De extremo a extremo
...
8,2 m y tres clases de suelo; el residuo culmina en los micrófonos del nadir,
...
(`flight_condition_weights`, `interpolated_source_level`), la derivación de
fuente de estacionario, ralentí y rodaje de la Tabla 3
(`hover_ring_hemisphere`, `hover_derived_hemisphere`), la cinemática de
...
**No cubierto.** El recurso del §A.3.5 para un tipo sin ningún dato de
estacionario (dos micrófonos laterales de vuelo nivelado corregidos al
círculo de estacionario de 150 m por el método integrado del Anexo 16 de la
OACI) es una cadena de corrección de medida que este módulo, que consume
hemisferios ya reducidos, no modela.
**Con phonometry no se distribuye ninguna base de datos de hemisferios, y no hay
lector de ficheros NORAH**: el método está implementado, los datos no, así que
los arrays salen de una campaña de medida o de la base de datos de referencia
NORAH2 y los monta quien lee. La refracción por gradientes atmosféricos queda
fuera de la propia Doc 32, igual que la interpolación entre condiciones de vuelo,
que sigue en su lugar la guía NORAH2.
