...
El EPNL que calcula esta página es la misma magnitud que pide la
certificación: el Capítulo 8 del Anexo 16 de la OACI hace volar el
helicóptero nivelado a 150 m sobre un micrófono central, con otros dos a
150 m a cada lado.
...
triangulan el plano $(V, \gamma)$ sin normalizar, así que pasarlas reproduce
la implementación de referencia bin a bin). Los miembros de clase con rotor
especular sustituyen `h.mirrored()` (Ec. 2, $\varphi \to -\varphi$) y los
desplazamientos por nivel de certificación entran como `level_offset`.
...
punto de `airspeed`, `path_angle`, `heading` y `bank_angle` de un
...
pura de la diferencia de camino $\delta$ (Ec. 42-44,
...
lorentziana $\mathrm{SEL} - L_{ASmax}$ del sobrevuelo, y de extremo a extremo contra los
