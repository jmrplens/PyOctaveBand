...
Las dos degradaciones no se parecen en nada sobre esa curva, y por eso el índice
...
sala: un micrófono de proximidad ve un nivel de habla de unos 86 dB(A) a
...
una medida en bucle cerrado: eso caza un archivo corrupto o sobrecomprimido, y
...
STI queda sesgado a la baja (un bucle cerrado ideal da STI ≈ 0,956 a 5 s frente a
≈ 0,998 a 18 s).
...
De ahí siguen tres cosas. El conocido requisito «STI ≥ 0,5» del trabajo de
...
La regla de reducción es la parte que se equivoca con facilidad. El apartado
...
- **Efectos dependientes del nivel.** El STI no es invariante con el nivel: el
  ...
  encima del $L_{Aeq}$ del habla continua en la posición) y pasa `level=` y
...
De esos cuatro, la dependencia del nivel es el que se descarta con más facilidad,
...
ruido propio de la sala y muerde el umbral de recepción de la Tabla A.3; por
encima de unos 80 dB el enmascaramiento auditivo de la Tabla A.2 deja que las
bandas graves fuertes enmascaren a las agudas. Entre medias hay una meseta
ancha, y por eso medir al nivel de operación no es una manía: una respuesta al
impulso medida a nivel alto y reescalada después se queda en la línea a trazos
plana y solo vale para un nivel que nadie anotó.*
...
**Cubierto.** La IEC 60268-16:2020 (Edición 5) para la opción de habla
