...
references:
  ...
  - type: standard
    ...
    note: "Las curvas de ponderación frecuencial A, C y Z normativas (la definición analítica del Anexo E a partir de cuatro frecuencias de esquina, normalizadas a 0 dB a 1 kHz), las tolerancias de clase 1 que el diseño high_accuracy mantiene a toda frecuencia de muestreo desde 8 kHz, y los límites de aceptación de clase 1 y clase 2 de la Tabla 3 que comprueba verify_weighting_class en la sección 6."
  - type: standard
    organization: "International Electrotechnical Commission"
    year: 1979
    title: "Sound level meters"
    designation: "IEC 651:1979"
    url: "https://webstore.iec.ch/en/publication/2311"
    note: "La edición retirada cuya Tabla V publica la máscara de tolerancias de Tipo 0, de grado de laboratorio, que verify_weighting_class ofrece como edition='1979' en la sección 6, leída de la adopción británica idéntica BS 5969:1981."
---
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/weighting_responses.svg" alt="Curvas de ponderación A, C y Z de IEC 61672-1 con zoom de la región positiva de la curva A (+1,27 dB a 2,5 kHz)" width="80%" loading="eager" />
...
se ajusta a la frecuencia de muestreo en lugar de transformar el prototipo
impreso, y el 6 demuestra que el resultado cumple la clase 1.
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/diagram_equal_loudness_weighting.svg" alt="Líneas isofónicas según ISO 226 a la izquierda, con la línea isofónica de 40 fonios resaltada; a la derecha la curva de ponderación A superpuesta a la línea isofónica de 40 fonios invertida, mostrando que A es la línea isofónica volteada congelada en un filtro realizable" width="92%" />
...
| `curve` | str | — | `'A'` (por defecto), `'B'`, `'C'`, `'D'`, `'G'`, `'AU'`, `'Z'`, `'468'` | `'G'` según ISO 7196 (infrasonido), `'B'`/`'D'` históricas y `'AU'` según IEC 61012 se tratan en [Ponderaciones especiales](/phonometry/es/signals/levels/special-weightings/); `'Z'` se implementa como un paso directo de una respuesta que la norma especifica plana; `'468'` es la curva de nivel de programa de ITU-R BS.468-4, y su falda es tan pronunciada que exige el diseño ajustado y rechaza `high_accuracy=False` |
| `high_accuracy` | bool | — | por defecto `True` | Ajusta el prototipo analógico a $f_\mathrm{s}$ en vez de transformarlo a ciegas; mantiene A/C en clase 1 a cualquier frecuencia de muestreo. Detalles en el §5 |
...
Las curvas de la sección 1 son analógicas: polos y ceros en el plano $s$.
Convertirlas en filtro digital con la transformación bilineal es exacto en
magnitud y erróneo en frecuencia — sitúa la respuesta del prototipo en
$2 f_\mathrm{s} \tan(\pi f / f_\mathrm{s})$ en vez de en $2 \pi f$ — y el error
crece de forma cuadrática hacia Nyquist. A $f_\mathrm{s} = 48$ kHz ese diseño
simple queda 15,7 dB por debajo del objetivo de diseño de la A en la fila de
19 952,6 Hz, y a $f_\mathrm{s} = 32$ kHz queda 61,4 dB por debajo en 15 848,9 Hz.

Por defecto (`high_accuracy=True`) phonometry no transforma el prototipo a
ciegas: **ajusta** un prototipo analógico de la misma estructura cuya
respuesta *en las frecuencias deformadas* es la del prototipo impreso en las
verdaderas, y transforma ese. Lo que se ejecuta es una única cascada de
secciones de segundo orden a la frecuencia de entrada, sin nada alrededor.
Medido frente al prototipo impreso sobre la banda propia de cada norma:

| Curva | 32 kHz | 44,1 kHz | 48 kHz | 96 kHz |
| :--- | ---: | ---: | ---: | ---: |
...
Por eso A y C verifican **clase 1 a cualquier frecuencia de muestreo desde
8 kHz**, y en todas las filas de la Tabla 3 su desviación se mantiene dentro
de los 0,05 dB a los que está redondeada la propia tabla. Antes la cadena
llegaba a sus secciones a través de una etapa de interpolación y otra de
diezmado, y el filtro antialias de esas etapas tenía su banda de transición
sobre la frecuencia de Nyquist de entrada: a $f_\mathrm{s} = 32$ kHz eso dejaba
la fila de 15 848,9 Hz 16,2 dB por debajo del objetivo de diseño de la A, por
encima del límite de clase 1 de −16,0 dB, de modo que ahí la A solo verificaba
clase 2.

La banda que controla el ajuste es el rango normalizado propio de cada curva,
recortado por arriba al 99,5 % de la frecuencia de Nyquist: suficiente para
contener toda frecuencia en la que estas normas fijan un requisito, siendo la
aproximación más estrecha la fila de 15 848,9 Hz de la Tabla 3, al 0,9906 de
Nyquist cuando $f_\mathrm{s} = 32$ kHz. Por encima de ese último medio por
ciento la respuesta no se afirma: ningún filtro digital puede seguir una curva
analógica más allá de Nyquist, y la magnitud de un filtro de coeficientes
reales tiene pendiente nula allí. De ahí sale la fila 468 de la tabla anterior
—su falda sigue cayendo a unos $-30$ dB/octava donde termina la banda— y es el
3 % de la tolerancia de $\pm 2$ dB de esa curva, no una carencia de exactitud.

<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/weighting_accuracy_hf.svg" alt="Precisión en alta frecuencia de la ponderación A a 48 kHz: curva analítica frente a bilineal simple y diseño ajustado a la frecuencia de muestreo, con error" width="80%" />

*El diseño bilineal simple (rojo) cruza la tolerancia de clase 1 cerca de
12,5 kHz; el diseño ajustado (azul) se apoya sobre la curva analítica.*
...
- `high_accuracy=False` da el diseño bilineal simple: la forma cerrada que un
  lector puede comprobar término a término frente a la norma, al coste
  anterior. Verifica clase 1 para $f_\mathrm{s} \ge 44\,100$ Hz, degrada a
  clase 2 a 32 000 y 22 050 Hz y no cumple ninguna clase a 16 000 Hz.
- La curva `'468'` rechaza `high_accuracy=False`: su falda deja el diseño
  simple 23 dB fuera a 16 kHz, e ITU-R BS.468-4 imprime una única máscara de
  tolerancia y ningún grado inferior al que recurrir.
- El **procesado por bloques (stateful)** no cuesta nada. Ambos diseños son
  secciones de segundo orden a la frecuencia de entrada, así que `stateful` y
...
# El diseño bilineal en forma cerrada, explícito
y = filters.weighting_filter(recording, fs, curve="A", high_accuracy=False)

# Procesado por bloques con estado (mismo diseño ajustado, estado entre bloques)
...
El diseño ajustado elimina la contribución *del filtro* al error en alta
...
**El grado de laboratorio vive en la edición retirada.** IEC 61672-1:2013
publica solo las clases 1 y 2; el **Tipo 0**, el más estricto de los cuatro
tipos de instrumento de **IEC 651:1979**, sobrevive en la Tabla V de esa
norma. Se selecciona con `edition="1979"`, igual que `verify_filter_class`
alcanza la clase 0 de IEC 61260:1995. La clase *N* es entonces el Tipo *N* de
la norma, y cada banda y el barrido llevan un margen por tipo, de
`margin_class0_db` a `margin_class3_db`:
...
Es una máscara distinta, no un cambio de nombre, y ve errores que la clase 1
no puede ver: el Tipo 0 mantiene +2/-3 dB tanto a 16 kHz como a 20 kHz, donde
la clase 1 se abre a +2,5/-16 dB y a +3/-inf. El diseño `high_accuracy=False`
a 48 kHz cae 15,7 dB en la fila de 20 kHz y aun así obtiene clase 1 por ella;
bajo la Tabla V esa fila se rechaza y el filtro queda calificado como Tipo 1.
La edición de 1979 cubre A, B y C, las ponderaciones que IEC 651 define: la
nota al pie de su Tabla V hace que una sola máscara gobierne todas las
características de ponderación, así que B se somete allí a esos mismos
límites en lugar de tomar prestados los de ANSI.
...
*Los diseños ajustados A y C (azul, morado) se mantienen cerca de
...
No con un diseño bilineal simple: a $f_\mathrm{s} = 48$ kHz el error de la curva A alcanza −2,7 dB a 12,5 kHz, fuera de la tolerancia de clase 1 de IEC 61672-1 (+2,0/−2,5 dB). El `high_accuracy=True` por defecto ajusta en su lugar el prototipo a la frecuencia de muestreo, lo que deja 0,0003 dB en cualquier punto de la tabla a 48 kHz y 0,008 dB a 32 kHz, así que la clase 1 se mantiene a cualquier frecuencia de muestreo desde 8 kHz. El §5 tiene las cifras y la banda que controla el ajuste.
