...
dan por fácil: el $L_{eq}$, o el espectro de bandas, de una grabación que no
cabe en memoria. Y *es* fácil, y además es exacta y no aproximada, porque el
$L_{eq}$ no tiene constante de tiempo ni detector: solo una suma de cuadrados y
...
- **El primer bloque sigue llevando el transitorio de arranque.** El estado
  ...
  $B = 0{,}231\,f_m$ para tercios de octava eso es alrededor de un segundo a
...
$L_{AFmax}$ hay que acumularlo como el máximo *dentro* de cada bloque. Y los
...
completa, así que no se admite en modo con estado; usa la vía offline
