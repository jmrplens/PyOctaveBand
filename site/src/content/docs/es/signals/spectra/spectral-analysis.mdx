...
Promediar $n_\mathrm{d}$ segmentos independientes da al estimador $2 n_\mathrm{d}$ grados de
...
ancho de banda de análisis finito $B_\mathrm{e}$ (expuesto como
...
media potencia $B_\mathrm{r}$, el sesgo normalizado de primer orden es la forma
...
una resonancia por debajo de 1 dB significa $\varepsilon_\mathrm{b} \ge -0{,}2$, y por
tanto $B_\mathrm{e} \le 0{,}775\,B_\mathrm{r}$; mantenerlo por debajo de una décima de decibelio
significa $B_\mathrm{e} \le 0{,}263\,B_\mathrm{r}$. La regla empírica de siempre, tres o cuatro
...
modo a 100 Hz con $Q = 50$ tiene $B_\mathrm{r} = f/Q = 2$ Hz. Mantener su sesgo por debajo
del 2 % (0,09 dB) pide $B_\mathrm{e} \le B_\mathrm{r}\sqrt{3 \times 0{,}02} = 0{,}49$ Hz y, para la
ventana de Hann, $B_\mathrm{e} = 1{,}5\,f_\mathrm{s}/\texttt{nperseg}$, así que a 48 kHz eso es
`nperseg` $\ge 147\,000$, es decir, segmentos de al menos **3,1 s**. Pedir
después un error aleatorio del 10 % necesita $n_\mathrm{d} = 100$ promedios efectivos, lo
...
como $B_\mathrm{e}^2$ y el error aleatorio sube como $n_\mathrm{d}^{-1/2}$, y se igualan cerca de
...
retardo de grupo $\tau_\mathrm{g} = -\mathrm{d}\varphi/(2\pi\,\mathrm{d}f)$; para un
...
de coherencia, $b[\hat{\gamma}^2] \approx (1-\gamma^2)^2/n_\mathrm{d}$ (ec. 9.75):
despreciable en cuanto $n_\mathrm{d}$
...
| Hundimientos abruptos justo en las resonancias | sesgo de resolución: el ancho de banda de análisis emborrona el pico de forma distinta en cada canal | alarga el segmento; la fórmula de $\varepsilon_\mathrm{b}$ del §1 dice cuánto |
...
en $(1-\gamma^2)^2/n_\mathrm{d}$, así que una coherencia baja leída con un puñado de
...
central $f_\mathrm{c}$ tiene un ancho de banda $B_n = f_\mathrm{c}\,(2^{1/2n} - 2^{-1/2n})$, es
decir, $0{,}2316\,f_\mathrm{c}$ en un tercio de octava y $0{,}7071\,f_\mathrm{c}$ en una octava
completa, así que

$$
L_\text{banda} = 10\log_{10}\!\left(\frac{G(f_\mathrm{c})\,B_n}{p_0^2}\right),
$$
...
densidad más $10\log_{10} B_n$ con $B_n \propto f_\mathrm{c}$: **todos los colores se leen
...
por semiancho de banda $p = NW$ (4 por defecto), y para un registro de $N_\mathrm{r}$
muestras a $f_\mathrm{s}$ fija el semiancho $W = p\,f_\mathrm{s}/N_\mathrm{r}$ en hercios. $2W$ es la
...
$n_\mathrm{d} = 6{,}7$ promedios efectivos dentro de él, así que su banda de confianza es
...
Los estimadores de Welch de Bendat y Piersol (*Random Data*, 4.ª ed., 2010,
secciones 5.2, 8.5, 9.1-9.2 y 11.5): `power_spectral_density` y
`cross_spectral_density` con el número efectivo de promedios, los intervalos
de confianza chi-cuadrado y el error de sesgo por resolución; el espectro de
salida coherente y la relación señal-ruido espectral; las figuras de mérito de
las ventanas de Harris (1978) de `window_metrics`; y el estimador multitaper
de Thomson (1982) de `multitaper_psd`, siguiendo los capítulos 7 y 8 de
Percival y Walden. `fractional_octave_smoothing` y `noise_signal` completan la
caja de herramientas.
</ScopeClaim>

<ScopeClaim status="not-covered" label="No cubierto">
Los estimadores de respuesta en frecuencia `transfer_function` y `coherence`,
y la sonda de intensidad acústica, comparten el núcleo de Welch de esta página
pero se documentan en las páginas de
[electroacústica](/phonometry/es/devices/electroacoustics/electroacoustics/) y
[intensidad acústica](/phonometry/es/devices/emission/intensity/), no aquí. Lo
mismo ocurre con la coherencia múltiple y parcial, en la página de [coherencia
MISO](/phonometry/es/signals/spectra/miso-coherence/). Esta página implementa
los estimadores del libro de Bendat y Piersol, no una norma de certificación,
así que no lleva números de apartado ni límites de cumplimiento que comprobar.
</ScopeClaim>
</Scope>
