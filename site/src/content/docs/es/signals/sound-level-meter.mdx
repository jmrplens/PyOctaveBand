...
por encima de su pico: el $L_{Cpeak}$ del paso 4 lee el verdadero pico entre
muestras, y una sola racha saturada lo invalida, y con él el $L_{Aeq}$.
...
- Ganancia fijada a mano y anotada; AGC, limitador y supresión de ruido
  desactivados.
- PCM sin comprimir, 24 bits, $f_s \ge 48$ kHz, un canal por micrófono.
...
segundo del evento sube el $L_{Aeq}$ y el $L_{10}$, y deja el $L_{50}$ y el
...
añade el nivel máximo con ponderación temporal $L_{AFmax}$ que da esta misma
traza `laf_t`, la dosis de ruido y los espectrogramas de octava; el $L_{den}$ y
...
energética reproduce el $L_{eq}$ sin ponderar y no el $L_{Aeq}$ del paso 4. Esa
...
apenas mueve el $L_{Aeq}$ (recortar duramente un tono de 1 kHz al 80 % de su
...
IEC 61672-2:2013 que necesita un instrumento físico para su aprobación de tipo,
como el ruido autogenerado, el rango de linealidad, la indicación de
sobrecarga o la respuesta direccional, ni los ensayos periódicos de
IEC 61672-3:2013 que recibe un instrumento en servicio. Un veredicto de clase de
esta página describe el algoritmo, no un aparato construido, y las tres
comprobaciones del paso 7 criban la *grabación*, no el instrumento. Los ensayos de conformidad
