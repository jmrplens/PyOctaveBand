...
calibrador y, si se le pasa `fs` como aquí, valida además la estabilidad a corto
plazo de la grabación igual que IEC 60942 cualifica el propio calibrador, de
modo que un micrófono mal acoplado se detecta aquí en lugar de corromper todos
los niveles posteriores. Sin `fs` se obtiene solo el factor, sin comprobación.
