...
con `NR = L_p1 - L_p2` la reducción de ruido entre los dos campos
reverberantes, `TL = 10 lg(1/tau)` la pérdida por transmisión de la partición
(la magnitud que UNE llama índice de reducción acústica `R`), `S_w` el área
de la partición y `S_2 alpha_2` el área de absorción sonora equivalente del
recinto receptor.
...
El término `tau S_w` es la potencia que la propia partición devuelve al
...
relevante es `L_p1 = L_W + 10 lg(4 / R_1)`. Eso es `steady_state_spl` con
`distance=None`, el límite `r -> infinito` de la misma Ecuación (6.43) de
...
volumen constante es la cota superior. Aquí vale `10 lg 4 = 6,02 dB`, y sin
él las respuestas impresas salen 6 dB por debajo.
...
`.required_transmission_loss` es la Ecuación (4.101) resuelta para `TL`, que
es la cifra que va a una especificación.
...
los paneles, `TL = IL + 10 lg(S_E / R_i)`, que es
...
`0,3` dentro del logaritmo y por tanto limita la pérdida por inserción de un
encapsulado totalmente revestido a `TL + 5,2 dB`. La Ecuación (4.115) de
