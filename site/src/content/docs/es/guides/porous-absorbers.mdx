...
y para el caso habitual de un armazón fabricado con un material mucho más rígido
que el propio armazón ($K_s \to \infty$, cierto en esqueletos de vidrio, roca y
polímero) los tres coeficientes elásticos salen del módulo de cizalla $N$, el
...
poroelástica aporta la suya de 6x6 y ambas se unen mediante las matrices de
acoplamiento impresas del apdo. 11.4. Dos capas poroelásticas contiguas se
...
1500 Hz. El pico de impedancia de la muestra más delgada del libro está impreso
como 860 Hz y sale en 863,5 Hz. Todo lo demás, incluido el comportamiento en
...
transferencia (`layered_absorber` con `PorousLayer`, `PoroelasticLayer`,
`PerforatedPlateLayer`, `MicroperforatedPlateLayer` y `MembraneLayer`) y los
