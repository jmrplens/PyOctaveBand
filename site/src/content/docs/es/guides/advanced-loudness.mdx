...
| Moore-Glasberg-Schlittenlacher | ISO 532-3:2023 | variable en el tiempo | sonos (STL/LTL) | Sonoridad variable en el tiempo con trazas de corto/largo plazo y el pico $N_\text{max}$ |
...
Integrar $df/\mathrm{ERB}_N(f)$ convierte eso en una escala de frecuencia cuya
unidad es una anchura de filtro auditivo, el **número ERB<sub>N</sub>**, cuya
unidad se llama **Cam** (por Cambridge):
...
de ISO 532-2; ambas formas concuerdan mejor del 0,2 % en todo el margen
audible, y esas cifras de más son las que hacen de `frequency_from_cam` una
inversa exacta. Las funciones no son un auxiliar privado del modelo de
sonoridad: `loudness_moore_glasberg` importa exactamente estas funciones, de
modo que la rejilla Cam de un `MooreGlasbergLoudness.erb_number` y un eje Cam
construido a mano no pueden separarse.
