...
references:
  - type: book
    ...
    primary: true
    note: "El modelo de potencia acústica del ventilador (Cap. 13, Ec. 13.1 con las Tablas 13.5-13.8), la atenuación de conductos, el conducto flexible, la pérdida por derivación, la reflexión del extremo, el ruido regenerado del silenciador y el efecto de sala del Cap. 14, y la hoja resuelta de ruido por conductos de la Tabla 14.9 sobre la que se construye esta guía."
  - type: book
...
[Silenciadores](/phonometry/es/guides/silencers/) y el resto de métodos de
instalación de
[Control de ruido industrial](/phonometry/es/guides/noise-control/) son las
...
publicada los enuncia de forma distinta.
...
`DuctPathResult.table()` le devuelve el signo a las filas de tipo
...
defecto `0.0`) y `None` lo desactiva por completo.
...
`unlined_circular_duct_attenuation` es una simple tasa por longitud de 0,03 a
0,07 dB/ft. Revestirlo cambia el orden de magnitud:
...
[constante de sala](/phonometry/es/guides/room-image-sources/) del plenum.
...
el caudal de diseño y en el sentido de diseño, es lo que corresponde poner en
la hoja; el modelo sirve para dimensionar el paso de aire antes de que haya
fabricante.
...
La unidad terminal es el elemento que una hoja real nunca modela. Ni ASHRAE
ni Long publican una forma cerrada para la potencia acústica de una rejilla o
un difusor: sale de los datos de fabricante medidos según ASHRAE Standard 70.
Lo que la biblioteca ofrece en su lugar son las dos reglas de cribado del
capítulo 49 de ASHRAE. `air_terminal_velocity_limit` (Tabla 9) da la máxima
velocidad en el cuello para un RC de diseño, para usarla cuando no existen
datos acústicos del equipo elegido, y `air_terminal_damper_correction`
(Tabla 10) da la penalización por estrangular una compuerta de regulación,
que es donde fracasan muchísimas instalaciones terminadas.
...
<ReportPreview
	...
	caption="Ficha del trayecto de ruido por conductos (DuctPathResult.report): el trayecto de impulsión de la hoja resuelta elemento a elemento, valorado NC-22.6 y cumpliendo NC 30 con 5 dB de margen a 63 Hz."
/>
...
polos de [Silenciadores](/phonometry/es/guides/silencers/), son
...
Dos resultados llevan este límite por usted. Todo `ReactiveSilencerResult`
informa ahora del primer corte de su sección más ancha en
...
- [Silenciadores](/phonometry/es/guides/silencers/): los elementos reactivos
  ...
  corte.
- [Control de ruido industrial](/phonometry/es/guides/noise-control/): el
...
- [Criterios de ruido de fondo (NC / RC Mark II)](/phonometry/es/guides/room-noise/):
  las familias de la ANSI/ASA S12.2-2019 contra las que se juzga el espectro
  recibido.
- [Campo estacionario de la sala](/phonometry/es/guides/room-image-sources/):
