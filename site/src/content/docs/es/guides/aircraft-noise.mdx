...
Sobre el sobrevuelo, $\mathrm{PNLT} = \mathrm{PNL} + C$, su máximo es
$\mathrm{PNLTM}$, y la métrica integra $\mathrm{PNLT}$ sobre la ventana de
10 dB por debajo (los registros más cercanos a $\mathrm{PNLTM} - 10$ a cada
lado) normalizada a 10 s, de modo que $\mathrm{EPNL} = \mathrm{PNLTM} + D$ con
