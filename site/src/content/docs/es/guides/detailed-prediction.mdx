...
**5. Ensamblaje.** $R' = -10\lg\sum 10^{-R/10}$ sobre los trece caminos de un
recinto con cuatro elementos de flanco, y $L'_n = 10\lg\sum 10^{L_n/10}$ sobre
...
res = detailed_airborne_prediction(
    ...
    flanking_paths=[d1, ...])          # los doce caminos de los cuatro elementos
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/detailed_prediction_paths.svg" alt="Cuotas apiladas por banda de la energía transmitida por los trece caminos de transmisión del edificio del Anexo L de ISO 12354-1, con el índice de reducción acústica aparente R' superpuesto: el camino directo Dd transporta la mitad de la energía a 50 Hz y desaparece por encima de 200 Hz, tras lo cual toman el relevo los caminos por flancos a través de las fachadas y las particiones interiores" width="92%" loading="eager" />
...
un suelo flotante mejor ayudarían; a partir de 100 Hz ha desaparecido del
reparto y son los caminos por flancos a través de las paredes los que fijan
$R'$, así que ningún trabajo sobre el forjado movería el resultado. El número
único $R'_w = 57$ dB no dice nada de eso.*
...
elementos de entramado simple sin cámara).
...
$R + L_n = 38 + 30\lg f$ en bandas de tercio de octava y $43 + 30\lg f$ en
...
| :--- | :--- | :--- | :--- | :--- |
| `element` | `HomogeneousElement` | — | — | Área, longitudes de lado, `m'`, `fc`, `ηint`, `Σ lk αk` y opcionalmente `ρ`/`cL` |
| `frequencies` | array | Hz | > 0 | Frecuencias centrales de banda |
| `bands` | str | — | `'third'` (por defecto) / `'octave'` | Fija la banda que lleva la rama `f ≈ fc` |
| `resonant_only` | bool | — | por defecto `False` | Elimina el término de transmisión forzada por debajo de `fc` (Anexo B.1, caminos por flancos) |
| `speed_of_sound` | float | m/s | por defecto `340` | El valor que fija el Anexo A de ISO 12354-1 |
| `air_density` | float | kg/m³ | por defecto `1.29` | `ρo` del modelo del Anexo B |
...
| `vibration_reduction_index` | float o array | dB | — | `Kij` de este camino |
| `coupling_length` | float | m | > 0 | Longitud de acoplamiento de la unión `lij` |
| `separating_area` | float | m² | > 0 | `Ss` (solo a ruido aéreo) |
...
banda, el `R'w` / `L'n,w` recuadrado, la desviación típica de 1,5 dB a 2,5 dB
...
- [Transmisión por flancos en laboratorio (ISO 10848)](/phonometry/es/guides/flanking-lab/):
  de dónde salen el `Kij` medido y la longitud de absorción equivalente.
...
- Referencia de la API: [`building.detailed_prediction`](/phonometry/es/reference/api/building/detailed-prediction/).
