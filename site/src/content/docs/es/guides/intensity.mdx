...
tiempo de promediación corto (la nota 9 sugiere `M = 10` y de 8 s a 12 s por
muestra). **F1**, el indicador de variabilidad temporal, es el coeficiente de
...
`F1 > 0,6`: reducir la variabilidad, medir en periodos más tranquilos o
...
de ISO 9614-1 califican el propio barrido. **F2**, el indicador
...
`δpI0` no es solo un número al que restar `K`; es aquello por lo que
IEC 61043 clasifica el hardware. La tabla 2 de la norma fija un `δpI0`
...
`+10 lg(x/25)`, con `x` en milímetros: los mismos 3 dB por duplicación a los
que llegaba el apartado anterior a partir de la física.

`intensity_class_compliance` compara un espectro de `δpI0` medido con ambas
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/intensity_class_es.svg" alt="Índice presión-intensidad residual medido de un instrumento de intensidad completo con separador de 12 mm, dibujado como curva escalonada sobre los mínimos de clase 1 y clase 2 de la tabla 2 de IEC 61043; la región de aceptación de clase 2 sombreada queda por encima de la máscara discontinua de clase 2, y la banda de 100 Hz aparece marcada donde el índice medido cae por debajo de la máscara continua de clase 1" width="92%" />

*Ambas máscaras de la tabla 2 bajan `10 lg(12/25) = −3,2 dB` por el separador
...
de octava para la clase 1, o de 45 Hz a 5,6 kHz en octavas para la clase 2), y
un veredicto calculado sobre menos bandas se marca con `range_limited` para
que no pueda leerse como una declaración de rango completo. El apartado 8
...
producida por un desfase entre canales `φs` da
...
tercio de octava hasta 250 Hz y después se aplana: por debajo del codo la
norma pide un emparejamiento de fase *constante*, y por encima un índice
constante. También por eso una décima de grado de desfase entre canales es
...
`δpI0` como el suelo de error de fase del instrumento, y la clasificación del
...
conversión entre `δpI0` y desfase en ambos sentidos. ISO 9614-1:1993 para los
indicadores de campo del Anexo A F1 a F4 (`temporal_variability_indicator`,
`field_indicators`) con el límite de la tabla B.3 sobre F1, y el criterio de
...
apartados 8 y 9, que suma las potencias parciales `Ii·Si` de los puntos de
medida discretos hasta `LW`, no está implementada como función aquí:
...
calculan `LW` a partir de la intensidad (ISO 9614-2 e ISO 9614-3) están en
la guía de
...
biblioteca: `δpI0` es un valor que quien mide obtiene en su propia cadena
