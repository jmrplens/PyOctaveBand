...
mejor con un frente plano. Dos herramientas lo cubren, ambas
unidireccionales por construcción (la velocidad consistente con el
leapfrog se escribe medio paso atrás, así que nada radia hacia atrás):
...
- `sim.add_plane_wave(direction, center=..., width=..., wavelength=...)`
  ...
  `"right"`; tras el frente la energía residual queda al nivel del ruido
  numérico. Es lo que usa la animación de difusión del QRD.
- `PlaneWaveSource(direction, waveform, offset=...)` registrada con
  ...
  cruza la línea y muere en la esponja de detrás (residuo de unos -38 dB
  para una CW).
