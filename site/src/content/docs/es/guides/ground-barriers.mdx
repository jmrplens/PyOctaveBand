...
Todos los métodos de esta sección dependen de los mismos tres números: los dos
segmentos difractados sobre el borde y el camino recto al que sustituyen.
Dibujados sobre la pantalla de 4 m de los ejemplos, difieren en solo 0,15 m.
...
Las tres curvas comparten una única geometría, y `.plot_geometry()` la
dibuja a escala: a estas distancias de tráfico rodado la pantalla de 4 m es
una lámina, y toda la pérdida por inserción descansa sobre una diferencia de
caminos de solo 0,15 m.
...
es toda la entrada del número de Fresnel `N = (2/λ)(A + B − d)` del que
dependen todos los métodos de arriba.*
