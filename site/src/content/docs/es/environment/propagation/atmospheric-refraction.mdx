...
references:
  ...
  - type: book
    ...
    note: "Capítulo 11 (refracción por gradientes de viento y temperatura, modelos de rayos y zonas de sombra). ISBN 978-1-4987-4007-4."
---
...
un **modelo de rayos** y una **ecuación parabólica (PE)** marchada en distancia.
...
existe en un método de ingeniería por bandas de octava: esto es lo que $C_{met}$
...
$b = u_{SR}(z_1)\,/\,\ln(1 + z_1/z_0)$. Una lectura de 4 m/s en un mástil de
...
$u_{SR}(z) = u(z)\cos(\pi - \alpha)$ con $\alpha$ el ángulo entre la dirección
...
$R_c(1 - \cos\theta_0)$. Para un perfil lineal con **refracción hacia arriba**
el rayo
...
donde este segundo $R_c$ es el caso particular rasante ($\theta_0 = 0$) de la
...
círculo a un rayo trazado recupera $R_c$ con precisión de máquina. Son además la
...
un solo número, $C_{met}$ incluido, es un promedio sobre esa dispersión, no una
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/refraction_homogeneous_check_es.svg" alt="Dos paneles apilados frente a la distancia hasta 520 m para una fuente de 500 Hz y un receptor ambos a 2 m sobre suelo rígido con el gradiente de velocidad del sonido desactivado. Arriba: el nivel relativo GFPE dibujado sobre la forma cerrada coherente de dos rayos, con las dos curvas siguiendo el mismo patrón de interferencia a través de un cero profundo cerca de 45 m y asentándose juntas en la suma coherente de más 6 dB hacia los 500 m. Abajo: el residuo entre el método y la forma cerrada, dentro de 0,6 dB en todas partes más allá del cero y llegando a 17 dB dentro de él, donde la propia forma cerrada pasa por menos 31 dB" width="88%" />
...
independientes de la distancia» de los que es el reflejo. Ambos suponen además un suelo
plano en $z = 0$; ninguno admite un perfil de elevación del terreno.

## Véase también

- Teoría: [Propagación en exteriores y exposición al ruido en el trabajo](/phonometry/es/reference/theory/environment-transport/#propagación-en-exteriores-y-exposición-al-ruido-en-el-trabajo-iso-9613-12-iso-9612): las hipótesis meteorológicas que hace ISO 9613-2, que es justo lo que la refracción desmonta.
