...
$H_{max}$ del grupo ($d > 2H_{max}$). Si $d \le 2H_{max}$, o si las condiciones
...
mismo emplazamiento no lo es, y ahí la nave hay que modelarla cara a cara. La
...
octava de 63 Hz a 8 kHz. Cuando solo se dispone de un $L_{WA}$ ponderado A, el
...
$G_s$, $G_m$ y $G_r$ no son una blandura difusa: cada uno es la *fracción porosa*
de una región cuya extensión fija la norma (7.3.1, figura 1). Medida a lo largo
de la distancia $d_p$ proyectada sobre el suelo, la **región de la fuente** ocupa
$30 h_s$ desde la fuente hacia el receptor (recortada a $d_p$), la **región del
receptor** ocupa $30 h_r$ hacia atrás desde el receptor, y la **región media** es
...
En el ejemplo de 200 m de esta sección ($h_s = h_r = 1,5$ m, así que
$d_p \approx d$) eso son 45 m de región de fuente, 45 m de región de receptor y
...
20 m toma $G_s = 25/45 \approx 0,55$, y una región media de 70 m de hierba y
40 m de asfalto toma $G_m = 70/110 \approx 0,64$.

La región media también puede desaparecer. La nota 2 de la Tabla 3 define
$q = 1 - 30(h_s + h_r)/d_p$ cuando $d_p > 30(h_s + h_r)$ y $q = 0$ por debajo, y
$A_m$ es proporcional a $q$: con estas dos alturas el umbral queda en
$d_p = 30(1,5 + 1,5) = 90$ m, así que en un recorrido de 60 m `ground_middle`
**se ignora por completo**, y pasar 0.0 o 1.0 devuelve el mismo $A_{gr}$, en
silencio. Ese mismo $q$ da la comprobación de coherencia de todo el término. Sobre
suelo duro en todas partes, $A_s = A_r = -1,5$ dB y $A_m = -3q$ en todas las
bandas, así que el recorrido de 200 m tiene que devolver $A_{gr} = -4,65$ dB
...
# Dz por sí solo: la pérdida por inserción por difracción de la Ec. (14), antes de la Ec. (12)
...
`barrier_attenuation` devuelve el propio $D_z$, la pérdida por inserción por
difracción en bruto de la Ec. (14), que *no* es lo que declara el compuesto. Para
...
$h_m$ **no** es el promedio de las dos alturas extremas. La Figura 3 la define
como $h_m = F/d$, con $F$ el área encerrada entre la recta fuente-receptor y el
perfil del terreno: sobre suelo plano se reduce a $(h_s + h_r)/2$, sobre un
...
recorte: $A_{gr}$ está acotado por abajo en 0, así que un camino que va alto
sobre el suelo no gana atenuación por suelo en absoluto (a $d = 200$ m,
$h_m = 8$ m todavía da 3,32 dB, pero $h_m = 8$ m a 50 m da 0,00 dB), mientras
...
20 dB, y $K_{met}$ ya le ha quitado de 1 a 2 dB a 200 m. Derecha: las dos curvas
son la misma barrera; la separación entre ellas es $A_{gr}$, y donde el suelo ya
valía más que la difracción (la octava de 250 Hz) el $A_{bar}$ neto es cero.*
...
* **CNOSSOS-EU** (métodos comunes de evaluación del ruido en Europa) es el
  ...
  indicadores $L_{den}$/$L_{night}$ que la Directiva exige. Esta biblioteca
...
- [Refracción atmosférica](/phonometry/es/environment/propagation/atmospheric-refraction/): lo que la hipótesis de condiciones favorables y el $C_{met}$ vienen a sustituir.
...
- Teoría: [Propagación en exteriores y exposición al ruido en el trabajo](/phonometry/es/reference/theory/environment-transport/#propagación-en-exteriores-y-exposición-al-ruido-en-el-trabajo-iso-9613-12-iso-9612): los términos de atenuación de ISO 9613-2 derivados uno a uno, y la absorción atmosférica de la Parte 1.
