...
construye con *diferencias* de nivel), el $L_{Aeq}$ declarado y el ajustado no lo
...
crecimiento y su diferencia de nivel; la prominencia $P$ y el ajuste $K_I$; la
categoría de fuente en la que el resultado sitúa a la fuente; y el $L_{Aeq}$
...
la $P$ determinante, $K_I$ y la categoría) y toma el resto de `ReportMetadata`,
...
prominencia prevista $P$ (apartado 7, Fórmula 1), el ajuste graduado $K_I$
...
misma fórmula de prominencia: el muestreo de $L_{pAF}$, la detección y fusión
...
cuando $K_I > 3$ para ruido característico de operaciones de trabajo, ofrecido
como alternativa cuando el método graduado no se puede aplicar. Solo está
implementado el $K_I$ graduado de la Fórmula 2; la alternativa plana de 5 dB
