...
simplificada, que no necesita ningún $T_s$.
...
$T_{s,j}$ es la segunda entrada más influyente de esta página y la que más a
...
la longitud de absorción, $\eta_\text{total} = 2{,}2/(f T_s)$
(ISO 10848-1:2017, Fórmula (19)). Como $a_j \propto 1/T_{s,j}$ y $K_{ij}$ lleva
$-10\log_{10}\sqrt{a_i a_j}$, un $T_s$ equivocado en un factor de dos desplaza
$K_{ij}$ 1,5 dB por cada elemento, y lo desplaza en la dirección que hace
parecer la unión mejor de lo que es cuando $T_s$ se sobrestima.
...
directo deja de ser fiable: $T_s > 70/f$ y $T_s > 2T_\text{det}$, con
...
prescinde por completo de $T_s$. La identidad del factor de pérdidas es la
comprobación de sensatez de un valor medido: a 500 Hz un $T_s$ de 0,35 s da
...
y la regla cambia según la magnitud: al medir $D_{n,f}$ para la transmisión a
través de $i$ y $j$, apantalla el otro elemento **en los dos recintos**; al
medir $L_{n,f}$, apantalla el otro elemento **en el recinto receptor**; al medir
...
hay que evitarlo. **Un $K_{ij}$ o un $D_{n,f}$ declarado sin decir cómo se
...
$m_\text{acc} < 1/(2\pi f Y_\text{dp})$, con la movilidad puntual de una placa
delgada isótropa estimada por la Fórmula (17) como
$Y_\text{dp} = 1/(8\sqrt{B\rho_s}) = 1/(2{,}3\,\rho_s c_L h)$. La desigualdad
...
curva ASTM E413 ajustada al $D_{n,c}$ medido y sus deficiencias sombreadas; la
...
Un techo no se valora por $R_{cl}$ sino por la
...
**Cubierto.** El índice de reducción vibratoria $K_{ij}$ de
ISO 10848-1:2006 (Fórmula (13)) desde la diferencia de niveles de velocidad
promediada direccionalmente (Fórmula (11)) y las longitudes de absorción
...
unión y de elemento de ensayo a los que se aplican; phonometry implementa solo las
...
específicos de instalación que describen las otras partes. La adquisición de
[La medición](#la-medicion-iso-10848-1-2017)
está documentada aquí y no se comprueba en ninguna parte: nada verifica que
los demás elementos se apantallaran, que el apantallamiento alcanzara
$\Delta R_\text{min}$, que se respetaran los recuentos y las separaciones de
posiciones, que el acelerómetro cumpliera la desigualdad de carga másica, ni
que $T_s$ se midiera sobre los elementos ya montados en lugar de suponerse.
Como ISO 10848
