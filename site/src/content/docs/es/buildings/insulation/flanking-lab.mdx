...
genéricas de $K_{ij}$/$D_{n,f}$/$L_{n,f}$ de la parte 1, más la comprobación
...
cumpliera la desigualdad de carga másica, ni que $T_s$ se midiera sobre los
...
cerrada ($K_{ij}$ simplificado, $a_j$ en $f_\text{ref}$, $\eta$).
</ScopeClaim>

<ScopeClaim status="covered" label="Cubierto">
La rama de techos suspendidos cubre la atenuación normalizada del techo
$D_{n,c}$ del apartado 3.3 de ISO 140-9:1985, la clase de atenuación de techo
...
el amortiguamiento del plenum, la cota $\tau_{cl} \le 1$ y la convergencia de
