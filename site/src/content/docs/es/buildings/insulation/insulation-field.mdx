...
**Apartado 10.4: también el tiempo de reverberación.** Con el mismo
disparador, los tiempos de reverberación de tercio de octava de 50 Hz, 63 Hz y
80 Hz dejan de medirse y un único valor de la banda de **octava de 63 Hz** los
sustituye en las tres. Es una prescripción sobre qué medir, no una afirmación
de que los valores sean iguales: en un recinto pequeño una caída de tercio de
octava rara vez tiene modos suficientes para ser de pendiente única, y en
construcción con entramado de madera o acero puede ser más corta que la propia
caída del filtro de tercio de octava del analizador. Por debajo del disparador
tampoco hay a qué recurrir, porque el apartado 10.3 limita el procedimiento por
defecto del tiempo de reverberación a 100 Hz y por encima en cuanto el recinto
es así de pequeño. El apartado es del recinto **receptor** en las tres partes,
incluso en la parte 1, donde el procedimiento de esquinas admite además el
recinto emisor.
...
bandas: reescribe las tres bandas con $L_\mathrm{LF}$ y pone el valor de la
octava de 63 Hz sobre esos mismos tres tiempos de reverberación, dejando
intactas las bandas de 100 Hz en adelante.

```python
# 50, 63 y 80 Hz más las tres primeras bandas del rango básico, para que se vea
# la sustitución.
...
receiving = building.LowFrequencyProcedure(
    ...
    reverberation_63_octave=0.72,         # apartado 10.4
)
...
`impact_insulation()` y `facade_insulation()` reciben el mismo objeto bajo el
argumento `low_frequency=` y llegan al mismo código: la Fórmula (16) de la
parte 2 y la Fórmula (5) de la parte 3 son la Fórmula (13) con otros
subíndices. Dos diferencias entre las partes son reales y se exigen. La parte 1
evalúa los dos recintos por separado, así que `airborne_insulation()` acepta
además un `source_low_frequency=`, y un recinto emisor pequeño junto a un
receptor grande mueve $L_1$ y deja $L_2$ y $T$ tal como se midieron. La parte 3
limita el procedimiento a los métodos de **altavoz** por elementos y global (la
NOTA 1 del apartado 6 deja constancia de que no hay experiencia de aplicarlo
con tráfico como fuente), de modo que pasarlo con `method="road_traffic"` da
...
pequeños](#recintos-pequenos-apartado-8) *sí* está implementado (el disparador
