...
mientras que los $-3$ dB de $R'_{tr,s}$ suponen que el sonido llega «desde
...
un resultado de altavoz se etiqueta $D_{ls,2m,nT}$ y uno de tráfico
$D_{tr,2m,nT}$ en lugar de llamarse los dos igual.
...
llevarse: el nivel de referencia de un $D_{2m,nT}$ se mide delante de una
superficie reflectante y es por tanto más alto que el campo libre que la misma
fuente produciría con el edificio quitado, así que un $D_{2m,nT}$ y un $D_{nT}$
interior no son la misma clase de número aunque tengan el mismo tamaño.

<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/diagram_facade_setup_es.svg" alt="Sección por una fachada de vivienda: un altavoz sobre el terreno con la línea de incidencia de 45 grados más menos 5 grados hasta el centro de un elemento de ensayo de 11,5 metros cuadrados, la distancia oblicua r acotada en 5 m para el método del elemento y 7 m para el método global y la perpendicular D debajo, un micrófono sujeto enrasado sobre el elemento de ensayo con sus tolerancias de 10 mm en paralelo y 3 mm en normal, y el micrófono global a 2,0 más menos 0,2 m del plano de la fachada a 1,5 m sobre el suelo del recinto receptor" width="92%" loading="eager" />
...
$D_{2m,nT}$ (o $D_{2m,n}$) y «cuantifican el aislamiento a ruido aéreo de una
...
Los métodos de **elemento** dan $R'_{45°}$ o $R'_{tr,s}$ y son la vía cuando lo
...
elementos o fachadas con $R'_w < 40$ dB (NOTA del apartado 10.1).
...
| :--- | :--- | :--- | :--- | :--- |
| `l1_2m` | array 1D o 2D | dB | uno/banda, o `(posiciones, bandas)` | Nivel 2 m frente a la fachada $L_{1,2m}$ |
...
| `volume` | float, opcional | m³ | > 0 | Recinto receptor $V$ (habilita $D_{2m,n}$; requerido para $R'$) |
| `surface_level` | array 1D/2D, opcional | dB | mismo número de bandas | Nivel superficial $L_{1,s}$ sobre el elemento (habilita $R'$) |
...
a la referencia desplazada, el índice de campo $D_{2m,nT,w}\ (C;\ C_{tr})$ en
...
$R_{p,w}$ de cada elemento) junto a la gráfica por elemento / $R'$ /
...
modelo en `notes` y el objetivo $D_{2m,nT,w}$ en `requirement`. Los metadatos,
...
suponen ya corregidos por ruido de fondo, y los requisitos de procedimiento de
la propia ISO 16283-3, los que expone
[Elegir y realizar el método](#elegir-y-realizar-el-metodo),
están documentados pero no se comprueban: nada verifica la incidencia de
45° ± 5°, las distancias oblicuas de 5 m y 7 m, la escalada de 3 a 10
posiciones de micrófono y su caso especial del retranqueo, los 50 pasos de
vehículo ni la simultaneidad de una medición con tráfico, ni que el método
elegido corresponda a la pregunta planteada. El procedimiento de esquina en
baja frecuencia del apartado 7 (la contraparte de fachada del
[procedimiento de ISO 16283-1](/phonometry/es/buildings/insulation/insulation-field/#recintos-pequenos-apartado-8),
...
Anexo E. Los ejemplos resueltos de
