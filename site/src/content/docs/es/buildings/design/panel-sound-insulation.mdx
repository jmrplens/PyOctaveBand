...
exponen `transmission_loss` y lo valoran como $R_w$. Del mismo modo, aquí se
...
$TL = 10\log_{10}[1 + (\pi f m''/\rho_0 c_0)^2] + 10\log_{10}(2\eta f/\pi f_c)$ con el factor
de pérdidas $\eta$. El valle se sitúa en el punto B de la carta de diseño de
Bies, $TL = 20\log_{10}(f_c m'') + 10\log_{10}\eta - 44$.
...
$\eta = 2{,}2/(fT_s)$, la misma magnitud que miden las guías de
...
cambia nada por debajo de $f_c/2$.
...
caída del aislamiento en $f_c \approx 2{,}1$ kHz concentra la mayor parte
...
ax.set_ylabel("Índice de reducción acústica $R$ (pérdida por transmisión $TL$) [dB]")
...
dibujan el aire bajo la placa con la ganancia representada en ellos
...
ax.set(xlabel="Frecuencia [Hz]",
       ylabel="Índice de reducción acústica $R$ (pérdida por transmisión $TL$) [dB]")
ax.legend()
plt.show()
```
...
$Y_c = 0$, donde $\eta_{ij}$ cae como $1/f$. Una llave elástica añade dos
potencias más en cuanto $|Y_c| = \omega/k$ supera a las movilidades de las
...
a $10\log_{10}(S/S_a)$: una abertura del 1 % nunca puede superar los 20 dB, sea cual
sea la pared.
...
tras el muro lleva la ganancia representada en cada panel
...
*Tres regímenes en un mismo eje. Por debajo de $f_c$ los dos vidrios se
...
$f_c$ = 2107 Hz. Por encima, $\sigma$ vuelve hacia 1 solo despacio: todavía 1,74
...
se empaquetan como un [`MobilityResult`](/phonometry/es/vibration/structural/mechanical-mobility/)
de ISO 7626.
...
| `width` (rendija) | float | m | > 0 | Anchura $w$ de la rendija. Tercer argumento posicional de `slit_transmission_coefficient`, **segundo** de `slit_resonance_frequencies` |
| `depth` (rendija) | float | m | > 0 | Fondo $d$ de la rendija (el espesor de la pared). Cuarto argumento posicional de `slit_transmission_coefficient`, **primero** de `slit_resonance_frequencies` |
...
- Teoría: [Acústica de salas y edificación](/phonometry/es/reference/theory/rooms-buildings/#acústica-de-salas-y-edificación-iso-18233-iso-3382-iso-16283-iso-10140-en-12354-iso-12999-iso-717-iso-354): la ley de masas, la caída del aislamiento en la coincidencia y el coeficiente de transmisión que devuelven los modelos de panel.
