...
### Tabla D.1: $Y_s$ a partir de las piezas de la propia máquina
...
nuevo la Fórmula (F.4), escrita en $\rho$, $c_L$ y $t$ en lugar de en masa y
...
$f_0 = (2\pi)^{-1}\sqrt{s(1+\eta^2)/M}$ y dejan $\lvert Y_s\rvert$ en su
...
$D_C$ pequeño. Esa es la resonancia del montaje, y $\eta$ es lo único que fija
...
dos entradas que toma esta página: $L_{Ws,c} = L_F - 5 - 10\log_{10}f$
(Fórmula D.9a) y
$D_{C,i} = -10\log_{10}(\omega M Y_i) + 10\log_{10}[1 + (\omega M Y_i)^2]$
(Fórmula D.9b, que es la Fórmula 19b con $Y_s = 1/\mathrm{j}\omega M$).
