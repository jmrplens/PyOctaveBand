...
ax.set(xlabel="Frecuencia [Hz]",
       ylabel=r"Nivel de potencia estructural $L_{W\mathrm{s}}$ [dB re 1 pW]")
ax.legend()
plt.show()
```
...
ax.semilogx(bands, l_wsn, "-s", label=r"$L_{W\mathrm{sn}}$, el valor declarado")
ax.semilogx(bands, installed, "-^", label=r"$L_{W\mathrm{s,inst}}$ sobre la pared")
ax.semilogx(bands, characteristic, "-D",
            label=r"$L_{W\mathrm{s,c}}$, la entrada de EN 12354-5")
