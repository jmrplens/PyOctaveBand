...
(`ranges`, `depths` y `travel_times` por rayo); `gaussian_beams` un
`GaussianBeamResult` (el campo `propagation_loss`, más el rayo central y la
anchura de cada haz); `parabolic_equation` un `ParabolicEquationResult` (el
campo `propagation_loss`). Los cuatro difieren en lo que aproximan, no en lo
que modelan, así que elegir entre ellos es una cuestión de régimen: la tabla
de la sección 6 es la respuesta corta.
