...
description: "Calcular el campo sonoro submarino cuando las formas cerradas no bastan: la expansión en modos normales de la guía de ondas estratificada, el trazado de rayos con el invariante de Snell y las zonas de convergencia, la búsqueda de eigenrayos que convierte un abanico trazado en la lista de llegadas de un receptor, un fondo libre de inclinarse como perfil lineal a trozos sujeto al abanico exacto de imágenes de la cuña ideal, el trazado de haces gaussianos para un campo que se mantiene finito en las cáusticas, y la ecuación parabólica split-step de Fourier, cada uno validado contra una forma cerrada exacta, con la guía de selección de modelo para elegir entre ellos."
references:
  - type: book
...
ellos y las formas cerradas. Los cuatro toman el mismo perfil $c(z)$
...
- **`ray_trace`** integra las ecuaciones de trayectoria de rayos
  ...
  ellos. `eigenrays` busca después en ese abanico trazado los caminos que
  unen la fuente con un receptor y lista sus llegadas, validado contra la red
  de imágenes de la guía de ondas ideal, donde cada llegada es forma cerrada.
...
$10^{-14}$ s. `RayTraceResult` sigue sin informar de ningún nivel propio: la
...
Pasar `bathymetry_ranges_m` y `bathymetry_depths_m` sustituye el fondo
horizontal por un **perfil de profundidad lineal a trozos** — la frontera
...
ángulo $\beta$ gira el rayo $2\beta$: los rebotes pendiente arriba empinan
...
# Una cuña isovelocidad: el fondo sube de 1000 m a 650 m en 4 km, una
# pendiente arriba de 5 grados. Un rebote en la superficie solo invierte el
# rayo; un rebote en el fondo lo empina el doble de la pendiente.
beta = np.radians(5.0)
...
# 0 20.0   como se lanzó
# 1 30.0   primer rebote de fondo: empinado 2 beta
# 2 30.0   la superficie solo invierte el signo
# 3 40.0   segundo rebote de fondo: 2 beta otra vez
```

La polilínea empieza en $r = 0$, de modo que la columna de agua en la fuente
...
El abanico dibuja todos los caminos que el perfil admite; a un receptor solo
...
leídos de su propia trayectoria, y con la divergencia $q$ de la sección
...
```python
# El canal isóvelo de 100 m se despliega en imágenes especulares, así que
# cada llegada se comprueba a mano: la n-ésima vuela en línea recta hasta una
# imagen del receptor, a sqrt(r^2 + z_n^2) metros.
...
print(arr.travel_times.size)          # 11 llegadas dentro del abanico de 48 grados

# El camino directo: 500 m en distancia, 10 m hacia abajo, sin toques, 1/R.
print(round(float(arr.travel_times[0]) * 1e3, 2))       # 333.4 ms
print(round(float(abs(arr.amplitudes[0])) * 500.1, 6))  # 1.0: eso es 1/R

# El primer eco salió de la fuente hacia arriba y llega desde arriba, con el
# signo invertido una vez por la superficie de presión nula.
print(int(arr.surface_reflections[1]), int(arr.bottom_reflections[1]))  # 1 0
print(round(float(arr.launch_angles[1]), 2))            # -9.31 grados: arriba
arr.plot(language="es")   # el esqueleto de la respuesta al impulso
```

<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/eigenray_arrivals_es.svg" alt="La estructura de llegadas en un receptor de un canal isovelocidad de 100 metros: once barras verticales frente al tiempo de propagación de 333 a 480 milisegundos, cada cabeza marcando la pérdida de propagación de su propio camino sobre un eje invertido, el camino directo dibujado el primero y el más fuerte en azul, los caminos reflejados coloreados según su número de reflexiones en los contornos de uno a cinco, llegando más tarde, más empinados y algo más débiles a lo largo de la escalera de imágenes, con una barra de color que da el número" width="92%" />

*El canal del propio fragmento, dibujado: una barra por eigenrayo, con la
cabeza en la pérdida de ese único camino. El camino directo llega el primero y
el más fuerte; el primer eco le sigue 4,4 ms por detrás, con el signo
invertido por la superficie; y cada toque adicional despliega una imagen más
del receptor, más tardía, más empinada y algo más débil escalera abajo. Sobre
reflectores perfectos nada salvo $1/R$ atenúa un camino, así que 147 ms de
dispersión de retardos cuestan apenas 3 dB: esta lista es el multitrayecto que
consume la ecuación del sonar y el esqueleto sobre el que se construye una
respuesta al impulso del canal.*

Una sola lista sirve para todas las frecuencias. Las amplitudes están
normalizadas a presión unidad a 1 m, en el mismo convenio $e^{-i\omega t}$
que el resto del módulo, y la búsqueda no pregunta nunca por una frecuencia:
el tono que oye un receptor es $p(\omega) = \sum_j a_j e^{i\omega\tau_j}$, su
pérdida $-20\lg|p(\omega)|$ en la misma escala que los otros tres métodos, y
la respuesta al impulso de una banda es esa suma transformada. Cada amplitud
lleva un $(-1)$ por toque en la superficie y el coeficiente del fondo por
toque en el fondo — el $\mp1$ de los reflectores perfectos o, con el mismo
par `seabed_density`/`seabed_sound_speed` que toman los haces, la
$\mathcal{R}$ de Rayleigh del ángulo rasante propio de cada rayo, módulo y
fase — y el $(-i)^m$ de la Ec. (3.79) por cada cáustica cruzada, la forma
discreta del $-\pi/2$ que la rama de la raíz cuadrada del método de haces
...
concuerdan hasta $10^{-13}$ s, $10^{-11}$ grados y $10^{-13}$ relativo; y
contra el rayo parabólico exacto del perfil $n^2$-lineal (Ec. 3.195), cuyo
...
$2\times10^{-7}$ s, siendo el residuo el muestreo del perfil y no la
búsqueda.
...
print(round(float(beams.initial_beam_widths[0]), 1))  # 35,9 m: W0, Ec. (3.86)
...
es más afilado que una regla empírica, y es una anchura por cada ángulo de
lanzamiento en vez de una por corrida. En aguas abiertas el §3.5.1 hace la
optimización de forma explícita: derivar la anchura en espacio libre de la
Ec. (3.86) respecto del desplazamiento complejo da
$W_0 = \sqrt{\lambda\,r_{\max}/\pi}$, la anchura que mejor resuelve el campo
en el extremo lejano del recorrido, que es donde peor se resuelve. Es también
donde la integral en ángulo de lanzamiento que hay detrás de la Ec. (3.92) es
una gaussiana de verdad y no una integral de Fresnel, y por eso no es solo una
elección elegante: frente al campo libre a 100 Hz a 2, 5 y 8 km el error en
$|p|$ es de $7,5\times10^{-5}$ con esa anchura, de $2,7\times10^{-2}$ con la
quinta parte de ella y de $4,1\times10^{-2}$ con quince veces. Un canal somero
añade encima su propia exigencia: su campo atrapado es un conjunto discreto de
modos separados $\lambda/(2D)$ en el seno del ángulo de lanzamiento, y un haz
mezcla ángulos de lanzamiento sobre su divergencia de campo lejano
$\lambda/(\pi W_0)$, así que resolver los modos vecinos hasta la mitad de su
separación pide $W_0 \ge 4D\cos\theta_0/\pi$: una anchura por cada ángulo de
lanzamiento, máxima para los haces tendidos, cuyos modos se agolpan, y
relajada por el coseno para los inclinados, con la huella vertical de todos
los haces en el mismo $4D/\pi$. El valor por defecto toma el mayor de los dos,
dentro de la banda de 10 a 50 longitudes de onda del libro, y entrega a todo
el abanico el óptimo de espacio libre cuando el canal es demasiado profundo en
longitudes de onda para que la anchura de guía quepa en la banda. Una versión
anterior, en cambio, *acotaba* $W_0$ a un cuarto de la profundidad de la
columna, leyendo el «no grandes en comparación con la profundidad del agua»
del libro como un techo sobre la anchura y no sobre la huella; lo que ese tope
decía proteger, la contabilidad que pliega un rayo reflejado de vuelta a la
columna, ya lo restituye la escalera de imágenes del receptor, y en aguas
someras costaba en silencio decibelios de nivel, siempre hacia lo demasiado
flojo. Las medidas están dos párrafos más abajo, tope contra valor por
defecto sobre los mismos oráculos exactos.
...
**El fondo también puede inclinarse aquí**: el mismo par
`bathymetry_ranges_m`/`bathymetry_depths_m` que toma `ray_trace` (sección 3)
...
solución exacta por imágenes — un abanico de $2\pi/\beta$ fuentes imagen
...
- **No hay campo cercano**, y este es el mayor error de los cuatro. La
  ...
  defecto crece como $\sqrt{r_{\max}}$, un recorrido más largo aleja aún más
  esa frontera. De cerca, usa `parabolic_equation`.
- **El régimen propio de la teoría de rayos** (Jensen §3.4.2): «la longitud de
  ...
  `normal_modes` sigue siendo el método en el que confiar, exacto al precio de
  un puñado de modos.
...
| `gaussian_beams` | Alta frecuencia, siempre que de los rayos haga falta un *nivel* | Esa misma geometría convertida en PL($z$,$r$): finita en las cáusticas, graduada en las zonas de sombra, fondo en pendiente incluido y, como los rayos sobre los que se apoya, con un coste que no crece con la frecuencia |
...
tercer estado del mismo paso Runge-Kutta, y `eigenrays` busca en un abanico
trazado las llegadas que unen una fuente con un receptor, cada una refinada
por bisección sobre trazados nuevos y cargada con la amplitud clásica de rayo
de la Ec. (3.65) con las fases de contorno y de cáustica en el registro
(Ecs. 3.79, 3.125-3.126). `ray_trace` y `gaussian_beams` aceptan además un
perfil de fondo lineal a trozos, la frontera facetada del cap. 3, que
refleja cada rayo sobre la faceta local (Ec. 3.121) con el par dinámico
cruzando el rebote según las Ecs. (3.122)-(3.123). Cada uno está validado contra una
forma cerrada exacta: los modos de la guía de ondas ideal de presión nula y su
suma de fuentes imagen (para los eigenrayos, la lista completa de llegadas de
esa red: tiempo, ángulo, contadores y amplitud por imagen), los
rayos en arco de círculo de un gradiente lineal junto con el tiempo de
propagación publicado a lo largo de ellos (Medwin y Clay 1998, Ec. (3.3.20)),
la divergencia esférica en campo libre y, para el fondo en pendiente, el
abanico cerrado exacto de imágenes de la cuña ideal, más la coincidencia
mutua de la pérdida de propagación modal y de la PE.
</ScopeClaim>
...
pueden cambiarlo por el lecho fluido disipativo del modelo de Rayleigh (solo
con fondo horizontal), y los dos métodos basados en rayos pueden inclinar el
fondo con un perfil de profundidad lineal a trozos, que es la única
dependencia de la distancia que hay: una columna de agua dependiente de la
distancia $c(r,z)$ queda excluida deliberadamente, porque la frontera en
pendiente tiene un oráculo exacto (el abanico cerrado de imágenes de la cuña
ideal) y $c(r,z)$ no tiene ninguno. No hay fondo elástico ni atenuación del
sedimento. El
trazado de rayos devuelve caminos y tiempos de propagación, no amplitudes; el
nivel por camino pertenece a `eigenrays`, cuya lista de llegadas es teoría de
rayos clásica y diverge honestamente para un receptor plantado sobre una
cáustica, mientras que `gaussian_beams` es lo que da un campo finito, sumando
haces en vez de buscando rayos. Su alternativa de haces geométricos (cap. 3, §3.3.5.5) con el
