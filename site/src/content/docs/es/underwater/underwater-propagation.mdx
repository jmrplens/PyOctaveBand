...
sin simular ningún campo. Al terminarla sabrás poner un número a la pérdida por
transmisión de un trayecto, elegir los modelos de absorción y de velocidad del
...
<span id="perdida-por-transmision"></span>

## Pérdida por transmisión

**Son dos magnitudes, y la ISO 18405 las mantiene separadas.** La *pérdida por
transmisión* es la reducción de un nivel especificado entre dos puntos
especificados; la *pérdida por propagación* es la diferencia entre el nivel de
fuente y el nivel de presión acústica cuadrático medio en una posición,
referida a 1 m². La ecuación del sonar necesita la magnitud referida a la
fuente, así que lo que esta página calcula en realidad es una pérdida por
propagación, y la norma desaconseja usar cualquiera de los dos nombres como
sinónimo del otro, aunque la literatura y el propio nombre de función
`transmission_loss` sigan el uso antiguo. Cualquiera de las dos magnitudes hay
que declararla con el tiempo de promediado y el margen de frecuencia a los que
se aplica. En las geometrías de esta página ambas son numéricamente
intercambiables, porque la distancia de referencia es 1 m en las dos, y por eso
la pérdida por propagación de Weston de más abajo se dibuja en el mismo eje que
la pérdida por transmisión de aquí.
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/underwater_transmission_loss_es.svg" alt="Pérdida por transmisión submarina frente a la distancia a 10 kHz, con las contribuciones de divergencia geométrica y absorción de volumen dibujadas por separado, la pérdida creciente hacia abajo" width="82%" loading="eager" />
...
$\alpha R$ toma el relevo del crecimiento: en 20 km, los 92,0 dB totales son
73,0 dB de divergencia más 19,0 dB de absorción a 0,95 dB/km. En 10 kHz la
absorción no llega a superar a la divergencia en todo este intervalo; en 100 kHz
...
número de onda acústico en la columna de agua, $\psi_c = \arccos(c_1/c_2)$ el
...
radián ($|R(\theta)| \approx e^{-\eta\theta}$) y $H_e$ la **profundidad efectiva
de Weston**: el nivel poco *por debajo* del fondo real en el que parece situarse
un contorno de presión nula, de modo que $H_e > H$ y las dos letras no son
...
comparable con la pérdida por transmisión de arriba y con el resultado modal
...
cuatro coinciden en torno a 1 m/s**: 0,98 m/s a 25 °C, 35 ‰ y 1000 m, y
...
$$
SE = SL - 2\,TL + TS - (NL - DI) - DT \ \ \text{(activa, monoestática)},
$$
...
veces, que es la razón de que $TL$ aparezca duplicada y de que exista siquiera
...
exceso de señal cae decibelio a decibelio con la pérdida por transmisión y cruza
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/detection_range_es.svg" alt="A la izquierda, la pérdida por transmisión en forma cerrada del ejemplo activo de 50 kilohercios de Ainslie cruzando una sola vez una figura de mérito de 82,7 decibelios, a 1319 metros; a la derecha, una pérdida por transmisión de modos normales a 30 hercios en una guía de 100 metros cruzando ocho veces una figura de mérito de 60 decibelios entre 1 y 6 kilómetros, con el primer cruce y el último marcados" width="92%" />
...
ax_m.plot(ranges / 1000.0, tl, label="TL de modos normales (30 Hz, 100 m)")
ax_m.axhline(60.0, linestyle="--", label="Figura de mérito = 60 dB")
print(underwater.detection_range_from_curve(60.0, ranges, tl))   # primer cruce
ax_m.invert_yaxis()
ax_m.set(xlabel="Distancia [km]", ylabel="Pérdida por transmisión [dB]")
ax_m.legend()
plt.show()
```
...
$\psi_c = \arccos(c_1/c_2)$, el mismo $\psi_c$ que fija las fronteras de los
...
La reflexión total por debajo de $\psi_c$ es una propiedad de la idealización
...
pasando por 5,21 dB en 30°, 7,89 dB en 45° y 9,05 dB en incidencia normal. Ese
...
exactamente 1 hasta 24,62° y a partir de ahí cae deprisa: 0,403 en 45° y 0,353
...
5,02 dB por octava, y pasar de 5 a 20 nudos la levanta 10,03 dB en 1 kHz, de
51,1 dB a 61,1 dB. Los dos mecanismos van luego en sentidos opuestos, así que el
total tiene un mínimo: el ruido térmico llega a 25,3 dB en 100 kHz y con 5 nudos
...
portacontenedores alcanza su máximo de 174,3 dB re 1 µPa²/Hz en 31,6 Hz, el
crucero 158,7 dB en 25,1 Hz, y el remolcador, lento y corto, 129,5 dB y más
arriba, en 125,9 Hz. De ahí salen dos cosas: la energía de la flota está por
...
del sonar pasivo y activo monoestático (Urick, vía Etter 2003), y
...
de alcance. `active_sonar_equation` es solo monoestática: no hay geometría
