...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/sound_speed_models.svg" alt="A la izquierda, el perfil de velocidad del sonido calculado con las ecuaciones UNESCO, Del Grosso, Mackenzie y Medwin sobre el mismo perfil de temperatura y salinidad desde la superficie hasta 5000 metros, con las cuatro curvas indistinguibles a esa escala; a la derecha, la diferencia de cada una respecto a UNESCO frente a la profundidad en un eje de más menos siete metros por segundo, con las profundidades más allá del límite de validez de mil metros de Medwin sombreadas y con Medwin apartándose casi seis metros por segundo a 5000 metros" width="92%" />
...
### De dónde salen $DI$ y $DT$

Los dos términos de arriba se escribieron a mano. Los dos tienen modelo, así
que ninguno tiene por qué escribirse.

El **índice de directividad** de un array lineal sin ponderar sale de su
longitud, de la longitud de onda y de la dirección a la que apunta (Ainslie,
*Principles of Sonar Performance Modelling*, Ecs. (6.49) a (6.57), folio
impreso 267). Es además la
**ganancia de array** cuando el ruido es isótropo y la señal una onda plana,
que es el caso para el que está escrita la ecuación del sónar, así que un solo
número responde a los dos:
...
El array gana 3 dB más en proa que en través, porque allí la huella del haz se
reduce a la mitad, y no gana nada en cuanto es mucho más corto que una longitud
de onda:
...
traves = underwater.array_directivity_index(100.0, 1.0)
proa = underwater.array_directivity_index(100.0, 1.0,
                                          steer_angle_rad=math.pi / 2)
print(f"{traves:.1f} dB, {proa:.1f} dB")                     # 23.0 dB, 26.0 dB
print(f"{underwater.array_directivity_index(0.01, 1.0):.3f} dB")  # 0.000 dB
```

El **umbral de detección** con 50 % de probabilidad de detección sale sólo de
la probabilidad de falsa alarma (Ec. (11.22), folio impreso 581), para la
estadística de una dominante más Rayleigh que el libro recomienda cuando no se
conoce la del blanco:
...
Exigir menos falsas alarmas cuesta relación señal a ruido, y el precio se
abarata según se paga: el logaritmo interior de la Ec. (11.22) es a su vez
logarítmico en $p_\mathrm{fa}$, así que la década por debajo de $10^{-2}$ cuesta
2,0 dB y la que hay por debajo de $10^{-5}$ solo 0,8. En $p_\mathrm{fa} = 1/2$
el umbral diverge y la llamada se rechaza: la mitad de los haces vacíos ya se están
declarando detecciones, y ahí no hay umbral que valga.
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/detection_range.svg" alt="A la izquierda, la pérdida de propagación en forma cerrada del ejemplo activo de 50 kilohercios de Ainslie cruzando una sola vez una figura de mérito de 82,7 decibelios, a 1319 metros; a la derecha, una pérdida de propagación de modos normales a 30 hercios en una guía de 100 metros cruzando ocho veces una figura de mérito de 60 decibelios entre 1 y 6 kilómetros, con el primer cruce y el último marcados" width="92%" />
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/seabed_reflection.svg" alt="Pérdida por reflexión en el fondo frente al ángulo rasante para un fondo arenoso rápido: pérdida nula por debajo del ángulo rasante crítico, creciendo bruscamente por encima" width="82%" />
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/seabed_reflection_coefficient.svg" alt="Módulo del coeficiente de reflexión del fondo frente al ángulo rasante para un fondo arenoso rápido: reflexión total por debajo del ángulo rasante crítico, decreciendo hasta el valor de incidencia normal por encima" width="82%" />
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/ocean_ambient_noise.svg" alt="Niveles espectrales de ruido ambiental de Wenz para dos velocidades de viento, con el ruido de viento cayendo 5 dB por octava y el ruido térmico creciendo por encima de unos 50 kHz" width="82%" />
