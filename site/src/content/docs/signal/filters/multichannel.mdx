...
[Correlation and delay](/phonometry/signal/spectra/correlation-delay/) and
...
- **Combining channels is your decision.** Levels come back one per
  ...
  [Levels](/phonometry/signal/levels/levels/) for why).
...
- **Stateful classes keep one state per channel.** In block processing the
  ...
  resets it; see [Block Processing](/phonometry/signal/filters/block-processing/).
...
[Correlation and delay](/phonometry/signal/spectra/correlation-delay/) and
[Multiple and partial coherence](/phonometry/signal/spectra/miso-coherence/) for
the cross-channel tools built on Bendat & Piersol's cross-correlation and
multiple-input models.

## See also

- [Correlation and delay](/phonometry/signal/spectra/correlation-delay/): the cross-channel time-domain questions (delay, alignment) the per-channel path deliberately leaves to you.
- [Multiple and partial coherence](/phonometry/signal/spectra/miso-coherence/): which of several correlated channels actually drives a response (Bendat & Piersol Ch. 7).
- [Block Processing](/phonometry/signal/filters/block-processing/): the streaming counterpart, with one filter state per channel.
- [Levels](/phonometry/signal/levels/levels/): the per-channel level metrics, and why dB values are combined energetically.
