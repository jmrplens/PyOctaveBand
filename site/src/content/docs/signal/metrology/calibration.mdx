...
- [Build a sound level meter](/phonometry/signal/sound-level-meter/): the walkthrough that starts from this calibration step and ends at a class-checked instrument.
- [Levels](/phonometry/signal/levels/levels/): every metric that consumes the `calibration_factor` derived here.
- [Multichannel and Performance](/phonometry/signal/filters/multichannel/): one sensitivity per channel when the channels differ.
- [GUM uncertainty](/phonometry/signal/metrology/gum-uncertainty/): propagating the calibrator tolerance and drift bound into a level's uncertainty.
