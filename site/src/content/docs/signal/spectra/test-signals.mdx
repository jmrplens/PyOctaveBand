...
[spectral analysis guide](/phonometry/signal/spectra/spectral-analysis/#5-colored-noise-generators)),
...
[Fast/Slow/Impulse ballistics](/phonometry/signal/levels/time-weighting/) reference
...
- `mode="linear"` (default) zero-pads the record past the shift, so samples
  ...
  kernel inside
  [`align_impulse_responses`](/phonometry/signal/spectra/correlation-delay/#3-impulse-response-delay-and-alignment).
...
The [window figures of merit](/phonometry/signal/spectra/spectral-analysis/#6-choosing-the-window)
...
[correlation and delay work](/phonometry/signal/spectra/correlation-delay/), where
...
- [Time Weighting](/phonometry/signal/levels/time-weighting/): the detector ballistics the tone bursts exercise (IEC 61672-1 Table 4).
- [Correlation and delay](/phonometry/signal/spectra/correlation-delay/): the alignment work built on the fractional-delay kernel.
- [Synchronous averaging](/phonometry/signal/spectra/synchronous-averaging/): period alignment with the same band-limited shift when $f_s T$ is not an integer.
- [Spectral analysis](/phonometry/signal/spectra/spectral-analysis/): the colored-noise verification and the window metrics.
