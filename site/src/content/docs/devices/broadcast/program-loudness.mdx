...
algorithm, and nothing here runs it. There is no normalisation *helper*
either: section 7 shows the single gain, and applying it is one
multiplication. And nothing on this page is a loudness **model** — K-weighting
is a fixed linear filter, so masking, level dependence, critical bands and
binaural summation are all outside it by construction; those live in
[Loudness (ISO 532)](/phonometry/perception/psychoacoustics/loudness/).
