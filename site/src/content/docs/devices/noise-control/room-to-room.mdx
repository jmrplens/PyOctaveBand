...
the $R_w$ used below, so the $R$ and $R_w$ of a laboratory test report drop
straight into this equation (see
[Insulation ratings (ISO 717)](/phonometry/buildings/insulation/insulation-ratings/))
— $S_w$ the area of the partition and $S_2 \alpha_2$ the equivalent absorption
...
$D_{nT} = D + 10\log_{10}(T/T_0)$ with $T_0 = 0.5$ s — the quantity most
residential regulations are written in — or to a reference absorption area of
10 m², $D_n = D - 10\log_{10}(A/10)$. The wall's own quantity is $R$, and the
...
$D_{nT,w}$ for the same wall are different numbers by construction, and
...
$-10\log_{10}[S_w/(S_2\alpha_2)]$, contains no property of the wall at all. It
