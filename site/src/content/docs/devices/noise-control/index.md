---
title: "Noise control"
description: "Industrial noise control on the path: the end-to-end fan-to-room duct calculation and the room-to-room chain against a design criterion, plus the element models they call — reactive four-pole silencers, HVAC duct attenuation and flow noise, and machine-enclosure insertion loss."
---
...
[Silencers](/phonometry/devices/noise-control/silencers/) covers the reactive
...
[Duct-Borne Noise](/phonometry/devices/noise-control/duct-path/); if it travels
through a wall, start at
[Room to Room](/phonometry/devices/noise-control/room-to-room/); open the
...
determined by the [Sound power and intensity](/phonometry/devices/emission/)
...
[Room noise criteria](/phonometry/buildings/rooms/room-noise/), plus whatever
...
modelled from liner properties anywhere in the library, and the lined-elbow and
plenum figures on the HVAC page are interpolated installation tables rather than
a liner model. **Mean flow is outside the element matrices**: convection,
temperature gradients and the flow-dependent impedance of perforates do not
appear, so a silencer carrying significant flow is predicted as though it were
not. And `enclosure_insertion_loss` **never predicts the panel's transmission
loss**: you supply R measured or from another model, and the module combines it
with the interior correction — predicting R itself is [Insulation
design](/phonometry/buildings/design/). Above the higher-order-mode cut-on
frequency the plane-wave assumption every duct model rests on stops holding,
which the duct-path page states and which no method here works around.
