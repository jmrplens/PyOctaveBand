---
title: "Sound Power"
description: "The sound power level LW as the device-independent emission descriptor: choosing among the six standardised determination routes (ISO 3744/3746, ISO 3745, ISO 3741, ISO 9614-2, ISO 9614-3, ISO/TS 7849), what the accuracy grades promise, and the ISO 4871 noise-emission declaration with its dual- and single-number forms and verification."
references:
  - type: book
...
four method guides: an enveloping *pressure* surface in the field
...
noise test codes call for — and ISO 3747 determines the sound power of a large
fixed machine in situ by comparison against a calibrated reference sound source
in a reverberant field. Neither is implemented here. In practice: for a small
appliance, ISO 3744 over a hemisphere in the same room is usually available at
the same grade; for the in-situ comparison case, the comparison formula on the
[ISO 3741 page](/phonometry/devices/emission/sound-power-reverberation/#2-the-comparison-method-eq-21)
is the same algebra, applied to a qualified room instead of an arbitrary one.

<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/sound_power_methods.svg" alt="The six sound power routes in two rows of three, each cell carrying the same five attributes. Top row: the ISO 3744/3746 enveloping pressure surface over a reflecting plane, the ISO 3745 fixed array in a wedge-lined anechoic room, and the ISO 3741 diffuse field of a reverberation room. Bottom row: the ISO 9614-2 serpentine intensity scan, the tighter ISO 9614-3 precision scan, and the ISO/TS 7849 accelerometer grid on a radiating casing with no microphone at all" width="92%" loading="eager" />

All six deliver the same quantity, and the last one gets there without a
...
ISO 3746 and *not implemented here*. The practical test: if the A-weighted level
...
The six determinations themselves live in their method guides: the enveloping
...
Scanning](/phonometry/devices/emission/sound-power-intensity/) and the
surface-velocity route in [Sound power from surface
vibration](/phonometry/devices/emission/vibration-sound-power/). So does the
seventh the decision path names, ISO 9614-1's discrete-point summation, which
lives in [Sound Intensity](/phonometry/devices/emission/intensity/) beside the
Annex A field indicators it lends the scanning parts. ISO 3743-1, ISO 3743-2
and ISO 3747, the remaining members of the ISO 3740 family, are not
implemented, and neither is the sound *energy* level
$L_J$ for single events. The emission sound pressure level $L_{p\mathrm{A}}$ that
...
- [Sound power from surface vibration (ISO/TS 7849)](/phonometry/devices/emission/vibration-sound-power/):
  the sixth route, for a machine no microphone can approach.
...
- Theory: [Sound power determination](/phonometry/reference/theory/environment-transport/#sound-power-determination-iso-374437453746-iso-3741-iso-9614-23): the enveloping-surface, reverberation-room and intensity derivations behind the four routes.
