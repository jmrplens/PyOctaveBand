...
The sound energy level $L_J$ of clause 9.2, the single-event counterpart of
$L_W$ defined in the same clauses, is not implemented, so an impulsive or
single-event source falls outside this guide. Neither is the Annex F
octave-band summation, so octave results have to be summed from the
