...
ISO 9614-1's own sound-power determination, clauses 8 and 9, which sums the
partial powers $I_i \cdot S_i$ of the discrete measurement points into $L_W$,
is not implemented as a function here: `sound_intensity` and
`field_indicators` report intensity and field quality, not a summed power. The
scanning routes that do compute $L_W$ from intensity (ISO 9614-2 and ISO
9614-3) live in [Sound Power by Intensity
Scanning](/phonometry/devices/emission/sound-power-intensity/). The
residual-intensity *test* of IEC 61043 is not performed by the library either:
