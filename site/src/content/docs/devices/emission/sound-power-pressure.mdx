...
print(round(float(five_at_once[0]), 2))          # 90.01 dB: 97 - 10 lg 5 (Eq. 20)
one_by_one = emission.mean_single_event_level(np.array([89.0, 91.0, 90.0, 90.5, 89.5]))
print(round(float(one_by_one), 2))               # 90.05 dB: the energy mean (Eq. 19)
```
...
print(round(ten_seconds.sound_energy_level_a - res.sound_power_level_a, 6))  # 10.0
```
