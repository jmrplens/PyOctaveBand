...
(Eq. 3.202). [`WALL_TIE_STIFFNESS`](/phonometry/reference/api/building/masonry-cavity-wall/#wall_tie_stiffness) carries Hopkins' Table A4, whose 50 mm
...
[`wall_tie_coupling_loss_factor`](/phonometry/reference/api/building/masonry-cavity-wall/#wall_tie_coupling_loss_factor) needs only the tie density. A rigid
...
| `tie` | A name from [`WALL_TIE_STIFFNESS`](/phonometry/reference/api/building/masonry-cavity-wall/#wall_tie_stiffness), an explicit dynamic stiffness `k` in N/m, or `None` for a rigid connection (`Yc = 0`, the screw/nail/bolt limit). |
...
| `connector_mobility` | Magnitude `\|Yc\| = omega/k` of the tie mobility per frequency, in m/(N.s); all zeros for a rigid connection. |
...
| `rigid_coupling_loss_factor` | The `Yc = 0` coupling loss factor per frequency, the ceiling a resilient tie is measured against. |
