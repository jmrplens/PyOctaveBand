...
* **Low frequency** ($ka \ll 1$): $R_1 \to (ka)^2 / 2$ so the
  radiated power
...
$$
D(\theta) = \frac{2 J_1(ka \sin\theta)}{ka \sin\theta}, \qquad D(0) = 1
$$

The main lobe narrows as `ka` grows; its first null is at
$ka \sin\theta = 3.8317$ (the first zero of `J1`), which exists only
once
$ka > 3.8317$. The **directivity factor** `Q` (on-axis intensity over
the
intensity of a point source of equal power radiating into the full sphere) and
the **directivity index** $DI = 10 \log_{10} Q$ follow from integrating
...
which tends to $Q = 2$ ($DI = 3.01$ dB, the half-space baffle
gain) at low
`ka` and to $Q \sim (ka)^2$ ($DI \sim 20 \log_{10} ka$) at
high `ka`.
...
Far-field directivity $D = 2 J_1(ka \sin\theta) / (ka \sin\theta)$.
...
the first zero of `J1` ($ka \sin\theta = 3.8317$).
...
Piston resistance function $R_1(x) = 1 - 2 J_1(x) / x$.

The real part of the normalized radiation impedance of a baffled circular
piston, as a function of $x = 2ka$ (Beranek & Mellow Eq. (4.30)).
It rises as $x^2 / 8 = (ka)^2 / 2$ at low `x` and tends to 1 at
high `x`.
...
$D(\theta) = 2 J_1(ka \sin\theta) / (ka \sin\theta)$ (Beranek &
Mellow
...
Radiation impedance and directivity of a rigid baffled circular
piston.

Evaluates the piston resistance $R_1(2ka)$ and reactance
