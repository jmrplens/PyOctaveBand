...
where $L = T / \ln(f_2/f_1)$ is the sweep rate. Windowing each
...
* **Synchronized swept-sine (Novak et al. 2015)** -- the sweep
  $x(t) = \sin[2\pi f_1 L \exp(t/L)]$ with the rate rounded so
  that $f_1 L$ is an integer (Eqs. 47/49). Time-shifting this
  ...
  band of `H_n` to $[n f_1, n f_2]$ (their Fig. 6) and avoids
  inverting spectral ripple. Generated by
  [`synchronized_sweep_signal`](/phonometry/reference/api/electroacoustics/swept-sine/#synchronized_sweep_signal); analysed with
...
distortion order at its exact arrival $-L \ln(n)$ (Farina 2000;
...
$[n f_1, n f_2]$ for the synchronized method,
$[n f_1, f_2]$ for the Farina inverse filter (outside it there
...
| `thd` | Total harmonic distortion at each excitation frequency $f$: $\sqrt{\sum_n \lvert H_n(n f) \rvert^2} / \lvert H_1(f) \rvert$ over the orders available below Nyquist and inside their deconvolved band. |
| `distortion_ratios` | Per-order ratios $\lvert H_n(n f) \rvert / \lvert H_1(f) \rvert$ on `thd_frequencies`; row `k` is order `k + 2`. An order reads 0 where its product $n f$ leaves the valid band. |
| `fs` | Sample rate, in Hz. |
| `f1` | Sweep start frequency, in Hz. |
| `f2` | Sweep stop frequency, in Hz. |
| `duration` | Actual sweep duration `T` used by the analysis, s. |
| `rate` | Sweep rate $L = T / \ln(f_2/f_1)$, in seconds. |
...
$x(t) = A \sin[2\pi f_1 L \exp(t/L)]$ with the rate
$L = \operatorname{round}(f_1 \tilde{T} / \ln(f_2/f_1)) / f_1$
(Eq. 49, with `seconds` as the requested duration
$\tilde{T}$). Because
$f_1 L$ is an integer, delaying the signal by $L \ln(n)$
is exactly equivalent to generating its n-th harmonic (Eq. 18), which
is what makes the harmonic phases measured by
[`swept_sine_distortion`](/phonometry/reference/api/electroacoustics/swept-sine/#swept_sine_distortion) properties of the system under test
rather than of the excitation. The rounding adjusts the duration
slightly: the generated signal lasts $T = L \ln(f_2/f_1)$
seconds rather than exactly `seconds` (the difference is at most
$\ln(f_2/f_1)/2$ periods of `f1`).
