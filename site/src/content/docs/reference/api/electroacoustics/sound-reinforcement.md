...
| Name | Description |
| :--- | :--- |
| `level_loudspeaker_at_microphone` | Direct-field level `L_H-M` the loudspeaker produces at the microphone, dB. |
| `level_loudspeaker_at_listener` | Direct-field level `L_H-L` the loudspeaker produces at an average listener, dB. |
| `microphone_directivity` | Directivity index `D_M(theta)` of the microphone toward the loudspeaker relative to the talker, dB (0 for an omnidirectional microphone, about [`CARDIOID_RELATIVE_DIRECTIVITY`](/phonometry/reference/api/electroacoustics/sound-reinforcement/#cardioid_relative_directivity) for a cardioid). |
...
| Name | Description |
| :--- | :--- |
| `open_loop_gain` | Open-loop system gain `Z_S = L_H-L - L_T-M`, dB; about -6 dB for a typical auditorium or church. |
| `level_loudspeaker_at_microphone` | Direct-field level `L_H-M` produced by the loudspeaker system at the microphone, dB. |
| `level_loudspeaker_at_listener` | Direct-field level `L_H-L` produced by the loudspeaker system at an average listener, dB. |
| `microphone_directivity` | Directivity index `D_M(theta)` of the microphone toward the loudspeaker relative to the talker, dB. |
...
| `nom_correction` | Number-of-open-microphones correction `dL_nom`, dB (Equation (18.23)). |
| `loop_gain` | Total loop gain `Z_S + G_S + dL_nom`, dB. The system oscillates at 0 dB (Equation (18.16)). |
...
| `maximum_level_at_microphone` | Largest `L_H-M` the loop tolerates, dB, for the given `Z_S` (Equations (18.20) to (18.22)). |
| `level_loudspeaker_at_microphone` | Input `L_H-M`, dB. |
| `level_loudspeaker_at_listener` | Input `L_H-L`, dB. |
| `microphone_directivity` | Input `D_M(theta)`, dB. |
...
**Returns:** The correction `dL_nom`, dB.
