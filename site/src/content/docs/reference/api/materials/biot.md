...
* **The 6x6 layer matrix** of chapter 11: the field vector
  ...
  which [`poroelastic_transfer_matrix`](/phonometry/reference/api/materials/biot/#poroelastic_transfer_matrix) returns
...
| `shear_modulus` | Complex shear modulus `N` of the frame, in Pa (`Im(N) >= 0`; a structural loss factor `eta` gives `N = N'(1 + j eta)`). |
...
| Name | Description |
| :--- | :--- |
| `shear_modulus` | Complex shear modulus `N` of the frame, in Pa (`Im(N) >= 0` for a lossy frame in the `e^{+j w t}` convention). |
| `poisson_ratio` | Poisson coefficient `nu` of the frame (`-1 < nu < 0,5`). |
...
| Exception | When |
| :--- | :--- |
| ValueError | for a non-positive or non-finite `N`, a negative `Im(N)` or a `nu` outside `(-1, 0,5)`. |
...
| `transverse_wavenumber` | In-plane wavenumber `kt = k sin(theta)`, in rad/m (Default: 0, normal incidence). Scalar or one value per frequency. |
