...
* fluctuation-strength zero-padding (Clause 5.1.2.2) and segmentation
  (Clause 5.1.5.2) with the fixed block/hop $s_\mathrm{b} = 65536$ /
  $s_\mathrm{h} = 16384$
  (Clause 9.1.1);
...
* Formula (127) prints the phase factor
  $\exp(-j 2\pi f_n (\tilde{s}_\mathrm{b} - n_\mathrm{ze} + n_\mathrm{zb} - 1))$; the DFT of
...
* Formula (144) subtracts 1 from the three-bin centroid before scaling by
  ...
  (bin k maps to $k \tilde{r}_\mathrm{s} / \tilde{s}_\mathrm{b}$) that offset shifts
  every modulation
