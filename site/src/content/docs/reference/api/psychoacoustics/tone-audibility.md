...
lives in [`phonometry.environment.assessment.measurement`](/phonometry/reference/api/environment/measurement/). The audibility of a tone
...
$N \ge 2$ = FG entry combining `N` tones). The decisive audibility
...
$$
U = \frac{\sqrt{\sum_j \left( 10^{0.1 \Delta L_j} U_j \right)^2}} {\sum_j 10^{0.1 \Delta L_j}}
$$
...
| `audibility2` | Audibility `ΔL2` of the second tone, in dB (Formula (14)). On a tie ($\Delta L_1 = \Delta L_2$) the first tone is taken as the more prominent. |
