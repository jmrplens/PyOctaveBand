...
| $K$ | — | $K_1$ background, $K_2$ reverberant field, $K_I$ impulse adjustment, and plain $K$ the bias-error factor of the intensity methods; $K_{met}$ is the ISO 9613-2 barrier meteorological factor |
...
interrupts an evening at home or a night's sleep — so $L_{den}$ is an
...
$L_{den}$ bands from 55 dB upward. The weighting also decides where a mitigation
...
against the day's 51.3 %, and 3 dB off the day moves $L_{den}$ by 1.3 dB where
...
*A 24-hour $L_{Aeq}$ profile split into day, evening and night, the +5/+10 dB
penalties and the resulting $L_{den}$ of 64.3 dB — an urban arterial façade,
...
lines above their own levels, and $L_{den}$ still lands within 0.1 dB of the
...
the adjustment $K_I$ implemented here follow the NT ACOU 112 formulation that the
...
barrier $A_{bar} = D_z$ and the ground term is kept (Eq. (13)). (The standard
...
long-term correction $C_{met}$, never by evaluating Eq. (3) directly. The
...
The quantity itself, $L_{EX,8h}$, is defined from sound exposure in
...
the engineering method is valid only while $K_{2A} \le 4$ dB (7 dB for the
...
writes $F_{pIn}$ and $F_S$. Each subsection here uses its own part's symbols. The
