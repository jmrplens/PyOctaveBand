...
and one such artefact can move the day's $L_{EX,8h}$; any unexplained peak
...
the answer is moved by a term that is not a measured level at all. $T_e$ is the
...
with $T_e = 7.5$ h the normalisation $10\log_{10}(7.5/8)$ subtracts 0.3 dB, so
$L_{EX,8h}$ = 88.2 dB — *below* the sampled average. The six full-day samples
energy-average to 89.5 dB, but that shift lasts 9.25 h, so the same term adds
$10\log_{10}(9.25/8)$ = 0.6 dB and $L_{EX,8h}$ = 90.1 dB, which is higher than
four of the six individual samples. That is not an error: normalising a long
shift to a nominal eight-hour day is *supposed* to penalise it. Get $T_e$ wrong
...
push against. Because peak $L_{p,Cpeak}$ carries no Annex C sampling model
...
applied, so the $L_{EX,8h}$ computed here — and the limit-value comparison the
...
$L_{p,Cpeak}$ (Table C.5, Note 1), which is why the peak is reported without
...
- [Levels](/phonometry/signals/levels/levels/): the `lex_8h` / `sound_exposure` dose primitives
  (IEC 61252) and the $L_{p,Cpeak}$ these strategies report alongside.
