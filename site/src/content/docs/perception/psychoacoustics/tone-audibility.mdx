...
(−19.8 dB at 95.7 Hz to −11.0 dB at 197.0 Hz), so both $L_T$ and $L_S$ move,
...
*Five spectra, one penalty. The mean audibility earns $K_t = 4$ dB, but its
uncertainty band reaches below the 6 dB boundary of ISO 1996-2 Table J.1, so
$K_t = 3$ dB is not excluded — and the remedy is more spectra, not a rounder
number.*
...
contiguous with the peak (above both $L_S + 6\ \text{dB}$ and $L_\text{peak} - 10\ \text{dB}$). The
...
$\le 26\,(1 + 0.001 f_T)$ Hz and edge steepness $\ge 24\ \text{dB}$), and audibility) and
...
(the three tone frequencies and $L_T = 72.15\ \text{dB}$).
...
written $\Delta L_{ta}$ and the tonal adjustment $K$, which are the
$\Delta L$ and $K_t$ of the sections above.
...
audibility $\Delta L_{ta}$) above the level-versus-frequency analysis plot with
the tones and their critical-band masking noise marked, the boxed decisive
audibility $\Delta L_{ta}$ together with the derived tonal adjustment $K$
...
$\Delta L_{ta}$ in dB (a quieter tone passes). Rendering needs reportlab and,
...
the critical bandwidth $\Delta f_c$ and corner frequencies (Formulae 2 to 5),
the mean narrow-band level $L_S$ and tone level $L_T$ (Formulae 6 and 8, Annex
D), the critical-band masking level $L_G$, the masking index $a_v$ and the
...
tonal adjustment $K_t$ (Table J.1).
</ScopeClaim>
