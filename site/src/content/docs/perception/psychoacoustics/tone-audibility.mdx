...
**Covered.** The ISO/PAS 20065:2016 engineering method in full: the clause 4.2
analyser requirements and the clause 4.3 merging of basic spectra that the
input rests on, with the clause 5.1 acceptance conditions and the clause 7
reporting list; the critical
bandwidth $\Delta f_c$ and corner frequencies (Formulae 2 to 5), the mean
narrow-band level $L_S$ and tone level $L_T$ (Formulae 6 and 8, Annex D),
the critical-band masking level $L_G$, the masking index $a_v$ and the
...
mean audibility to the tonal adjustment $K_t$ (Table J.1).

**Not covered.** The distinctness edge-steepness test follows the DIN
...
recorded in the [errata registry](/phonometry/reference/errata/). Building the
narrow-band FFT spectrum itself from a raw time-domain recording is not part of
this module: every function here takes an already-computed spectrum (levels and
