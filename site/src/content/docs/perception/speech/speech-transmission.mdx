...
threshold corrections of Tables A.2 and A.3 (`level=` and `ambient=`), and the
Annex F rating letters returned as `STIResult.rating`.
</ScopeClaim>
...
d), so there is nothing left to implement.
</ScopeClaim>
</Scope>
