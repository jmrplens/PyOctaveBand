...
infinite surface. A truncated $\alpha_{dif}$ is a fitted engineering quantity
...
**Do not compare this with a datasheet $\alpha_s$.** The Paris integral is
...
is why measured $\alpha_s$ routinely exceeds 1 and why the same product
differs between two rooms. Expect the measured value to run above the
predicted $\alpha_{dif}$ by roughly 0.1 to 0.2 in the mid bands, more as the
...
against a datasheet $\alpha_s$ — the finite-size surplus would be absorbed
...
against a measured $\alpha_s$ still has to be closed with a measurement.
...
and the finite-size difference between a predicted $\alpha_{dif}$ and a
measured $\alpha_s$ is stated but not modelled — nothing here reproduces the
