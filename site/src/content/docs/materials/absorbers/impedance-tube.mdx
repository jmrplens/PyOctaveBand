...
- **The air, measured rather than assumed (Clause 7.2).** $c_0$ and $\rho c_0$
  ...
  `air_density_iso10534(temperature_c=..., atmospheric_pressure_kpa=...)`. Both
  equations are written in kelvin and both arguments are in degrees Celsius,
  like every other temperature in the library; the conversion happens inside,
  so the printed 293 K stays literal against the page. The fiche snippet below
