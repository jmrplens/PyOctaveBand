...
12 dB of headroom above its peak: the $L_{Cpeak}$ of step 4 reads the true
inter-sample peak, and one clipped run invalidates it and the $L_{Aeq}$ with
it.
...
- Uncompressed PCM, 24 bit, $f_s \ge 48$ kHz, one channel per microphone.
...
event second lifts $L_{Aeq}$ and $L_{10}$, and leaves $L_{50}$ and $L_{90}$ on
the background.*
...
which adds the maximum time-weighted level $L_{AFmax}$ that this same `laf_t`
track yields, noise dose and octave spectrograms; the $L_{den}$ and rating
levels continue in
...
unweighted $L_{eq}$ and not the $L_{Aeq}$ of step 4. That sum is the sanity
...
moves $L_{Aeq}$ — hard-clipping a 1 kHz tone at 80 % of its peak costs 0.9 dB —
...
linearity range, overload indication or directional response, nor the
IEC 61672-3:2013 periodic tests a working instrument receives. A class
verdict from this page describes the algorithm, not a built device, and the
three checks of step 7 screen the *recording*, not the instrument. The
