...
$L_{eq}$, or the band spectrum, of a recording that does not fit in memory.
It *is* easy, and it is exact rather than approximate, because $L_{eq}$ has no
...
- **The first block still carries the onset transient.** State starts at
  ...
  a few times $1/B$, and with $B = 0.231\,f_m$ for one-third octaves that is
...
$L_{AFmax}$ has to be accumulated as the maximum *inside* each block. And the
