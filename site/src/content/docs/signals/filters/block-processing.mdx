...
[Frequency Weighting](/phonometry/signals/levels/weighting/), and a streamed
...
# The reference has to use the design streaming can use: high_accuracy=False.
...
| `high_accuracy` (weighting) | resolves to `False` by default (the legacy bilinear design, see [Frequency Weighting](/phonometry/signals/levels/weighting/)); explicitly passing `True` raises `ValueError` | The polyphase resampling inside is block-incompatible |
...
Banks](/phonometry/signals/filters/filter-banks/) instead. `high_accuracy`
...
Weighting](/phonometry/signals/levels/weighting/) for the offline
high-accuracy design. Rank statistics such as $L_{90}$ do not stream either:
