...
$$
f_m = 1000 \cdot G^{x/b}\ (b\ \text{odd}), \qquad
f_m = 1000 \cdot G^{(2x+1)/2b}\ (b\ \text{even}), \qquad
...
is $B = f_m\,(G^{1/2b} - G^{-1/2b})$, which for one-third octaves is
$0.231\,f_m$, and two consequences follow from it.
...
2. Mid frequencies follow $f_m = 1000 \cdot G^{x/b}$ for odd $b$ and
$f_m = 1000 \cdot G^{(2x+1)/2b}$ for even $b$ — so 1 kHz is a band mid
frequency in the odd fractions and a band *edge* in the even ones — and the
edges are $f_1 = f_m G^{-1/2b}$ and $f_2 = f_m G^{+1/2b}$ in both cases; every
