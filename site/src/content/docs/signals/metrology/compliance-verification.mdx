...
with `weighting_class_limits` and `class_limits` publishing the masks, and
`filter_class_compliance` / `intensity_class_compliance` adding `.plot()` and
the accredited fiche); how to read and cite the published conformance report;
and the verified scope of the pattern-evaluation and periodic-test parts.
</ScopeClaim>
