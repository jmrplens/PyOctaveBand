...
the principal calibrator level that `target_spl` assumes, the Table 2 and
Table A.1 class tolerances quoted in the uncertainty discussion above, and the
short-term level-fluctuation check of clause 5.3.3 against the class 1 limits of
Table 2, which `sensitivity(ref, fs=fs)` runs on the reference recording.
