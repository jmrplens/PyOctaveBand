...
its own resolution bandwidth inside a single segment unless $B_e \ge r\,T_B$.
With $B_e = 1.5/T_B$ that gives an optimum $T_B = \sqrt{1.5/r}$. The siren below
...
check the resulting $B_e$ against the closest two components that must stay
...
only the part inside one $B_e$ — some 25 dB lower here, for a 1024-point segment
at 16 kHz. To recover a level from noise, convert cells to a density by
subtracting $10\log_{10} B_e$ and integrate over the band of interest, or use
...
that the wider main lobe raises $B_e$ and therefore the closest spacing you can
