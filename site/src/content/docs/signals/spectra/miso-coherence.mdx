...
(Eqs. 9.98-9.101), all exposed by `miso_coherence` and
`MISOCoherenceResult`.

**Not covered.** The chapter's multiple-input frequency-response (gain
factor $L_{iy}$) estimates, the linear model that predicts the output from
the conditioned inputs, are computed internally to build the coherent
output spectra but are not returned: `MISOCoherenceResult` exposes the
coherence and spectral-decomposition side only, not a fitted multi-input
transfer function. The method is also single-output (MISO); a system with
several correlated outputs needs one `miso_coherence` call per output.
