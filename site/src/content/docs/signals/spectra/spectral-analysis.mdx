...
under 1 dB means $\varepsilon_b \ge -0.2$, hence $B_e \le 0.775\,B_r$; keeping
it under a tenth of a decibel means $B_e \le 0.263\,B_r$. The familiar rule of
...
at 100 Hz with $Q = 50$ has $B_r = f/Q = 2$ Hz. Holding its bias under 2 %
(0.09 dB) needs $B_e \le B_r\sqrt{3 \times 0.02} = 0.49$ Hz, and for the Hann
taper $B_e = 1.5\,f_s/\texttt{nperseg}$, so at 48 kHz that is
...
the two errors on one axis: the bias falls as $B_e^2$ and the random error rises
...
| Sharp dips exactly at the resonances | resolution bias: the analysis bandwidth smears the peak differently in the two channels | lengthen the segment; the $\varepsilon_b$ formula of section 1 says by how much |
...
The conversion is one term. A fractional-octave band of centre frequency $f_c$
has bandwidth $B_n = f_c\,(2^{1/2n} - 2^{-1/2n})$, that is $0.2316\,f_c$ in one
third of an octave and $0.7071\,f_c$ in a full octave, so

$$
L_\text{band} = 10\log_{10}\!\left(\frac{G(f_c)\,B_n}{p_0^2}\right),
$$
...
$B_n \propto f_c$: **every colour reads 3 dB per octave higher in a band display
...
$p = NW$ (default 4), and for a record of $N_r$ samples at $f_s$ it fixes the
half-bandwidth $W = p\,f_s/N_r$ in hertz. $2W$ is the resolution of the estimate
