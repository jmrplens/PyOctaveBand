...
random error then needs $n_d = 100$ effective averages, which at 50 % overlap
...
as $n_d^{-1/2}$, and they are equal near `nperseg = 7400` for this resonance —
...
$(1-\gamma^2)^2/n_d$, so a low coherence read from a handful of averages is even
...
2048-sample segment fits only $n_d = 6.7$ effective averages into it, so its
