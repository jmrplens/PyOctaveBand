...
axes[2].plot(nm.ranges / 1000.0, nm.transmission_loss, label="Normal modes")
axes[2].plot(pe.ranges / 1000.0, pe.transmission_loss[zi], label="PE")
axes[2].invert_yaxis()
axes[2].set(xlabel="Range [km]", ylabel="Transmission loss [dB]")
axes[2].legend()
plt.show()
```
...
`transmission_loss` field). The three differ in what they approximate, not in
...
583.3 m analytic turning depth against 583.7 m traced, and the dashed overlay is
the exact circular arc of radius $R = c_0/(g\cos\theta_0)$ the solver is checked
against. A result draws its own fan with `rays.plot()`.*
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/pe_paraxial_error.svg" alt="Left, the transmission loss against range at a 60 metre receiver in a 100 metre waveguide at 50 hertz, computed with normal modes and with the parabolic equation, both drawn faintly as fine structure and boldly as range averages, with the parabolic equation running 1.6 decibels lossier at every range; right, the modal grazing angles of the six propagating modes as bars against the plus or minus 20 degree paraxial band, four of them outside it" width="92%" />
...
pe_tl = np.interp(r, pe_field.ranges, pe_field.transmission_loss[iz2])

def range_average(tl, width=61):        # incoherent, in the energy domain
    return -10.0 * np.log10(np.convolve(10.0 ** (-tl / 10.0),
                                        np.ones(width) / width, mode="same"))

inner = (r > 300.0) & (r < 5700.0)
sm_nm = range_average(np.asarray(modes.transmission_loss))
sm_pe = range_average(pe_tl)
print(round(float(np.mean(sm_pe[inner] - sm_nm[inner])), 2))   # 1.64 dB

fig, (ax_tl, ax_ang) = plt.subplots(1, 2, figsize=(13.5, 5.6),
                                    gridspec_kw={"width_ratios": [2, 1]})
ax_tl.plot(r / 1000.0, modes.transmission_loss, alpha=0.35)
ax_tl.plot(r / 1000.0, pe_tl, alpha=0.35)
ax_tl.plot(r[inner] / 1000.0, sm_nm[inner], label="Normal modes, averaged")
ax_tl.plot(r[inner] / 1000.0, sm_pe[inner], label="PE, averaged")
ax_tl.set_ylim(30.0, 92.0)
ax_tl.invert_yaxis()
ax_tl.set(xlabel="Range [km]", ylabel="Transmission loss [dB]")
ax_tl.legend()
...
The three solvers are configured by the caller, and every snippet above sets a
...
example of the
[transmission-loss section](/phonometry/underwater/underwater-propagation/#transmission-loss) the
...
transmission-loss curve of that guide's
[transmission-loss section](/phonometry/underwater/underwater-propagation/#transmission-loss)
...
se = underwater.passive_sonar_equation(
    source_level=140.0, transmission_loss=np.linspace(40.0, 120.0, 400),
...
for label, freq, law, r0 in cases:
    ax.plot(ranges / 1000.0,
            underwater.transmission_loss(ranges, freq, law=law,
                                         transition_range=r0, **env).tl,
            label=label)
...
ax.invert_yaxis()
ax.set(xlabel="Range [km]", ylabel="Transmission loss [dB]")
ax.legend()
plt.show()
```
...
`normal_modes`, `ray_trace` and `parabolic_equation` implement the Jensen et
al. (2011) numerical solvers for a range-independent stratified ocean: the
finite-difference Sturm-Liouville modal sum (Ch. 5, Eqs. 5.3-5.17), the
Runge-Kutta ray-trajectory integration (Ch. 3, Eqs. 3.23-3.24) and the
...
against an exact closed form: the ideal pressure-release waveguide's modes,
...
spreading, plus the mutual agreement of the modal and PE transmission loss.
</ScopeClaim>
...
not computed, and nothing searches for the eigenrays that join a given source
and receiver. The PE is the standard small-angle (Tappert) form, not a
...
shallow water (few propagating modes, $M \approx kD/\pi$, the reference
solution for its regime) and the parabolic equation for low-frequency, long
one-way paths (the full TL($z$,$r$) field with refraction). When two solvers
