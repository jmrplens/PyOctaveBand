...
description: "Computing the underwater sound field when closed forms are not enough: the normal-mode expansion of the stratified waveguide, ray tracing with Snell's invariant and convergence zones, the eigenray search that turns a traced fan into a receiver's arrival list, Gaussian beam tracing for a field that stays finite at caustics, and the split-step Fourier parabolic equation, each validated against an exact closed form, with the model-selection guidance to choose between them."
references:
  - type: book
...
between them and the closed forms. All four assume a range-independent
...
(Eqs. 3.79, 3.125-3.126). Each is validated
...
it for the lossy fluid seabed of the Rayleigh model, but there is no elastic
bottom, no sediment attenuation and no real bathymetry,
