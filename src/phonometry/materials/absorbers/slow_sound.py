...
* **Visco-thermal effective parameters.** The slit of height ``h`` uses the
  ...
  :math:`\rho_\mathrm{s} = \rho_0 [1 - \tanh((h/2) G_\rho) / ((h/2) G_\rho)]^{-1}`
  and :math:`\kappa_\mathrm{s} = \kappa_0 [1 + (\gamma - 1) \tanh((h/2) G_\kappa) /
  ((h/2) G_\kappa)]^{-1}`
...
def slit_effective_properties(
    ...
    *,
    ...
    .. math::
       ...
       \kappa_\mathrm{s} = \kappa_0 \left[1
    ...
    :param fluid: State of the air in the slit
        (:class:`~phonometry.fluids.Fluid`); the
...
def rectangular_duct_properties(
    ...
    *,
    ...
    :param fluid: State of the air in the duct
        (:class:`~phonometry.fluids.Fluid`); the
        density ``rho0``, viscosity ``eta``, Prandtl number ``Pr``, ratio of
        specific heats ``gamma`` and static pressure ``P0`` are read from it.
...
def _neck_slit_correction_2d(neck_width: float, slit_height: float) -> float:
    ...
    :math:`0.82 (w_\mathrm{n} / 2) = 0.41 w_\mathrm{n}` as printed in the source, exactly as
    ...
    :math:`w_\mathrm{n} > h`, where the polynomial turns negative and the resonator
...
def _slit_resonator_ducts(
    ...
    rho_n, kap_n = slit_effective_properties(f, slit_height=wn, fluid=fluid)
    rho_c, kap_c = slit_effective_properties(f, slit_height=wc, fluid=fluid)
...
def _square_resonator_ducts(
    ...
    rho_n, kap_n = rectangular_duct_properties(
        f, side=wn, fluid=fluid, sum_terms=sum_terms
    )
    rho_c, kap_c = rectangular_duct_properties(
        f, side=wc, fluid=fluid, sum_terms=sum_terms
    )
...
def helmholtz_resonator_impedance(
    ...
    *,
    ...
    .. math::
       ...
       Z_{\mathrm{HR}} = -j \frac{\cos(k_\mathrm{n} l_\mathrm{n}) \cos(k_\mathrm{c} l_\mathrm{c})
       - Z_\mathrm{n} k_\mathrm{n} \mathrm{dl} \cos(k_\mathrm{n} l_\mathrm{n}) \sin(k_\mathrm{c} l_\mathrm{c}) / Z_\mathrm{c}
       - Z_\mathrm{n} \sin(k_\mathrm{n} l_\mathrm{n}) \sin(k_\mathrm{c} l_\mathrm{c}) / Z_\mathrm{c}}
       {\sin(k_\mathrm{n} l_\mathrm{n}) \cos(k_\mathrm{c} l_\mathrm{c}) / Z_\mathrm{n}
       - k_\mathrm{n} \mathrm{dl} \sin(k_\mathrm{n} l_\mathrm{n}) \sin(k_\mathrm{c} l_\mathrm{c}) / Z_\mathrm{c}
       + \cos(k_\mathrm{n} l_\mathrm{n}) \sin(k_\mathrm{c} l_\mathrm{c}) / Z_\mathrm{c}}

    with :math:`Z_\mathrm{n} = \sqrt{\kappa_\mathrm{n} \rho_\mathrm{n}} / w_\mathrm{n}^2`,
    :math:`k_\mathrm{n} = \omega \sqrt{\rho_\mathrm{n} / \kappa_\mathrm{n}}` (and likewise for the
    ...
    :param fluid: State of the air in the neck and cavity
        (:class:`~phonometry.fluids.Fluid`); the
        density ``rho0``, viscosity ``eta``, Prandtl number ``Pr``, ratio of
        specific heats ``gamma`` and static pressure ``P0`` are read from it.
...
def slit_helmholtz_absorber(
    ...
    *,
    ...
    :math:`T = M_{\mathrm{dl}} (M_\mathrm{s} M_{\mathrm{HR}} M_\mathrm{s}) \cdots` over the
    ...
    :param fluid: State of the air the panel radiates into and the slit and
...
def critical_coupling_design(
    ...
    *,
    ...
    :param end_correction: Include the resonator radiation end corrections.
    :param slit_radiation: Include the slit-to-free-air radiation correction.
    :param fluid: State of the air the panel is designed for
        (:class:`~phonometry.fluids.Fluid`): its
        speed of sound ``c0``, density ``rho0``, viscosity ``eta``, Prandtl
        number ``Pr``, ratio of specific heats ``gamma`` and static pressure
        ``P0``.
