...
its reciprocal is the **point impedance** :math:`Z = 1/Y` (the same
motion-per-force / force-per-motion pair as ISO 7626-1 mechanical mobility, so
these theoretical values slot straight into
:class:`~phonometry.vibration.MobilityResult` and
:func:`~phonometry.vibration.convert_frf`). For an *infinite* structure the
...
.. math::
   ...
   \text{Thin plate, bending, edge:} \quad
   Z = 3.5 \sqrt{B' m''}, \qquad Y = \frac{1}{3.5 \sqrt{B' m''}}
...
def plate_bending_stiffness(
    ...
    r"""Bending stiffness of a thin plate per unit width (Cremer Eq. 4.22).
    ...
    :raises ValueError: for a non-positive modulus/thickness or
        :math:`\lvert\nu\rvert \ge 1`.
    """
...
def infinite_plate_impedance(
    ...
    r"""Point impedance of an infinite thin plate (Cremer Table 5.1).

    :math:`Z = C \sqrt{B' m''}` with :math:`C = 8` for a force at the plate
    centre and :math:`C = 3.5` for a force at a free edge. The impedance is
    purely real and
    ...
    :param mass_per_area: Mass per unit area ``m''``, in kg/m^2.
    :param location: ``"centre"`` (:math:`C = 8`) or ``"edge"``
        (:math:`C = 3.5`).
...
def infinite_plate_mobility(
    ...
    r"""Point mobility of an infinite thin plate
    :math:`Y = 1 / (C \sqrt{B' m''})`.
...
def injected_power(force: ArrayLike, mobility: ArrayLike) -> NDArray[np.float64]:
    r"""Time-averaged vibrational power injected by a point force (Cremer 5.23).

    :math:`W = 0.5\, |F|^2 \operatorname{Re}\{Y\}`: only the real part
