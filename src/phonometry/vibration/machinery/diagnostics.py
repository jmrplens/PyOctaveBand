...
outer race rotating it becomes :math:`(f_s/2)(1 + g)` (Eq. 8.6).

**Gears** (Eq. 8.3). The gear-meshing (tooth-passing) frequency of a wheel
with ``n_teeth`` teeth is :math:`\mathrm{GMF} = n_{\text{teeth}} f_s`, with
...
sideband groups at :math:`k\, \mathrm{GMF} \pm m f_s`.

**Induction motors** (Eqs. 8.19, 8.20). The three lines always present in a
motor bearing signal are ``fs`` (mechanical unbalance), :math:`2 f_s`
(misalignment with the driven load) and :math:`2 f_e` (non-uniform air gap,
torque pulses and the electrical faults). Norton's Eq. (8.19) writes the
supply frequency as :math:`f_e = f_s p / 2` for ``p`` magnetic poles, which
is the synchronous, zero-slip form; this module uses the slip-consistent
:math:`f_e = f_s p / (2 (1 - s))`, identical at :math:`s = 0` and the only
version that makes the rotor-slot harmonics of Eq. (8.20),

.. math::

   f_{sh} = f_e \left[ \frac{2 R}{p}(1 - s) \pm 2 (n - 1) \right]
   \tag{8.20}

collapse to the physical rotor-bar passing rate :math:`R f_s` at
...
of an impeller with ``N`` blades is :math:`f_b = n N f_s`; a pump's
...
sets up rotating pressure patterns with :math:`m_L = n N \pm k V` lobes for
``V`` vanes (Eq. 8.16), turning at :math:`n N f_s / m_L` (Eq. 8.17): a
...
def bearing_fault_frequencies(
    ...
    *,
    ...
    :math:`\mathrm{BPFO} + \mathrm{BPFI} = Z f_s` always, and
...
def induction_motor_frequencies(
    ...
    *,
    ...
    :math:`f_{sh} = f_e\left[\left(\dfrac{2R}{p}\right)(1-s) \pm 2(n-1)\right]
    = R f_s \pm 2(n-1) f_e`
    ...
    taken as :math:`f_e = f_s p / (2(1-s))`: Norton's Eq. (8.19) writes
    :math:`f_e = f_s p / 2`, which is the same expression at zero slip but
    does not reduce Eq. (8.20) to the physical rotor-bar passing rate
    ``R fs`` when the machine is loaded.

    With a non-zero slip the pole-pass line :math:`F_P = p\,f_{\text{slip}}`
...
def _lobe_patterns(
    ...
    :math:`m_L = nN \pm kV` lobes turning at ``n N fs / mL``. The pattern
...
def blade_pass_frequencies(
    ...
    *,
    ...
    :math:`f_b = n\, N_{\text{blades}} \times N/60` for ``n = 1 .. harmonics``,
    the discrete tone
    ...
    :math:`m_L = n\, N_{\text{blades}} \pm k\, N_{\text{vanes}}`
    lobes (Eq. 8.16) turning at
    :math:`M_L = n\, N_{\text{blades}} f_s / m_L` (Eq. 8.17). Those
