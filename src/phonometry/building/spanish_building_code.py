...
DB_HR_NORMALISED_SPECTRA: dict[str, tuple[float, ...]] = {
    "pink": (
...
def d2m_nt_atr(
    ...
    *,
    ...
    :param level_difference: Band standardized facade level difference
        ``D2m,nT``, in dB.
    :param frequencies: Band centre frequencies, in Hz (optional).
    :param spectrum: ``"traffic"`` (default), ``"railway"`` or ``"aircraft"``,
        per the dominant outdoor noise.
    :return: A :class:`DbHrGlobalIndexResult` named ``"D2m,nT,Atr"``.
    """
    return db_hr_global_index(
        level_difference, spectrum, frequencies=frequencies, name="D2m,nT,Atr"
    )
...
def db_hr_facade_requirement(
    ...
    *,
    ...
    :return: The :class:`DbHrRequirement` on ``D2m,nT,Atr``.
