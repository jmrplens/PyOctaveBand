...
normalized impact sound pressure level :math:`L_n = L_i + 10 \log_{10}(A/A_0)`
(Clause 3.2,
...
:math:`L = 10 \log_{10}(10^{L_{sb}/10} - 10^{L_b/10})` for a signal-to-background
margin
...
class LabImpactInsulationResult:
    ...
    :ivar l_n: Normalized impact sound pressure level
        :math:`L_n = L_i + 10 \log_{10}(A/A_0)` per band, in dB (Clause 3.2,
        Formula (1)).
    :ivar absorption: Equivalent sound absorption area :math:`A = 0.16\,V/T`
        per band, in m² (ISO 10140-4:2010, Formula (5)).
...
def background_correction(
    ...
    - :math:`6\,\mathrm{dB} < \mathrm{margin} < 15\,\mathrm{dB}`: the level
      ...
      :math:`L = 10 \log_{10}(10^{L_{sb}/10} - 10^{L_b/10})`.
    ...
    :math:`10 \log_{10}(10^{L_{sb}/10} - 10^{L_b/10})`, but that
...
def lab_impact_insulation(
    ...
    *,
    ...
    :math:`L_n = L_i + 10 \log_{10}(A/A_0)` (Clause 3.2, Formula (1)) with the
    Sabine
