...
:math:`\Delta f_\mathrm{c} = 25.0 + 75.0 (1.0 + 1.4 (f_\mathrm{T}/1000)^2)^{0.69}` Hz
(Formula (2)). Assuming a geometric placement of the corner frequencies,
:math:`f_\mathrm{T} = \sqrt{f_1 f_2}` (Formula (3)),
:math:`f_1 = -\Delta f_\mathrm{c}/2 + \sqrt{\Delta f_\mathrm{c}^2 + 4 f_\mathrm{T}^2}/2` (Formula (4))
and :math:`f_2 = f_1 + \Delta f_\mathrm{c}` (Formula (5)).
...
masking noise is :math:`L_\mathrm{G} = L_\mathrm{S} + 10 \log_{10}(\Delta f_\mathrm{c}/\Delta f)`
(Formula (12), ``Δf`` the line spacing). The masking index is
:math:`a_\mathrm{v} = -2 - \log_{10}[1 + (f/502)^{2.5}]` dB (Formula (13)) and the
audibility of a tone of level ``LT`` (Formula (8)) is
:math:`\Delta L = L_\mathrm{T} - L_\mathrm{G} - a_\mathrm{v}` dB (Formula (14)). A tone is present when
:math:`\Delta L > 0`.
...
(:math:`L_\mathrm{S} = 49.22` dB, :math:`L_\mathrm{T} = 67.96` dB for the 137.3 Hz tone) and
...
:math:`L_\mathrm{T} = 72.15` dB and the decisive FG audibility
...
:math:`f_\mathrm{D} = 21 \cdot 10^{1.2 |\log_{10}(f_\mathrm{T}/212)|^{1.8}}` Hz
...
restricts the method to :math:`f_\mathrm{T} \ge 90` Hz and the ISO/PAS 20065 scope
starts at 50 Hz; the two-tone separation frequency (Formula (19)) is printed
for :math:`f_\mathrm{T} < 1000` Hz (with a lower bound of 88 Hz in the DIN print,
...
edge-steepness test follows the DIN 45681 :math:`f_\mathrm{T}/\sqrt{2}`-on-both-edges
...
def critical_band_corners(tone_frequency: float) -> tuple[float, float]:
    ...
    :math:`f_1 = -\Delta f_\mathrm{c}/2 + \sqrt{\Delta f_\mathrm{c}^2 + 4 f_\mathrm{T}^2}/2` and
    :math:`f_2 = f_1 + \Delta f_\mathrm{c}`, so that :math:`\sqrt{f_1 f_2} = f_\mathrm{T}` and
    :math:`f_2 - f_1 = \Delta f_\mathrm{c}`.
...
def masking_index(frequency: float) -> float:
    ...
    :math:`a_\mathrm{v} = -2 - \log_{10}[1 + (f/502)^{2.5}]` dB. The value is negative and
    grows more
...
def critical_band_level(
    ...
    :math:`L_\mathrm{G} = L_\mathrm{S} + 10 \log_{10}(\Delta f_\mathrm{c}/\Delta f)` dB, spreading the mean
...
def audibility_from_levels(
    ...
    :math:`\Delta L = L_\mathrm{T} - L_\mathrm{G} - a_\mathrm{v}` dB.
...
def energy_sum_level(
    ...
    *no* bandwidth correction, :math:`L_\mathrm{T} = L_1` (Formula (7)). For
    :math:`K > 1` lines,
    :math:`L_\mathrm{T} = 10 \log_{10}[\sum_i 10^{L_i/10}] + 10 \log_{10}(\Delta f/\Delta f_\mathrm{e})` dB
    (Formula (8)); the window correction
    :math:`10 \log_{10}(\Delta f/\Delta f_\mathrm{e})` is ``−1.76 dB`` for a Hanning window
    (:math:`\Delta f_\mathrm{e} = 1.5 \Delta f`, Annex A) and ``0 dB`` for a
    rectangular window
    (:math:`\Delta f_\mathrm{e} = \Delta f`). The DIN 45681:2005-03 Annex J reference
    ...
    :param line_levels: Narrow-band levels ``Li`` of the tonal lines to sum,
        in dB.
    :param effective_bandwidth_factor: :math:`\Delta f_\mathrm{e}/\Delta f`; 1.5
...
def mean_narrowband_level(
    ...
    *,
    ...
    .. math::
       ...
       L_\mathrm{S} = 10 \log_{10}\!\left[ \frac{1}{M} \sum_i 10^{L_i/10} \right]
       + 10 \log_{10}\frac{\Delta f}{\Delta f_\mathrm{e}}
    ...
    :math:`10 \log_{10}(\Delta f/\Delta f_\mathrm{e})` is
    ``−1.76 dB`` for the recommended Hanning window
    (:math:`\Delta f_\mathrm{e} = 1.5 \Delta f`).
...
def tone_level(
    ...
    *,
    ...
    ``tone_frequency`` whose level stays above both :math:`L_\mathrm{S} + 6` dB and
    :math:`L_\mathrm{peak} - 10` dB (Clause 5.3.3); their energy sum with the window
...
def _is_distinct(
    ...
    *asymmetric* edge-steepness formulas, :math:`f_\mathrm{T}/2` on the lower edge and
    :math:`f_\mathrm{T}` (no divisor) on the upper. DIN 45681:2005-03 prints
    :math:`f_\mathrm{T}/\sqrt{2}` on BOTH edges and its executable Annex J reference
...
def combined_tone_level(
    ...
    *,
    ...
    :math:`L_{\mathrm{T}m} = 10 \log_{10}[\sum_n 10^{L_{\mathrm{T}m,n}/10}]`, the energy sum of the
    tonal lines of all
...
def two_tone_separation_frequency(tone_frequency: float) -> float:
    ...
    :math:`f_\mathrm{D} = 21 \cdot 10^{1.2 |\log_{10}(f_\mathrm{T}/212)|^{1.8}}` Hz. When *exactly
    ...
    :math:`|f_{\mathrm{T}1} - f_{\mathrm{T}2}|` (Formula (18)) exceeds this threshold. ``fT`` is
    the frequency
    of the more prominent tone (the larger audibility ``ΔL``). The threshold is
    ``21 Hz`` at :math:`f_\mathrm{T} = 212` Hz and grows on either side; Formula (19)
    is stated for :math:`50~\text{Hz} < f_\mathrm{T} < 1000~\text{Hz}` (Clause 5.3.8,
    Annex D, Note 3).
...
def resolve_tones_separately(
    ...
    :math:`|f_{\mathrm{T}1} - f_{\mathrm{T}2}|` (Formula (18)) exceeds the separation
    frequency ``fD``
...
def audibility_uncertainty(
    ...
    *,
    ...
    .. math::
       ...
       \sigma^2 = \left[ \frac{\sum w_\mathrm{T}^2}{\left(\sum w_\mathrm{T}\right)^2}
       + \frac{\sum w_\mathrm{S}^2}{\left(\sum w_\mathrm{S}\right)^2} \right] \sigma_L^2
       + \left( 4.34 \, \frac{\Delta f}{\Delta f_\mathrm{c}} \right)^2
    ...
    critical-bandwidth term uses :math:`\sigma_{\Delta f_\mathrm{c}} = \Delta f`
    (Formula (26)). The
...
def tone_audibility(
    ...
    :math:`\Delta L = L_\mathrm{T} - L_\mathrm{G} - a_\mathrm{v}`
