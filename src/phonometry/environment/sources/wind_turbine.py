...
* :func:`apparent_sound_power_level` -- the A-weighted apparent sound power
  ...
  (:func:`slant_distance`),
  :math:`L_{W\mathrm{A}} = L_p - 6 + 10 \cdot \log_{10}(4\pi R_1^2/S_0)` (Formula 26).
...
def apparent_sound_power_level(
    ...
    :math:`L_{W\mathrm{A},i} = L_{p,i} - 6 + 10 \cdot \log_{10}(4\pi R_1^2/S_0)` per
...
def critical_bandwidth(fc: float) -> float:
    ...
    (f_\mathrm{c}/1000)^2]^{0.69}`. For a candidate tone in the
...
class WindTurbineTonalityResult:
    ...
    :ivar tonality: Tonality :math:`\Delta L_\mathrm{tn} = L_{p\mathrm{t}} - L_{p\mathrm{n}}`, in dB.
...
def wind_turbine_tonality(
    ...
    *,
    ...
    (Formula 31), the tonality :math:`\Delta L_\mathrm{tn} = L_{p\mathrm{t}} - L_{p\mathrm{n}}`
    (Formula 32), the
    audibility criterion ``L_a`` (Formula 34) and the tonal audibility
    :math:`\Delta L_\mathrm{a} = \Delta L_\mathrm{tn} - L_\mathrm{a}` (Formula 33).
