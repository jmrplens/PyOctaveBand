...
def steady_state_spl(
    ...
    *,
    ...
    :param source_model: Sound power model of Norton & Karczub 2e Table 4.5:
        ``"constant_power"`` (default, ``Pi = Pi_0``, the position of the source
        does not change its radiated power), ``"constant_volume"``
        (``Pi = Pi_0 Q``, reflecting boundaries raise the radiated power by
        ``10 lg Q``; the conservative upper bound) or ``"constant_pressure"``
        (``Pi = Pi_0 / Q``, the theoretical lower bound). See
