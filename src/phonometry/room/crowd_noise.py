...
r"""Crowd self-noise in a restaurant: the cocktail-party equilibrium.
...
* the **signal**, the direct field of the person across the table
  (Equation (17.50)),
  :math:`L_p^{\text{signal}} = L_W + 10 \log_{10}[Q / (4 \pi r^2)]`;

* the **noise**, the reverberant field built up by ``N`` simultaneous talkers
  spread over the room (Equation (17.51)),
  :math:`L_p^{\text{noise}} = L_W + 10 \log_{10} N
  + 10 \log_{10}[4 / (N A_{\text{tab}})]`;
...
absorption area *per occupied table*, so that :math:`N A_{\text{tab}}` is the
room's total
...
.. math::
   ...
   A_{\text{tab}} > 16 \pi\, 10^{-0.6}\, r_s^2 / Q
   \approx 6.31\, r_s^2 \quad (Q = 2)

   A_{\text{tab}} < 16 \pi\, 10^{-0.9}\, r_t^2 / Q
   \approx 3.16\, r_t^2 \quad (Q = 2)

.. note::
   Long prints the first constant as ``6.33``; the closed form with the same
   :math:`Q = 2` and ``-6 dB`` that yield his second constant ``3.16`` gives
...
def speech_direct_level(
    ...
    *,
    ...
    r"""Direct-field level of one talker (Long Equation (17.50)).

    :math:`L_p = L_W + 10 \log_{10}[Q / (4 \pi r^2)]`, the signal a listener
...
def crowd_noise_level(
    ...
    *,
    ...
    r"""Reverberant level of ``N`` simultaneous talkers (Long Equation (17.51)).

    :math:`L_p = L_W + 10 \log_{10} N + 10 \log_{10}(4 / A)` with the room's
    total equivalent absorption area ``A``
    (:math:`= N A_{\text{tab}}` when the absorption is
